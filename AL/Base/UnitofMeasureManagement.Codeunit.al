codeunit 5402 "Unit of Measure Management"
{
    ...
    // P800133109
    procedure GetQtyRoundingPrecision(Item: Record Item; UnitOfMeasureCode: Code[10]; var QtyRoundingPrecision: Decimal; var QtyRoundingPrecisionBase: Decimal)
    begin
        QtyRoundingPrecision := GetQtyRoundingPrecision(Item, UnitOfMeasureCode);
        QtyRoundingPrecisionBase := GetQtyRoundingPrecision(Item, Item."Base Unit of Measure");
    end;

    // P800133109
    procedure GetQtyRoundingPrecision(ItemNo: Code[20]; UnitOfMeasureCode: Code[10]; var QtyRoundingPrecision: Decimal; var QtyRoundingPrecisionBase: Decimal)
    var
        Item: Record Item;
    begin
        Item.Get(ItemNo); 
    ...
    // P800133109
    procedure CalcBaseQty(ItemNo: Code[20]; UOMCode: code[10]; Qty: Decimal) QtyRounded: Decimal
    var
        ItemUnitOfMeasure: Record "Item Unit of Measure";
    begin
        ItemUnitOfMeasure.Get(ItemNo, UOMCode);
        QtyRounded := CalcBaseQty(ItemNo, '', '', Qty, ItemUnitOfMeasure."Qty. per Unit of Measure", ItemUnitOfMeasure."Qty. Rounding Precision", '', '', '');
    end;
    ...
    // P800133109
    procedure RoundAndValidateQty(ItemNo: Code[20]; UOM: Code[10]; Qty: Decimal; FieldName: Text) QtyRounded: Decimal
    var
        ItemUnitOfMeasure: Record "Item Unit of Measure";
    begin
        ItemUnitOfMeasure.Get(ItemNo, UOM);
        QtyRounded := RoundQty(Qty, ItemUnitOfMeasure."Qty. Rounding Precision");
        if (ItemUnitOfMeasure."Qty. Rounding Precision" > 0) and (Qty <> QtyRounded) then
            Error(InvalidPrecisionErr, Qty, FieldName, 0.00001);
