codeunit 48 PostingSetupManagement
{
    ...
    procedure SendECPostingSetupNotification(ECPostingSetup: Record "Extra Charge Posting Setup"; FieldCaption: Text)
    begin
        // P80053245
        if not IsPostingSetupNotificationEnabled then
            exit;
        ...
        SendPostingSetupNotification(
          StrSubstNo(MissingAccountTxt, FieldCaption, ECPostingSetup.TableCaption),
          SetupMissingAccountTxt, 'ShowECPostingSetup',
          ECPostingSetup."Gen. Bus. Posting Group" + '|' + ECPostingSetup."Gen. Prod. Posting Group", ECPostingSetup."Extra Charge Code");
    end;

    procedure ShowECPostingSetup(SetupNotification: Notification)
    var
        ECPostingSetupRec: Record "Extra Charge Posting Setup";
        ECPostingSetupPage: Page "Extra Charge Posting Setup";
        BusPostingGroupCode: Code[20];
        ProdPostingGroupCode: Code[20];
    ...
    begin
        // P80053245
        Clear(ECPostingSetupPage);
        BusPostingGroupCode := SetupNotification.GetData('GroupCode1');
        Index := StrPos(BusPostingGroupCode, '|');
        ProdPostingGroupCode := CopyStr(BusPostingGroupCode, Index + 1);
        BusPostingGroupCode := CopyStr(BusPostingGroupCode, 1, Index - 1);
        ExtraChargeCode := SetupNotification.GetData('GroupCode2');
        if ECPostingSetupRec.Get(BusPostingGroupCode, ProdPostingGroupCode, ExtraChargeCode) then
            ECPostingSetupPage.SetRecord(ECPostingSetupRec);
        ECPostingSetupPage.SetTableView(ECPostingSetupRec);
        ECPostingSetupPage.RunModal;
    end;

    [Scope('Personalization')]
    procedure SendAccrualPostingGroupNotification(AccrualPostingGroup: Record "Accrual Posting Group"; FieldCaption: Text)
    begin
        // P80053245
        if not IsPostingSetupNotificationEnabled then
            exit;
        ...
        SendPostingSetupNotification(
          StrSubstNo(MissingAccountTxt, FieldCaption, AccrualPostingGroup.TableCaption),
          SetupMissingAccountTxt, 'ShowAccrualPostingGroups', AccrualPostingGroup.Code, '');
    end;

    procedure ShowAccrualPostingGroups(SetupNotification: Notification)
    var
        AccrualPostingGroup: Record "Accrual Posting Group";
        AccrualPostingGroups: Page "Accrual Posting Groups";
        PostingGroupCode: Code[20];
    begin
        // P80053245
        Clear(AccrualPostingGroups);
        PostingGroupCode := SetupNotification.GetData('GroupCode1');
        if PostingGroupCode <> '' then begin
            AccrualPostingGroup.Get(PostingGroupCode);
            AccrualPostingGroups.SetRecord(AccrualPostingGroup);
        end;
        AccrualPostingGroups.SetTableView(AccrualPostingGroup);
        AccrualPostingGroups.RunModal;
    end;
}
