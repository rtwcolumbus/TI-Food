...
{
    // PRW17.10.03
    // P8001340, Columbus IT, Jack Reynolds, 13 AUG 14
    //   Fix problem posting multiple warehouse entries
    ...
    begin
        with ItemJnlLine do begin
            QuantityBase := Abs("Quantity (Base)"); // P8001340
            QuantityAlt := Abs("Quantity (Alt.)");  // P8001340
            WhseEntry.Reset();
            ...
            if WhseEntry.Find('+') then
                repeat
                    ...
                    // P8001340
                    if (QuantityAlt <> 0) and (QuantityBase <> 0) then begin
                        "Quantity (Alt.)" := Round(QuantityAlt * "Quantity (Base)" / QuantityBase, 0.00001);
