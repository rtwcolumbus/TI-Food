page 9000 "Whse. WMS Role Center"
{
    ...
    //   Add P800 Adv. Whse. elements
    // 
    // P8000810, VerticalSoft, Don Bresee, 11 APR 10
    //   Add P800 Whse. elements
    // 
    // PRW19.00
    ...
    actions
    {
        area(reporting)
        {
            ...
            action("Prod. &Replenishment")
            {
                ApplicationArea = FOODBasic;
                Caption = 'Prod. &Replenishment';
                Image = Production;
        ...
        area(embedding)
        {
            ...
            action(Staging)
            {
                ...
                Image = Stages;
                RunObject = Page "Whse. Staged Pick List";
                RunPageView = WHERE("Staging Status" = FILTER(.. "Partially Staged"));
            }
            action("Staging Complete")
            {
                ApplicationArea = FOODBasic;
                Caption = 'Staging Complete';
                Image = Completed;
                RunObject = Page "Whse. Staged Pick List";
                RunPageView = WHERE("Staging Status" = CONST("Completely Staged"));
            }
            action(BinContents)
            {
                ApplicationArea = Warehouse;
        ...
        area(creation)
        {
            ...
            {
                ApplicationArea = FOODBasic;
                Caption = 'Staged Pic&k';
                Image = InventoryPick;
                //The property 'PromotedCategory' can only be set if the property 'Promoted' is set to 'true'
                //PromotedCategory = Process;
                RunObject = Page "Whse. Staged Pick";
                RunPageMode = Create;
            }
        ...
        area(processing)
        {
            ...
            action("Pro&duction Picking")
            {
                ApplicationArea = FOODBasic;
                Caption = 'Pro&duction Picking';
                Image = PickLines;
            ...
            action("Order Shippin&g")
            {
                ApplicationArea = FOODBasic;
                Caption = 'Order Shippin&g';
                Image = Shipment;
                RunObject = Page "Order Shipping";
            }
            action("Order &Recei&ving")
            {
                ApplicationArea = FOODBasic;
                Caption = 'Order &Recei&ving';
                Image = Receipt;
