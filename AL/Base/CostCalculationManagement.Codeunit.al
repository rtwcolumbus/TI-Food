codeunit 5836 "Cost Calculation Management"
{
    ...
    //   CalcABCRtngCostPerUnit - figure out parameters and call P800BOMFns.CalcABCRtngCostPerUnit
    // 
    // PRW15.00.01
    ...
    // 
    // PRW16.00.01
    // P8000695, VerticalSoft, Jack Reynolds, 05 MAY 09
    //   Fix problem with calculation of indirect cost
    // 
    // PRW16.00.05
    // P8000925, Columbus IT, Jack Reynolds, 29 MAR 11
    //   Use equipment code on production order line
    // 
    // PRW16.00.06
    ...
    // 
    // PRW17.00.01
    // P8001210, Columbus IT, Jack Reynolds, 16 SEP 13
    //   Fix problem with standard cost based on SKU
    ...
    begin
        with ProdOrderLine do begin
            ...
            then begin
                ...
                Item.Get("Item No.");
                // P8001210
                if SKU.Get(ProdOrderLine."Location Code", ProdOrderLine."Item No.", ProdOrderLine."Variant Code") then
                    ItemCostMgmt.TransferCostsFromSKUToItem(SKU, Item);
                // P8001210
                QtyBase := "Quantity (Base)";
            end;
            QtyBase := P800ProdOrderMgmt.CalcProdLineCostQtyBase(ProdOrderLine, InvtAdjmtEntryOrder."Completely Invoiced"); // PR3.60, P8001132
    ...
    begin
        with ProdOrderLine do begin
            ...
            P800ProdOrderMgmt.AddProdLineExpCost(ProdOrderLine, ExpMatCost); // PR3.60
    ...
    procedure CalcABCRtngCostPerUnit(RoutingRec: RecordRef; Item: Record Item; CalcDate: Date; LocationCode: Code[10]; var ProdBOMNo: Code[20]; var ProdBOMVersion: Code[10]; var DirUnitCost: Decimal; var IndirCostPct: Decimal; var OvhdRate: Decimal; var UnitCost: Decimal; var UnitCostCalculation: Option Time,Unit): Boolean
    var
    ...
    begin
        ...
        case RoutingRec.Number of
            DATABASE::"Routing Line":
                begin
                    RoutingRec.SetTable(RoutingLine);
                    if not RoutingLine."Use ABC Cost" then
                        exit;
                    WorkCenter.Get(RoutingLine."Work Center No.");
                    if not WorkCenter."Specific Unit Cost" then
                        exit;
                    RoutingNo := RoutingLine."Routing No.";
                    RoutingLinkCode := RoutingLine."Routing Link Code";
                    if ProdBOMNo = '' then begin
                        ProdBOMNo := Item."Production BOM No.";
                        ProdBOMVersion := VersionMgt.GetBOMVersion(ProdBOMNo, CalcDate, true);
                    end;
                    EquipNo := P800BOMFns.GetCostingEquipment(ProdBOMNo, ProdBOMVersion, LocationCode); // P8000557A, P8001030
                end;
            DATABASE::"Prod. Order Routing Line":
                begin
                    RoutingRec.SetTable(ProdOrderRtngLine);
                    if not ProdOrderRtngLine."Use ABC Cost" then
                        exit;
                    ...
                    RoutingNo := ProdOrderRtngLine."Routing No.";
                    RoutingLinkCode := ProdOrderRtngLine."Routing Link Code";
                    if ProdBOMNo = '' then begin
                        ProdBOMNo := ProdOrderLine."Production BOM No.";
                        ProdBOMVersion := ProdOrderLine."Production BOM Version Code";
                    end;
                    //ProdOrder.GET(ProdOrderRtngLine.Status,ProdOrderRtngLine."Prod. Order No."); // P8000557A   P8000925
                    EquipNo := ProdOrderLine."Equipment Code";                                       // P8000557A P8000925
                end;
            DATABASE::"Planning Routing Line":
                begin
                    RoutingRec.SetTable(PlanningRtngLine);
                    if not PlanningRtngLine."Use ABC Cost" then
                        exit;
                    WorkCenter.Get(PlanningRtngLine."Work Center No.");
                    if not WorkCenter."Specific Unit Cost" then
                        exit;
                    ReqLine.Get(PlanningRtngLine."Worksheet Template Name", PlanningRtngLine."Worksheet Batch Name",
                      PlanningRtngLine."Worksheet Line No.");
                    RoutingNo := ReqLine."Routing No.";
                    RoutingLinkCode := PlanningRtngLine."Routing Link Code";
                    if ProdBOMNo = '' then begin
                        ProdBOMNo := ReqLine."Production BOM No.";
                        ProdBOMVersion := ReqLine."Production BOM Version Code";
                    end;
                    EquipNo := P800BOMFns.GetCostingEquipment(ProdBOMNo, ProdBOMVersion, LocationCode); // P8000557A, P8001030
                end;
        end;

        UnitCostCalculation := UnitCostCalculation::Unit;
        P800BOMFns.CalcABCRtngCostPerUnit(
          Item."No.",
          ProdBOMNo, ProdBOMVersion,
          RoutingNo, RoutingLinkCode,
          EquipNo, // P8000557A
          DirUnitCost, IndirCostPct, OvhdRate, UnitCost);
