...
{
    // PR2.00.05
    //   Added Unit of Measure and Production BOM No. with Table Relation for Lookup
    //   Add key on Production BOM No.
    //   OnDelete to include any existing Item Variant Variables
    // 
    // PR3.60.03
    //   Add key on Production BOM No.
    // 
    // PRW17.00
    // P8001132, Columbus IT, Jack Reynolds, 28 MAR 13
    //   Additional check and deletions in OnDelete trigger
    // 
    // PRW19.00.01
    ...
    fields
    {
        ...
        field(37002580; "Unit of Measure Code"; Code[10])
        {
            Caption = 'Unit of Measure Code';
            Description = 'PR2.00.05';
            TableRelation = "Item Unit of Measure".Code WHERE("Item No." = FIELD("Item No."));
        }
        field(37002581; "Production BOM No."; Code[20])
        {
    ...
    keys
    {
        ...
        key(Key37002000; "Production BOM No.")
        {
        }
    }
    ...
    begin
        ...
        StdCostWorksheet.SetRange("No.", "Item No.");
        StdCostWorksheet.SetRange("Variant Code", Code);
        if not StdCostWorksheet.IsEmpty then
            Error(Text001, Code, StdCostWorksheet.TableCaption);

        ProdOrderLine.SetRange("Item No.", "Item No.");
        ProdOrderLine.SetRange("Variant Code", Code);
        if not ProdOrderLine.IsEmpty then
            Error(Text001, Code, ProdOrderLine.TableCaption);

        FamilyLine.SetRange("Item No.", "Item No.");
        FamilyLine.SetRange("Variant Code", Code);
        if not FamilyLine.IsEmpty then
            Error(Text001, Code, FamilyLine.TableCaption);

        PreProcActivity.SetRange("Item No.", "Item No.");
        PreProcActivity.SetRange("Variant Code", Code);
        if not PreProcActivity.IsEmpty then
            Error(Text001, Code, PreProcActivity.TableCaption);

        RepackOrder.SetRange(Status, RepackOrder.Status::Open);
        RepackOrder.SetRange("Item No.", "Item No.");
        RepackOrder.SetRange("Variant Code", Code);
        if not RepackOrder.IsEmpty then
            Error(Text001, Code, RepackOrder.TableCaption);

        RepackOrderLine.SetRange(Type, RepackOrderLine.Type::Item);
        RepackOrderLine.SetRange("No.", "Item No.");
        RepackOrderLine.SetRange("Variant Code", Code);
        RepackOrderLine.SetRange(Status, RepackOrderLine.Status::Open);
        if not RepackOrderLine.IsEmpty then
            Error(Text001, Code, RepackOrderLine.TableCaption);
        // P8001132
        ...
        // PR2.00.05 Begin
        ItemVarVbl.SetRange("Item No.", "Item No.");
        ItemVarVbl.SetRange("Variant Code", Code);
        ItemVarVbl.DeleteAll;
        // PR2.00.05 End
        ...
        // P8001132
        RecPriceTemplate.SetRange("Item No.", "Item No.");
        RecPriceTemplate.SetRange("Variant Code", Code);
        RecPriceTemplate.DeleteAll;

        Item.SetRange("Production Grouping Item", "Item No.");
        Item.SetRange("Production Grouping Variant", Code);
        Item.ModifyAll("Production Grouping Variant", '');
        // P8001132
    end;
