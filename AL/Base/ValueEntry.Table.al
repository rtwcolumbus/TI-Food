table 5802 "Value Entry"
{
    ...
    //   GetQuantity function to return quantity or invoiced quantity (regular or alternate)
    // 
    // PRW16.00.05
    ...
    fields
    {
        ...
        field(37002080; "Sales Quantity"; Decimal)
        {
            Caption = 'Sales Quantity';
            DecimalPlaces = 0 : 5;
        }
        field(37002660; "Extra Charge"; Decimal)
        {
            AccessByPermission = TableData "Extra Charge" = R;
            AutoFormatType = 1;
            CalcFormula = Sum ("Value Entry Extra Charge".Charge WHERE("Entry No." = FIELD("Entry No.")));
            Caption = 'Extra Charge';
            Editable = false;
            FieldClass = FlowField;
        }
        field(37002661; "Extra Charge (Expected)"; Decimal)
        {
            AccessByPermission = TableData "Extra Charge" = R;
            AutoFormatType = 1;
            CalcFormula = Sum ("Value Entry Extra Charge"."Expected Charge" WHERE("Entry No." = FIELD("Entry No.")));
            Caption = 'Extra Charge (Expected)';
            Editable = false;
            FieldClass = FlowField;
        }
        field(37002662; "Extra Charge (ACY)"; Decimal)
        {
            AccessByPermission = TableData "Extra Charge" = R;
            AutoFormatType = 1;
            CalcFormula = Sum ("Value Entry Extra Charge"."Charge (ACY)" WHERE("Entry No." = FIELD("Entry No.")));
    ...
    keys
    {
        ...
        {
            SumIndexFields = "Invoiced Quantity", "Sales Amount (Expected)", "Sales Amount (Actual)", "Cost Amount (Expected)", "Cost Amount (Actual)", "Cost Amount (Non-Invtbl.)", "Purchase Amount (Actual)", "Expected Cost Posted to G/L", "Cost Posted to G/L", "Item Ledger Entry Quantity", "Sales Amount (FOB)", "Sales Amount (Freight)";
        }
        ...
        {
            SumIndexFields = "Discount Amount", "Cost Amount (Non-Invtbl.)", "Cost Amount (Actual)", "Cost Amount (Expected)", "Sales Amount (Actual)", "Sales Amount (Expected)", "Invoiced Quantity", "Sales Amount (FOB)", "Sales Amount (Freight)";
        }
        ...
        {
            SumIndexFields = "Invoiced Quantity", "Sales Amount (Actual)", "Purchase Amount (Actual)", "Sales Quantity";
        }
    ...
    procedure GetQuantity(FldNo: Integer; AltQty: Boolean): Decimal
    begin
        // P8000500A
        ...
        if (ItemLedgEntry.CostInAlternateUnits = AltQty) then begin
            case FldNo of
                FieldNo("Item Ledger Entry Quantity"):
            ...
            case FldNo of
                FieldNo("Item Ledger Entry Quantity"):
                    if ("Item Ledger Entry Type" in ["Item Ledger Entry Type"::"Positive Adjmt.", "Item Ledger Entry Type"::"Negative Adjmt."])
              or
                ...
                FieldNo("Invoiced Quantity"):
                    if AltQty then begin
                        if "Invoiced Quantity" = ItemLedgEntry."Invoiced Quantity" then
                            exit(ItemLedgEntry."Invoiced Quantity (Alt.)")
                        else
                            if ItemLedgEntry."Invoiced Quantity" <> 0 then
                                exit(Round(ItemLedgEntry."Invoiced Quantity (Alt.)" * "Invoiced Quantity" / ItemLedgEntry."Invoiced Quantity", 0.00001));
                    end else begin
                        if "Invoiced Quantity" = ItemLedgEntry."Invoiced Quantity (Alt.)" then
                            exit(ItemLedgEntry."Invoiced Quantity")
                        else
                            if ItemLedgEntry."Invoiced Quantity (Alt.)" <> 0 then
                                exit(Round(ItemLedgEntry."Invoiced Quantity" * "Invoiced Quantity" / ItemLedgEntry."Invoiced Quantity (Alt.)", 0.00001));
                    end;
            end;
        end;
    end;
