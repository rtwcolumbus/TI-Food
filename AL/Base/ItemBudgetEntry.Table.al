table 7134 "Item Budget Entry"
{
    ...
    // 
    // PRW17.00
    // P8001137, Columbus IT, Jack Reynolds, 22 FEB 13
    //   Fix problems for entries that are not items
    ...
    keys
    {
        ...
        key(Key5; "Budget Name", "Analysis Area", "Item No.", "Source Type", "Source No.", Date, "Location Code", "Global Dimension 1 Code", "Global Dimension 2 Code", "Budget Dimension 1 Code", "Budget Dimension 2 Code", "Budget Dimension 3 Code")
        {
            SumIndexFields = "Quantity (Alt.)", "Sales Amount (FOB)", "Sales Amount (Freight)";
        }
    }
    ...
    begin
        ...
        case true of
            "Sales Amount" <> 0:
                "Sales Amount (FOB)" := "Sales Amount";
            "Sales Amount (FOB)" <> 0:
                "Sales Amount" := "Sales Amount (FOB)";
            "Sales Amount (Freight)" <> 0:
                "Sales Amount" := "Sales Amount (Freight)";
        end;
        // P8000921
    end;
    ...
    procedure CalcAlternateQty()
    var
        Item: Record Item;
    begin
        // P8000500A
        if "Item No." = '' then // P8001137
            exit;                 // P8001137

        Item.Get("Item No.");
        if Item.TrackAlternateUnits then
            "Quantity (Alt.)" := Round(Quantity * Item.AlternateQtyPerBase, 0.00001)
        else
            "Quantity (Alt.)" := 0;
    end;
}
