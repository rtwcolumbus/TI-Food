codeunit 74 "Purch.-Get Receipt"
{
    ...
    // 
    // PRW111.00.03
    // P800806363, To Increase, Jack Reynolds, 13 AUG 19
    //   Alternate quantity issue when getting receipt lines
    //
    // PRW115.00.03
    // P800127968, To Increase, Gangabhushan, 03 MAR 21
    //   CS00178333 | Get Receipt Lines for GL Account Issue
    ...
    begin
        ...
        if TransferLine then begin
            ...
            if PurchRcptLine.Type = PurchRcptLine.Type::Item then // P800127968
            	InsertAltQtyLinesFromPurReceipt(PurchRcptLine, PurchLine); // P80070933
    ...
    local procedure InsertAltQtyLinesFromPurReceipt(pPurchRcptLine: Record "Purch. Rcpt. Line"; var pPurchaseLine: Record "Purchase Line")
    var
    ...
    begin
        // P80070933
        Item.Get(pPurchaseLine."No.");
        if not Item."Catch Alternate Qtys." then // P80080636
            exit;
        pPurchaseLine."Alt. Qty. Transaction No." := P800UtilityFns.GetNextTransNo;
        ;
        Clear(QtyAlt);

        with ItemLedgerEntry do begin
            SetRange("Document No.", pPurchRcptLine."Document No.");
            SetRange("Document Line No.", pPurchRcptLine."Line No.");
            SetRange("Document Type", ItemLedgerEntry."Document Type"::"Purchase Receipt");
            if FindSet then
                repeat
                    QtyAlt := ("Quantity (Alt.)" - "Invoiced Quantity (Alt.)"); // P80080636
                    if QtyAlt <> 0 then begin
                        TotQtyAlt += QtyAlt;
                        LineNum += 10000;
                        AltQtyMgmt.CreateAltQtyLine(AlternateQuantityLine, pPurchaseLine."Alt. Qty. Transaction No.", LineNum, DATABASE::"Purchase Line",
                            pPurchaseLine."Document Type", pPurchaseLine."Document No.", '', '', pPurchaseLine."Line No.");
                        AlternateQuantityLine."Quantity (Base)" := ItemLedgerEntry.Quantity - ItemLedgerEntry."Invoiced Quantity";
                        AlternateQuantityLine.Quantity := Round(AlternateQuantityLine."Quantity (Base)" / pPurchaseLine."Qty. per Unit of Measure", 0.00001);
                        AlternateQuantityLine."Quantity (Alt.)" := QtyAlt;
                        AlternateQuantityLine."Lot No." := ItemLedgerEntry."Lot No.";
                        AlternateQuantityLine."Serial No." := ItemLedgerEntry."Serial No.";
                        AlternateQuantityLine.Modify;
                    end;
                until Next = 0;
        end;

        pPurchaseLine.Validate("Qty. to Receive (Alt.)", TotQtyAlt);
        AltQtyTracking.UpdatePurchTracking(PurchLine);
        pPurchaseLine.Modify;
    end;
