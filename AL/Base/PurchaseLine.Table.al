table 39 "Purchase Line" // Version: FOODNA
{
    // --------------------------------------------------------------------------------
    ...
    // 
    // PR1.00
    //   Default Item No. Lookup restricted to raw material and packaging items
    //   Modify validation on Qty. to Receive
    //   New Process 800 fields
    //     Original Quantity
    // 
    // PR1.00.01
    //   Item No. Lookup change to filter on Requisition System
    // 
    // PR1.00.02
    //   No. - only set quantity if line number is unchanged
    // 
    // PR1.10
    //   OnDelete - remove records in Purchase Lot Specification
    //   New Fields
    //     Vendor Lot No.
    //     Lot Specifications
    // 
    ...
    //   Remove Vendor Lot No, Lot Specifications
    // 
    // PR2.00.01
    ...
    //     Qty. to Invoice (Alt.)
    //     Qty. Invoiced (Alt.)
    //     Return Qty. to Ship (Alt.)
    //     Return Qty. Shipped (Alt.)
    //   Move No. OnLookup to function to be called on forms
    // 
    // PR3.60.02
    //   Suppress backorder prompt when Qty. to Receive set though UpdateWithWarehouseReceive
    // 
    // PR3.60.03
    //   Fix problems with Qty. to Receive on fixed weight items
    // 
    // PR3.70
    ...
    //   New Function
    //     ShowExtraCharges, ExtraChargeUnitCost
    //   Added "Quantity (Alt.)", "Qty. Received (Alt.)" as sum index fields to key
    //     Document Type,Type,No.,Variant Code,Drop Shipment,Location Code,Expected Receipt Date
    // 
    // PR3.70.02
    //   Fix permission error on extra charges
    // 
    // PR3.70.03
    //   Accruals
    //   Fix bug in DeleteExtraCharges
    // 
    // PR3.70.04
    ...
    //   fix problem with line discount when over/under receiving
    // 
    ...
    //   Add key - Document Type,Buy-from Vendor No.,Type
    // 
    // PR4.00.03
    // P8000325A, VerticalSoft, Jack Reynolds, 01 MAY 06
    //   Modifiy SP2 changes for alternate quantities
    // 
    ...
    //   Add fields for recording purchases against a work order
    // 
    ...
    //   Fix problem with setting qty. to receive to zero and no backorder
    // 
    // PR4.00.06
    ...
    // 
    // PRW15.00.01
    // P8000569A, VerticalSoft, Jack Reynolds, 13 FEB 08
    //   Division by zero issues with costing quantity
    // 
    ...
    //   Add missing alternate quantity code to InitQtyToReceive2
    // 
    // PRW15.00.03
    ...
    //   Fix problem setting quantity to receive
    // 
    // P8000694, VerticalSoft, Jack Reynolds, 04 MAY 09
    //   Change name of Accrual fields
    // 
    // P8000715, VerticalSoft, Jack Reynolds, 06 AUG 09
    //   Fix problem with over receipts from Warehouse receipt
    // 
    // P8000727, VerticalSoft, Jack Reynolds, 01 SEP 09
    //   Over/Under receive warnings only for orders
    // 
    // PRW16.00.02
    // P8000756, VerticalSoft, Jack Reynolds, 04 JAN 10
    //   UpdateAmounts - prepayments and alternate quantities
    // 
    // P8000758, VerticalSoft, Jack Reynolds, 18 JAN 10
    //   Fix problem with item tracking entries not being created
    // 
    // PRW16.00.03
    ...
    //   Added the Production Date field for Freshness
    // 
    ...
    // 
    // P8000929, Columbus IT, Don Bresee, 08 APR 11
    //   Skip automatic lot assignment for Commodity Manifest lines
    // 
    ...
    // 
    // P8001133, Columbus IT, Jack Reynolds, 28 MAR 13
    //   Updated for dimension sets
    // 
    // PRW17.00.01
    // P8001189, Columbus IT, Jack Reynolds, 07 AUG 13
    //   Fix problem with multiple lots and ADC Receiving
    // 
    // PRW17.10
    ...
    //   Fix problem with backorder confirmation on Qty. to Receive
    // 
    // P8001297, Columbus IT, Jack Reynolds, 25 FEB 14
    //   Fix problem with over receiving with ADC Warehouse Receive
    // 
    // PRW17.10.03
    ...
    //   FOOD-TOM Separation
    // 
    // P8007526, To-Increase, Jack Reynolds, 08 SEP 16
    //   Fix problem calculating unit cost (UpdateDirectUnitCost)
    // 
    ...
    // 
    // PRNA10.0.01
    // P80039744, To-Increase, Jack Reynolds, 30 MAY 17
    //   Fix issues that order must be open to add container lines
    // 
    // PRW110.0.02
    ...
    // 
    // P80082100, To-Increase, Gangabhushan, 16 SEP 19
    //   CS00075140 - Error in table 39 Code (when you change the quantity on a Purchase Invoice Line, the line amount is not changing)
    // 
    ...
    fields
    {
        ...
        field(5; Type; Enum "Purchase Line Type")
        {
            ...
            begin
                ...
                TestCommodityManifest(FieldCaption(Type)); // P8000891
                ...
                if ProcessFns.AccrualsInstalled() then
                    AccrualMgmt.PurchDeleteLines(Rec);
                // P8000044A

                TempPurchLine := Rec;
                DeleteExtraCharges; // PR3.70.01
                Init();
        ...
        field(6; "No."; Code[20])
        {
            ...
            begin
                ...
                TestCommodityManifest(FieldCaption("No.")); // P8000891
                ...
                // P8000044A, P8000119A
                if ProcessFns.AccrualsInstalled() then
                    AccrualMgmt.PurchBeginRecalcLines(Rec);
                // P8000044A
                ...
                // P8000241A
                if ProcessFns.AccrualsInstalled() then
                    AccrualFldMgmt.PurchLineValidate(FieldNo("No."), xRec, Rec);
                // P8000241A
                ...
                // P8000044A, P8000119A
                if ProcessFns.AccrualsInstalled() then
                    AccrualMgmt.PurchEndRecalcLines(Rec);
                // P8000044A
        ...
        field(7; "Location Code"; Code[10])
        {
            ...
            begin
                ...
                if (Type in [Type::Item, Type::FOODContainer]) then
                    if "Location Code" <> xRec."Location Code" then begin
                        GetItem(Item);
                ...
                UpdateDates();
                //P8001373
                if Type = Type::FOODContainer then
                    ContainerFns.EditPurchaseLineLocation(Rec);
                //P8001373
        ...
        field(15; Quantity; Decimal)
        {
            ...
            begin
                ...
                P800CoreFns.CheckPurchaseLineFieldEditable(Rec, FieldNo(Quantity), CurrFieldNo);  //P8001373
                TestStatusOpen();

                TestCommodityManifest(FieldCaption(Quantity)); // P8000891
                ...
                // P8001324
                    ...
                    if (CurrFieldNo = FieldNo(Quantity)) and (Type = Type::FOODContainer) and ("Document Type" = "Document Type"::Order) then begin // P80082100
                        if "Outstanding Quantity" < GetContainerQuantity('') then
                            Error(Text37002015, FieldCaption("Outstanding Quantity")); // P80082100
                ...
                if not (CurrFieldNo in [FieldNo("Qty. to Receive"), FieldNo("Over-Receipt Quantity")]) then
                UpdateWithWarehouseReceive();
                if (xRec.Quantity <> Quantity) or (xRec."Quantity (Base)" <> "Quantity (Base)") then begin
                    ...
                    IsHandled := "Allow Quantity Change";  // P8000715
        ...
        field(17; "Qty. to Invoice"; Decimal)
        {
            ...
            begin
                ...
                if "Qty. to Invoice" = MaxQtyToInvoice then
                    ...
                    "Qty. to Invoice (Base)" := CalcBaseQty("Quantity Invoiced" + "Qty. to Invoice", FieldCaption("Qty. to Invoice"), FieldCaption("Qty. to Invoice (Base)")) - "Qty. Invoiced (Base)";
                    // P8000550A
                ...
                if TrackAlternateUnits then           // PR3.60
                    AltQtyMgmt.SetPurchLineAltQty(Rec); // PR3.60
                ...
                //UpdateLotTracking(FALSE); // P8000043A, P8000342A
            end;
        } 
        field(18; "Qty. to Receive"; Decimal)
        {
            ...
            var
                ...
                ContQtyToReceive: Decimal;
                ContQtyToReceiveBase: Decimal;
                ContQtyToReceiveAlt: Decimal;
            begin
                // P80070336
                if ((CurrFieldNo = FieldNo("Qty. to Receive")) and CostInAlternateUnits) then
                    TestStatusOpen;
                // P80070336
                ...
                if CurrFieldNo <> 0 then // P80046533
                    ...
                    if (Type = Type::FOODContainer) then begin
                        if "Qty. to Receive" < GetContainerQuantity(true) then // P80046533
                            Error(Text37002015, FieldCaption("Qty. to Receive"));
                    end else
                        if GetContainerQuantity(true) > 0 then // P80046533
                            if "Qty. to Receive" < GetContainerQuantity(true) then // P80046533
                                FieldError("Qty. to Receive", Text37002103);
                // P8001323

                if "Qty. to Receive" = Quantity - "Quantity Received" - GetContainerQuantity('') then begin // P80046533
                    IsHandled := false;
		            ...
		            "Qty. to Receive (Base)" := CalcBaseQty("Quantity Received" + "Qty. to Receive", FieldCaption("Qty. to Receive"), FieldCaption("Qty. to Receive (Base)")) - "Qty. Received (Base)";
                    // P8000550A
                    ...
                    if (Type = Type::Item) and ("No." <> '') and TrackAlternateUnits then
                        ...
                        //                             "Qty. to Receive" * "Qty. per Unit of Measure", "Qty. to Receive (Alt.)");
                        AltQtyMgmt.InitAlternateQtyToHandle(
                      "No.", "Alt. Qty. Transaction No.", "Quantity (Base)", "Qty. to Receive (Base)",
        ...
        field(22; "Direct Unit Cost"; Decimal)
        {
            ...
            begin
                ...
                // P8000044A, P8000119A
                if ProcessFns.AccrualsInstalled() then
                    AccrualMgmt.PurchBeginNewCostLines(Rec);
                // P8000044A

                P800CoreFns.CheckPurchaseLineFieldEditable(Rec, FieldNo("Direct Unit Cost"), CurrFieldNo); //P8001373
                Validate("Line Discount %");

                // P8000044A, P8000119A
                if ProcessFns.AccrualsInstalled() then
                    AccrualMgmt.PurchEndNewCostLines(Rec, (CurrFieldNo = FieldNo("Direct Unit Cost")) or SettingUnitPrice); // P8006632
                // P8000044A
            end;
        }
        ...
        field(28; "Line Discount Amount"; Decimal)
        {
            ...
            begin
                ...
                // PR3.70.03
                if ProcessFns.AccrualsInstalled() then
                    AccrualMgmt.PurchRecalcLines(Rec); // P8000044A
                // PR3.70.03
                                ...
                                "Amount Including VAT" :=
                                  ...
                                  Round(
                                    SalesTaxCalculate.CalculateTax(
                                      ...
                                      "VAT Base Amount", GetCostingQuantity(FieldNo(Quantity), 'BASE'), PurchHeader."Currency Factor"), // P8001366
                                ...
                                Amount :=
                                  Round(
                                    SalesTaxCalculate.ReverseCalculateTax(
                                      "Tax Area Code", "Tax Group Code", "Tax Liable", PurchHeader."Posting Date",
                                      "Amount Including VAT", GetCostingQuantity(FieldNo(Quantity), 'BASE'), PurchHeader."Currency Factor"), // P8001366
                                    Currency."Amount Rounding Precision");
        ...
        field(103; "Line Amount"; Decimal)
        {
            ...
            begin
                ...
                MaxLineAmount := Round(GetCostingQty * "Direct Unit Cost", Currency."Amount Rounding Precision");
        ...
        field(110; "Prepmt. Line Amount"; Decimal)
        {
            ...
            begin
                ...
                if "Prepmt. Line Amount" > "Line Amount" then
                    FieldError("Prepmt. Line Amount", StrSubstNo(Text039, LineAmtExclAltQtys())); // P8000466A
                Validate("Prepayment %", "Prepmt. Line Amount" * 100 / LineAmtExclAltQtys());     // P8000466A
            end;
        }
        ...
        field(5402; "Variant Code"; Code[10])
        {
            ...
            begin
                ...
                if CurrFieldNo = FieldNo("Variant Code") then begin
                    GetPurchHeader;
                    TestApprovedItem(PurchHeader."Allow Unapproved Items");
                end;
                // P8001230

                TestCommodityManifest(FieldCaption("Variant Code")); // P8000891
                ...
                if Type in [Type::Item, Type::FOODContainer] then // P8001373
        ...
        field(5407; "Unit of Measure Code"; Code[10])
        {
            ...
            begin
                P800CoreFns.CheckPurchaseLineFieldEditable(Rec, FieldNo("Unit of Measure Code"), CurrFieldNo); //P8001373
                TestStatusOpen();
                TestField("Quantity Received", 0);
        ...
        field(5415; "Quantity (Base)"; Decimal)
        {
            ...
            var
                IsHandled: Boolean;
            begin
                P800CoreFns.CheckPurchaseLineFieldEditable(Rec, FieldNo("Quantity (Base)"), CurrFieldNo); //P8001373
                IsHandled := false;
        ...
        field(5803; "Return Qty. to Ship"; Decimal)
        {
            ...
            var
                IsHandled: Boolean;
            begin
                P800CoreFns.CheckPurchaseLineFieldEditable(Rec, FieldNo("Return Qty. to Ship"), CurrFieldNo); //P8001373
                // P80070336
                if ((CurrFieldNo = FieldNo("Return Qty. to Ship")) and CostInAlternateUnits) then
                    TestStatusOpen;
                // P80070336
                if (CurrFieldNo <> 0) and
                   (Type = Type::Item) and
                ...
                //P8001373
                if GetContainerQuantity(true) > 0 then // P80046533
                    if "Return Qty. to Ship" < GetContainerQuantity(true) then // P80046533
                        FieldError("Return Qty. to Ship", Text37002103);
                //P8001373

                OnValidateReturnQtyToShipOnAfterCheck(Rec, CurrFieldNo);

                if "Return Qty. to Ship" = Quantity - "Return Qty. Shipped" - GetContainerQuantity('') then // P80046533
                    InitQtyToShip()
                else begin
                    // P8000550A
                    ...
                    "Return Qty. to Ship (Base)" := CalcBaseQty("Return Qty. Shipped" + "Return Qty. to Ship", FieldCaption("Return Qty. to Ship"), FieldCaption("Return Qty. to Ship (Base)")) - "Return Qty. Shipped (Base)";
                    // P8000550A
                    // PR3.60.03
                    if (Type = Type::Item) and ("No." <> '') and TrackAlternateUnits then
                        // P8000550A
                        // AltQtyMgmt.InitAlternateQty("No.", "Alt. Qty. Transaction No.",
                        //                             "Return Qty. to Ship" * "Qty. per Unit of Measure", "Return Qty. to Ship (Alt.)");
                        AltQtyMgmt.InitAlternateQtyToHandle(
                      "No.", "Alt. Qty. Transaction No.", "Quantity (Base)",
                      "Return Qty. to Ship (Base)", "Return Qty. Shipped (Base)",
        ...
        field(5804; "Return Qty. to Ship (Base)"; Decimal)
        {
            ...
            begin
                ...
                P800CoreFns.CheckPurchaseLineFieldEditable(Rec, FieldNo("Return Qty. to Ship (Base)"), CurrFieldNo); //P8001373
                TestField("Qty. per Unit of Measure", 1);
        ...
        field(37002020; "Lot No."; Code[50])
        {
            ...
            begin
                ...
                // P80079756
                if ("Lot No." = '') then begin    // P8001106, P8008351
                    "Supplier Lot No." := ''; // P8001106
                    "Creation Date" := 0D;                 // P8008351
                    "Country/Region of Origin Code" := ''; // P8008351
                end;                                     // P8008351
        ...
        field(37002021; "Creation Date"; Date)
        {
            ...
            begin
                // P8008351
                if ("Lot No." = P800Globals.MultipleLotCode) then
                    FieldError("Creation Date", Text37002003);
        ...
        field(37002026; "Supplier Lot No."; Code[50])
        {
            ...
            begin
                // P8001106
                if ("Lot No." = P800Globals.MultipleLotCode) then // P8008351
                    FieldError("Supplier Lot No.", Text37002003);
                if ("Supplier Lot No." <> '') then
                    TestField("Lot No.");
                if "Line No." <> 0 then begin
                    Modify;
                    UpdateLotTracking(false);
        ...
        field(37002081; "Quantity (Alt.)"; Decimal)
        {
            ...
            begin
                // P8000044A
                if ProcessFns.AccrualsInstalled() then
                    AccrualMgmt.PurchRecalcLines(Rec);
                // P8000044A
            end;
        }
        field(37002082; "Qty. to Receive (Alt.)"; Decimal)
        {
            AccessByPermission = TableData "Alternate Quantity Line" = R;
            ...
            begin
                // PR3.60
                AltQtyMgmt.TestPurchAltQtyInfo(Rec, false);
                ...
                AltQtyMgmt.SetPurchLineAltQty(Rec);
                // PR3.60
        ...
        field(37002083; "Qty. Received (Alt.)"; Decimal)
        {
            AccessByPermission = TableData "Alternate Quantity Line" = R;
            BlankZero = true;
        ...
        field(37002086; "Return Qty. to Ship (Alt.)"; Decimal)
        {
            AccessByPermission = TableData "Alternate Quantity Line" = R;
            AutoFormatExpression = GetItemNo();
            AutoFormatType = 37002080;
            ...
            Caption = 'Return Qty. to Ship (Alt.)';
            Description = 'PR3.60';
            ...
            var
                Item: Record Item;
            begin
                P800CoreFns.CheckPurchaseLineFieldEditable(Rec, FieldNo("Return Qty. to Ship (Alt.)"), CurrFieldNo); //P8001373
                // PR3.60
                AltQtyMgmt.TestPurchAltQtyInfo(Rec, false);

                GetItem(Item); // P80066030
                if (CurrFieldNo = FieldNo("Return Qty. to Ship (Alt.)")) then begin
                    TestStatusOpen; // P80070336
                    Item.TestField("Catch Alternate Qtys.", true);
                    TestField("Return Qty. to Ship");
                    // TestStatusOpen; // P8000282A
                    AltQtyMgmt.CheckSummaryTolerance1("Alt. Qty. Transaction No.", "No.",
                                                     FieldCaption("Return Qty. to Ship (Alt.)"),
                                                     "Return Qty. to Ship (Base)", "Return Qty. to Ship (Alt.)");
                end;
        ...
        field(37002087; "Return Qty. Shipped (Alt.)"; Decimal)
        {
            AccessByPermission = TableData "Alternate Quantity Line" = R;
            BlankZero = true;
            CaptionClass = StrSubstNo('37002080,1,2,%1,%2', Type, "No.");
            Caption = 'Return Qty. Shipped (Alt.)';
            DecimalPlaces = 0 : 5;
            Description = 'PR3.60';
            Editable = false;
        }
        ...
        {
            CalcFormula = Sum ("Document Accrual Line"."Payment Amount (LCY)" WHERE("Accrual Plan Type" = CONST(Purchase),
        ...
        field(37002126; "Accrual Source No."; Code[20])
        {
            ...
            trigger OnValidate()
            begin
                AccrualFldMgmt.PurchLineValidate(FieldNo("Accrual Source No."), xRec, Rec); // P8000241A
            end;
        }
        field(37002127; "Accrual Source Doc. Type"; Option)
        {
            ...
            trigger OnValidate()
            begin
                AccrualFldMgmt.PurchLineValidate(FieldNo("Accrual Source Doc. Type"), xRec, Rec); // P8000241A
            end;
        }
        field(37002128; "Accrual Source Doc. No."; Code[20])
        {
            ...
            trigger OnValidate()
            begin
                AccrualFldMgmt.PurchLineValidate(FieldNo("Accrual Source Doc. No."), xRec, Rec); // P8000241A
            end;
        }
        field(37002129; "Accrual Source Doc. Line No."; Integer)
        {
            ...
            trigger OnValidate()
            begin
                AccrualFldMgmt.PurchLineValidate(FieldNo("Accrual Source Doc. Line No."), xRec, Rec); // P8000241A
            end;
        }
        field(37002130; "Scheduled Accrual No."; Code[10])
        {
            ...
            trigger OnValidate()
            begin
                AccrualFldMgmt.PurchLineValidate(FieldNo("Scheduled Accrual No."), xRec, Rec); // P8002746
            end;
        }
        ...
        field(37002663; "Extra Charge"; Decimal)
        {
            ...
            AutoFormatExpression = "Currency Code";
            AutoFormatType = 1;
            CalcFormula = Sum ("Document Extra Charge".Charge WHERE("Table ID" = CONST(39),
                                                                    "Document Type" = FIELD("Document Type"),
                                                                    "Document No." = FIELD("Document No."),
                                                                    "Line No." = FIELD("Line No.")));
        ...
        field(37002664; "Country/Region of Origin Code"; Code[10])
        {
            ...
            begin
                // P8008351
                if ("Lot No." = P800Globals.MultipleLotCode) then
                    FieldError("Country/Region of Origin Code", Text37002003);
        ...
        field(37002688; "Comm. Payment Class Code"; Code[10])
        {
            ...
            begin
                // P8000902
                if not ("Commodity P.O. Type" in ["Commodity P.O. Type"::Producer, "Commodity P.O. Type"::Broker]) then
                    FieldError("Commodity P.O. Type");
                TestField("Comm. Payment Class Code");
                CommCostMgmt.CalcPurchLine(Rec, false);
            end;
        }
        field(37002689; "Commodity Cost Calculated"; Boolean)
        {
            Caption = 'Commodity Cost Calculated';
            Editable = false;
        ...
        field(37002690; "Commodity Unit Cost"; Decimal)
        {
            ...
            begin
                // P8000902
                GetGLSetup;
                "Commodity Amount" := Round(GetCostingQty() * "Commodity Unit Cost", GLSetup."Amount Rounding Precision");
            end;
        }
        field(37002691; "Commodity Amount"; Decimal)
        {
            AutoFormatType = 1;
            BlankZero = true;
            Caption = 'Commodity Amount';
            Editable = false;
        }
        field(37002692; "Commodity Received Lot No."; Code[50])
        {
            Caption = 'Commodity Received Lot No.';
            Editable = false;
        ...
        field(37002694; "Producer Zone Code"; Code[20])
        {
            ...
            begin
                // P8000902
                TestField("Commodity P.O. Type", "Commodity P.O. Type"::Hauler);
                TestField("Producer Zone Code");
                CommCostMgmt.CalcPurchLine(Rec, false);
            end;
        }
        ...
        {
            CalcFormula = Lookup ("Purchase Header"."Commodity Item No." WHERE("Document Type" = FIELD("Document Type"),
                                                                               "No." = FIELD("Document No.")));
        ...
        field(37002697; "Rejection Action"; Option)
        {
            ...
            begin
                ...
                if ("Rejection Action" > 0) then begin
                    ...
                    TestField("Commodity Rejected", true);
                end;
                Validate("Comm. Payment Class Code");
            end;
        }
        field(37002698; "Blended Comm. Unit Cost"; Decimal)
        {
            AutoFormatType = 2;
            BlankZero = true;
            CalcFormula = Lookup ("Purchase Header"."Blended Comm. Unit Cost" WHERE("Document Type" = FIELD("Document Type"),
                                                                                    "No." = FIELD("Document No.")));
            Caption = 'Blended Comm. Unit Cost';
            Editable = false;
            FieldClass = FlowField;
        }
        ...
        {
            CalcFormula = Sum ("Whse. Staged Pick Source Line"."Qty. Outstanding" WHERE("Source Type" = CONST(39),
        ...
        field(37002800; "Work Order No."; Code[20])
        {
            AccessByPermission = TableData "Work Order" = R;
            ...
            begin
                // P8000335A
                MaintFldMgmt.PurchLineValidate(FieldNo("Work Order No."), xRec, Rec);

                CreateDim(
                  DATABASE::"Work Order", "Work Order No.",
                  DimMgt.TypeToTableID3(Type), "No.",
                  DATABASE::Job, "Job No.",
                  DATABASE::"Responsibility Center", "Responsibility Center");
            end;
        }
        field(37002801; "Maintenance Entry Type"; Option)
        {
            AccessByPermission = TableData "Work Order" = R;
            Caption = 'Maintenance Entry Type';
            OptionCaption = ' ,,Stock,Nonstock,Contract';
            OptionMembers = " ",,Stock,Nonstock,Contract;

            trigger OnValidate()
            begin
                // P8000335A
                MaintFldMgmt.PurchLineValidate(FieldNo("Maintenance Entry Type"), xRec, Rec);
            end;
        }
        field(37002802; "Maintenance Trade Code"; Code[10])
        {
            AccessByPermission = TableData "Work Order" = R;
            ...
            begin
                // P8000335A
                MaintFldMgmt.PurchLineValidate(FieldNo("Maintenance Trade Code"), xRec, Rec);
            end;
        }
        field(37002803; Hours; Decimal)
        {
            AccessByPermission = TableData "Work Order" = R;
            Caption = 'Hours';
            DecimalPlaces = 0 : 5;
            ...
            begin
                // P8000335A
                MaintFldMgmt.PurchLineValidate(FieldNo(Hours), xRec, Rec);
            end;
        }
        field(37002804; "Part No."; Code[20])
        {
            AccessByPermission = TableData "Work Order" = R;
            Caption = 'Part No.';
            ...
            begin
                // P8000335A
                MaintFldMgmt.PurchLineValidate(FieldNo("Part No."), xRec, Rec);
            end;
        }
    ...
    keys
    {
        ...
        {
            SumIndexFields = "Outstanding Qty. (Base)", "Quantity (Alt.)", "Qty. Received (Alt.)";
        }
        ...
        key(Key15; "Commodity Manifest No.", "Commodity Manifest Line No.", "Commodity P.O. Type")
        {
        }
        key(Key16; "Document Type", "Document No.", "Commodity Cost Calculated")
        {
            SumIndexFields = "Quantity (Base)", "Commodity Amount";
        }
        key(Key17; Type, "No.", "Variant Code", "Commodity Received Lot No.")
        {
        }
        key(Key18; "Document Type", Type, "No.")
        {
            Enabled = false;
            SumIndexFields = "Outstanding Qty. (Base)";
        }
        key(Key19; "Recalculate Invoice Disc.")
        {
        }
        key(Key20; "Outstanding Quantity")
        {
        }
        key(Key21; "Location Code", "Quantity Invoiced")
        {
        }
    }
    ...
    begin
        ...
        if ("Commodity Manifest No." <> '') then                                               // P8000891
            Error(Text37002686, FieldCaption("Commodity Manifest No."), "Commodity Manifest No."); // P8000891
    ...
    begin
        ...
        // PR3.70.03, P8000119A
        if ProcessFns.AccrualsInstalled() then
            AccrualMgmt.PurchInsertLines(Rec);
        // PR3.70.03

        LockTable();
        PurchHeader."No." := '';

        if ProcessFns.FreshProInstalled then // PR3.70.02
            ExtraChargeMgt.InsertDocExtraCharge(DATABASE::"Purchase Line", "Document Type", "Document No.", "Line No."); // P8000928, P8001032
    ...
    begin
        ...
        if ((Type = Type::Item) and ("No." <> xRec."No.")) or "Drop Shipment" then // P80058334
            UpdateLotTracking("No." <> xRec."No."); // P8000758
    end;
    ...
    var
        ...
        Text37002000: Label 'Quantity to receive exceeds outstanding quantity.\\OK to proceed?';
        Text37002001: Label 'Outstanding quantity exceeds quantity to receive.\\Backorder remainder?';
        Text37002002: Label 'The update has been interrupted to respect the warning.';
        Text37002003: Label 'may not be edited';
        AccrualFldMgmt: Codeunit "Accrual Field Management";
        MaintFldMgmt: Codeunit "Maintenance Purchase Mgmt.";
        Text37002004: Label 'Item %1 is not approved for vendor %2.';
        Text37002685: Label 'You cannot change %1 when the purchase order line is associated to %2 %3.';
        Text37002686: Label 'You cannot delete the purchase order line when it is associated to %1 %2.';
        ...
        Text37002103: Label 'cannot be less than quantity assigned through containers';
        Text37002015: Label '%1 cannot be less than number of containers assigned.';
    ...
    begin
        ...
        then begin
            "Qty. to Receive" := "Outstanding Quantity" - GetContainerQuantity(''); // P80046533
    ...
    begin
        ...
        then begin
            "Return Qty. to Ship" := "Outstanding Quantity" - GetContainerQuantity(''); // P80046533
            "Return Qty. to Ship (Base)" := UOMMgt.CalcBaseQty("Return Qty. to Ship", "Qty. per Unit of Measure"); // P80046533
        end else
    ...
    begin
        ...
        if CurrFieldNo = FieldNo("No.") then                      // P8001230
            TestApprovedItem(PurchHeader."Allow Unapproved Items"); // P8001230
        ...
        if Type = Type::Item then // P8001375
            if "Document Type" = "Document Type"::Order then                       // P8001047
        ...
        if ProcessFns.TrackingInstalled then
            P800ItemTracking.SetDefaultCOO(PurchHeader, Rec, Item."Item Tracking Code");
        // P8000938
        InitDeferralCode();
    ...
    begin
	    ...
	    Type::FOODContainer,
            Type::Resource:
                begin
	            if Type = Type::FOODContainer then begin // P8007526
	                GetContainerType;
            	    ...
            	    if CalledByFieldNo <> 0 then begin // P80083775
                        if not IsHandled then begin
    ...
    begin
        ...
        // PR3.60
        if GetCostingQty <> 0 then // P8000569A
            DiscountAmountPerQty :=
              Round(("Line Discount Amount" + "Inv. Discount Amount") / GetCostingQty,
                GLSetup."Unit-Amount Rounding Precision");
        // PR3.60
        ...
        if ("Line Amount" = "Inv. Discount Amount") and not
            ...
            then begin
                ...
                if not PurchLine2.IsEmpty() then begin
                    PurchLine2.CalcSums(
                      "Line Amount", "Inv. Discount Amount", Amount, "Amount Including VAT", "Quantity (Base)", "Quantity (Alt.)", "VAT Difference", "Tax To Be Expensed");
                    ...
                    TotalQuantityBase := PurchLine2.GetCostingQuantity(FieldNo(Quantity), 'BASE'); // P8001366, P8001386
                            ...
                            if "Use Tax" then
                                ...
                                Amount :=
                                  Round(
                                    SalesTaxCalculate.ReverseCalculateTax(
                                      "Tax Area Code", "Tax Group Code", "Tax Liable", PurchHeader."Posting Date",
                                      TotalAmountInclVAT + "Amount Including VAT", TotalQuantityBase + GetCostingQuantity(FieldNo(Quantity), 'BASE'), // P8001366
                            ...
                            if "Use Tax" then
                                ...
                                "Amount Including VAT" :=
                                  ...
                                  Round(
                                    SalesTaxCalculate.CalculateTax(
                                      "Tax Area Code", "Tax Group Code", "Tax Liable", PurchHeader."Posting Date",
                                      TotalAmount + Amount, TotalQuantityBase + GetCostingQuantity(FieldNo(Quantity), 'BASE'), // P8001366
                                      PurchHeader."Currency Factor"),
                                    Currency."Amount Rounding Precision") -
    ...
    begin
        ...
        if CostInAlternateUnits then begin
            ...
            "Unit Price (LCY)" := Item."Unit Price"; // P8000325A
        end;
        // PR3.60
    ...
    begin
        ...
        if TrackAlternateUnits then begin
            Rec.Find('=');
            AltQtyMgmt.UpdatePurchLine(Rec);
            AltQtyMgmt.SetPurchLineAltQty(Rec);
        end;
        // PR3.60 End
        GetLotNo; // P8000043A
    ...
    begin
        ...
        if "Work Order No." <> '' then begin
            ...
            if WorkOrder."Dimension Set ID" <> 0 then begin
                DimensionSetIDArr[1] := "Dimension Set ID";
                DimensionSetIDArr[2] := WorkOrder."Dimension Set ID";
                "Dimension Set ID" :=
                    ...
                    then begin
                        ...
                        if VATAmountLine.Modified then begin
                            ...
                            if QtyType = QtyType::General then
                                ...
                                LineAmountToInvoice :=
                                  Round("Line Amount" * GetCostingQuantity(FieldNo("Qty. to Invoice"), '') / GetCostingQuantity(FieldNo(Quantity), ''), Currency."Amount Rounding Precision"); // P8001366
                ...
                repeat
                    if not ZeroAmountLine(QtyType) then begin
                        ...
                        case QtyType of
                            QtyType::General:
                                begin
                                    VATAmountLine.Quantity += GetCostingQuantity(FieldNo(Quantity), 'BASE'); // P8001366
                            ...
                            QtyType::Invoicing:
                                begin
                                    case true of
                                      ...
                                      (not PurchHeader.Receive) and PurchHeader.Invoice and (not "Prepayment Line"):
                                            if "Receipt No." = '' then begin
                                                QtyToHandle := GetAbsMin(GetCostingQuantity(FieldNo("Qty. to Invoice"), ''), GetCostingQuantity(FieldNo("Qty. Rcd. Not Invoiced"), '')); // P8001366
                                                VATAmountLine.Quantity += GetAbsMin(GetCostingQuantity(FieldNo("Qty. to Invoice"), 'BASE'), GetCostingQuantity(FieldNo("Qty. Rcd. Not Invoiced"), 'BASE')); // P8001366
                                            end else begin
                                                QtyToHandle := GetCostingQuantity(FieldNo("Qty. to Invoice"), ''); // P8001366
                                                VATAmountLine.Quantity += GetCostingQuantity(FieldNo("Qty. to Invoice"), 'BASE'); // P8001366
                                            end;
                                        IsCreditDocType and (not PurchHeader.Ship) and PurchHeader.Invoice:
                                            if "Return Shipment No." = '' then begin
                                                QtyToHandle := GetAbsMin(GetCostingQuantity(FieldNo("Qty. to Invoice"), ''), GetCostingQuantity(FieldNo("Return Qty. Shipped Not Invd."), '')); // P8001366
                                                VATAmountLine.Quantity += GetAbsMin(GetCostingQuantity(FieldNo("Qty. to Invoice"), 'BASE'), GetCostingQuantity(FieldNo("Return Qty. Shipped Not Invd."), 'BASE')); // P8001366
                                            end else begin
                                                QtyToHandle := GetCostingQuantity(FieldNo("Qty. to Invoice"), ''); // P8001366
                                                VATAmountLine.Quantity += GetCostingQuantity(FieldNo("Qty. to Invoice"), 'BASE'); // P8001366
                                            end;
                                    ...
                                    if PurchHeader."Invoice Discount Calculation" <> PurchHeader."Invoice Discount Calculation"::Amount then
                                        VATAmountLine.SumLine(
                                          AmtToHandle, Round("Inv. Discount Amount" * QtyToHandle / GetCostingQuantity(FieldNo(Quantity), ''), Currency."Amount Rounding Precision"), // P8001366
                            ...
                            QtyType::Shipping:
                                begin
                                    ...
                                    then begin
                                        QtyToHandle := GetCostingQuantity(FieldNo("Return Qty. to Ship"), ''); // P8001366
                                        VATAmountLine.Quantity += GetCostingQuantity(FieldNo("Return Qty. to Ship"), 'BASE'); // P8001366
                                    end else begin
                                        QtyToHandle := GetCostingQuantity(FieldNo("Qty. to Receive"), ''); // P8001366
                                        VATAmountLine.Quantity += GetCostingQuantity(FieldNo("Qty. to Receive"), 'BASE'); // P8001366
                                    end;
                                    AmtToHandle := GetLineAmountToHandleInclPrepmt(QtyToHandle);
                                    OnCalcVATAmountLinesOnBeforeVATAmountLineSumLine(Rec, VATAmountLine, QtyType);
                                    VATAmountLine.SumLine(
    ...
    begin
        ...
        if IsInventoriableItem() then
            case true of
                ("Document Type" in ["Document Type"::Quote, "Document Type"::Order]) and (Quantity >= 0):
                    begin                         // PR3.60.02
                        ...
                        if Location.RequireReceive("Location Code") then
                            Validate("Qty. to Receive", GetContainerQuantity(true)) // P80046533
                        else
                            Validate("Qty. to Receive", "Outstanding Quantity" - GetContainerQuantity(false)); // P80046533
                    end;                          // PR3.60.02
                ("Document Type" in ["Document Type"::Quote, "Document Type"::Order]) and (Quantity < 0):
                ...
                ("Document Type" = "Document Type"::"Return Order") and (Quantity >= 0):
                    if Location.RequireShipment("Location Code") then
                        Validate("Return Qty. to Ship", GetContainerQuantity(true))   // P8007152, P80046533
                    else
                        Validate("Return Qty. to Ship", "Outstanding Quantity" - GetContainerQuantity(false)); // P80046533
                ("Document Type" = "Document Type"::"Return Order") and (Quantity < 0):
                    if Location.RequireReceive("Location Code") then
        ...
        if not (CurrFieldNo in [FieldNo("Qty. to Receive"), FieldNo("Return Qty. to Ship")]) then // P8001132
        SetDefaultQuantity();
    ...
    begin
        ...
        if CostInAlternateUnits then
            exit(
              CurrExchRate.ExchangeAmtLCYToFCY(
                GetDate, "Currency Code", "Overhead Rate", PurchHeader."Currency Factor"));
        // PR3.60
    ...
    begin
        ...
        if (not (Type in [Type::Item, Type::FOODContainer])) or IsNonInventoriableItem()  then //P8001373
            exit;
    ...
    begin
        ...
        if ("Qty. to Invoice" <> 0) and ("Prepmt. Amt. Inv." <> 0) then begin
            ...
            if ("Prepayment %" = 100) and not IsFinalInvoice then
                "Prepmt Amt to Deduct" := GetLineAmountToHandle(GetCostingQuantity(FieldNo("Qty. to Invoice"), '')) - "Inv. Disc. Amount to Invoice" // P8001366
            else
                "Prepmt Amt to Deduct" :=
                  Round(
                    ("Prepmt. Amt. Inv." - "Prepmt Amt Deducted") *
                    GetCostingQuantity(FieldNo("Qty. to Invoice"), '')                                                                                         // P8001366
                     / (GetCostingQuantity(FieldNo(Quantity), '') - GetCostingQuantity(FieldNo("Quantity Invoiced"), '')), Currency."Amount Rounding Precision") // P8001366
        end else
    ...
    begin
        ...
        if PurchSetup."Default Qty. to Receive" = PurchSetup."Default Qty. to Receive"::Blank then begin
            if (("Document Type" = "Document Type"::Order) and ("Over-Receipt Quantity" = 0)) or ("Document Type" = "Document Type"::Quote) then begin
                GetContainerQuantities("Qty. to Receive", "Qty. to Receive (Base)", "Qty. to Receive (Alt.)", false, true); // P8004338, P80046533
                "Qty. to Invoice" := "Qty. to Receive";
                "Qty. to Invoice (Base)" := "Qty. to Receive (Base)";
                "Qty. to Invoice (Alt.)" := "Qty. to Receive (Alt.)"; // P8000808
            end;
            if "Document Type" = "Document Type"::"Return Order" then begin
                // P8001373
                //"Return Qty. to Ship" := 0;
                //"Return Qty. to Ship (Base)" := 0;
                //"Return Qty. to Ship (Alt.)" := 0; // P8000808
                GetContainerQuantities("Return Qty. to Ship", "Return Qty. to Ship (Base)", "Return Qty. to Ship (Alt.)", false, true); // P80046533
                "Qty. to Invoice" := "Return Qty. to Ship";
                "Qty. to Invoice (Base)" := "Return Qty. to Ship (Base)";
                "Qty. to Invoice (Alt.)" := "Return Qty. to Ship (Alt.)"; // P8000808
            end;
        end;
    ...
    begin
        if Type = Type::" " then
            exit(true);
        if GetCostingQuantity(FieldNo(Quantity), '') = 0 then            // P8006835
            exit(true);
        if "Direct Unit Cost" = 0 then
            exit(true);
        if QtyType = QtyType::Invoicing then
            if GetCostingQuantity(FieldNo("Qty. to Invoice"), '') = 0 then // P8006835
                exit(true);
        exit(false);
    end;
    ...
    begin
        ...
        case Type of
            ...
            Type::FOODAccrualPlan:
                exit(AccrualFldMgmt.DocumentLineLookupNo(Text)); // P8000241A
                                                                 //P8001373
            Type::FOODContainer:
                begin
    ...
    procedure GetCostingQuantity(FldNo: Integer; Base: Code[4]): Decimal
    begin
        // P8001366
        if CostInAlternateUnits then begin
            case FldNo of
                ...
                FieldNo("Return Qty. Shipped"):
                    exit("Return Qty. Shipped (Alt.)");
                FieldNo("Qty. to Invoice"):
                    exit("Qty. to Invoice (Alt.)");
                FieldNo("Quantity Invoiced"):
                    exit("Qty. Invoiced (Alt.)");
                FieldNo("Qty. Rcd. Not Invoiced"):
                    exit("Qty. Received (Alt.)" - "Qty. Invoiced (Alt.)");
                FieldNo("Return Qty. Shipped Not Invd."):
                    exit("Return Qty. Shipped (Alt.)" - "Qty. Invoiced (Alt.)");
            end;
        end else
            if Base = 'BASE' then begin
                case FldNo of
                    ...
                    FieldNo("Qty. Rcd. Not Invoiced"):
                        exit("Qty. Rcd. Not Invoiced (Base)");
                    FieldNo("Return Qty. Shipped Not Invd."):
                        exit("Ret. Qty. Shpd Not Invd.(Base)");
                end;
            end else begin
                case FldNo of
                    FieldNo(Quantity):
                    ...
                    FieldNo("Qty. Rcd. Not Invoiced"):
                        exit("Qty. Rcd. Not Invoiced");
                    FieldNo("Return Qty. Shipped Not Invd."):
                        exit("Return Qty. Shipped Not Invd.");
                end;
            end;
    end;
    ...
    begin
        ...
        if "Line No." <> 0 then
            ExtraChargeMgt.ShowExtraCharge(DATABASE::"Purchase Line",     // P8001032
              "Document Type", "Document No.", "Line No.", ShortcutECCharge) // P8000928
        else
    ...
    begin
        ...
        if "Line No." <> 0 then begin
            ExtraChargeMgt.SaveExtraCharge(DATABASE::"Purchase Line",        // P8001032
              "Document Type", "Document No.", "Line No.", FieldNumber, Charge); // P8000928
    ...
    begin
        // PR3.70.01
        CalcFields("Extra Charge"); // PR3.70.02
        if CostInAlternateUnits then begin
    ...
    begin
        CalcFields("Extra Charge"); // PR3.70.02
        exit("Line Amount" + "Extra Charge"); // PR3.70.01
    end;
    ...
    begin
        ...
        if ProcessFns.TrackingInstalled then begin
            ...
            "Supplier Lot No." := EasyLotTracking.GetSupplierLotNo("Lot No."); // P8001106
            "Creation Date" := EasyLotTracking.GetLotCreationDate("Lot No.");                 // P8008351
            "Country/Region of Origin Code" := EasyLotTracking.GetCountryOfOrigin("Lot No."); // P8008351
        end;
    end;
    ...
    begin
        // P8000043A
        // P80079197
        OnBeforeUpdateLotTracking(Rec, xRec, ForceUpdate, Handled);
        if Handled then
            exit;
        // P80079197
        ...
        if UseWhseLineQty then begin
            QtyBase := "Quantity (Base)"; // P800108979
            ...
            case "Document Type" of
                "Document Type"::Order, "Document Type"::Invoice:
                    if Location.LocationType = 1 then begin // P8000629A
                        ...
                        if not StartingADC then begin // P8001297
                                                      // P80077569
                            QtyToHandle := "Qty. to Receive (Base)";
                            QtyToHandleAlt := "Qty. to Receive (Alt.)";
                            QtyToInvoice := "Qty. to Invoice (Base)"; // P8000629A
                ...
                "Document Type"::"Credit Memo", "Document Type"::"Return Order":
                    if Location.LocationType = 1 then begin // P8000629A
                        ...
                        QtyToHandleAlt := "Return Qty. to Ship (Alt.)";
                        QtyToInvoice := "Qty. to Invoice (Base)"; // P8000629A
                                                                  // P8000629A
                    end else begin
                        // P80077569
        ...
        EasyLotTracking.SetSupplierLotNo(xRec."Supplier Lot No.", "Supplier Lot No."); // P8001106
        EasyLotTracking.SetLotCreationDate(xRec."Creation Date", "Creation Date");                                 // P8008351
        EasyLotTracking.SetCountryOfOrigin(xRec."Country/Region of Origin Code", "Country/Region of Origin Code"); // P8008351
        EasyLotTracking.ReplaceTracking(xRec."Lot No.", "Lot No.", "Alt. Qty. Transaction No.",
          QtyBase, QtyToHandle, QtyToHandleAlt, QtyToInvoice); // P8000629A, P8001189
    end;
    ...
    begin
        ...
        if PurchHeader."Commodity Manifest Order" then // P8000929
            exit;                                        // P8000929
        ...
        if Posting then begin
            ...
            //xPurchLine."Expected Receipt Date" := PurchHeader."Posting Date"; // P8001314
            xPurchLine := PurchLine;                                            // P8001314
        end else begin
            PurchLine."Expected Receipt Date" := 0D;
            xPurchLine."Expected Receipt Date" := 0D;
        end;
        if P800Tracking.AutoAssignLotNo(PurchLine, xPurchLine, "Lot No.") then begin
            // P8001234
        ...
        if (Type = Type::Item) and ("No." <> '') then
            if CostInAlternateUnits() then begin
                ...
                exit(Round(Quantity * ("Qty. per Unit of Measure" / ItemUOM."Qty. per Unit of Measure") *
                           "Direct Unit Cost" * (1 - "Line Discount %" / 100), Currency."Amount Rounding Precision"));
            end;
        exit("Line Amount");
    end;
    ...
    begin
        ...
        if TrackAlternateUnits then begin
            case "Document Type" of
                ...
                "Document Type"::"Credit Memo":
                    AltQtyMgmt.InitAlternateQtyToHandle(
                      "No.", "Alt. Qty. Transaction No.", "Quantity (Base)", "Return Qty. to Ship (Base)",
                      "Return Qty. Shipped (Base)", "Quantity (Alt.)", "Return Qty. Shipped (Alt.)", "Return Qty. to Ship (Alt.)");
            end;
            AltQtyMgmt.SetPurchLineAltQty(Rec);
        end;
    end;

    local procedure TestCommodityManifest(FldCaption: Text[50])
    begin
        // P8000891
        if (CurrFieldNo <> 0) and ("Commodity Manifest No." <> '') then
            Error(Text37002685, FldCaption, FieldCaption("Commodity Manifest No."), "Commodity Manifest No.");
    end;

    procedure CalcCommProductRejected()
    var
        PstdCommManifest: Record "Posted Comm. Manifest Header";
        CommManifest: Record "Commodity Manifest Header";
    begin
        // P8000926
        case true of
            PstdCommManifest.Get("Commodity Manifest No."):
                "Commodity Rejected" := PstdCommManifest."Product Rejected";
            CommManifest.Get("Commodity Manifest No."):
                "Commodity Rejected" := CommManifest."Product Rejected";
            else
                "Commodity Rejected" := false;
        end;
    end;

    procedure ADCStarting(NewStartingADC: Boolean)
    begin
        // P8001189
        StartingADC := NewStartingADC;
    end;

    procedure TestApprovedItem(AllowUnapprovedItems: Boolean)
    var
        Item: Record Item;
    ...
    begin
        ...
        if Item.VendorApprovalRequired then begin
            ...
            ItemVendor.SetRange("Variant Code", "Variant Code");
            ItemVendor.SetRange(Approved, true);
            if ItemVendor.IsEmpty then
                Error(Text37002004, "No.", "Buy-from Vendor No.");
        end;
    end;
    ...
    begin
        //P8001373
        ContainerFns.ContainersFromDocument(Rec);
        //P8001373
    end;
    ...
    begin
        ...
        case Type of
            Type::FOODContainer:
                begin
                    QtyToHandle := ContainerFns.GetContainerCount(DATABASE::"Purchase Line", "Document Type", "Document No.", "Line No.", ExcludeWhseDoc, ShipReceive);
    ...
    begin
        ...
        if Round(Quantity * "Direct Unit Cost", Currency."Amount Rounding Precision") <> 0 then begin
            LineDiscountPct := Round(
                "Line Discount Amount" / Round(GetCostingQty * "Direct Unit Cost", Currency."Amount Rounding Precision") * 100, // P80066030
                0.00001);
    ...
    local procedure BackorderProcessing()
    begin
        if CurrFieldNo <> 0 then
            if "Document Type" = "Document Type"::Order then // P8000727
                if (((("Qty. to Receive" < 0) xor (Quantity < 0)) and (Quantity <> 0) and ("Qty. to Receive" <> 0)) or // P8001213, P8001296
                    ((Abs("Qty. to Receive") + GetContainerQuantity(false)) < Abs("Outstanding Quantity")) or // P80046533
                ...
                then begin
                    if not Confirm(Text37002001, true) then begin
                        SuspendStatusCheck(true);
                        Validate(Quantity, Quantity + "Qty. to Receive" + GetContainerQuantity(false) - "Outstanding Quantity"); // P80046533
    ...
    begin
        ...
        if "Line Amount" <> Round(GetCostingQty * "Direct Unit Cost", Currency."Amount Rounding Precision") - "Line Discount Amount" then begin // PR3.70
            "Line Amount" :=
              Round(GetCostingQty * "Direct Unit Cost", Currency."Amount Rounding Precision") - "Line Discount Amount"; // PR3.70
            "VAT Difference" := 0;
            LineAmountChanged := true;
        end;
    end;
                ...
                "Document Type"::"Return Order":
                    begin
                        ...
                        if WarehouseShipmentLine.FindFirst then
                            BinCode := WarehouseShipmentLine."Bin Code";
                    end;
            end;
        end;
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeUpdateLotTracking(var PurchLine: Record "Purchase Line"; var xPurchLine: Record "Purchase Line"; ForceUpdate: Boolean; var Handled: Boolean)
    begin
        // P80079197
    end;
