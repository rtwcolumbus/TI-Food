report 1306 "Standard Sales - Invoice"
{
    ...
    //   Adjust for alternate quantity
    // 
    // PRW111.00
    // P80053245, To Increase, Jack Reynolds, 23 MAR 18
    //   Upgrade for NAV 2018
    RDLCLayout = './layout/StandardSalesInvoice.rdlc';
    WordLayout = './layout/StandardSalesInvoice.docx';
    ...
    procedure GetUnitofMeasure(var SalesInvoiceLine: Record "Sales Invoice Line")
    var
        Item: Record Item;
        UnitOfMeasure: Record "Unit of Measure";
    ...
    begin
        // P8001361
        if (SalesInvoiceLine.Type = SalesInvoiceLine.Type::Item) and (SalesInvoiceLine."No." <> '') then begin
            Item.Get(SalesInvoiceLine."No.");
            if not Item.PriceInAlternateUnits then
                exit;

            if UnitOfMeasure.Get(Item."Alternate Unit of Measure") then;
            SalesInvoiceLine."Unit of Measure" := UnitOfMeasure.Description;
            if Header."Language Code" <> '' then begin
                ...
                if UnitOfMeasureTranslation.Find('-') then
                    SalesInvoiceLine."Unit of Measure" := UnitOfMeasureTranslation.Description;
            end;
        end;
    end;
