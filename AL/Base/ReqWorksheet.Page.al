page 291 "Req. Worksheet"
{
    ...
    //   Modified to show information about current item/SKU (availability, usage history and projection,
    //     vendors, planning parameters)
    // 
    // PRW16.00
    ...
    layout
    {
        area(content)
        {
            group(Control37002006)
            {
                ...
                {
                    ShowCaption = false;
                    field(BegEndDateText; BegEndDateText)
                    {
            ...
            repeater(Control1)
            {
                ...
                field("Vendor No."; "Vendor No.")
                {
                    ...
                    begin
                        // P8000312A
                        if LookupVendor(Vend, true) then begin // P80053245
                            Text := Vend."No.";
                            exit(true);
                        end else
                            exit(false);
    ...
    actions
    {
        area(navigation)
        {
            group("&Line")
            {
                ...
                action("Purchase Entries")
                {
                    ApplicationArea = FOODBasic;
                    Caption = 'Purchase Entries';
                    Image = Entries;
            ...
            group("F&unctions")
            {
                ...
                action(CalculatePlan)
                {
                    ...
                    begin
                        ...
                        ReqWkshName.LoadItemView(Item);
                        CalculatePlan.SetTableView(Item);
                        // P8000312A
                ...
                action(CarryOutActionMessage)
                {
                    ...
                    begin
                        ...
                        // P8000314A
                        if PurchHeader.Get(PurchHeader."Document Type"::Order, PerformAction.GetFirstOrder) then
    ...
    procedure GetDateVars()
    var
        ReqWkshName: Record "Requisition Wksh. Name";
    begin
        // P8000312A
        FilterGroup(2);
        ReqWkshName.Get(GetFilter("Worksheet Template Name"), GetFilter("Journal Batch Name"));
        FilterGroup(0);
        BegDate := ReqWkshName."Last Date";
        EndDate := ReqWkshName."End Date";
        if BegDate = 0D then
            BegDate := WorkDate;
        if EndDate = 0D then
            EndDate := WorkDate;
        if (BegDate <> 0D) and (EndDate <> 0D) then begin
            DaysView := EndDate - BegDate;
            BegEndDateText := StrSubstNo('%1..%2', BegDate, EndDate);
        end else begin
            DaysView := 0;
            BegEndDateText := '';
        end;
