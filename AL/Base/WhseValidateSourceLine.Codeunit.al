codeunit 5777 "Whse. Validate Source Line"
{
    ...
    // 
    // PRW16.00.05
    // P8000976, Columbus IT, Jack Reynolds, 07 SEP 11
    //   Fix problem validating quantity on consumption journal lines
    // 
    // PRW19.00.01
    ...
    var
        ...
        N138WhseShptLine: Record "Warehouse Shipment Line";
    ...
    begin
        ...
        with NewSalesLine do begin
            ...
            if not SkipQtyCheck then //N138F0000.n, P80066030
    ...
    begin
        ...
        with NewServiceLine do begin
            ...
            VerifyFieldNotChanged(NewRecRef, OldRecRef, FieldNo("No."));
            VerifyFieldNotChanged(NewRecRef, OldRecRef, FieldNo("Location Code"));
            if not SkipQtyCheck then //N138F0000.n
                VerifyFieldNotChanged(NewRecRef, OldRecRef, FieldNo(Quantity));
            VerifyFieldNotChanged(NewRecRef, OldRecRef, FieldNo("Variant Code"));
            VerifyFieldNotChanged(NewRecRef, OldRecRef, FieldNo("Unit of Measure Code"));
        end;
            ...
            case "Entry Type" of
                ...
                "Entry Type"::Consumption:
                    begin
                        ...
                        if not Ishandled then
                        if Location.Get("Location Code") and Location."Require Pick" and Location."Require Shipment" and // P8000976
                          Location."Require Production Picking"                                                          // P8000976
                        then                                                                                             // P8000976
    ...
    procedure N138GetWhseShipmentLine(var ChangeShipmentLine: Boolean; var CurrWhseShptLine: Record "Warehouse Shipment Line")
    begin
        ChangeShipmentLine := SkipQtyCheck;
        CurrWhseShptLine := N138WhseShptLine;
    end;
