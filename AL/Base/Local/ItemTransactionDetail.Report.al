report 10136 "Item Transaction Detail" // Version: FOODNA
{
    // PR4.00
    ...
    //   Add alternate quantity logic to PriorItemLedgerEntry data item
    // 
    // PRW16.00.03
    ...
    // 
    // PRW16.00.04
    // P8000860, VerticalSoft, Jack Reynolds, 24 AUG 10
    //   Fix overflow error on Decimal to Integer
    // 
    // PRNA7.10
    ...
    DefaultLayout = RDLC;
    RDLCLayout = './layout/local/ItemTransactionDetail.rdlc';
    ...
    dataset
    {
        dataitem(Item; Item)
        {
            ...
            column(QuantityOnHandAltStart; QuantityOnHandAltStart)
            {
                DecimalPlaces = 0 : 5;
            }
            ...
            dataitem("Item Ledger Entry"; "Item Ledger Entry")
            {
                ...
                column(QuantityAlt; "Quantity (Alt.)")
                {
                }
                column(QuantityOnHandAlt; QuantityOnHandAlt)
                {
                    DecimalPlaces = 0 : 5;
                }
                column(SumQuantityAlt; SumQuantityAlt)
                {
                    DecimalPlaces = 0 : 5;
                }
                ...
                begin
                    ...
                    //IF "Invoiced Quantity" <> Quantity THEN // P8000239A
                    if GetCostingInvQty <> GetCostingQty then // P8000239A
                        "Cost Amount (Actual)" := "Cost Amount (Expected)";

                    if ItemLedgerFilter = '' then begin
                        QuantityOnHand := QuantityOnHand + Quantity;
                        QuantityOnHandAlt := QuantityOnHandAlt + "Quantity (Alt.)"; // P8000239A
                        ValueOnHand := ValueOnHand + "Cost Amount (Actual)";
                    end;

                    SumQuantity := SumQuantity + Quantity;
                    SumQuantityAlt := SumQuantityAlt + "Quantity (Alt.)"; // P8000646
                ...
                begin
                    ...
                    then
                        SetCurrentKey("Item No.", "Variant Code", "Location Code", "Posting Date");
                    CurrReport.CreateTotals(Quantity, "Quantity (Alt.)", "Cost Amount (Actual)"); // P8000239A

                    SumQuantity := 0;
                    SumQuantityAlt := 0; // P8000646A
            ...
            begin
                if ItemLedgerFilter = '' then begin
                    ...
                    CalcFields("Net Change", "Net Change (Alt.)"); // P8000239A
                    QuantityOnHand := "Net Change";
                    QuantityOnHandAlt := "Net Change (Alt.)"; // P8000239A
                    ValueEntry.Reset();
                ...
                QuantityOnHandAltStart := QuantityOnHandAlt; // xxx
    ...
    labels
    {
        QtyOnHandAltCaption = 'Quantity on Hand (Alt.)';
        QtyAltCaption = 'Quantity (Alt.)';
    }
    ...
    var
        ...
        QuantityOnHandAlt: Decimal;
        QuantityOnHandAltCF: Decimal;
        QuantityOnHandAltStart: Decimal;
        SumQuantityAlt: Decimal;
}
