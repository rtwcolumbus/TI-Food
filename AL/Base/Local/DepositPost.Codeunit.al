codeunit 10140 "Deposit-Post" // Version: FOODNA
{
    // PR3.70.08
    ...
    begin
        ...
        if GenJnlLine.Find('-') then
            repeat
                ...
                MarkDeductions(GenJnlLine); // P8000170A, PR4.00
    ...
    begin
        // P8000170A
        if GenJnlLine.DeductionsExist then begin // PR4.00
            CustLedger.Find('+');
            PaymentEntry := CustLedger;
            PaymentEntry.Insert;
            DedGenJnlLine := GenJnlLine;           // PR4.00
    ...
    begin
        ...
        if PaymentEntry.Find('-') then begin
            DedGenJnlLine.Find('-'); // PR4.00
            DedMgt.LockTables;
            repeat
                DeductionLine.SetRange("Source Table No.", DATABASE::"Gen. Journal Line");       // PR4.00
                DeductionLine.SetRange("Source ID", DedGenJnlLine."Journal Template Name");      // PR4.00
                DeductionLine.SetRange("Source Batch Name", DedGenJnlLine."Journal Batch Name"); // PR4.00
                DeductionLine.SetRange("Source Ref. No.", DedGenJnlLine."Line No.");
                DedMgt.PostDeductions(PaymentEntry, DeductionLine, DedGenJnlLine."Posting Date"); // PR4.00, P8000920
                DedGenJnlLine.Next;                                                             // PR4.00
