#if not CLEAN19
report 10148 "List Price Sheet" // Version: FOODNA
{
    // PRNA5.00.01
    // P8000516A, VerticalSoft, Jack Reynolds, 13 SEP 07
    //   Modified to accomodate structural changes in Sales Price table that replaces Item No. by Item Type and Item Code 1
    // 
    // PRNA6.00.04
    // P8000854, VerticalSoft, Jack Reynolds, 05 AUG 10
    //   Fix problem with missing key on sales Price table
    
    DefaultLayout = RDLC;
    RDLCLayout = './layout/local/ListPriceSheet.rdlc';
    ...
    dataset
    {
        dataitem(Item; Item)
        {
            ...
            {
                DataItemLink = "Item Code" = FIELD("No."), "Variant Code" = FIELD("Variant Filter");
                DataItemTableView = SORTING("Sales Type", "Sales Code", "Item Type", "Item Code", "Starting Date", "Ending Date", "Currency Code", "Variant Code", "Unit of Measure Code", "Price Type", "Pricing Method", "Cost Reference", "Use Break Charge", "Minimum Quantity", "Maximum Quantity") WHERE("Item Type" = CONST(Item));
