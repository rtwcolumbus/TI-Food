...
{
    // PR3.60
    //   Fix handling of Family Line UOM
    // 
    // PR4.00
    ...
    // 
    // PR4.00.04
    // P8000377A, VerticalSoft, Jack Reynolds, 08 SEP 06
    //   Re-GET production order record prior to setting default production sequence
    // 
    // PRW15.00.01
    // P8000600A, VerticalSoft, Jack Reynolds, 14 MAY 08
    //   Fix problem with assigning production order line numbers to match family line numbers
    // 
    // PRW16.00.04
    // P8000877, Columbus IT, Jack Reynolds, 02 MAR 11
    //   Copy equipment code form header to lines
    // 
    // PRW16.00.06
    // P8001092, Columbus IT, Don Bresee, 17 AUG 12
    //   Add "Variant Code" field
    //   Add "Co-Product Cost Share" field
    // 
    // PRW17.00
    // P8001142, Columbus IT, Don Bresee, 11 MAR 13
    //   Rework Replenishment logic
    // PRW114.00.02
    // P80092071, To-increase, Gangabhushan, 30 JAN 2020
    //   CS00090841 - Issue in supply Driven Planning - Order (Prod. Order number)  can only have shared components
    //   Removed duplicate code in function ProcessProdOrderLines
    ...
    begin
        ...
        if ProdOrder."Family Process Order" then         // PR3.60
            CoProdMgmt.ProcessSharedComponents(ProdOrder); // PR3.60
        ...
        if FamilyLine.FindSet() then
            repeat
                if FamilyLine."Item No." <> '' then begin
                    InitProdOrderLine(FamilyLine."Item No.", FamilyLine."Variant Code", ProdOrder."Location Code", FamilyLine."Line No."); // P8001092
                    ...
                    if ProdOrder."Family Process Order" then begin
                        ProdOrderLine.Validate("Production BOM No.", ProdOrder."Source No.");
                    ...
                    //
                    //ProdOrderLine."Line No." := FamilyLine."Line No."; // P8000600A
                    ProdOrderLine."By-Product" := FamilyLine."By-Product";
                    ProdOrderLine."Primary Co-Product" := FamilyLine.IsPrimaryCoProduct(); // P8001092
                    ProdOrderLine.Validate("Unit of Measure Code", FamilyLine."Unit of Measure Code");
                    ...
                    // PR3.60
                    ProdOrderLine."Co-Product Cost Share" := FamilyLine."Co-Product Cost Share"; // P8001092
    ...
    begin
        ...
        if TempSalesPlanningLine.FindSet() then
            repeat
                ...
                InitProdOrderLine(SalesLine."No.", SalesLine."Variant Code", SalesLine."Location Code", 0); // P8000600A
    ...
    begin
        ...
        P800ProdOrderMgt.DeleteSharedComponents(ProdOrder); // PR3.60
        ...
        case ProdOrder."Source Type" of
            ProdOrder."Source Type"::Item:
                begin
                    ...
                    InitProdOrderLine(ProdOrder."Source No.", VariantCode, ProdOrder."Location Code", 0); // P8000600A
    ...
    local procedure InitProdOrderLine(ItemNo: Code[20]; VariantCode: Code[10]; LocationCode: Code[10]; LineNo: Integer)
    var
        Item: Record Item;
        IsHandled: Boolean;
    begin
        ...
        if LineNo <> 0 then                   // P8000600A
            ProdOrderLine."Line No." := LineNo  // P8000600A
        else                                  // P8000600A
        ...
        if ProdOrderLine.FindSet(true) then
            repeat
                ...
                if not IsHandled then begin
                    ...
                    //IF NOT CalcProdOrder.Calculate(ProdOrderLine,Direction,TRUE,TRUE,TRUE,LetDueDateDecrease) THEN     // P8001081
                    if not CalcProdOrder.Calculate(ProdOrderLine, Direction,                                              // P8001081
          not ProdOrder."Family Process Order", not ProdOrder."Family Process Order", true, LetDueDateDecrease) // P8001081
        then
                        ErrorOccured := true;
                end;
    ...
    begin
        ...
        InitProdOrderLine(ProdOrderComp."Item No.", ProdOrderComp."Variant Code", ProdOrderComp."Location Code", 0); // P8000600A
