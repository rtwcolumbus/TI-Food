codeunit 311 "Item-Check Avail."
{
    ...
    begin
        ...
        case ItemJnlLine."Entry Type" of
            ItemJnlLine."Entry Type"::Sale:
                AvailableFor := LotStatus.FieldNo("Available for Sale");
            ItemJnlLine."Entry Type"::Consumption:
                AvailableFor := LotStatus.FieldNo("Available for Consumption");
            ItemJnlLine."Entry Type"::"Negative Adjmt.":
                AvailableFor := LotStatus.FieldNo("Available for Adjustment");
        end;
        // P8001083
    ...
    begin
        ...
        InvtSetup.Get;
        exit(InitialQtyAvailable + ItemNetChange - OldItemNetResChange < -InvtSetup."Near-Zero Qty. Value");
        // P8000548A
    end;
    ...
    begin
        ...
        QtyAvailToPromise(Item, CompanyInfo,                                     // P8001132
          LotStatusExclusionFilter, ExcludePurch, ExcludeSalesRet, ExcludeOutput); // P8001132
        EarliestAvailDate := EarliestAvailabilityDate(Item, CompanyInfo,         // P8001132
          LotStatusExclusionFilter, ExcludePurch, ExcludeSalesRet, ExcludeOutput); // P8001132
    ...
    local procedure QtyAvailToPromise(var Item: Record Item; CompanyInfo: Record "Company Information"; LotStatusExclusionFilter: Text[1024]; ExcludePurch: Boolean; ExcludeSalesRet: Boolean; ExcludeOutput: Boolean)
    var
        IsHandled: Boolean;
    begin
        IsHandled := false;
        ...
        AvailableToPromise.SetAvailableFor(LotStatusExclusionFilter, ExcludePurch, ExcludeSalesRet, ExcludeOutput); // P8001132
    ...
    local procedure EarliestAvailabilityDate(var Item: Record Item; CompanyInfo: Record "Company Information"; LotStatusExclusionFilter: Text[1024]; ExcludePurch: Boolean; ExcludeSalesRet: Boolean; ExcludeOutput: Boolean): Date
    var
        AvailableQty: Decimal;
        NewItemNetChangeBase: Decimal;
        OldItemNetChangeBase: Decimal;
    begin
        // P8001132 - parameters added for LotStatusExclusionFilter, ExcludePurch, ExcludeSalesRet, ExcludeOutput
    ...
    begin
        ...
        AvailableFor := LotStatus.FieldNo("Available for Transfer"); // P8001083
    ...
    begin
        with AssemblyHeader do begin
            ...
            CompanyInfo.Get();
            QtyAvailToPromise(Item, CompanyInfo, '', false, false, false); // P8001132
    ...
    begin
        if not Item.Get(ItemNo) then
            exit(false);
        if (Item.Type = Item.Type::FOODContainer) then // P8001290
            exit(false);                             // P8001290
