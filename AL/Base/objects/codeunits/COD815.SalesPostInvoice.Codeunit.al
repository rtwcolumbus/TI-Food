codeunit 815 "Sales Post Invoice" implements "Invoice Posting"
{
    ...
    begin
        ...
        if InvoiceDiscountPosting then begin
            ...
            if not IsHandled then begin
                ...
                if (InvoicePostingBuffer.Amount <> 0) or (InvoicePostingBuffer."Amount (ACY)" <> 0) then begin
                    ...
                    UpdateInvoicePostingBuffer(InvoicePostingBuffer, true, SalesLine); // P800133109
        ...
        if LineDiscountPosting then begin
            ...
            if not IsHandled then begin
                ...
                if (InvoicePostingBuffer.Amount <> 0) or (InvoicePostingBuffer."Amount (ACY)" <> 0) then begin
                    ...
                    if InvoicePostingParameters."Tax Type" = InvoicePostingParameters."Tax Type"::"Sales Tax" then
                        InvoicePostingBuffer.ClearVATFields();
                    UpdateInvoicePostingBuffer(InvoicePostingBuffer, true, SalesLine); // P8007748
        ...
        if (SalesSetup."Delivered Pricing Calc. Method" <> SalesSetup."Delivered Pricing Calc. Method"::None) then begin
            InvoicePostingBuffer.SetAmounts(
              0, 0, SalesLine."Line Amount (Freight)", SalesLineACY."Line Amount (Freight)", 0, 0, 0);
            if (InvoicePostingBuffer.Amount <> 0) or
               (InvoicePostingBuffer."Amount (ACY)" <> 0)
            then begin
                InvoicePostingBuffer.SetAccount(
                  GenPostingSetup.GetSalesAccountFreight, TotalVAT, TotalVATACY, TotalAmount, TotalAmountACY);
                InvoicePostingBuffer.UpdateVATBase(TotalVATBase, TotalVATBaseACY);
                UpdateInvoicePostingBuffer(InvoicePostingBuffer, true, SalesLine);
            end;
        end;
        // P800133109
        ...
        UpdateInvoicePostingBuffer(InvoicePostingBuffer, false, SalesLine); // P8007748, P800133109
    ...
    begin
        ...
        if not IsHandled then
            // if (SalesLine.Type = SalesLine.Type::"G/L Account") or (SalesLine.Type = SalesLine.Type::"Fixed Asset") then             // P800133109
            if (SalesLine.Type in [SalesLine.Type::"G/L Account", SalesLine.Type::"Fixed Asset", SalesLine.Type::FOODAccrualPlan]) then // P800133109
    ...
    local procedure UpdateInvoicePostingBuffer(InvoicePostingBuffer: Record "Invoice Posting Buffer"; ForceGLAccountType: Boolean; SalesLine: Record "Sales Line")
    var
    ...
    begin
        ...
        // P800133109
        if (InvoicePostingBuffer.Type = InvoicePostingBuffer.Type::FOODAccrualPlan) then begin
            AccrualFldMgmt.SalesPostUpdatePostingBuffer(InvoicePostingBuffer, SalesLine);
            if ForceGLAccountType then begin
                RestoreAPType := true;
                InvoicePostingBuffer.Type := InvoicePostingBuffer.Type::"G/L Account";
            end;
        ...
        if RestoreAPType then
            TempInvoicePostingBuffer.Type := TempInvoicePostingBuffer.Type::FOODAccrualPlan;
        // P800133109
    end;
    ...
    begin
        ...
        // P800133109
        if (InvoicePostingBuffer.Type = InvoicePostingBuffer.Type::FOODAccrualPlan) then
            AccrualFldMgmt.SalesPostPrepareGenJnlLine(
              SalesHeader, InvoicePostingBuffer."FOOD Accrual Line No.", GenJnlLine);
        // P800133109
