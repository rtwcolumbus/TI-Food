codeunit 5850 "Invt. Doc.-Post Receipt"
{
        ...
        with InvtDocHeader do begin
            ...
            if InvtDocLine.Find('-') then
                repeat
                    ...
                    // P800127049
                    InvtRcptLine."FOOD Country/Reg of Orig Code" := InvtDocLine."FOOD Country/Reg of Orig Code";
                    InvtRcptLine."FOOD Creation Date" := InvtDocLine."FOOD Creation Date";
                    InvtRcptLine."FOOD Quantity (Alt.)" := InvtDocLine."FOOD Quantity (Alt.)";
                    // P800127049
                    ...
                    AltQtyMgt.InvtDocLineToPostedInvtDocLine(InvtDocLine, InvtRcptLine); // P800127049

                    PostItemJnlLine(InvtRcptHeader, InvtRcptLine, InvtDocLine); // P800127049
    ...
    local procedure PostItemJnlLine(InvtRcptHeader2: Record "Invt. Receipt Header"; InvtRcptLine2: Record "Invt. Receipt Line"; InvtDocLine: Record "Invt. Document Line")
    var
    ...
    begin
        ...
        // P800127049
        ItemJnlLine."Alt. Qty. Transaction No." := InvtDocLine."FOOD Alt. Qty. Transaction No.";
        ItemJnlLine."Override Loose Lot Control" := not InvtDocHeader.Correction;
        ItemJnlLine."Creation Date" := InvtDocLine."FOOD Creation Date";
        ItemJnlLine."Country/Region of Origin Code" := InvtDocLine."FOOD Country/Reg of Orig Code";
        ItemJnlLine."Container License Plate" := InvtDocLine."FOOD Container License Plate";
        ItemJnlLine."Old Container License Plate" := InvtDocLine."FOOD Old License Plate";
        ItemJnlLine."Old Container ID" := InvtDocLine."FOOD Old Container ID";
        ItemJnlLine."New Container License Plate" := InvtDocLine."FOOD New License Plate";
        ItemJnlLine."New Container ID" := InvtDocLine."FOOD New Container ID";

        if InvtRcptHeader2.Correction then begin
            ItemJnlLine."Quantity (Alt.)" := -InvtRcptLine2."FOOD Quantity (Alt.)";
            ItemJnlLine."Invoiced Qty. (Alt.)" := -InvtRcptLine2."FOOD Quantity (Alt.)";
            // Need to change signe of Alt Qty Lines
        end
        else begin
            ItemJnlLine."Quantity (Alt.)" := InvtRcptLine2."FOOD Quantity (Alt.)";
            ItemJnlLine."Invoiced Qty. (Alt.)" := InvtRcptLine2."FOOD Quantity (Alt.)";
