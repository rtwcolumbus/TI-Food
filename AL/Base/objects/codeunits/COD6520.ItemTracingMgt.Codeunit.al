codeunit 6520 "Item Tracing Mgt."
{
    ...
    //   Add support for tracing through BOM ledger (repack)
    // 
    // P8000599A, VerticalSoft, Don Bresee, 28 MAY 08
    //   Use new SetDescription routine
    // 
    // PRW16.00
    // P8000642, VerticalSoft, Jack Reynolds, 19 NOV 08
    //   Remove calls to SetDescription
    // 
    // PRW16.00.01
    // P8000721, VerticalSoft, Jack Reynolds, 11 AUG 09
    //   Fix problem with shared components
    // 
    // PRW16.00.03
    // P8000807, VerticalSoft, Jack Reynolds, 01 APR 10
    //   Fix problem with item tracing and production orders
    // 
    // PRW17.00
    ...
    var
        ...
        Text37002000: Label '%1 Consumption %2';
        Text37002001: Label '%1 Output %2';
        Text37002002: Label 'Lot Combination Consumption %1';
        Text37002003: Label 'Lot Combination Output %1';
        Text37002004: Label 'Sales Repack Consumption %1';
        Text37002005: Label 'Sales Repack Output %1';
    ...
    begin
        ...
        with ItemLedgEntry2 do begin
            if not IsBOMOrderType() then // P8001134
            ...
            if ((("Entry Type" = "Entry Type"::Consumption) or ("Entry Type" = "Entry Type"::"Assembly Consumption")) and
               ...
               ((("Entry Type" = "Entry Type"::Output) or ("Entry Type" = "Entry Type"::"Assembly Output")) and
                (Direction = Direction::Backward)) or                                                     // P8001134
               (("Entry Type" = "Entry Type"::"Negative Adjmt.") and (Direction = Direction::Forward)) or // P8001134
               (("Entry Type" = "Entry Type"::"Positive Adjmt.") and (Direction = Direction::Backward))   // P8001134
            then begin
                ItemLedgEntry.Reset();
                ...
                if ("Entry Type" = "Entry Type"::Consumption) or ("Entry Type" = "Entry Type"::"Assembly Consumption") or // P8001134
                   ("Entry Type" = "Entry Type"::"Negative Adjmt.")                                                       // P8001134
                then begin
                    if ShowComponents <> ShowComponents::No then begin
                        ...
                        SetBOMILEFilters(ItemLedgEntry2, ItemLedgEntry, false); // P8001134
                    ...
                    if ("Order Type" = "Order Type"::Production) and ("Order Line No." = 0) then // P8000721, P8001132
                        ItemLedgEntry.SetRange("Order Line No.");                                  // P8000721, P8001132
                    ItemLedgEntry.SetFilter("Entry Type", '%1|%2', ItemLedgEntry."Entry Type"::Output,
                      ItemLedgEntry."Entry Type"::"Assembly Output");
                    SetBOMILEFilters(ItemLedgEntry2, ItemLedgEntry, true); // P8001134
                    ItemLedgEntry.SetRange(Positive, true);
                end else begin
                    if ShowComponents = ShowComponents::No then
                        exit;
                    if "Order Type" = "Order Type"::Production then                      // P8001132
                        ItemLedgEntry.SetFilter("Order Line No.", '0|%1', "Order Line No."); // P8000721, P8001132
                    ItemLedgEntry.SetFilter("Entry Type", '%1|%2', ItemLedgEntry."Entry Type"::Consumption,
                      ItemLedgEntry."Entry Type"::"Assembly Consumption");
                            ...
                            DATABASE::"Lot No. Information":
                                begin
                                    ...
                                    if "Entry Type" = "Entry Type"::"Negative Adjmt." then
                                        Description2 := StrSubstNo(Text37002002, LotNoInfo."Lot No.")
                                    else
                                        Description2 := StrSubstNo(Text37002003, LotNoInfo."Lot No.");
                                end;
                            DATABASE::"Repack Order":
                                begin
                                    RecRef.SetTable(RepackOrder);
                                    if "Entry Type" = "Entry Type"::"Negative Adjmt." then
                                        Description2 := StrSubstNo(Text37002000, RepackOrder.TableCaption, RepackOrder."No.") // P8000599A
                                    else
                                        Description2 := StrSubstNo(Text37002001, RepackOrder.TableCaption, RepackOrder."No."); // P8000599A
                                end;
                            DATABASE::"Sales Shipment Header":
                                begin
                                    RecRef.SetTable(SalesShptHeader);
                                    if "Entry Type" = "Entry Type"::"Negative Adjmt." then
                                        Description2 := StrSubstNo(Text37002004, SalesShptHeader."No.")
                                    else
                                        if "Entry Type" = "Entry Type"::"Positive Adjmt." then
                                            Description2 := StrSubstNo(Text37002005, SalesShptHeader."No.");
                                end;
                                // P8000498A, P8001132
                        end;
    ...
    begin
        ...
        TempTrackEntry."Quantity (Alt.)" := ItemLedgEntry."Quantity (Alt.)";                     // P8000466A
        TempTrackEntry."Remaining Quantity (Alt.)" := ItemLedgEntry."Remaining Quantity (Alt.)"; // P8000466A
        ...
        with TrackingEntry do begin
            ...
            case "Entry Type" of
              ...
              "Entry Type"::"Negative Adjmt.":
                    if not SetBOMRecordID(TrackingEntry) then // P8000498A
                        if ItemLedgEntry.Get("Item Ledger Entry No.") then begin
                            RecRef.GetTable(ItemLedgEntry);
                            "Record Identifier" := RecRef.RecordId;
                        end;
    ...
    begin
        ...
        case RecID.TableNo of
            ...
            DATABASE::"Repack Order":
                begin
                    RecRef.SetTable(RepackOrder);
                    PAGE.RunModal(PAGE::"Finished Repack Order", RepackOrder);
                end;
                // P8000498A
        end;
    end;
    ...
    procedure SetBOMRecordID(var TrackingEntry: Record "Item Tracing Buffer"): Boolean
    var
        LotNoInfo: Record "Lot No. Information";
        RepackOrder: Record "Repack Order";
        SalesShptHeader: Record "Sales Shipment Header";
    ...
    begin
        // P8000498A, P8001132
        with ItemLedgEntry do
            if Get(TrackingEntry."Item Ledger Entry No.") then
                if IsBOMOrderType() then
                    case "Order Type" of
                        "Order Type"::FOODLotCombination:
                            if LotNoInfo.Get("Item No.", "Variant Code", "Order No.") then begin
                                RecRef.GetTable(LotNoInfo);
                                TrackingEntry."Record Identifier" := RecRef.RecordId;
                                exit(true);
                            end;
                        "Order Type"::FOODRepack:
                            if RepackOrder.Get("Order No.") then begin
                                RecRef.GetTable(RepackOrder);
                                TrackingEntry."Record Identifier" := RecRef.RecordId;
                                exit(true);
                            end;
                        "Order Type"::FOODSalesRepack:
                            if SalesShptHeader.Get("Order No.") then begin
                                RecRef.GetTable(RepackOrder);
                                TrackingEntry."Record Identifier" := RecRef.RecordId;
                                exit(true);
                            end;
    ...
    local procedure SetBOMILEFilters(var ItemLedgEntry: Record "Item Ledger Entry"; var ItemLedgFilters: Record "Item Ledger Entry"; FindBOMEntry: Boolean)
    begin
        // P8001134
        if ItemLedgEntry.IsBOMOrderType() then
            with ItemLedgFilters do begin
