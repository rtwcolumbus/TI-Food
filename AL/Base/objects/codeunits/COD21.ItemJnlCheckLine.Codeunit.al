codeunit 21 "Item Jnl.-Check Line"
{
    ...
    // 
    // PR3.70.01
    //   Don't require Source No. for consumption of shared components
    // 
    // PR3.70.03
    //   Don't allow sales and purchases with zero quantity but non-zero alternate quantity
    // 
    // PR3.70.07
    // P8000140A, Myers Nissi, Jack Reynolds, 19 NOV 04
    //   RunCheck - add checks for containers
    // 
    // PR3.70.09
    // P8000201A, Myers Nissi, Jack Reynolds, 04 MAR 05
    //   RunCheck - allow zero quantity if value entry type is revaluation
    // 
    // PRW15.00.03
    ...
    //   2015 Upgrade - Code merge and refactorin
    // 
    // PRW18.00.03
    // P8006484, To-Increase, Jack Reynolds, 15 MAR 16
    //   Bypass container checks for adjust cost
    // 
    // PRW19.00
    // P8006256, To Increase, Jack Reynolds, 05 JAN 16
    //   Allow revaluation of in-transit locations
    // 
    // P8004516, To Increase, Jck Reynolds, 18 MAR 18
    ...
    // 
    // PRW110.0.01
    // P80039745, To-Increase, Dayakar Battini, 25 MAY 17
    //   By pass transit location checking for revaluation
    // 
    // PRW111.00.01
    ...
    var
        ...
        ContFns: Codeunit "Container Functions";
        ...
        with ItemJournalLine do begin
            ...
            if not IsHandled then
                if InvtSetup."Location Mandatory" and
                   ...
                   not Adjustment
                then begin
                    ShouldCheckLocationCode := (Type <> Type::Resource) and (Item.Type in [Item.Type::Inventory, Item.Type::FOODContainer]) and // P80066030
            ...
            if Item.IsInventoriableType() and (not Item."Non-Warehouse Item") then
            ...
            // P8001342
            if (not CalledFromAdjustment) and (not Correction) then // P8006484
                if Process800Fns.ContainerTrackingInstalled then begin
                    ContFns.CheckContainerAssignment2("Old Container ID", ItemJournalLine); // P80056718
                    ContFns.CheckContainerAssignment("New Container ID");
                    ContFns.ItemJnlCheckLooseInventory(ItemJournalLine);
                end;
            // P8001342
            ...
            //IF ("Entry Type" IN ["Entry Type"::Consumption,"Entry Type"::Output]) AND              // PR3.70.01
            if ((("Entry Type" = "Entry Type"::Consumption) and (ItemJournalLine."Order Line No." <> 0)) // PR3.70.01, P8001132
                or ("Entry Type" = "Entry Type"::Output)) and                                        // PR3.70.01
    ...
    begin
        ...
        with ItemJnlLine do
            ...
            ("Quantity (Alt.)" = 0) and ("Scrap Quantity (Alt.)" = 0) and ("Invoiced Qty. (Alt.)" = 0) and // PR3.60
                   TimeIsEmpty() and ("Invoiced Qty. (Base)" = 0)
                then
                    Error(ErrorInfo.Create(Text007, true))
            end else
                if not "Warehouse Adjustment" then begin // P800160088
    ...
    begin
        ...
        with ItemJnlLine do begin
            ...
            if ("Entry Type" = "Entry Type"::Output) and (not LastOutputOperation(ItemJnlLine)) or UsesReplenishmentBin(ItemJnlLine) then // P8000631A, P8001352
                exit;
    ...
    local procedure UsesReplenishmentBin(var ItemJnlLine: Record "Item Journal Line"): Boolean
    var
        ProdOrderLine: Record "Prod. Order Line";
    begin
        // P8000631A
        with ItemJnlLine do
            if ("Entry Type" in ["Entry Type"::Consumption, "Entry Type"::Output]) then
                if ProdOrderLine.Get(ProdOrderLine.Status::Released, "Order No.", "Order Line No.") then // P8001132
                    exit(ProdOrderLine."Replenishment Area Code" <> '');
        exit(false);
    end;
    ...
    begin
        ...
        with ItemJnlLine do
            if ((("Entry Type" <> "Entry Type"::Transfer) and ("Value Entry Type" <> "Value Entry Type"::Revaluation)) or // P8004516
                ("Order Type" <> "Order Type"::Transfer)) and                                                             // P8004516
               not Adjustment
            then begin
                if ("Value Entry Type" <> "Value Entry Type"::Revaluation) then begin   // P80039745
