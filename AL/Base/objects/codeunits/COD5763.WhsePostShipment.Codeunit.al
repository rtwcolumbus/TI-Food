codeunit 5763 "Whse.-Post Shipment"
{
    ...
    // 
    // PRW16.00.03
    // P8000798, VerticalSoft, Jack Reynolds, 25 MAR 10
    //   Fix problem with PrintReport and printer selection
    // 
    // PRW16.00.05
    ...
    var
        ...
        FromDeliveryTrip: Boolean;
        ShptError: Label 'This shipment is linked to Delivery trip. Please post from Delivery trips.';
    ...
    begin
        with WhseShptLine do begin
            CheckShptOfDelTrip(WhseShptLine); // P80083600
            SetCurrentKey("No.");
            SetRange("No.", "No.");
            ...
            if Find('-') then
                repeat
                    ...
                    TestField(Short, false); //N138F0000.n
                    WhseRqst.Get(
                    ...
                    if not IsNonWarehouseItem() then // P8001290
            ...
            case "Source Type" of
                DATABASE::"Sales Line":
                    begin
                        ...
                        // P800162917
                        // SalesPost.SetPostingLocation(WhseShptHeader."Location Code"); // P8000282A
                ...
                DATABASE::"Purchase Line": // Return Order
                    begin
                        ...
                        // P800162917
                        // PurchPost.SetPostingLocation(WhseShptHeader."Location Code"); // P8000282A
                        BindSubscription(FoodManualSubscriptions);
                        FoodManualSubscriptions.SetPostingLocation("Location Code");
                        // P800162917
    ...
    procedure ResultMessage() MessageText: Text[250]
    begin
        // P80039781
        MessageText := Text003;
        ...
        MessageText := StrSubstNo(MessageText, CounterSourceDocOK, CounterSourceDocTotal);
    end;
    ...
    begin
        ...
        if WhseShptLine.IsNonWarehouseItem() then // P8001290
            exit;                                   // P8001290
    ...
    begin
        ...
        if Location."Bin Mandatory" then begin
            ...
            if TempWhseJnlLine2.Find('-') then
                repeat
                    AdjustQtyFromWhseTracking(                                                                                 // P8000503A
                      PostedWhseShptLine."Whse. Shipment No.", PostedWhseShptLine."Whse Shipment Line No.", TempWhseJnlLine2); // P8000503A, P8001132
            ...
            if SalesLine.Find('-') then
                repeat
                    ...
                    if Find('-') then begin
                        ...
                        ModifyLine :=
                          (SalesLine.Type <> SalesLine.Type::FOODContainer) and // P8001347
                          ((SalesLine.Type <> SalesLine.Type::"G/L Account") or (SalesLine."Container Line No." = 0)) and // P8001323
                    ...
                    if ModifyLine and SalesLine.TrackAlternateUnits() then  // P8000282A
                        AltQtyMgmt.UpdateSalesOnWhsePost(SalesLine, Invoice); // P8000282A
            ...
            if PurchLine.Find('-') then
                repeat
                    ...
                    if Find('-') then begin
                        ...
                        ModifyLine :=
                          (PurchLine.Type <> PurchLine.Type::FOODContainer) and // P8001323
                          ((PurchLine."Qty. to Receive" <> 0) or            // P8001323
                          (PurchLine."Return Qty. to Ship" <> 0) or
                          (PurchLine."Qty. to Invoice" <> 0));              // P8001323
                    ...
                    if ModifyLine and PurchLine.TrackAlternateUnits() then  // P8000282A
                        AltQtyMgmt.UpdatePurchOnWhsePost(PurchLine, false);   // P8000282A
            ...
            if TransLine.Find('-') then
                repeat
                    ...
                    if Find('-') then begin
                        ...
                        // ModifyLine := TransLine."Qty. to Ship" <> 0;                                                  // P8001347
                        ModifyLine := (TransLine.Type <> TransLine.Type::Container) and (TransLine."Qty. to Ship" <> 0); // P8001347
                        if ModifyLine then begin
                            TransLine.Validate("Qty. to Ship", 0);
                            TransLine.Validate("Qty. to Receive", 0);
                        end;
                    end;
                    if ModifyLine and TransLine.TrackAlternateUnits() then              // P8000282A
                        AltQtyMgmt.UpdateTransOnWhsePost(TransLine, Direction::Outbound); // P8000282A
    ...
    local procedure AdjustQtyFromWhseTracking(WhseShptNo: Code[20]; WhseShptLineNo: Integer; var TempWhseJnlLine2: Record "Warehouse Journal Line" temporary)
    var
        WhseItemTrkgLine: Record "Whse. Item Tracking Line";
        Qty: Decimal;
        QtyBase: Decimal;
    begin
        ...
        with WhseItemTrkgLine do begin
            ...
            SetRange("Source ID", WhseShptNo);
            SetRange("Source Type", DATABASE::"Warehouse Shipment Line");
            SetRange("Source Ref. No.", WhseShptLineNo);
            SetRange("Lot No.", TempWhseJnlLine2."Lot No.");
            if Find('-') then
                repeat
                    Qty := Qty + Quantity;
            ...
            if (QtyBase <> 0) then begin
                if (QtyBase = TempWhseJnlLine2."Qty. (Base)") then
                    TempWhseJnlLine2.Quantity := -Qty
                else
                    TempWhseJnlLine2.Quantity :=
                      Round(TempWhseJnlLine2."Qty. (Base)" * (Qty / QtyBase), 0.00001);
                TempWhseJnlLine2."Qty. (Absolute)" := Abs(TempWhseJnlLine2.Quantity);
            end;
        end;
        // P8000503A
    end;

    procedure CallFromDeliveryTrip(pDeliveryTrip: Boolean)
    begin
        FromDeliveryTrip := pDeliveryTrip; // P80083600
    end;

    procedure CheckShptOfDelTrip(var WarehouseShipmentLine: Record "Warehouse Shipment Line")
    var
        ProcessFns: Codeunit "Process 800 Functions";
    ...
    begin
        // P80083600
        if not FromDeliveryTrip then begin
            if ProcessFns.DistPlanningInstalled then begin
                SalesLine.SetRange("Document Type", WarehouseShipmentLine."Source Subtype");
                ...
                if SalesLine.FindFirst then
                    if DeliveryTripMgt.GetWhseReqSLS(SalesLine, WarehouseRequest) then
                        if WarehouseRequest."Delivery Trip" <> '' then
                            Error(ShptError);
            end;
        end;
    end;
