codeunit 8 AccSchedManagement
{
    ...
    var
        ...
        Recalculate: Boolean;
        AccSchedUnits: Codeunit "Account Schedule Units";
    ...
    begin
        ...
        if not IsHandled then begin
            ...
            if TempAccSchedCellValue.Get(AccSchedLine."Line No.", ColumnLayout."Line No.") then begin
                ...
                // P8001019
                if (AccSchedLine."Acc. Schedule Unit Code" <> '') and
                  (AccSchedLine."Totaling Type" = AccSchedLine."Totaling Type"::Formula) and
                  (ColumnLayout."Ledger Entry Type" = ColumnLayout."Ledger Entry Type"::FOODUnits) and
                  (ColumnLayout."Column Type" = ColumnLayout."Column Type"::Formula)
                then begin
                    AccSchedLine.CopyFilters(AccountScheduleLine);
                    SetGLAccRowFilters(GLAcc, AccSchedLine);
                    SetGLAccColumnFilters(GLAcc, AccSchedLine, ColumnLayout);
                    Result := AccSchedUnits.CalcUnits(GLAcc, AccSchedLine, ColumnLayout);
                end;
                // P8001019
    ...
    begin
        ...
        if ColumnLayout."Column Type" <> ColumnLayout."Column Type"::Formula then begin
            ...
            case ColumnLayout."Ledger Entry Type" of
                ...
                ColumnLayout."Ledger Entry Type"::FOODUnits:
                    ColValue := AccSchedUnits.CalcUnits(GLAcc, AccSchedLine, ColumnLayout); // P8001019
            end;
    ...
    begin
        // P8001386
        if (ColumnLayout2."Ledger Entry Type" = ColumnLayout2."Ledger Entry Type"::FOODUnits) and
          (ColumnLayout2."Rounding Factor"::None = ColumnLayout2."Rounding Factor"::None)
        then
            exit(Format(Value, 0, '<Precision,5><Standard Format,0>'))
        else
            // P8001386
    ...
    begin
        ...
        with AccScheduleLine do begin
            ...
            // P8001213
            if TempColumnLayout."Ledger Entry Type" = TempColumnLayout."Ledger Entry Type"::FOODUnits then
                DrillDownUnits(AccScheduleLine, TempColumnLayout)
            else
            // P8001213
    ...
    procedure DrillDownUnits(var AccSchedLine: Record "Acc. Schedule Line"; var ColumnLayout: Record "Column Layout")
    var
        GLAcc: Record "G/L Account";
    begin
        // P8001019
        SetGLAccRowFilters(GLAcc, AccSchedLine);
        SetGLAccColumnFilters(GLAcc, AccSchedLine, ColumnLayout);
        AccSchedUnits.DrillDownUnits(GLAcc, AccSchedLine, ColumnLayout);
    end;
