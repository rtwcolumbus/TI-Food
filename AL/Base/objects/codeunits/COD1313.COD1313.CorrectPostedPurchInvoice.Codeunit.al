codeunit 1313 "Correct Posted Purch. Invoice"
{
	...
	ReceivedQtyNoReturnedAlt: Decimal;
    begin
            ...
            repeat
                if not IsCommentLine(PurchInvLine) then begin
                    ...
                    if PurchInvLine.Type = PurchInvLine.Type::Item then begin
                        ...
                        if Item.IsInventoriableType() and (not Item.IsContainerType) then // P80066030
                            if (PurchInvLine.Quantity > 0) and (PurchInvLine."Job No." = '') and WasNotCancelled(PurchInvHeader."No.") then begin
                                PurchInvLine.CalcReceivedPurchNotReturned(ReceivedQtyNoReturned, ReceivedQtyNoReturnedAlt, RevUnitCostLCY, false);          // P8001213
                                ThrowItemReturnedError := (PurchInvLine.Quantity <> ReceivedQtyNoReturned) or (PurchInvLine."Quantity (Alt.)" <> ReceivedQtyNoReturnedAlt);
