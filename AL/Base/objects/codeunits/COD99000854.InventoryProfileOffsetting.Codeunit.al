codeunit 99000854 "Inventory Profile Offsetting"
{
    ...
    //   Add logic to handle Co-Product Production Orders
    // 
    // P8001082, Columbus IT, Jack Reynolds, 15 JAN 13
    //   Support for pre-process
    // 
    // PRW17.00
    // P8001138, Columbus IT, Jack Reynolds, 22 FEB 13
    ...
    // 
    // PRW17.10.02
    // P8001292, Columbus IT, Jack Reynolds, 20 FEB 14
    //   Fix problem with clearing alt. qty. data in the tracking entries
    // 
    // PRW17.10.03
    // P8001317, Columbus IT, Jack Reynolds, 28 APR 14
    //   Fix problem with pre-processing and overflow
    // 
    // PRW110.0.02
    // P80050544, To-Increase, Dayakar Battini, 23 JAN 18
    //   Upgrade to 2017 CU13
    // 
    // PRW111.00.02
    // P80076591, To-Increase, Gangabhushan, 18 JUN 19
    //   CS00068612 - Lot Tracking loses Lot Creation Date
    //
    // PRW111.00.03
    //   P80089477, To-Increase, Gangabhushan, 18 DEC 19
    //     CS00085935 - Planning worksheet removes reservation entry for transfer order
    //
    // P800120327, To-Increase, Gangabhushan, 26 MAR 21
    //   CS00152020 | Problem with lot number, disappear on transfer orders when they use planning worksheet    
    ...
    var
        ...
        DeletedTracking: Record "Reservation Entry" temporary;
        AltQtyItem: Boolean;
    ...
    begin
        ...
        DeletedTracking.Reset();     // P8001292
        DeletedTracking.DeleteAll(); // P8001292
        PlanMRP := MRPPlanning;
        ...
        AltQtyItem := Item."Alternate Unit of Measure" <> ''; // P8001292
        // P8001083
    ...
    begin
        ...
        if not ExcludeSalesRet then // P8001083, P8001132
        ...
        if ProdOrderComp.FindLinesWithItemToPlan(Item, true) then
            repeat
                ...
                if ShouldProcess then begin
                    ...
                    if (ProdOrderComp."Prod. Order Line No." = 0) then // P8001092
                        ReqLine.SetRange("Ref. Line No.");               // P8001092
        ...
        if TransLine.FindLinesWithItemToPlan(Item, false, true) then
            repeat
                ...
                if ShouldProcess then begin
                    ...
                    InventoryProfile.TransferFromOutboundTransfer(TransLine, TempItemTrkgEntry, LotStatusExclusionFilter); // P8001083
    ...
    begin
        ...
        if ItemLedgEntry.FindLinesWithItemToPlan(Item, false) then
            repeat
                // P8001083
                if not LotStatusMgmt.ExcludeLot(ItemLedgEntry."Item No.", ItemLedgEntry."Variant Code", ItemLedgEntry."Lot No.",
    ...
    begin
        ...
        if not ExcludePurch then // P8001083, P8001132
    ...
    var
        ...
        ShouldProcess: Boolean;
    begin
        if not ExcludeOutput then // P8001083
            if ProdOrderLine.FindLinesWithItemToPlan(Item, true) then
                repeat
                    if IncludeProdOutputInSupply(ProdOrderLine) then begin // P8001092
                        ...
                        if ShouldProcess then begin
                            ...
                            then begin
                                ...
                                if not (CapLedgEntry.IsEmpty() and ItemLedgEntry.IsEmpty) then
                                    ...
                                    // ProdOrderComp.SetRange("Prod. Order Line No.",ProdOrderLine."Line No.");         // P8001092
                                    ProdOrderComp.SetFilter("Prod. Order Line No.", '%1|%2', 0, ProdOrderLine."Line No."); // P8001092
    ...
    var
        AsmHeader: Record "Assembly Header";
    begin
        if not ExcludeOutput then // P8001132
        ...
        if TransLine.FindLinesWithItemToPlan(Item, true, true) then
            repeat
                ...
                if ShouldProcess then begin
                    InventoryProfile.Init();
                    InventoryProfile."Line No." := NextLineNo();
                    InventoryProfile.TransferFromInboundTransfer(TransLine, TempItemTrkgEntry, LotStatusExclusionFilter); // P8001083
    ...
    begin
        ...
        DemandInvtProfile.SetCurrentKey(
          "Item No.", "Variant Code", "Location Code", "Pre-Process", "Due Date"); // P8001082
    ...
    begin
        ...
        DemandInvtProfile.SetCurrentKey(
          "Item No.", "Variant Code", "Location Code", "Pre-Process", "Due Date", "Attribute Priority", "Order Priority"); // P8001082
                ...
                repeat
                    ...
                    if not IsTrkgForSpecialOrderOrDropShpt(ReservEntry) then begin
                        if ShouldDeleteReservEntry(ReservEntry, ToDate) then begin
                            ...
                            then
                                if ReservEntry1.Get("Entry No.", not Positive) then begin // P8001292
                                    SaveDeletedTracking(ReservEntry1); // P8001292
                                    ReservEntry1.Delete();
                                end; // P8001292
                            SaveDeletedTracking(ReservEntry); // P8001292
                            Delete();
                        end else
    ...
    begin
        // P80089477
        if not IsInBoundLotAvailable(ReservEntry) then
            exit(false);
        // P80089477
            ...
            repeat
                ...
                if TempItemTrkgEntry.Find('-') then begin
                    ...
                    repeat
                        ...
                        ChildInvProfile."Lot Creation Date" := TempItemTrkgEntry."Lot Creation Date"; // P80076591
    ...
    begin
        ...
        if TempSKU.Find('-') then
            repeat
                for PreProcess := false to true do begin // P8001082
                    if PreProcess then begin                                                    // P8001082, P8001317
                        TempSKU.Validate("Reordering Policy", TempSKU."Reordering Policy"::Order); // P8001082
                        PlanningGetParameters.AdjustInvalidSettings(TempSKU);                     // P8001317
                    end;
                    ...
                    FilterDemandSupplyRelatedToSKU(DemandInvtProfile, PreProcess); // P8001082
                    FilterDemandSupplyRelatedToSKU(SupplyInvtProfile, PreProcess); // P8001082
    ...
    begin
        ...
        DemandInvtProfile.SetCurrentKey("Item No.", "Variant Code", "Location Code", "Pre-Process", "Due Date", "Attribute Priority", "Order Priority"); // P8001082
        SupplyInvtProfile.SetCurrentKey("Item No.", "Variant Code", "Location Code", "Pre-Process", "Due Date", "Attribute Priority", "Order Priority"); // P8001082
    ...
    local procedure FilterDemandSupplyRelatedToSKU(var InventoryProfile: Record "Inventory Profile"; PreProcess: Boolean)
    begin
        // P8001082 - Add parameter for PreProcess
        InventoryProfile.SetRange("Item No.", TempSKU."Item No.");
        InventoryProfile.SetRange("Variant Code", TempSKU."Variant Code");
        InventoryProfile.SetRange("Location Code", TempSKU."Location Code");
        InventoryProfile.SetRange("Pre-Process", PreProcess); // P8001082
    end;
    ...
    begin
        ...
        repeat
            ...
            RestoreDeletedTrackingData(ReservEntry); // P8001292
            ReservEntry.Insert();
        ...
        then
            if SupplyInvtProfile."Planning Line No." = 0 then
                with ReqLine do begin
                    ...
                    if SupplyInvtProfile."Action Message" = SupplyInvtProfile."Action Message"::New then begin
                        ...
                        case TempSKU."Replenishment System" of
                            ...
                            TempSKU."Replenishment System"::"Prod. Order":
                                begin
                                    "Pre-Process" := SupplyInvtProfile."Pre-Process"; // P8001082
                    ...
                    "Accept Action Message" := not "Pre-Process"; // P8001082
    ...
    begin
        ...
        FilterDemandSupplyRelatedToSKU(TempReminderInvtProfile, false); // P8001082
    ...
    begin
        ...
        DemandInvtProfile.SetCurrentKey(
          "Item No.", "Variant Code", "Location Code", "Pre-Process", "Due Date", "Attribute Priority", "Order Priority"); // P8001082, P8001138
        ...
        with ReqLine do begin
            ...
            if FindSet(true) then
                repeat
                    ...
                    if "Pre-Process" then       // P8001082
                        AcceptActionMsg := false; // P8001082
                ...
                case "Ref. Order Type" of
                    "Ref. Order Type"::"Prod. Order":
                        begin
                            ...
                            // ProdOrderComp.SETRANGE("Prod. Order Line No.","Ref. Line No.");         // P8001092
                            ProdOrderComp.SetFilter("Prod. Order Line No.", '%1|%2', 0, "Ref. Line No."); // P8001092
                            if ProdOrderComp.Find('-') then
                                repeat
                                    PlanComponent.InitFromRequisitionLine(ReqLine);
                                    PlanComponent.TransferFromComponent(ProdOrderComp);
                                    UpdateCoProdPlanComp(PlanComponent, ProdOrderComp, ReqLine); // P8001092
    ...
    local procedure UpdateCoProdPlanComp(var PlanComponent: Record "Planning Component"; var ProdOrderComp: Record "Prod. Order Component"; var ReqLine: Record "Requisition Line")
    var
        BOMHeader: Record "Production BOM Header";
        BOMLine: Record "Production BOM Line";
    begin
        // P8001092
        if (ProdOrderComp."Prod. Order Line No." = 0) then
            with PlanComponent do begin
                "Quantity per" := ProdOrderComp.GetCoProdCompQtyPer(ReqLine."No.");
                Quantity := "Quantity per";
                "Quantity (Base)" := Quantity * "Qty. per Unit of Measure";
            end;
    end;

    local procedure IncludeProdOutputInSupply(var ProdOrderLine: Record "Prod. Order Line"): Boolean
    var
        ProdOrder: Record "Production Order";
    ...
    begin
        ...
        if ProdOrder.Get(ProdOrderLine.Status, ProdOrderLine."Prod. Order No.") then
            if ProdOrder."Family Process Order" then
                exit(ProdOrderLine."Primary Co-Product");
        exit(true);
    end;

    procedure SaveDeletedTracking(ReservEntry: Record "Reservation Entry")
    begin
        // P8001292
        DeletedTracking.SetCurrentKey("Source ID", "Source Ref. No.", "Source Type", "Source Subtype", "Source Batch Name", "Source Prod. Order Line");
        DeletedTracking.SetRange("Source Type", ReservEntry."Source Type");
        DeletedTracking.SetRange("Source Subtype", ReservEntry."Source Subtype");
        DeletedTracking.SetRange("Source ID", ReservEntry."Source ID");
        DeletedTracking.SetRange("Source Batch Name", ReservEntry."Source Batch Name");
        DeletedTracking.SetRange("Source Prod. Order Line", ReservEntry."Source Prod. Order Line");
        DeletedTracking.SetRange("Source Ref. No.", ReservEntry."Source Ref. No.");
        DeletedTracking.SetRange("Lot No.", ReservEntry."Lot No.");
        DeletedTracking.SetRange("Serial No.", ReservEntry."Serial No.");
        DeletedTracking.SetRange(Positive, ReservEntry.Positive);
        if DeletedTracking.FindFirst then begin
            DeletedTracking."Quantity (Base)" += ReservEntry."Quantity (Base)";
            DeletedTracking."Qty. to Handle (Base)" += ReservEntry."Qty. to Handle (Base)";
            DeletedTracking."Qty. to Invoice (Base)" += ReservEntry."Qty. to Invoice (Base)";
            DeletedTracking."Quantity (Alt.)" += ReservEntry."Quantity (Alt.)";
            DeletedTracking."Qty. to Handle (Alt.)" += ReservEntry."Qty. to Handle (Alt.)";
            DeletedTracking."Qty. to Invoice (Alt.)" += ReservEntry."Qty. to Invoice (Alt.)";
            DeletedTracking.Modify;
        end else begin
            DeletedTracking := ReservEntry;
            DeletedTracking.Insert;
        end;
    end;

    procedure RestoreDeletedTrackingData(var ReservEntry: Record "Reservation Entry")
    var
        Factor: Decimal;
    begin
        // P8001292
        ...
        if DeletedTracking.FindFirst then begin
            if AltQtyItem then
                if Abs(DeletedTracking."Quantity (Base)") <= Abs(ReservEntry."Quantity (Base)") then begin
                    ReservEntry."Quantity (Alt.)" := DeletedTracking."Quantity (Alt.)";
                    DeletedTracking."Quantity (Base)" := 0;
                    DeletedTracking."Quantity (Alt.)" := 0;
                end else begin
                    Factor := ReservEntry."Quantity (Base)" / DeletedTracking."Quantity (Base)";
                    ReservEntry."Quantity (Alt.)" := Round(DeletedTracking."Quantity (Alt.)" * Factor, 0.00001);
                    DeletedTracking."Quantity (Base)" -= ReservEntry."Quantity (Base)";
                    DeletedTracking."Quantity (Alt.)" -= ReservEntry."Quantity (Alt.)";
                end;

            if Abs(DeletedTracking."Qty. to Handle (Base)") <= Abs(ReservEntry."Quantity (Base)") then begin
                if AltQtyItem then begin
                    ReservEntry."Qty. to Handle (Alt.)" := DeletedTracking."Qty. to Handle (Alt.)";
                    DeletedTracking."Qty. to Handle (Alt.)" := 0;
                end;
                ReservEntry."Qty. to Handle (Base)" := DeletedTracking."Qty. to Handle (Base)";
                DeletedTracking."Qty. to Handle (Base)" := 0;
            end else begin
                if AltQtyItem then begin
                    Factor := ReservEntry."Qty. to Handle (Base)" / DeletedTracking."Qty. to Handle (Base)";
                    ReservEntry."Qty. to Handle (Alt.)" := Round(DeletedTracking."Qty. to Handle (Alt.)" * Factor, 0.00001);
                    if Abs(ReservEntry."Quantity (Alt.)") < Abs(ReservEntry."Qty. to Handle (Alt.)") then
                        ReservEntry."Qty. to Handle (Alt.)" := ReservEntry."Quantity (Alt.)";
                    DeletedTracking."Qty. to Handle (Alt.)" -= ReservEntry."Qty. to Handle (Alt.)";
                end;
                DeletedTracking."Qty. to Handle (Base)" -= ReservEntry."Qty. to Handle (Base)";
            end;

            if Abs(DeletedTracking."Qty. to Invoice (Base)") <= Abs(ReservEntry."Quantity (Base)") then begin
                if AltQtyItem then begin
                    ReservEntry."Qty. to Invoice (Alt.)" := DeletedTracking."Qty. to Invoice (Alt.)";
                    DeletedTracking."Qty. to Invoice (Alt.)" := 0;
                end;
                ReservEntry."Qty. to Invoice (Base)" := DeletedTracking."Qty. to Invoice (Base)";
                DeletedTracking."Qty. to Invoice (Base)" := 0;
            end else begin
                if AltQtyItem then begin
                    Factor := ReservEntry."Qty. to Invoice (Base)" / DeletedTracking."Qty. to Invoice (Base)";
                    ReservEntry."Qty. to Invoice (Alt.)" := Round(DeletedTracking."Qty. to Invoice (Alt.)" * Factor, 0.00001);
                    if Abs(ReservEntry."Quantity (Alt.)") < Abs(ReservEntry."Qty. to Invoice (Alt.)") then
                        ReservEntry."Qty. to Invoice (Alt.)" := ReservEntry."Quantity (Alt.)";
                    DeletedTracking."Qty. to Invoice (Alt.)" -= ReservEntry."Qty. to Invoice (Alt.)";
                end;
                DeletedTracking."Qty. to Invoice (Base)" -= ReservEntry."Qty. to Invoice (Base)";
            end;
    ...
    local procedure IsInBoundLotAvailable(ReservationEntry: Record "Reservation Entry"): Boolean
    var
    ...
    begin
        ...
        case ReservationEntry."Source Type" of
            DATABASE::"Transfer Line":
                begin
                    if (ReservationEntry."Lot No." = '') or (LotStatusExclusionFilter = '') then // P800120327
                        exit(true);

                    if not LotInfo.Get(ReservationEntry."Item No.", ReservationEntry."Variant Code", ReservationEntry."Lot No.") then
                        exit(true);
                end;
            DATABASE::"Purchase Line":
                begin
                    if (ReservationEntry."Source Subtype" <> PurchaseLine."Document Type"::Order) or (ReservationEntry."Lot No." = '') or (LotStatusExclusionFilter = '') then
                        exit(true);

                    if not LotInfo.Get(ReservationEntry."Item No.", ReservationEntry."Variant Code", ReservationEntry."Lot No.") then
                        exit(not ExcludePurch)
                    else
                        if not LotInfo.Posted then
                            exit(not ExcludePurch);
                end;
            DATABASE::"Sales Line":
                begin
                    if (ReservationEntry."Source Subtype" <> SalesLine."Document Type"::"Return Order") or (ReservationEntry."Lot No." = '') or (LotStatusExclusionFilter = '') then
                        exit(true);

                    if not LotInfo.Get(ReservationEntry."Item No.", ReservationEntry."Variant Code", ReservationEntry."Lot No.") then
                        exit(not ExcludeSalesRet)
                    else
                        if not LotInfo.Posted then
                            exit(not ExcludeSalesRet);
                end;
            DATABASE::"Prod. Order Line":
                begin
                    if (ReservationEntry."Lot No." = '') or (LotStatusExclusionFilter = '') then
                        exit(true);

                    if not LotInfo.Get(ReservationEntry."Item No.", ReservationEntry."Variant Code", ReservationEntry."Lot No.") then
                        exit(not ExcludeOutput)
                    else
                        if not LotInfo.Posted then
                            exit(not ExcludeOutput);
                end;
        ...
        if LotInfo."Lot Status Code" = '' then
            exit(true);

        LotStatus.SetFilter(Code, LotStatusExclusionFilter);
        // P800120327
        LotStatus.FILTERGROUP(9);
        LotStatus.SETRANGE(Code, LotInfo."Lot Status Code");
        // P800120327        
        exit(LotStatus.IsEmpty);
    end;
