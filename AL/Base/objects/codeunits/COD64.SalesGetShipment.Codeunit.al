codeunit 64 "Sales-Get Shipment"
{
            ...
            if FindSet() then begin
                ...
                repeat
                    ...
                    if SalesShptHeader."No." <> "Document No." then begin
                        ...
                        IF SalesShptLine.Type = SalesShptLine.Type::Item THEN // P800127968
                            InsertAltQtyLinesFromSalesShipment(SalesShptLine, SalesLine); // P80070933                        
    ...
    local procedure InsertAltQtyLinesFromSalesShipment(pSalesShptLine: Record "Sales Shipment Line"; var pSalesLine: Record "Sales Line")
    var
    ...
    begin
        ...
        if not Item."Catch Alternate Qtys." then // P80080636
            exit;
        pSalesLine."Alt. Qty. Transaction No." := P800UtilityFns.GetNextTransNo;
        Clear(QtyAlt);

        with ItemLedgerEntry do begin
            SetRange("Document No.", pSalesShptLine."Document No.");
            SetRange("Document Line No.", pSalesShptLine."Line No.");
            SetRange("Document Type", ItemLedgerEntry."Document Type"::"Sales Shipment");
            if FindSet() then
                repeat
                    ...
                    if QtyAlt <> 0 then begin
                        TotQtyAlt += QtyAlt;
                        LineNum += 10000;
                        AltQtyMgmt.CreateAltQtyLine(AlternateQuantityLine, pSalesLine."Alt. Qty. Transaction No.", LineNum, Database::"Sales Line",
                                                      pSalesLine."Document Type", pSalesLine."Document No.", '', '', pSalesLine."Line No.");
                        AlternateQuantityLine."Quantity (Base)" := ItemLedgerEntry.Quantity - ItemLedgerEntry."Invoiced Quantity";
                        AlternateQuantityLine.Quantity := ROUND(AlternateQuantityLine."Quantity (Base)" / pSalesLine."Qty. per Unit of Measure", 0.00001);
        ...
        pSalesLine.Validate("Qty. to Ship (Alt.)", TotQtyAlt);
