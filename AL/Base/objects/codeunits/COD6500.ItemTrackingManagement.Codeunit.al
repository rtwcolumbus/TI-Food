...
{
    // PR3.70
    //   Alternate Quantity in new 3.70 functionality
    // 
    // PR3.70.03
    //   In SumUpItemTracking expand filter on quantity to handle to also filter on alternate quantity
    // 
    // PR3.70.07
    // P8000121A, Myers Nissi, Jack Reynolds, 23 SEP 04
    //   AdjustInvoiceRounding - adjusts Qty. to Invoice on temporary invoiceing specification table so that the
    //     total matches the Qty. to Invoice on the posted document lines
    // 
    ...
    //   Remove P8000121A
    //   CollectItemEntryRelation - add logic for alternate quantity
    // 
    // PR4.00.02
    // P8000282A, VerticalSoft, Jack Reynolds, 23 MAR 06
    //   SynchronizeWhseItemTracking - call function SynchronizeLinkedSources
    // 
    // PR4.00.04
    // P8000322A, Don Bresee, 01 AUG 06
    //   SynchronizeWhseItemTracking - add shipped quantity to source specification
    //   Check for blocked lots on pick for sales
    //   Add Lot No. to Whse. Worksheet
    //   Add Staged Picks
    // 
    // P8000404A, VerticalSoft, Jack Reynolds, 05 OCT 06
    //   Fix Navision bug that shows incorrect quantity availble in AssistEditLotSerialNo
    // 
    // PR4.00.05
    // P8000452B, VerticalSoft, Jack Reynolds, 19 FEB 07
    //   Fix problem with SynchronizeItemTracking when reducing transfer line quantity to ship to zero
    // 
    // P8000413A, VerticalSoft, Jack Reynolds, 02 APR 07
    //   Previous project P8000404A which fixed a Navision bug has been fixed in SP3 so P8000404A has been removed
    // 
    // PR5.00
    // P8000466A, VerticalSoft, Don Bresee, 12 JUN 07
    //   AssistEditLotSerialNo moved to codeunit 6501
    //   Moved IsLotAvailableForSale to codeunit 6501
    //   Alternate quantity support for new code
    // 
    // PRW15.00.01
    ...
    //   Bring the NAV 2009 versions of SynchronizeWhseItemTracking and ItemTrkgQtyPostedOnSource into this codeunit
    // 
    // P8000655, VerticalSoft, Jack Reynolds, 08 JAN 09
    //   Fix division by zero problem in RetrieveILEFromPostedInv
    // 
    // PRW16.00
    // P8000642, VerticalSoft, Jack Reynolds, 15 JAN 09
    //   Remove references to P8000654
    // 
    // PRW16.00.02
    ...
    // 
    // PRW16.00.05
    // P8000985, Columbus IT, Jack Reynolds, 21 OCT 11
    //   Fix problem with incorrect quantity on Warehouse Entries
    // 
    // PRW16.00.06
    // P8001108, Columbus IT, Jack Reynolds, 22 OCT 12
    //   Fix problem with transfer orders and partial picks
    // 
    // PRW17.00
    ...
    // 
    // PRW110.0.01
    // P80041932, To-Increase, Dayakar Battini, 26 JUN 17
    ...
    // 
    // PRW111.00.02
    // P80063543, To Increase, Jack Reynolds, 12 OCT 18
    //   Implement Microsoft fix
    //   Remove P800 reference when implementing NAV 2018 CU11 or later
    // 
    // P80068216, To Increase, Jack Reynolds, 10 DEC 18
    //   Allow blocking of COMMIT when running Item Tracking Lines page
    // 
    // P80068316, To Increase, Jack Reynolds, 12 DEC 18
    //   Undo pick for transfer otrder and containers
    // 
    // P80070693, To-Increase, Gangabhushan, 22 MAR 19
    //   TI-12871 - TO-Warehouse Shipment Error when undo the Registered Picks
    // 
    // PRW111.00.03
    // P80082330, To Increase, Jack Reynolds, 09 SEP 19
    //   Fix problem with Reservation Entry Qty. to Handle
    //
    // PRW114.00.03
    // P800101090, To-Increase, Gangabhushan, 01 JUL 20
    //   CS00112360 | Rounding Issues
    ...
    begin
        ...
        ItemTrackingLines.RegisterItemTrackingLines(
          TempSourceTrackingSpecification, DueDate, TempTrackingSpecification)
    end;
    ...
    begin
        ...
        ToTrackingSpecification."Qty. to Handle (Alt.)" := 0;  // PR3.60
        ToTrackingSpecification."Qty. to Invoice (Alt.)" := 0; // PR3.60
        ...
        ToTrackingSpecification."Quantity Handled (Alt.)" := FromReservEntry."Qty. to Handle (Alt.)";   // PR3.60
        ToTrackingSpecification."Quantity Invoiced (Alt.)" := FromReservEntry."Qty. to Invoice (Alt.)"; // PR3.60
    ...
    begin
        ...
        if ItemJnlLine."Phys. Inventory" then
            ReservEntry.SetFilter("Source Subtype", '%1|%2', ItemJnlLine."Entry Type"::"Positive Adjmt.", ItemJnlLine."Entry Type"::"Negative Adjmt.")
        else
            // P8007748
    ...
    begin
        ...
        // P80046533
        // NonZero := ReservEntry.GETFILTER("Qty. to Handle (Base)") = '<>0'; // PR3.70.03
        // IF NonZero THEN                                                    // PR3.70.03
        //  ReservEntry.SETRANGE("Qty. to Handle (Base)");                   // PR3.70.03
        if ReservEntry.GetFilter("Qty. to Handle (Base)") = '<>0' then begin
            ReservEntry.SetRange("Qty. to Handle (Base)");
            ReservEntry.FilterGroup(-1);
            ReservEntry.SetFilter("Qty. to Handle (Base)", '<>0');
            ReservEntry.SetFilter("Qty. to Handle (Alt.)", '<>0');
            ReservEntry.FilterGroup(0);
        end;
        // P80046533
        TempHandlingSpecification.Reset();
        ...
        if ReservEntry.FindSet() then begin
            ...
            repeat
                //    IF NonZero OR (ReservEntry."Qty. to Handle (Base)" <> 0) OR (ReservEntry."Qty. to Handle (Alt.)" <> 0) THEN // PR3.70.03, P80046533
                if ReservEntry.TrackingExists() then begin
                    ...
                    if TempHandlingSpecification.FindFirst() then begin
                        ...
                        TempHandlingSpecification."Quantity (Alt.)" += ReservEntry."Quantity (Alt.)";                   // PR3.60
                        TempHandlingSpecification."Qty. to Handle (Alt.)" += ReservEntry."Qty. to Handle (Alt.)";       // PR3.60
                        TempHandlingSpecification."Qty. to Invoice (Alt.)" += ReservEntry."Qty. to Invoice (Alt.)";     // PR3.60
            ...
            repeat
                if ReservEntry.TrackingExists() then begin
                    ...
                    if SwapSign then begin
                        ...
                        TempReservEntry."Quantity (Alt.)" := -TempReservEntry."Quantity (Alt.)"; // PR3.70
                        TempReservEntry."Qty. to Handle (Alt.)" := -TempReservEntry."Qty. to Handle (Alt.)"; // PR3.70
                        TempReservEntry."Qty. to Invoice (Alt.)" := -TempReservEntry."Qty. to Invoice (Alt.)"; // PR3.70
    ...
    begin
        ...
        exit(CollectItemEntryRelation(TempItemLedgEntry, SourceType, SourceSubtype, SourceID, SourceBatchName, SourceProdOrderLine, SourceRefNo, TotalQty, 0));
    end;

    procedure CollectItemEntryRelation(var TempItemLedgEntry: Record "Item Ledger Entry" temporary; SourceType: Integer; SourceSubtype: Integer; SourceID: Code[20]; SourceBatchName: Code[10]; SourceProdOrderLine: Integer; SourceRefNo: Integer; TotalQty: Decimal; TotalQtyAlt: Decimal)Result: Boolean
    var
    ...
    begin
        ...
        if IsHandled then
            exit(Result);
        // P8000267B - add parameter for TotalQtyAlt
        Quantity := 0;
        ...
        if ItemEntryRelation.FindSet() then
            repeat
                ...
                QuantityAlt := QuantityAlt + ItemLedgEntry."Quantity (Alt.)"; // P8000267B
            until ItemEntryRelation.Next() = 0;
        exit((Quantity = TotalQty) and (QuantityAlt = TotalQtyAlt)); // P8000267B
    end;
            ...
            if FindSet() then
                repeat
                    ...
                    TempWhseJnlLine2."Qty. (Absolute)" :=
                      Round(
                        TempWhseJnlLine2."Qty. (Absolute)" * SplitFactor, UOMMgt.QtyRndPrecision()); // P800101090
                    //TempWhseJnlLine2."Quantity (Absolute, Alt.)" := ABS("Quantity (Alt.)"); // P8000591A    // P80041932
                    if (TempWhseJnlLine.Quantity > 0) or (TempWhseJnlLine."Quantity (Alt.)" > 0) then begin // P8000591A
                        TempWhseJnlLine2."Qty. (Base)" := TempWhseJnlLine2."Qty. (Absolute, Base)";
                        TempWhseJnlLine2.Quantity := TempWhseJnlLine2."Qty. (Absolute)";
                        //TempWhseJnlLine2."Quantity (Alt.)" := TempWhseJnlLine2."Quantity (Absolute, Alt.)"; // P8000591A    // P80041932
                    end else begin
                        TempWhseJnlLine2."Qty. (Base)" := -TempWhseJnlLine2."Qty. (Absolute, Base)";
                        TempWhseJnlLine2.Quantity := -TempWhseJnlLine2."Qty. (Absolute)";
                        //TempWhseJnlLine2."Quantity (Alt.)" := -TempWhseJnlLine2."Quantity (Absolute, Alt.)"; // P8000591A    // P80041932
        ...
        with WhseWkshLine do begin
            ...
            if WhseItemEntryRelation.FindSet() then
                repeat
                    ...
                    WhseItemTrackingLine.Quantity :=                                                                           // P8000985
                      Round(WhseItemTrackingLine."Quantity (Base)" / WhseItemTrackingLine."Qty. per Unit of Measure", 0.00001); // P8000985
    ...
    begin
        ...
        WhseItemTrackingLine.Quantity := -SourceReservEntry.Quantity; // P8000985
    ...
    begin
        case WhseItemTrkgLine."Source Type" of
            ...
            // P8000322A
            DATABASE::"Whse. Staged Pick Line":
                WhseItemTrkgLine."Source Type Filter" := WhseItemTrkgLine."Source Type Filter"::"Staged Pick";
        // P8000322A
        end;
    ...
    begin
        ...
        // P8001290
        Item.Get(ItemNo);
        if Item."Non-Warehouse Item" then
            exit(false);
        // P8001290
    ...
    begin
        ...
        if ReservEntry2.IsEmpty() then begin
            ...
            // P80046533
            //  IF FromReservEntry."Source Type" = DATABASE::"Transfer Line" THEN // P8000452B
            //    FromReservEntry.SETFILTER("Qty. to Handle (Base)",'<>0');       // P8000452B
            // P80046533
            ...
            if TempTrkgSpec1.FindSet() then
                repeat
                    ...
                    if TempTrkgSpec2.FindFirst() then begin
                        ShouldInsertTrkgSpec := (TempTrkgSpec2."Quantity (Base)" * SignFactor2 <> TempTrkgSpec1."Quantity (Base)" * SignFactor1) or // P8000267B
                           (TempTrkgSpec2."Quantity (Alt.)" * SignFactor2 <> TempTrkgSpec1."Quantity (Alt.)" * SignFactor1);    // P8000267B
                        ...
                        if ShouldInsertTrkgSpec then begin
                            ...
                            // P80068316
                            // P80070693
                            OnGetUndoPick(FromUndoPick);
                            if FromUndoPick then
                                // P80070693
                                TempTrkgSpec3.Validate("Qty. to Handle (Base)",
                                  (TempTrkgSpec1."Qty. to Handle (Base)" * SignFactor1 - TempTrkgSpec2."Qty. to Handle (Base)" * SignFactor2));
                            // P80068316
                            // P8000267B
                            TempTrkgSpec3."Quantity (Alt.)" :=
                              TempTrkgSpec1."Quantity (Alt.)" * SignFactor1 - TempTrkgSpec2."Quantity (Alt.)" * SignFactor2;
                            TempTrkgSpec3."Qty. to Handle (Alt.)" :=
                              TempTrkgSpec1."Qty. to Handle (Alt.)" * SignFactor1 - TempTrkgSpec2."Qty. to Handle (Alt.)" * SignFactor2;
                            // P8000267B
                        ...
                        if TempTrkgSpec1."Quantity (Alt.)" <> 0 then                                               // P8000267B
                            TempTrkgSpec3.Validate("Quantity (Alt.)", TempTrkgSpec1."Quantity (Alt.)" * SignFactor1); // P8000267B
                        TempTrkgSpec3."Entry No." := LastEntryNo + 1;
                        TempTrkgSpec3.Insert();
                    end;
            ...
            if TempTrkgSpec2.FindFirst() then
                repeat
                    ...
                    // P80063375
                    if TempTrkgSpec3."Quantity (Base)" < 0 then
                        TempTrkgSpec3."Qty. to Handle (Base)" := -Abs(TempTrkgSpec2."Qty. to Handle (Base)")
                    else
                        TempTrkgSpec3."Qty. to Handle (Base)" := Abs(TempTrkgSpec2."Qty. to Handle (Base)");
                    // P80063375
                    if TempTrkgSpec2."Quantity (Alt.)" <> 0 then                                                // P8000267B
                        TempTrkgSpec3.Validate("Quantity (Alt.)", -TempTrkgSpec2."Quantity (Alt.)" * SignFactor2); // P8000267B
                    TempTrkgSpec3."Entry No." := LastEntryNo + 1;
                    TempTrkgSpec3.Insert();
                    LastEntryNo := TempTrkgSpec3."Entry No.";
            ...
            if not TempTrkgSpec3.IsEmpty() then begin
                ...
                TempSourceSpec."Quantity (Alt.)" := ReservMgt.GetSourceRecordValue(ReservEntry2, false, 5); // P8000267B
                ...
                ItemTrackingLines.SetBlockCommit(BlockCommit); // P80068216
    ...
    begin
        ...
        if TempReservEntry."Source Type" = DATABASE::"Transfer Line" then begin
            //TransLine.GET(TempReservEntry."Source ID",TempReservEntry."Source Ref. No."); // P8000267B
            // P8000267B
            if not TransLine.Get(TempReservEntry."Source ID", TempReservEntry."Source Ref. No.") then begin
                TransHeader.Get(TempReservEntry."Source ID");
                TransLine."Transfer-from Code" := TransHeader."Transfer-from Code";
                TransLine."Transfer-to Code" := TransHeader."Transfer-to Code";
                TransLine."Shipment Date" := TransHeader."Shipment Date";
                TransLine."Receipt Date" := TransHeader."Receipt Date";
            end;
            // P8000267B
    ...
    begin
        ...
        if TempTrackingSpec.FindSet() then
            repeat
                ...
                TempTrackingSpec.CalcSums("Quantity (Base)"); // P80046533
                ...
                TrackingSpec."Quantity (Base)" :=
                  TempTrackingSpec."Quantity (Base)" + // P80046533
                ...
                ItemTrackingLines.SetBlockCommit(true); // P80046533
    ...
    local procedure AddWhseShippedQty(var SourceSpecification2: Record "Tracking Specification")
    var
        ItemEntryRelation: Record "Item Entry Relation";
        ItemLedgEntry: Record "Item Ledger Entry";
    begin
        // P8000322A
        if IsWhseShptSource(SourceSpecification2) then
            with ItemEntryRelation do begin
                SetCurrentKey("Order No.", "Order Line No.");
                SetRange("Order No.", SourceSpecification2."Source ID");
                SetRange("Order Line No.", SourceSpecification2."Source Ref. No.");
                SetRange("Lot No.", SourceSpecification2."Lot No.");
                SetRange("Serial No.", SourceSpecification2."Serial No.");
                if Find('-') then
                    repeat
                        ItemLedgEntry.Get("Item Entry No.");
                        SourceSpecification2."Quantity (Base)" :=
                          SourceSpecification2."Quantity (Base)" - ItemLedgEntry.Quantity;
                    until (Next = 0);
            end;
        // P8000322A
    end;
    ...
    procedure CheckWhsePickForSaleLot(var WhseActLine: Record "Warehouse Activity Line")
    var
        LotNoInfo: Record "Lot No. Information";
    begin
        // P8000322A
        with WhseActLine do
            if ("Source Type" = DATABASE::"Sales Line") and ("Activity Type" = "Activity Type"::Pick) and ("Lot No." <> '') then
                if LotNoInfo.Get("Item No.", "Variant Code", "Lot No.") then
                    LotNoInfo.TestField(Blocked, false);
        // P8000322A
    end;

    procedure WhseAssistEdit(LocationCode: Code[10]; ItemNo: Code[20]; VariantCode: Code[10]; BinCode: Code[20]; var Text: Text[1024]): Boolean
    var
        TempTrackingSpecification: Record "Tracking Specification" temporary;
    ...
    begin
        ...
        with TempTrackingSpecification do begin
            ...
            "Bin Code" := BinCode;
            ItemTrackingDCMgt.AssistEditTrackingNo(      // P8000466A
              TempTrackingSpecification, true, -1, 1, 0);
            if ("Lot No." = '') then
                exit(false);
            Text := "Lot No.";
            exit(true);
    ...
    begin
        with TotalWhseItemTrackingLine do begin
            ...
            case SourceType of
              ...
              DATABASE::"Warehouse Shipment Line",
              DATABASE::"Whse. Internal Put-away Line",
              DATABASE::"Whse. Staged Pick Line", // P8007748
              DATABASE::"Whse. Internal Pick Line",
              DATABASE::"Assembly Line",
              DATABASE::Job,
    ...
    begin
        ...
        with ReservEntry do begin
            ...
            // P80082330
            if (SalesLine."Qty. to Ship" = 0) and (SalesLine."Return Qty. to Receive" = 0) then begin
    ...
    begin
        ...
        with ReservEntry do begin
            ...
            // P80082330
            if (PurchaseLine."Qty. to Receive" = 0) and (PurchaseLine."Return Qty. to Ship" = 0) then begin
                "Qty. to Handle (Base)" := 0;
                "Qty. to Invoice (Base)" := 0;
            end;
            // P80082330
    ...
    [IntegrationEvent(false, false)]
    local procedure OnGetUndoPick(var pUndoPick: Boolean)
    begin
    end;
