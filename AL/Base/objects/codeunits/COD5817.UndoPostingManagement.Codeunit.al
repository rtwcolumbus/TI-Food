codeunit 5817 "Undo Posting Management"
{
    ...
    // ID          Date        Description
    // --------------------------------------------------------------------------------
    // N138F0000 , 01-07-2015, Initial Version
    // --------------------------------------------------------------------------------
    // 
    // PR4.00.01
    // P8000267B, VerticalSoft, Jack Reynolds, 12 DEC 05
    //   Add code for alternate quantities
    // 
    // PRW15.00.01
    // P8000585A, VerticalSoft, Jack Reynolds, 03 MAR 08
    //   Undo posting of lot information
    // 
    // PRW19.00.01
    ...
    var
        ...
        IsHandled: Boolean;
    begin
        if Settlement then exit; //N138F0000.n
        IsHandled := false;
        ...
        with TempItemLedgEntry do begin
            ...
            repeat
                ...
                if not IsHandled then
                    ...
                    // P8000267B
                    if Positive then
                        TestField("Remaining Quantity (Alt.)", "Quantity (Alt.)");
                    // P8000267B
    ...
    begin
        ...
        repeat
            ...
            ItemJnlLine."Quantity (Alt.)" := -TempApplyToItemLedgEntry."Quantity (Alt.)"; // P8000267B
    ...
    begin
        ...
        CollectItemLedgEntries(TempItemLedgEntry, SourceType,DocumentNo, LineNo, BaseQty, 0, EntryRef);
    end;

    procedure CollectItemLedgEntries(var TempItemLedgEntry: Record "Item Ledger Entry" temporary; SourceType: Integer; DocumentNo: Code[20]; LineNo: Integer; BaseQty: Decimal; AltQty: Decimal; EntryRef: Integer)
    var
    ...
    begin
        ...
        if EntryRef <> 0 then begin
            ...
            then begin                             // P8000267B
                BaseQty := BaseQty * -1;
                AltQty := AltQty * -1;               // P8000267B
            end;                                   // P8000267B
            CheckMissingItemLedgers(TempItemLedgEntry, SourceType, DocumentNo, LineNo, BaseQty, AltQty); // P800131264, P8000267B
        end;
    end;

    local procedure CheckMissingItemLedgers(var TempItemLedgEntry: Record "Item Ledger Entry" temporary; SourceType: Integer; DocumentNo: Code[20]; LineNo: Integer; BaseQty: Decimal; AltQty: Decimal)
    var
        ItemTrackingMgt: Codeunit "Item Tracking Management";
        IsHandled: Boolean;
    begin
        // P800131264, P8000267B - add parameter for AltQty
        IsHandled := false;
        OnBeforeCheckMissingItemLedgers(TempItemLedgEntry, SourceType, DocumentNo, LineNo, BaseQty, IsHandled);
        if IsHandled then
            exit;

        if not ItemTrackingMgt.CollectItemEntryRelation(TempItemLedgEntry, SourceType, 0, DocumentNo, '', 0, LineNo, BaseQty, AltQty) then // P800131264, P8000267B
    ...
    begin
        // P80096141 - Original signature
        UpdatePurchLine(PurchLine, UndoQty, UndoQtyBase, 0, TempUndoneItemLedgEntry);   
    end;

    procedure UpdatePurchLine(PurchLine: Record "Purchase Line"; UndoQty: Decimal; UndoQtyBase: Decimal; UndoQtyAlt: Decimal; var TempUndoneItemLedgEntry: Record "Item Ledger Entry" temporary)
    var
            ...
            case "Document Type" of
                "Document Type"::"Return Order":
                    begin
                        ...
                        "Return Qty. Shipped (Alt.)" := "Return Qty. Shipped (Alt.)" - UndoQtyAlt; // P8000267B
                        InitOutstanding();
                ...
                "Document Type"::Order:
                    begin
                        ...
                        "Qty. Received (Alt.)" := "Qty. Received (Alt.)" - UndoQtyAlt; // P8000267B
                        InitOutstanding();
                        if PurchSetup."Default Qty. to Receive" = PurchSetup."Default Qty. to Receive"::Blank then
                            "Qty. to Receive" := 0
                        else
    ...
    begin
        // P80096141 - Original signature
        UpdateSalesLine(SalesLine, UndoQty, UndoQtyBase, 0, TempUndoneItemLedgEntry);   
    end;

    procedure UpdateSalesLine(var SalesLine: Record "Sales Line"; UndoQty: Decimal; UndoQtyBase: Decimal; UndoQtyAlt: Decimal; var TempUndoneItemLedgEntry: Record "Item Ledger Entry" temporary)
    var
            ...
            case "Document Type" of
                "Document Type"::"Return Order":
                    begin
                        ...
                        "Return Qty. Received (Alt.)" := "Return Qty. Received (Alt.)" - UndoQtyAlt; // P8000267B
                ...
                "Document Type"::Order:
                    begin
                        ...
                        "Qty. Shipped (Alt.)" := "Qty. Shipped (Alt.)" - UndoQtyAlt; // P8000267B
                        OnUpdateSalesLineOnBeforeInitOustanding(SalesLine, UndoQty, UndoQtyBase);
                        InitOutstanding();
                        if SalesSetup."Default Quantity to Ship" = SalesSetup."Default Quantity to Ship"::Blank then
                            "Qty. to Ship" := 0
    ...
    begin
        ...
        if ProcessFns.TrackingInstalled then         // P8000585A
            P800Tracking.UndoPostLotData(ItemJnlLine); // P8000585A
