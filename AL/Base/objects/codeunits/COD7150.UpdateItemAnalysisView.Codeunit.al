codeunit 7150 "Update Item Analysis View"
{
    ...
    //   Modified to maintain fields for alternate quantities
    // 
    // PRW16.00.05
    // P8000921, Columbus IT, Don Bresee, 08 APR 11
    //   Add "Sales Amount (FOB)" and "Sales Amount (Freight)" fields
    // 
    // P8000922, Columbus IT, Don Bresee, 24 MAR 11
    //   Fix to totalling of budget entries
    // 
    // PRW17.00
    ...
    //
    // PRW111.00.03
    // P80096534, To-Increase, Gangabhushan, 31 MAR 20
    //   CS00100305 - Inventory Analysis View_Qty (ALT)
    ...
    begin
        ...
        if TempItemAnalysisViewEntry.Find() then begin
            ...
            // P8000500A
            begin
                AddValue(TempItemAnalysisViewEntry.Quantity,
                  GetQuantity(ItemAnalysisViewSource.ColumnNo(ILEQuantity), false)); // P8001132
                AddValue(TempItemAnalysisViewEntry."Quantity (Alt.)",
                  GetQuantity(ItemAnalysisViewSource.ColumnNo(ILEQuantity), true));  // P8001132
            end;
            AddValue(TempItemAnalysisViewEntry."Invoiced Quantity",
              GetQuantity(ItemAnalysisViewSource.ColumnNo(InvoicedQuantity), false)); // P8001132
            AddValue(TempItemAnalysisViewEntry."Invoiced Quantity (Alt.)",
              GetQuantity(ItemAnalysisViewSource.ColumnNo(InvoicedQuantity), true));  // P8001132
                                                                                      // P8000500A

            AddValue(TempItemAnalysisViewEntry."Sales Amount (Actual)", ItemAnalysisViewSource.SalesAmountActual);
            AddValue(TempItemAnalysisViewEntry."Sales Amount (FOB)", ItemAnalysisViewSource.SalesAmountFOB);         // P8000921, P8001132
            AddValue(TempItemAnalysisViewEntry."Sales Amount (Freight)", ItemAnalysisViewSource.SalesAmountFreight); // P8000921, P8001132
            ...
            // P8000500A
            begin
                TempItemAnalysisViewEntry.Quantity := GetQuantity(ItemAnalysisViewSource.ColumnNo(ILEQuantity), false);         // P8001132
                TempItemAnalysisViewEntry."Quantity (Alt.)" := GetQuantity(ItemAnalysisViewSource.ColumnNo(ILEQuantity), true); // P8001132
            end;
            TempItemAnalysisViewEntry."Invoiced Quantity" := GetQuantity(ItemAnalysisViewSource.ColumnNo(InvoicedQuantity), false);       // P8001132
            TempItemAnalysisViewEntry."Invoiced Quantity (Alt.)" := GetQuantity(ItemAnalysisViewSource.ColumnNo(InvoicedQuantity), true); // P8001132
                                                                                                                                          // P8000500A

            TempItemAnalysisViewEntry."Sales Amount (Actual)" := ItemAnalysisViewSource.SalesAmountActual;
            TempItemAnalysisViewEntry."Sales Amount (FOB)" := ItemAnalysisViewSource.SalesAmountFOB;         // P8000921, P8001132
            TempItemAnalysisViewEntry."Sales Amount (Freight)" := ItemAnalysisViewSource.SalesAmountFreight; // P8000921, P8001132
    ...
    begin
        ...
        if TempItemAnalysisViewBudgEntry.Find() then begin
            ...
            AddValue(TempItemAnalysisViewBudgEntry."Sales Amount (FOB)", ItemBudgetEntry."Sales Amount (FOB)");         // P8000921
            AddValue(TempItemAnalysisViewBudgEntry."Sales Amount (Freight)", ItemBudgetEntry."Sales Amount (Freight)"); // P8000921
            ...
            AddValue(TempItemAnalysisViewBudgEntry."Quantity (Alt.)", ItemBudgetEntry."Quantity (Alt.)"); // P8000500A
            TempItemAnalysisViewBudgEntry.Modify();
        end else begin
            TempItemAnalysisViewBudgEntry."Sales Amount" := ItemBudgetEntry."Sales Amount";
            TempItemAnalysisViewBudgEntry."Sales Amount (FOB)" := ItemBudgetEntry."Sales Amount (FOB)";         // P8000921
            TempItemAnalysisViewBudgEntry."Sales Amount (Freight)" := ItemBudgetEntry."Sales Amount (Freight)"; // P8000921
            ...
            TempItemAnalysisViewBudgEntry."Quantity (Alt.)" := ItemBudgetEntry."Quantity (Alt.)"; // P8000500A
            ...
            repeat
                ...
                then begin
                    ...
                    if not ItemAnalysisViewEntry.Insert() then begin
                        ...
                        // P80096534
                        AddValue(ItemAnalysisViewEntry."Quantity (Alt.)", TempItemAnalysisViewEntry."Quantity (Alt.)");
                        AddValue(ItemAnalysisViewEntry."Invoiced Quantity (Alt.)", TempItemAnalysisViewEntry."Invoiced Quantity (Alt.)");
                        // P80096534
                        ...
                        // P8000921
                        AddValue(ItemAnalysisViewEntry."Sales Amount (FOB)", TempItemAnalysisViewEntry."Sales Amount (FOB)");
                        AddValue(ItemAnalysisViewEntry."Sales Amount (Freight)", TempItemAnalysisViewEntry."Sales Amount (Freight)");
                        // P8000921
        ...
        if TempItemAnalysisViewBudgEntry.FindSet() then
            repeat
                ...
                if not ItemAnalysisViewBudgetEntry.Insert() then begin
                    ...
                    // P8000921
                    AddValue(ItemAnalysisViewBudgetEntry."Sales Amount (FOB)", TempItemAnalysisViewBudgEntry."Sales Amount (FOB)");
                    AddValue(ItemAnalysisViewBudgetEntry."Sales Amount (Freight)", TempItemAnalysisViewBudgEntry."Sales Amount (Freight)");
                    // P8000921
                    // P8000922
                    // AddValue(ItemAnalysisViewBudgetEntry."Sales Amount",TempItemAnalysisViewBudgEntry."Cost Amount");
                    AddValue(ItemAnalysisViewBudgetEntry."Cost Amount", TempItemAnalysisViewBudgEntry."Cost Amount");
                    // AddValue(ItemAnalysisViewBudgetEntry."Sales Amount",TempItemAnalysisViewBudgEntry.Quantity);
                    AddValue(ItemAnalysisViewBudgetEntry.Quantity, TempItemAnalysisViewBudgEntry.Quantity);
                    AddValue(ItemAnalysisViewBudgetEntry."Quantity (Alt.)", TempItemAnalysisViewBudgEntry."Quantity (Alt.)");
                    // P8000922
    ...
    procedure GetQuantity(ColNo: Integer; AltQty: Boolean): Decimal
    begin
        // P8001132
        with ItemAnalysisViewSource do begin
            if ItemLedgEntry."Entry No." <> ItemLedgerEntryNo then
                ItemLedgEntry.Get(ItemLedgerEntryNo);

            if (ItemLedgEntry.CostInAlternateUnits = AltQty) then begin
                case ColNo of
                    ColumnNo(ILEQuantity):
                        exit(ILEQuantity);
                    ColumnNo(InvoicedQuantity):
                        exit(InvoicedQuantity);
                end;
            end else begin
                case ColNo of
                    ColumnNo(ILEQuantity):
                        if (ItemLedgerEntryType in [ItemLedgerEntryType::"Positive Adjmt.", ItemLedgerEntryType::"Negative Adjmt."]) or
                          (ILEQuantity <> 0)
                        then
                            if AltQty then
                                exit(ItemLedgEntry."Quantity (Alt.)")
                            else
                                exit(ItemLedgEntry.Quantity);
                    ColumnNo(InvoicedQuantity):
                        if AltQty then begin
                            if InvoicedQuantity = ItemLedgEntry."Invoiced Quantity" then
                                exit(ItemLedgEntry."Invoiced Quantity (Alt.)")
                            else
                                if ItemLedgEntry."Invoiced Quantity" <> 0 then
                                    exit(Round(ItemLedgEntry."Invoiced Quantity (Alt.)" * InvoicedQuantity / ItemLedgEntry."Invoiced Quantity", 0.00001));
                        end else begin
                            if InvoicedQuantity = ItemLedgEntry."Invoiced Quantity (Alt.)" then
                                exit(ItemLedgEntry."Invoiced Quantity")
                            else
                                if ItemLedgEntry."Invoiced Quantity (Alt.)" <> 0 then
                                    exit(Round(ItemLedgEntry."Invoiced Quantity" * InvoicedQuantity / ItemLedgEntry."Invoiced Quantity (Alt.)", 0.00001));
                        end;
                end;
            end;
        end;
