codeunit 5760 "Whse.-Post Receipt"
{
    ...
    // ID          Date        Description
    // --------------------------------------------------------------------------------
    // N138F0000 , 23-10-2014, Initial Version
    // --------------------------------------------------------------------------------
    // 
    // PR4.00.02
    ...
    //   Temp table for item relation entries must be cleared after the entries are inserted.
    // 
    // P8000358A, VerticalSoft, Phyllis McGovern, 26 JUL 06
    //   Added logic to clear 'ADC Started'
    //   Changed function 'CreatePutAwayDoc' Local property to 'no'
    // 
    // PR4.00.06
    ...
    // 
    // PRW17.10.01
    // P8001261, Columbus IT, Jack Reynolds, 14 JAN 14
    //   Fix permission problem when not creating put-aways
    // 
    // PRW19.00.01
    ...
    begin
        ...
        with WhseRcptLine do begin
            ...
            WhseRcptHeader."ADC Started" := false; // P8000358A
            ...
            FindSet(true, true);
            GetLocation("Location Code");                                // P8000282A
            PutAwayRequired := Location.RequirePutaway("Location Code"); // P8000282A
            ...
            repeat
                ...
                if ShouldCreatePutAway then begin
                    CreatePutAwayDoc(WhseRcptHeader, WhseRcptLine); // P8000282A
                    if not SuppressCommit then
                        Commit();
                end;
            ...
            */
            if PutAwayRequired then // P8001261
                            ...
                            repeat
                                ...
                                if not IsHandled then begin
                                    ...
                                    if ModifyLine and SalesLine.TrackAlternateUnits() then // P8000282A
                                        AltQtyMgmt.UpdateSalesOnWhsePost(SalesLine, false);  // P8000282A
                        ...
                        if TransLine.Find('-') then
                            repeat
                                ...
                                if FindFirst() then begin
                                    ...
                                    ModifyLine := (TransLine.Type <> TransLine.Type::Container) and (TransLine."Qty. to Receive" <> 0); // P8001323
                                    if ModifyLine then
                                ...
                                if ModifyLine and TransLine.TrackAlternateUnits() then             // P8000282A
                                    AltQtyMgmt.UpdateTransOnWhsePost(TransLine, Direction::Inbound); // P8000282A
    ...
    begin
        ...
        if not IsHandled then
            with WhseRcptLine2 do
                if "Source Document" = "Source Document"::"Purchase Order" then begin
                    ModifyLine := (PurchLine.Type <> PurchLine.Type::FOODContainer) and (PurchLine."Qty. to Receive" <> 0); // P8001323
                    if ModifyLine then
                        PurchLine.Validate("Qty. to Receive", 0);
                end else begin
                    ModifyLine := (PurchLine.Type <> PurchLine.Type::FOODContainer) and (PurchLine."Return Qty. to Ship" <> 0); // P8001323
                    if ModifyLine then
                ...
                DATABASE::"Purchase Line":
                    begin
                        ...
                        if not IsHandled then begin
                            ...
                            // P800162917
                            // PurchPost.SetPostingLocation(WhseRcptHeader."Location Code"); // P8000282A
                ...
                DATABASE::"Sales Line": // Return Order
                    begin
                        ...
                        if not IsHandled then begin
                            ...
                            // P800162917
                            // SalesPost.SetPostingLocation(WhseRcptHeader."Location Code"); // P8000282A
                            BindSubscription(FoodManualSubscriptions);
                            FoodManualSubscriptions.SetPostingLocation("Location Code");
                            // P800162917
    ...
    begin
        ...
        MessageText := StrSubstNo(MessageText, CounterSourceDocOK, CounterSourceDocTotal, CounterPutAways);
    end;
    ...
    begin
        ...
        with PostedWhseRcptLine do begin
            ...
            GetLocation("Location Code");
            // IF NOT Location."Require Put-away" THEN BEGIN                                     // P8001290
            if (not Location."Require Put-away") or WhseRcptLine.IsNonWarehouseItem() then begin // P8001290
        ...
        PostedWhseRcptHeader.UpdateDocumentStatus; // P8001290

        if WhseRcptLine.IsNonWarehouseItem() then // P8001290
            exit;                                   // P8001290
    ...
    procedure CreatePutAwayDoc(WhseRcptHeader: Record "Warehouse Receipt Header"; WhseRcptLine: Record "Warehouse Receipt Line")
    var
        WhseActivHeader: Record "Warehouse Activity Header";
    ...
    begin
        ...
        // P8000282A
        PostedWhseRcptLine.SetRange("Source Type", WhseRcptLine."Source Type");
        PostedWhseRcptLine.SetRange("Source Subtype", WhseRcptLine."Source Subtype");
        PostedWhseRcptLine.SetRange("Source No.", WhseRcptLine."Source No.");
        // P8000282A
        PostedWhseRcptLine.SetFilter(Status, '<%1', PostedWhseRcptLine.Status::"Completely Put Away"); // P8001290
