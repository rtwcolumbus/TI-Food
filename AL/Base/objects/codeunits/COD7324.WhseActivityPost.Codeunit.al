codeunit 7324 "Whse.-Activity-Post"
{
    ...
    //   Support for running from order shipping and receiving; alternate quantities
    // 
    // PR4.00.06
    // P8000501A, VerticalSoft, Jack Reynolds, 03 AUG 07
    //   Create invoices for extra charges
    // 
    // PRW16.00.01
    // P8000662, VerticalSoft, Jack Reynolds, 22 JAN 09
    //   Fix problem updatinbg line number on source document lines
    // 
    // PRW16.00.05
    ...
    // 
    // PRW16.00.06
    // P8001039, Columbus IT, Don Bresee, 28 FEB 12
    //   Add Rounding Adjustment logic for Warehouse
    //   Remove local ItemJnlPostLine from PostConsumptionLine and PostOutputLine, add as global
    // 
    // PRW17.00
    ...
    //
    // PRW111.00.03
    // P80095316, To Increase, Jack Reynolds, 05 MAR 20
    //   Problem setting "Quantity Handled (Alt.)"
    // 
    // PRW113.00
    ...
    var
        ...
        TempWhseRoundingAdjmts: Record "Warehouse Journal Line" temporary;
        ...
        with WhseActivHeader do begin
            ...
            if WhseActivLine.Find('-') then begin
                ...
                repeat
                    ...
                    // P8000282A
                    if WhseActivLine.TrackAlternateUnits() then
                        AltQtyMgmt.CheckBaseQty(
                          WhseActivLine."Item No.", WhseActivLine."Serial No.", WhseActivLine."Lot No.",
                          WhseActivLine."Alt. Qty. Transaction No.", WhseActivLine.FieldCaption("Qty. to Handle (Base)"),
                          WhseActivLine."Qty. to Handle (Base)");
                    // P8000282A
            ...
            if WhseActivLine.Find('-') then
                repeat
                    ...
                    if (WhseActivLine."Qty. Outstanding" = WhseActivLine."Qty. to Handle") or ForceDelete then
                        ...
                        QuantityToHandleAlt := WhseActivLine."Qty. to Handle (Alt.)"; // P80095316
                        ...
                        // P8000282A
                        if WhseActivLine.TrackAlternateUnits() then begin
                            WhseActivLine.VALIDATE("Quantity Handled (Alt.)",
                              WhseActivLine."Quantity Handled (Alt.)" + QuantityToHandleAlt); // P80095316
                            WhseActivLine.VALIDATE("Qty. to Handle (Alt.)", 0);
                        end;
                        // P8000282A
    ...
    begin
        ...
        with WhseActivLine do begin
            ...
            if TempWhseActivLine.Find('-') then begin
                ...
                TempWhseActivLine."Qty. to Handle (Alt.)" += "Qty. to Handle (Alt.)"; // P8000282A
            ...
            case "Source Type" of
                DATABASE::"Purchase Line":
                    begin
                        ...
                        PurchLine.SetRange("Location Code", "Location Code"); // P8000282A
                        ...
                        if PurchLine.Find('-') then
                            repeat
                                ...
                                if not IsHandled then begin
                                    ...
                                    // P8000282A
                                    if PurchLine.TrackAlternateUnits() then
                                        if "Source Document" = "Source Document"::"Purchase Order" then
                                            PurchLine.Validate("Qty. to Receive (Alt.)", 0)
                                        else
                                            PurchLine.Validate("Return Qty. to Ship (Alt.)", 0);
                                    // P8000282A
                ...
                DATABASE::"Sales Line":
                    begin
                        ...
                        SalesLine.SetRange("Location Code", "Location Code"); // P8000282A
                            ...
                            repeat
                                ...
                                if not IsHandled then begin
                                    ...
                                    if SalesLine.TrackAlternateUnits() then
                                        if "Source Document" = "Source Document"::"Sales Order" then
                                            SalesLine.Validate("Qty. to Ship (Alt.)", 0)
                                        else
                                            SalesLine.Validate("Return Qty. to Receive (Alt.)", 0);
                                    // P8000282A
                    ...
                    begin
                        ...
                        if TransLine.Find('-') then
                            repeat
                                ...
                                // P8000282A
                                if TransLine.TrackAlternateUnits() then
                                    if ("Source Document" = "Source Document"::"Outbound Transfer") then
                                        TransLine.Validate("Qty. to Ship (Alt.)", 0)
                                    else
                                        TransLine.Validate("Qty. to Receive (Alt.)", 0);
                                // P8000282A
                    ...
                    begin
                        if "Activity Type" = "Activity Type"::"Invt. Pick" then begin
                            ...
                            if TrackAlternateUnits() then
                                if "Activity Type" = "Activity Type"::"Invt. Pick" then
                                    "Qty. to Handle (Alt.)" := -"Qty. to Handle (Alt.)";
                            // P8000282A
                        ...
                        if "Source Document" = "Source Document"::"Purchase Order" then begin
                            ...
                            if TrackAlternateUnits() then                                            // P8000282A
                                PurchLine.Validate("Qty. to Receive (Alt.)", "Qty. to Handle (Alt.)"); // P8000282A
                            ...
                            if TrackAlternateUnits() then                                                 // P8000282A
                                PurchLine.Validate("Return Qty. to Ship (Alt.)", -"Qty. to Handle (Alt.)"); // P8000282A
                            PurchLine."Return Qty. to Ship (Base)" := -"Qty. to Handle (Base)";
                            if InvoiceSourceDoc then
                                PurchLine.Validate("Qty. to Invoice", -"Qty. to Handle");
                        end;
                        PurchLine."Bin Code" := "Bin Code";
                        PurchLine.GetLotNo; // P8000662			
                ...
                DATABASE::"Sales Line":
                    begin
                        ...
                        if "Source Document" = "Source Document"::"Sales Order" then begin
                            ...
                            if TrackAlternateUnits() then                                          // P8000282A
                                SalesLine.Validate("Qty. to Ship (Alt.)", -"Qty. to Handle (Alt.)"); // P8000282A
                            ...
                            if TrackAlternateUnits() then                                                   // P8000282A
                                SalesLine.Validate("Return Qty. to Receive (Alt.)", "Qty. to Handle (Alt.)"); // P8000282A
                            SalesLine."Return Qty. to Receive (Base)" := "Qty. to Handle (Base)";
                            if InvoiceSourceDoc then
                                SalesLine.Validate("Qty. to Invoice", "Qty. to Handle");
                        end;
                        SalesLine."Bin Code" := "Bin Code";
                        SalesLine.GetLotNo; // P8000662
                        SalesLine.Modify();
                ...
                DATABASE::"Transfer Line":
                    begin
                        ...
                        if "Activity Type" = "Activity Type"::"Invt. Put-away" then begin
                            ...
                            if TrackAlternateUnits() then                                            // P8000282A
                                TransLine.Validate("Qty. to Receive (Alt.)", "Qty. to Handle (Alt.)"); // P8000282A
                            ...
                            if TrackAlternateUnits() then                                         // P8000282A
                                TransLine.Validate("Qty. to Ship (Alt.)", "Qty. to Handle (Alt.)"); // P8000282A
            ...
            case "Source Type" of
                DATABASE::"Purchase Line":
                    begin
                        ...
                        // P800162917
                        // PurchPost.SetPostingLocation("Location Code"); // P8000282A
                        BindSubscription(FoodManualSubscriptions);
                        FoodManualSubscriptions.SetPostingLocation("Location Code");
                        // P800162917
                        PurchPost.SetWhseActHeader(WhseActivHeader);   // P8000282A
                        OnPostSourceDocumentOnBeforePurchPostRun(WhseActivHeader, PurchHeader);
                        PurchPost.Run(PurchHeader);
                        UnbindSubscription(FoodManualSubscriptions); // P800162917
                        PurchPost.GetWhseRoundingAdjmts(TempWhseRoundingAdjmts); // P8001039
                        if "Source Document" = "Source Document"::"Purchase Order" then begin
                ...
                DATABASE::"Sales Line":
                    begin
                        ...
                        // P800162917
                        // SalesPost.SetPostingLocation("Location Code"); // P8000282A
                        BindSubscription(FoodManualSubscriptions);
                        FoodManualSubscriptions.SetPostingLocation("Location Code");
                        // P800162917
                        SalesPost.SetWhseActHeader(WhseActivHeader);   // P8000282A
                        ...
                        UnbindSubscription(FoodManualSubscriptions); // P800162917
                        SalesPost.GetWhseRoundingAdjmts(TempWhseRoundingAdjmts); // P8001039
                        if "Source Document" = "Source Document"::"Sales Order" then begin
                ...
                DATABASE::"Transfer Line":
                    begin
                        ...
                        if Type = Type::"Invt. Put-away" then begin
                            ...
                            TransferPostReceipt.SetWhseActHeader(WhseActivHeader); // P8000282A
                            TransferPostReceipt.Run(TransHeader);
                            TransferPostReceipt.GetWhseRoundingAdjmts(TempWhseRoundingAdjmts); // P8001039
                            ...
                            if not TransHeader."Direct Transfer" then begin
                                TransferPostShip.SetWhseActHeader(WhseActivHeader); // P8000282A
    ...
    begin
        ...
        with WhseActivHeader do begin
            ...
            WhseJnlRegisterLine.SetWhseRoundingAdjmts(TempWhseRoundingAdjmts); // P8001039
    ...
    begin
        with TempWhseActivLine do begin
            ...
            until Next() = 0;
            ItemJnlPostLine.GetWhseRoundingAdjmts(TempWhseRoundingAdjmts); // P8001039
            Clear(ItemJnlPostLine);                                        // P8001039
    ...
    local procedure DeleteSourceAltQtys()
    var
                        ...
                        begin
                            ...
                            if TransLine.TrackAlternateUnits() then
                                case "Source Subtype" of
                                    Direction::Outbound:
                                        AltQtyMgmt.DeleteAltQtyLines(TransLine."Alt. Qty. Trans. No. (Ship)");
                                    Direction::Inbound:
                                        AltQtyMgmt.DeleteAltQtyLines(TransLine."Alt. Qty. Trans. No. (Receive)");
                                end;
                        end;
                end;
            until (Next = 0);
    ...
    begin
        // P8001039
        if not TempWhseRoundingAdjmts.IsEmpty then begin
            RoundingAdjmtMgmt.SetWhseAdjmts(TempWhseRoundingAdjmts);
            TempWhseRoundingAdjmts.DeleteAll;
            WhseJnlRegisterLine.ClearWhseRoundingAdjmts;
            while RoundingAdjmtMgmt.WhseAdjmtsToPost() do begin
