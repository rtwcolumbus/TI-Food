codeunit 90 "Purch.-Post" // Version: FOODNA
{
    // --------------------------------------------------------------------------------
    ...
    // ID          Date        Description
    // --------------------------------------------------------------------------------
    // TOM4524     29-10-2015  Refactoring by Microsoft in NAV 2016
    // --------------------------------------------------------------------------------
    // 
    // PR2.00.04
    //   Document Management
    ...
    //   Add logic for assignment of posting numbers from order number
    // 
    // PR3.70.01
    //   Pass receiving reason code, farm, brand through item journal line
    //   Extra Charges
    // 
    // PR3.70.03
    //   Add logic to post document accruals
    //   Clear ADC Started
    //   Fix problem with extra charges posted to non-Item lines
    // 
    // PR3.70.05
    // P8000062B, Myers Nissi, Jack Reynolds, 21 JUN 04
    //   FillInvPostingBuffer - remove extra charge posting; now part of inventory posting
    // 
    // PR3.70.06
    // P8000107A, Myers Nissi, Jack Reynolds, 02 SEP 04
    //   AddSalesTaxLineToSalesTaxCalc - when calculating line amount use alt. quantity if appropriate
    // 
    // P8000080A, Myers Nissi, Jack Reynolds, 15 SEP 04
    //   Modifications for posting pickup loads
    // 
    // PR3.70.07
    ...
    // 
    // PR4.00.03
    // P8000326A, VerticalSoft, Jack Reynolds, 24 APR 06
    //   Fix problem with skipping receipt numbers
    // 
    // PR4.00.04
    // P8000335A, VerticalSoft, Jack Reynolds, 20 SEP 06
    //   Validating and posting of maintenance ledger entries
    // 
    // P8000382A, VerticalSoft, Jack Reynolds, 21 SEP 06
    //   Fix incorrect posting of Purchase Amount (Expected) in value entry
    // 
    // PR5.00
    ...
    //   Fix problem updating sales lines for drop shipments
    // 
    // PRW16.00.04
    ...
    //   Added Production Date for Freshness Date calculation.
    // 
    ...
    //   Update purchase lines when modifying posting date
    // 
    ...
    //   Update sales contract history for drop shipments
    // 
    ...
    //   Fix problem with Sales Comment Line Document Type for posted drop shipments
    // 
    // PRW17.10.04
    ...
    // 
    // P80047943, To-Increase, Dayakar Battini, 02 NOV 17
    ...
    // 
    // PRW111.00.01
    // P80061798, To-Increase, Jack Reynolds, 10 JUL 18
    //   Fix problem with loose inventory check when posting return orders
    // 
    // PRW111.00.03
    ...
    // 
    // P80079756, To-Increase, Gangabhushan, 05 AUG 19
    //   CS00071760 - Users are unable to post a purchase order - which is created from Sub Contract Work sheet
    // 
    // PRW113.00
    ...
    begin
        ...
        // P8000080A Begin
        if Receive and (PickupLoadNo <> "Pickup Load No.") then begin
            if "Pickup Load No." <> '' then begin
                if PickupLoadLine.Get("Pickup Load No.", "No.") then
        ...
        RefreshTempLines(PurchHeader, TempPurchLineGlobal);  // P80047148, Pp8870395
        ...
        if ProcessFns.AltQtyInstalled() then
            AltQtyMgmt.CopyWhseActToPurchase(TempWhseActHeader, PurchHeader);
        // P8000282A
        ...
        SetCommManifestFilters(PurchHeader, TempPurchLineGlobal); // P8000891, P8007748
        ...
        if TempPurchLineGlobal.FindSet() then
            repeat
                ...
                if ((TempPurchLineGlobal."Bin Code" = '') or (TempPurchLineGlobal."Document Type" = TempPurchLineGlobal."Document Type"::"Return Order")) and // P80061798, P80070395
                   ProcessFns.ContainerTrackingInstalled
                then // P8001342
                    ContainerFns.SetContainerDocumentLineSource(TempPurchLineGlobal, 0); // P8001342
                TempPurchLineGlobal.AutoLotNo(true); // P8000250B
        ...
        if PurchHeader.Invoice then begin
            ...
            PostInvoice(PurchHeader, TempPurchLineGlobal);
        end;
    ...
    var
        ...
        MaintFldMgmt: Codeunit "Maintenance Purchase Mgmt.";
        ...
        PickupLoadNo: Code[20];
        PostingLocation: Code[10];
        DeliveryTripNo: Code[20];
        CommManifestNo: Code[20];
        Text37002000: Label '-1';
        Text37002685: Label 'You cannot manually receive an order that is associated to Commodity Manifests.';
    ...
    begin
        ...
        if PostingLocation <> '' then                          // P8001323
                                                               //PurchLine.SETRANGE("Location Code",PostingLocation); // P8001323, P80048210
            PurchLine.SetFilter("Location Code", '%1|%2', '', PostingLocation); // P80048210
        // P80073095
    ...
    begin
        with PurchHeader do begin
            ...
            // P8000282A
            if ProcessFns.AltQtyInstalled() then
                AltQtyMgmt.CheckPurchRelease(PurchHeader);
            // P8000282A
    ...
    begin
        with PurchHeader do begin
            ...
            // P8000891, P8000902
            if "Commodity Manifest Order" and (CommManifestNo = '') then begin
                if Receive and (not Invoice) then
                    Error(ErrorInfo.Create(Text37002685, true, PurchHeader));
                TestField("Comm. Receiving Complete", true, ErrorInfo.Create('', true, PurchHeader));
                Receive := ("Commodity P.O. Type" = "Commodity P.O. Type"::Hauler);
            end;
            // P8000891
    ...
    begin
        ...
        with PurchLine do begin
            if PurchLine.Type = PurchLine.Type::FOODContainer then begin   //P8001373
                                                                           //P8001373
                ContainerType.SetRange("Container Item No.", PurchLine."No.");
                if ContainerType.FindFirst then
                    PostAsItem := ContainerType."Container Purchase Processing" = ContainerType."Container Purchase Processing"::Purchase
                else
                    PostAsItem := false;
            end else
                PostAsItem := PurchLine.Type = PurchLine.Type::Item;
            if PostAsItem then
                //P8001373
            ...
            if Quantity <> 0 then begin
                ...
                if not IsHandled then
                    ...
                    DivideAmount(PurchHeader, PurchLine, 1, "Qty. to Invoice", "Qty. to Invoice (Alt.)", TempVATAmountLine, TempVATAmountLineRemainder); // PR3.60
            end else
                TestField(Amount, 0);
            ...
            // RoundAmount(PurchHeader,PurchLine,"Qty. to Invoice");                       // PR3.60
            RoundAmount(PurchHeader, PurchLine, "Qty. to Invoice", "Qty. to Invoice (Alt.)"); // PR3.60

            if (Type = Type::Item) and ProcessFns.FreshProInstalled then            // PR3.70.01
                ExtraChargeMgmt.StartPurchasePosting(PurchHeader, PurchLine, Currency); // PR3.70.01
            ...
            RoundingAdjmtMgmt.SetFixedAltQtyPurchLine(PurchLine); // P8000550A

            RemQtyToBeInvoiced := "Qty. to Invoice";
            RemQtyToBeInvoicedBase := "Qty. to Invoice (Base)";
            RemQtyToBeInvoicedAlt := PurchLine."Qty. to Invoice (Alt.)"; // PR3.60
            ...
            PostItemTrackingLine(PurchHeader, PurchLine, PostAsItem); // P8007748
            ...
            case Type of
                ...
                Type::Item, Type::FOODContainer:  //P8001373
                    PostItemLine(PurchHeader, PurchLine, TempDropShptPostBuffer, PostAsItem); // P8007748
                Type::Resource:
            ...
            if not IsHandled then
                ...
                then
                    InsertReceiptLine(PurchRcptHeader, PurchRcptLine, PurchLine, CostBaseAmount, PostAsItem); // P8007748
            ...
            if not IsHandled then
                ...
                then
                    InsertReturnShipmentLine(ReturnShptHeader, PurchLine, CostBaseAmount, PostAsItem); // P8007748
            ...
            if PurchHeader.Invoice then
                if "Document Type" in ["Document Type"::Order, "Document Type"::Invoice] then begin
                    ...
                    if not IsHandled then begin
                        ...
                        if xPurchLine."Alt. Qty. Transaction No." <> 0 then                                 // PR3.60
                            AltQtyMgmt.CreateInvoiceEntries(DATABASE::"Purch. Inv. Line", PurchInvHeader."No."); // PR3.60
                                                                                                                 // PR3.70.01 Begin
                        if (PurchInvLine.Type = PurchInvLine.Type::Item) and ProcessFns.FreshProInstalled then // PR3.70.03
                            ExtraChargeMgmt.MoveToDocumentLine(
                              DATABASE::"Purch. Inv. Line", PurchInvLine."Document No.", PurchInvLine."Line No.");
                        // PR3.70.01 End
                        ...
                        InvoicePostingInterface.CreatePostedDeferralSchedule(
                          ...
                          PurchInvHeader."No.", PurchInvLine."Line No.", PurchInvHeader."Posting Date");
                        InvoiceTransportCosts(PurchInvLine."Transport Cost Entry No"); //N138F0000.n, TOM4524
                    ...
                    if not IsHandled then begin
                        ...
                        // PR3.60
                        if xPurchLine."Alt. Qty. Transaction No." <> 0 then
                            if ("Document Type" = "Document Type"::"Return Order") or
                              PurchSetup."Return Shipment on Credit Memo"
                            then
                                AltQtyMgmt.CreateInvoiceEntries(DATABASE::"Purch. Cr. Memo Line", PurchCrMemoHeader."No.") // PR3.60
                            else begin
                                AltQtyMgmt.PurchLineToCreditMemoLine(xPurchLine, PurchCrMemoLine);
                                AltQtyMgmt.UpdateAltQtyEntriesInvQty(TempValueEntryRelation, PurchCrMemoLine.RowID1, ItemLedgShptEntryNo);
                            end;
                        // PR3.60
            ...
            if ProcessFns.AccrualsInstalled() then
                PostAccrualBatch.PostPurchLineAccruals(PurchHeader, PurchLine, SrcCode); // P8001133
            // P8000274A

            // P8000335A
            if "Document Type" in ["Document Type"::Order, "Document Type"::Invoice] then begin
                if ProcessFns.MaintenanceInstalled then
                    MaintFldMgmt.PurchPostLine(PurchHeader, PurchLine,
                      PurchRcptLine, PurchInvLine, SrcCode, ItemJnlPostLine); // P8001133
            end;
            // P8000335A
        end;
    ...
    local procedure PostInvoice(var PurchHeader: Record "Purchase Header"; var PurchLine: Record "Purchase Line")
    var
#if not CLEAN20    
    ...
    begin
        ...
        with PurchHeader do begin
            // P8000241A
            if ProcessFns.AccrualsInstalled() then
                AccrualFldMgmt.PurchPrePost(PurchLine);
            // P8000241A
                ...
                if "Tax Area Code" <> '' then begin
                    ...
                    if Invoice then
                        TaxAmountDifference.ClearDocDifference(
                            "Sales Tax Document Area"::Purchase.AsInteger(), "Document Type".AsInteger(), "No.");
            ...
            if ProcessFns.AccrualsInstalled() then begin
                PostAccrualBatch.PostPurchHeaderAccruals(PurchHeader, SrcCode); // P8001133
                PostAccrualsToGL(PurchHeader); // P8007748
            end;
            // PR3.70.03
        end;
    ...
    local procedure PostItemLine(PurchHeader: Record "Purchase Header"; var PurchLine: Record "Purchase Line"; var TempDropShptPostBuffer: Record "Drop Shpt. Post. Buffer" temporary; PostAsItem: Boolean)
    var
    ...
    begin
        ...
        with PurchHeader do begin
            if PostAsItem then begin //P8001373
                if RemQtyToBeInvoiced <> 0 then
                    ItemLedgShptEntryNo :=
                      PostItemJnlLine(
                        PurchHeader, PurchLine,
                        RemQtyToBeInvoiced, RemQtyToBeInvoicedBase,
                        RemQtyToBeInvoicedAlt, // PR3.60
                        RemQtyToBeInvoiced, RemQtyToBeInvoicedBase,
                        RemQtyToBeInvoicedAlt, // PR3.60
                        0, '', DummyTrackingSpecification, PostAsItem); // P8007748
                    ...
                    if Abs(PurchLine."Return Qty. to Ship") > Abs(RemQtyToBeInvoiced) then
                        ItemLedgShptEntryNo :=
                          PostItemJnlLine(
                            ...
                            PurchLine."Return Qty. to Ship (Alt.)" - RemQtyToBeInvoicedAlt, // PR3.60
                                                                                            //0,0,0,'',DummyTrackingSpecification);            // PR3.60
                            0, 0, 0, 0, '', DummyTrackingSpecification, PostAsItem); // PR3.60, P8007748
                end else begin
                    if Abs(PurchLine."Qty. to Receive") > Abs(RemQtyToBeInvoiced) then
                        ItemLedgShptEntryNo :=
                          PostItemJnlLine(
                            ...
                            PurchLine."Qty. to Receive (Alt.)" - RemQtyToBeInvoicedAlt, // PR3.60
                                                                                        // 0,0,0,'',DummyTrackingSpecification);           // PR3.60
                            0, 0, 0, 0, '', DummyTrackingSpecification, PostAsItem); // PR3.60, P8007748
                ...
                if Ship or Receive then
                    if ProcessFns.ContainerTrackingInstalled then // P80046533
                        if ContainerFns.PostPurchaseContainerLine(PurchHeader, PurchLine, ReturnShptHeader, PurchRcptHeader, SrcCode, ItemJnlLine) then
                            PostPurchaseContainerLine(ItemJnlLine, PurchLine);
    ...
    begin
        ...
        if (PurchLine."Qty. to Receive" <> 0) and (PurchLine."Sales Order Line No." <> 0) then begin
            ...
            TempDropShptPostBuffer."Quantity (Alt.)" := PurchLine."Qty. to Receive (Alt.)"; // P8000505A
            ...
            TempDropShptPostBuffer."Item Shpt. Entry No." :=
              PostAssocItemJnlLine(PurchHeader, PurchLine, TempDropShptPostBuffer.Quantity, TempDropShptPostBuffer."Quantity (Base)", TempDropShptPostBuffer."Quantity (Alt.)"); // P8007748
    ...
    local procedure PostItemTrackingLine(PurchHeader: Record "Purchase Header"; PurchLine: Record "Purchase Line"; PostAsItem: Boolean)
    var
        TempTrackingSpecification: Record "Tracking Specification" temporary;
    ...
    begin
        ...
        TrackingSpecificationExists := TrackingSpecificationExists and PostAsItem; //P8001373

        PostItemTracking(PurchHeader, PurchLine, TempTrackingSpecification, TrackingSpecificationExists, PostAsItem);
    ...
    procedure PostItemJnlLine(PurchHeader: Record "Purchase Header"; PurchLine: Record "Purchase Line"; QtyToBeReceived: Decimal; QtyToBeReceivedBase: Decimal; QtyToBeReceivedAlt: Decimal; QtyToBeInvoiced: Decimal; QtyToBeInvoicedBase: Decimal; QtyToBeInvoicedAlt: Decimal; ItemLedgShptEntryNo: Integer; ItemChargeNo: Code[20]; TrackingSpecification: Record "Tracking Specification"; PostAsItem: Boolean) Result: Integer
    var
        ItemJnlLine: Record "Item Journal Line";
        ...
        CostingQty: Decimal;
    begin
        // PR3.60 - Add parameters - QtyToBeReceivedAlt, QtyToBeInvoicedAlt
        ...
        with ItemJnlLine do begin
            ...
            PostItemJnlLineCopyDocumentFields(ItemJnlLine, PurchHeader, PurchLine, QtyToBeInvoiced, QtyToBeReceived, QtyToBeReceivedAlt); // P800-MegaApp
            ...
            // PR3.60
            "Quantity (Alt.)" := QtyToBeReceivedAlt;
            "Invoiced Qty. (Alt.)" := QtyToBeInvoicedAlt;
            if (QtyToBeReceivedAlt <> 0) then
                "Alt. Qty. Transaction No." := PurchLine."Alt. Qty. Transaction No.";
            // PR3.60
            ...
            if (QtyToBeInvoiced <> 0) or (QtyToBeInvoicedAlt <> 0) then // P8000971
                CalcItemJnlLineToBeInvoicedAmounts(ItemJnlLine, PurchHeader, PurchLine, QtyToBeInvoiced, QtyToBeInvoicedBase, QtyToBeInvoicedAlt, ItemChargeNo) // P8000971
                else
                CalcItemJnlLineToBeReceivedAmounts(ItemJnlLine, PurchHeader, PurchLine, QtyToBeReceived, QtyToBeReceivedAlt); // P8000971
                ...
                if PurchLine."Work Order No." <> '' then // P8000335A
                    ItemJnlLine."Don't Apply" := true;     // P8000335A
                ...
                // IF (PurchLine.Type = PurchLine.Type::Item) AND PurchHeader.Invoice THEN //P8001373
                if PostAsItem and PurchHeader.Invoice then //P8001373
    ...
    local procedure CalcItemJnlLineToBeInvoicedAmounts(var ItemJnlLine: Record "Item Journal Line"; var PurchaseHeader: Record "Purchase Header"; var PurchaseLine: Record "Purchase Line"; QtyToBeInvoiced: Decimal; QtyToBeInvoicedBase: Decimal; QtyToBeInvoicedAlt: Decimal; ItemChargeNo: Code[20])
    var
        Factor: Decimal;
    ...
    begin
        ...
        if ItemChargeNo <> '' then
            ...
            if PurchaseLine.CostInAlternateUnits() then begin
                if (QtyToBeInvoicedAlt <> 0) and (PurchaseLine.Type = PurchaseLine.Type::Item) then // P8007748
                    Factor := QtyToBeInvoicedAlt / PurchaseLine."Qty. to Invoice (Alt.)"
                else
    ...
    local procedure CalcItemJnlLineToBeReceivedAmounts(var ItemJnlLine: Record "Item Journal Line"; var PurchaseHeader: Record "Purchase Header"; var PurchaseLine: Record "Purchase Line"; QtyToBeReceived: Decimal; QtyToBeReceivedAlt: Decimal)
    var
        IsHandled: Boolean;
        CostingQty: Decimal;
    begin
        ...
        if PurchaseLine.CostInAlternateUnits then
            CostingQty := QtyToBeReceivedAlt
        else
            CostingQty := QtyToBeReceived;
        // P8000382A

        if PurchaseHeader."Prices Including VAT" then
            ItemJnlLine.Amount :=
                (CostingQty * PurchaseLine."Direct Unit Cost" * (1 - PurchaseLine."Line Discount %" / 100) /  // P8000382A
                (1 + PurchaseLine."VAT %" / 100)) + RemAmt
        else
            ItemJnlLine.Amount :=
                (CostingQty * PurchaseLine."Direct Unit Cost" * (1 - PurchaseLine."Line Discount %" / 100)) + RemAmt; // P8000382A
        RemAmt := ItemJnlLine.Amount - Round(ItemJnlLine.Amount);
    ...
    local procedure PostItemJnlLineCopyDocumentFields(var ItemJnlLine: Record "Item Journal Line"; PurchHeader: Record "Purchase Header"; PurchLine: Record "Purchase Line"; QtyToBeInvoiced: Decimal; QtyToBeReceived: Decimal; QtyToBeReceivedAlt: Decimal)
    begin
        OnPostItemJnlLineOnBeforeCopyDocumentFields(ItemJnlLine, PurchHeader, PurchLine, WhseReceive, WhseShip, InvtPickPutaway);

        with ItemJnlLine do
            if (QtyToBeReceived = 0) and (QtyToBeReceivedAlt = 0) then // P8000971
                if PurchLine.IsCreditDocType() then
                    CopyDocumentFields(
    ...
    begin
        ...
        if not IsHandled then begin
            with TempItemChargeAssgntPurch do begin
                ...
                if "Document Type" in ["Document Type"::"Return Order", "Document Type"::"Credit Memo"] then
                    // P8000971
                    if PurchLine.CostInAlternateUnits then
                        QtyToInvoice :=
                          CalcQtyToInvoice(PurchLine."Return Qty. to Ship (Alt.)", PurchLine."Qty. to Invoice (Alt.)")
                    else
                        // P8000971
                    QtyToInvoice :=
                    ...
                    if PurchLine.CostInAlternateUnits then
                        QtyToInvoice :=
                          CalcQtyToInvoice(PurchLine."Qty. to Receive (Alt.)", PurchLine."Qty. to Invoice (Alt.)")
                    else
                        // P8000971
                        QtyToInvoice :=
                ...
                if PurchHeader."Currency Code" <> '' then
                    ItemJnlLine2."Discount Amount" := Round(
                        CurrExchRate.ExchangeAmtFCYToLCY(
                          ...
                          ItemJnlLine2.GetCostingQty(ItemJnlLine2.FieldNo("Invoiced Qty. (Base)")) /  // P8000971
            ...
            if IsEmpty() then
                ...
                repeat
                    ...
                    then begin                                                                                // P8000971
                        ...
                        NonDistrItemJnlLine."Quantity (Alt.)" -= "Quantity (Alt.)"; // P8000971
    ...
    begin
        ...
        if PurchRcptLine."Item Rcpt. Entry No." <> 0 then
            ...
            ItemTrackingMgt.CollectItemEntryRelation(TempItemLedgEntry,
              ...
              '', 0, PurchRcptLine."Line No.", PurchRcptLine."Quantity (Base)", PurchRcptLine."Quantity (Alt.)"); // P8000267B
        end;
        ...
        if DistributeCharge then
            PostDistributeItemCharge(
              PurchHeader, PurchLine, TempItemLedgEntry, PurchRcptLine.GetCostingQtyBase, // P8000971
            ...
            PostItemCharge(PurchHeader, PurchLine,
              ...
              PurchRcptLine."Quantity (Alt.)", // PR3.60
    ...
    begin
        ...
        if ReturnShptLine."Item Shpt. Entry No." <> 0 then
            ...
            ItemTrackingMgt.CollectItemEntryRelation(TempItemLedgEntry,
              ...
              '', 0, ReturnShptLine."Line No.", ReturnShptLine."Quantity (Base)", ReturnShptLine."Quantity (Base)"); // P8000267B
        end;
        ...
        if DistributeCharge then
            PostDistributeItemCharge(
              PurchHeader, PurchLine, TempItemLedgEntry, -ReturnShptLine.GetCostingQtyBase, // P8000971
            ...
            PostItemCharge(PurchHeader, PurchLine,
              ...
              -ReturnShptLine."Quantity (Alt.)", // PR3.60
            ...
            if TransRcptLine."Item Rcpt. Entry No." = 0 then
                ...
                if ItemApplnEntry.FindSet() then
                    repeat
                        ...
                        Factor := ItemApplnEntry.GetCostingQty / TransRcptLine.GetCostingQtyBase; // P8000971
                        CalcAmountToPostFCY :=
                          //((TotalAmountToPostFCY / TransRcptLine."Quantity (Base)") * ItemApplnEntry.Quantity) + // P8000971
                          ((TotalAmountToPostFCY * Factor)) +                                                      // P8000971
                          RemAmountToPostFCY;
                        AmountToPostFCY := Round(CalcAmountToPostFCY);
                        RemAmountToPostFCY := CalcAmountToPostFCY - AmountToPostFCY;
                        CalcAmountToPostLCY :=
                          //((TotalAmountToPostLCY / TransRcptLine."Quantity (Base)") * ItemApplnEntry.Quantity) + // P8000971
                          ((TotalAmountToPostLCY * Factor)) +                                                      // P8000971
                          RemAmountToPostLCY;
                        AmountToPostLCY := Round(CalcAmountToPostLCY);
                        RemAmountToPostLCY := CalcAmountToPostLCY - AmountToPostLCY;
                        CalcDiscAmountToPost :=
                          //((TotalDiscAmountToPost / TransRcptLine."Quantity (Base)") * ItemApplnEntry.Quantity) + // P8000971
                          ((TotalDiscAmountToPost * Factor)) +                                                      // P8000971
                          RemDiscAmountToPost;
                        ...
                        PurchLine2."Unit Cost" :=
                          //ROUND(AmountToPostFCY / ItemApplnEntry.Quantity,GLSetup."Unit-Amount Rounding Precision");    // P8000971
                          Round(AmountToPostFCY / ItemApplnEntry.GetCostingQty, GLSetup."Unit-Amount Rounding Precision"); // P8000971
                        PurchLine2."Unit Cost (LCY)" :=
                          //ROUND(AmountToPostLCY / ItemApplnEntry.Quantity,GLSetup."Unit-Amount Rounding Precision");    // P8000971
                          Round(AmountToPostLCY / ItemApplnEntry.GetCostingQty, GLSetup."Unit-Amount Rounding Precision"); // P8000971
                        if "Document Type" in ["Document Type"::"Return Order", "Document Type"::"Credit Memo"] then
                        ...
                        PostItemJnlLine(
                          ...
                          ItemApplnEntry."Quantity (Alt.)", // PR3.60
                          PurchLine2."Appl.-to Item Entry", "Item Charge No.", DummyTrackingSpecification, true); // P8007748
    ...
    begin
        with TempItemChargeAssgntPurch do begin
            ItemTrackingMgt.CollectItemEntryRelation(TempItemLedgEntry,
              ...
              '', 0, TransRcptLine."Line No.", TransRcptLine."Quantity (Base)", TransRcptLine."Quantity (Alt.)"); // P8000267B
            OnPostItemChargePerITTransferOnAfterCollectItemEntryRelation(PurchHeader, PurchLine, TransRcptLine, TempItemLedgEntry);
            PostDistributeItemCharge(
              PurchHeader, PurchLine, TempItemLedgEntry, TransRcptLine.GetCostingQtyBase, // P8000971
    ...
    begin
        ...
        if DistributeCharge then
            PostDistributeItemCharge(
              PurchHeader, PurchLine, TempItemLedgEntry, -SalesShptLine.GetCostingQtyBase, // P8000971
    ...
    begin
        ...
        if DistributeCharge then
            PostDistributeItemCharge(
              PurchHeader, PurchLine, TempItemLedgEntry, ReturnRcptLine.GetCostingQtyBase, // P8000971
    ...
    begin
        ...
        if TempItemLedgEntry.FindSet() then begin
            repeat
                //Factor := TempItemLedgEntry.Quantity / NonDistrQuantity;    // P8000971
                Factor := TempItemLedgEntry.GetCostingQty / NonDistrQuantity; // P8000971
    ...
    local procedure PostAssocItemJnlLine(PurchHeader: Record "Purchase Header"; PurchLine: Record "Purchase Line"; QtyToBeShipped: Decimal; QtyToBeShippedBase: Decimal; QtyToBeShippedAlt: Decimal): Integer
    var
        ItemJnlLine: Record "Item Journal Line";
    ...
    begin
        ...
        if (QtyToBeShippedAlt <> 0) then
            ItemJnlLine."Alt. Qty. Transaction No." := PurchLine."Alt. Qty. Transaction No.";
        // PR3.60
    ...
    begin
        ...
        with PurchLine do begin
            case Type of
                ...
                // P800241A, P800-MegaApp
                Type::FOODAccrualPlan:
                    AccrualFldMgmt.PurchPostCheckLine(PurchLine);
                // P8000241A
                else
                ...
                repeat
                    ...
                    if ProcessFns.AltQtyInstalled then begin
                        AltQtyMgmt.TestDropShipAltQtys(
                          SalesOrderLine."Alt. Qty. Transaction No.", SalesOrderLine.TableCaption,
                          SalesOrderHeader."No.", PurchLine.TableCaption);
                        SalesOrderLine."Qty. Shipped (Alt.)" :=
                          SalesOrderLine."Qty. Shipped (Alt.)" + TempDropShptPostBuffer."Quantity (Alt.)";  // P8000506A, P8000760
                    end;
                    // PR3.60
    ...
    begin
        with PurchHeader do begin
            ...
            if PostingLocation <> '' then                              // P8000282A
                TempPurchLine.SetRange("Location Code", PostingLocation); // P8000282A
                    ...
                    repeat
                        if SalesOrderHeader."No." <> TempPurchLine."Sales Order No." then begin
                            ...
                            if SalesOrderHeader."Shipping No." = '' then begin
                                ...
                                AssignShipmentPostingNo(SalesOrderHeader);
                                // PR3.60
                                SalesOrderHeader.Modify();
                            end;
            ...
            if ShouldUpdateReceivingNo then
                ...
                then
                    if not PreviewMode then begin
                        ...
                        // PR3.61.01
                        //"Receiving No." := NoSeriesMgt.GetNextNo("Receiving No. Series","Posting Date",TRUE); // P8000326A
                        "Receiving No." := AssignPostingNo(PurchHeader, "Receiving No. Series", "Last Receiving No."); // P8007748
                                                                                                                       // PR3.61.01
                        ModifyHeader := true;
            ...
            if Ship and ("Return Shipment No." = '') then
                ...
                then
                    if not PreviewMode then begin
                        ...
                        "Return Shipment No." := AssignPostingNo(PurchHeader, "Return Shipment No. Series", "Last Return Shipment No."); // P8007748
                                                                                                                                         // PR3.61.01
                        ModifyHeader := true;
                    ...
                    then begin
                        if not PreviewMode then begin
                            ...
                            if not IsHandled then
                                ...
                                "Posting No." := AssignPostingNo(PurchHeader, "Posting No. Series", "Last Posting No."); // P8007748
                                                                                                                       // PR3.61.01
                            ModifyHeader := true;
                        end;
                    end;
    ...
    begin
        ...
        with PurchLine do begin
            // if not (PurchHeader.Receive or RoundingLineInserted) then begin   // P8000282A
            if (not (PurchHeader.Receive or RoundingLineInserted)) or            // P8000282A
              ((PostingLocation <> '') and ("Location Code" <> PostingLocation)) // P8000282A
            then begin
                "Qty. to Receive" := 0;
                "Qty. to Receive (Base)" := 0;
                "Qty. to Receive (Alt.)" := 0; // PR3.60
            end;

            // if not (PurchHeader.Ship OR RoundingLineInserted) then begin      // P8000282A
            if (not (PurchHeader.Ship or RoundingLineInserted)) or               // P8000282A
              ((PostingLocation <> '') and ("Location Code" <> PostingLocation)) // P8000282A
            then begin
                "Return Qty. to Ship" := 0;
                "Return Qty. to Ship (Base)" := 0;
                "Return Qty. to Ship (Alt.)" := 0; // PR3.60
            end;
            ...
            if (PurchHeader."Document Type" = PurchHeader."Document Type"::Invoice) and ("Receipt No." <> '') then begin
                ...
                "Qty. Received (Alt.)" := "Quantity (Alt.)"; // PR3.60
                "Qty. to Receive" := 0;
                "Qty. to Receive (Base)" := 0;
                "Qty. to Receive (Alt.)" := 0; // PR3.60
            end;
            ...
            then begin
                ...
                "Return Qty. Shipped (Alt.)" := "Quantity (Alt.)"; // PR3.60
                "Return Qty. to Ship" := 0;
                "Return Qty. to Ship (Base)" := 0;
                "Return Qty. to Ship (Alt.)" := 0; // PR3.60
            end;
    ...
    begin
        ...
        with PurchHeader do begin
            ...
            DocExtraCharge.SetFilter("Table ID", '%1|%2', DATABASE::"Purchase Header", DATABASE::"Purchase Line"); // P8001032
    ...
    begin
        ...
        with PurchHeader do begin
            if ProcessFns.ContainerTrackingInstalled then                                // P80046533
                ContainerFns.CloseContainerForPostedDocument(PurchHeader, PostingLocation, TempWhseShptHeader, TempWhseRcptHeader); // P80046533, P80097092
            ...
            then begin
                ...
                if not IsHandled then
                    UpdateWhseDocuments();
                WhsePurchRelease.SetCalledFromPost; // P8000549A
                ...
                case "Document Type" of
                    "Document Type"::Invoice:
                        begin
                            InsertTrackingSpecification(PurchHeader);  // P80047943
                            PostUpdateInvoiceLine(PurchHeader);
                            // InsertTrackingSpecification(PurchHeader); // Moved before calling PostUpdateInvoiceLine  // P80047943
                        end;
                    "Document Type"::"Credit Memo":
                        begin
                            InsertTrackingSpecification(PurchHeader);  // P80047943
                            PostUpdateCreditMemoLine();
                            // InsertTrackingSpecification(PurchHeader);  // Moved before calling PostUpdateCreditMemoLine  // P80047943
                        end;
                    else begin
                            ResetTempLines(TempPurchLine);
            ...
            if not EverythingInvoiced then                    // P8007609
                WhseRqstUpdateCompletelyHandled(PurchHeader);   // P8007609
    ...
    begin
                ...
                if (InvoicePostBuffer.Amount <> 0) or (InvoicePostBuffer."Amount (ACY)" <> 0) then begin
                    ...
                    if InvoicePostBuffer.Type = InvoicePostBuffer.Type::"Fixed Asset" then begin
                        FillInvoicePostBufferFADiscount(
                          ...
                          TotalVAT, TotalVATACY, TotalAmount, TotalAmountACY, TotalVATBase, TotalVATBaseACY, PurchLine); // P8007748
                        ...
                        UpdateInvoicePostBuffer(InvoicePostBuffer, true, PurchLine); // P8007748, P800133109
        ...
        if (PurchLine.Type in [PurchLine.Type::"G/L Account", PurchLine.Type::"Fixed Asset", PurchLine.Type::FOODAccrualPlan]) then
            // P8000756
        ...
        UpdateInvoicePostBuffer(InvoicePostBuffer, false, PurchLine); // P8007748, P800133109
...
#if not CLEAN20
    local procedure FillInvoicePostBufferFADiscount(var InvoicePostBuffer: Record "Invoice Post. Buffer" temporary; GenPostingSetup: Record "General Posting Setup"; AccountNo: Code[20]; TotalVAT: Decimal; TotalVATACY: Decimal; TotalAmount: Decimal; TotalAmountACY: Decimal; TotalVATBase: Decimal; TotalVATBaseACY: Decimal; PurchLine: Record "Purchase Line")
    var
        DeprBook: Record "Depreciation Book";
        IsHandled: Boolean;
    begin
...
#if not CLEAN20
    local procedure UpdateInvoicePostBuffer(InvoicePostBuffer: Record "Invoice Post. Buffer"; ForceGLAccountType: Boolean; PurchLine: Record "Purchase Line")
    var
        RestoreAPType: Boolean;
    begin
        // P8007748 - add parameter PurchLine
        if InvoicePostBuffer.Type = InvoicePostBuffer.Type::"Fixed Asset" then begin
            FALineNo := FALineNo + 1;
        ...
        // P8000241A
        if (InvoicePostBuffer.Type = InvoicePostBuffer.Type::FOODAccrualPlan) then begin
            AccrualFldMgmt.PurchPostUpdatePostingBuffer(InvoicePostBuffer, PurchLine);
            // P800133109
        ...
        // P800133109
        if RestoreAPType then
            TempInvoicePostBuffer.Type := TempInvoicePostBuffer.Type::FOODAccrualPlan;
        // P800133109            
    end;
#endif
    ...
    procedure DivideAmount(PurchHeader: Record "Purchase Header"; var PurchLine: Record "Purchase Line"; QtyType: Option General,Invoicing,Shipping; PurchLineQty: Decimal; PurchLineQtyAlt: Decimal; var TempVATAmountLine: Record "VAT Amount Line" temporary; var TempVATAmountLineRemainder: Record "VAT Amount Line" temporary)
    var
        OriginalDeferralAmount: Decimal;
        UseAltQty: Boolean;
        OldPurchLineQty: Decimal;
    begin
        // PR3.60 - Add parameter PurchLineQtyAlt
        if RoundingLineInserted and (RoundingLineNo = PurchLine."Line No.") then
            exit;

        // PR3.60
        UseAltQty := PurchLine.CostInAlternateUnits();
        if UseAltQty then begin
            OldPurchLineQty := PurchLine.Quantity;
            PurchLine.Quantity := PurchLine."Quantity (Alt.)";
            PurchLineQty := PurchLineQtyAlt;
        end;
        // PR3.60
...
#pragma warning restore AA0005
        ...
        // PR3.60
        if UseAltQty then
            PurchLine.Quantity := OldPurchLineQty;
        // PR3.60
    ...
    local procedure RoundAmount(PurchHeader: Record "Purchase Header"; var PurchLine: Record "Purchase Line"; PurchLineQty: Decimal; PurchLineQtyAlt: Decimal)
    var
        NoVAT: Boolean;
        IsHandled: Boolean;
    begin
        ...
        with PurchLine do begin
            ...
            Increment(TotalPurchLine."Quantity (Alt.)", PurchLineQtyAlt); // PR3.60
            ...
            if CostInAlternateUnits() then
                Increment(TotalPurchLineLCY."Unit Cost (LCY)",
                          Round(PurchLineQtyAlt * "Unit Cost (LCY)") - Round(PurchLineQty * "Unit Cost (LCY)"));
            // PR3.60
        end;
    ...
    begin
        with PurchLine do begin
            ...
            // PR3.60
            "Qty. to Receive (Alt.)" := -"Qty. to Receive (Alt.)";
            "Return Qty. to Ship (Alt.)" := -"Return Qty. to Ship (Alt.)";
        ...
        with PurchHeader do begin
            ...
            if OldPurchLine.FindSet() then
                repeat
                    ...
                    case QtyType of
                        QtyType::General:
                            PurchLineQtyAlt := PurchLine."Quantity (Alt.)";
                        QtyType::Invoicing:
                            PurchLineQtyAlt := PurchLine."Qty. to Invoice (Alt.)";
                        QtyType::Shipping:
                            begin
                                if "Document Type" in ["Document Type"::"Return Order", "Document Type"::"Credit Memo"] then
                                    PurchLineQtyAlt := PurchLine."Return Qty. to Ship (Alt.)"
                                else
                                    PurchLineQtyAlt := PurchLine."Qty. to Receive (Alt.)";
                            end;
                    end;
                    // PR3.60
                    //DivideAmount(PurchHeader,PurchLine,QtyType,PurchLineQty,TempVATAmountLine,TempVATAmountLineRemainder);               // PR3.60
                    DivideAmount(PurchHeader, PurchLine, QtyType, PurchLineQty, PurchLineQtyAlt, TempVATAmountLine, TempVATAmountLineRemainder); // PR3.60
                    ...
                    PurchLine."Quantity (Alt.)" := PurchLineQtyAlt; // PR3.60
                    if PurchLineQty <> 0 then begin
                        ...
                        // RoundAmount(PurchHeader,PurchLine,PurchLineQty);              // PR3.60
                        RoundAmount(PurchHeader, PurchLine, PurchLineQty, PurchLineQtyAlt); // PR3.60
        ...
        if TempInvoicingSpecification.FindSet() then begin
            repeat
                ...
                if not TempTrackingSpecification.Insert() then begin
                    ...
                    // P8000756  // P80050544
                    TempTrackingSpecification."Quantity Invoiced (Alt.)" += TempInvoicingSpecification."Qty. to Invoice (Alt.)";
                    TempTrackingSpecification."Qty. to Invoice (Alt.)" += TempInvoicingSpecification."Qty. to Invoice (Alt.)";
                    // P8000756  // P80050544
    ...
    begin
        ...
        if not TempTrackingSpecification.IsEmpty() then begin
            ...
            if ProcessFns.ContainerTrackingInstalled then begin
                UpdateDocumentLine.SetApplication(false, DATABASE::"Purchase Line", PurchHeader."Document Type", PurchHeader."No.", 0); // Value of first parameter doesn't matter
    ...
    procedure PostItemCharge(PurchHeader: Record "Purchase Header"; var PurchLine: Record "Purchase Line"; ItemEntryNo: Integer; QuantityBase: Decimal; QuantityAlt: Decimal; AmountToAssign: Decimal; QtyToAssign: Decimal; IndirectCostPct: Decimal)
    var
        DummyTrackingSpecification: Record "Tracking Specification";
        ...
        CostingQty: Decimal;
    begin
        // PR3.60 Add parameter: QuantityAlt
        with TempItemChargeAssgntPurch do begin
            ...
            if PurchLineToPost."Currency Code" <> '' then
                PurchLineToPost."Unit Cost" := Round(
                    PurchLineToPost.Amount / CostingQty, Currency."Unit-Amount Rounding Precision") // PR3.60, P8001213
            else
                PurchLineToPost."Unit Cost" := Round(
                    PurchLineToPost.Amount / CostingQty, GLSetup."Unit-Amount Rounding Precision"); // PR3.60, P8001213
            ...
            PostItemJnlLine(
              PurchHeader, PurchLineToPost, 0, 0, 0, QuantityBase, QuantityBase, QuantityAlt, // PR3.60
              PurchLineToPost."Appl.-to Item Entry", "Item Charge No.", DummyTrackingSpecification, false); // P8007748  // P80048121
    ...
    begin
        ...
        with TempPrepmtDeductLCYPurchLine do begin
            ...
            if "Document Type" = "Document Type"::Order then begin                           // P8001366
                ...
                "Qty. to Invoice (Alt.)" := GetQtyToInvoiceAlt(PurchLine, PurchHeader.Receive); // P8001366, P8007748
            end else                                                                         // P8001366
                ...
                if FindSet(true) then begin
                    ...
                    repeat
                        ...
                        if "Document Type" = "Document Type"::Invoice then begin
                            ...
                            PurchLine."Qty. to Invoice (Alt.)" := PurchInvoiceLine."Qty. to Invoice (Alt.)"; // P8001366
    ...
    begin
        with PurchLine do begin
            ...
            "Qty. to Invoice (Alt.)" := GetQtyToInvoiceAlt(PurchLine, Receive); // P8001366, P8007748
            CalcPrepaymentToDeduct();
    ...
    begin
        with PurchLine do begin
            ...
            // P8001366
            "Quantity (Alt.)" := PurchOrderLine."Quantity (Alt.)";
            "Qty. Received (Alt.)" := PurchOrderLine."Qty. Received (Alt.)";
            "Qty. Invoiced (Alt.)" := PurchOrderLine."Qty. Invoiced (Alt.)";
            // P8001366
                ...
                repeat
                    if IsFinalInvoice() then begin
                        ...
                        if "Document Type" = "Document Type"::Order then
                            DiffToLineDiscAmt := DiffToLineDiscAmt * GetCostingQuantity(FieldNo(Quantity), '') / GetCostingQuantity(FieldNo("Qty. to Invoice"), ''); // P8001366
    ...
    local procedure UpdateInvoicedQtyOnReturnShptLine(var ReturnShptLine: Record "Return Shipment Line"; QtyToBeInvoiced: Decimal; QtyToBeInvoicedBase: Decimal; QtyToBeInvoicedAlt: Decimal)
    begin
        with ReturnShptLine do begin
            "Quantity Invoiced" := "Quantity Invoiced" - QtyToBeInvoiced;
            "Qty. Invoiced (Base)" := "Qty. Invoiced (Base)" - QtyToBeInvoicedBase;
            ReturnShptLine."Qty. Invoiced (Alt.)" := "Qty. Invoiced (Alt.)" - QtyToBeInvoicedAlt; // PR3.60
    ...
    local procedure UpdateQtyToBeInvoicedForReceipt(var QtyToBeInvoiced: Decimal; var QtyToBeInvoicedBase: Decimal; var QtyToBeInvoicedAlt: Decimal; TrackingSpecificationExists: Boolean; PurchLine: Record "Purchase Line"; PurchRcptLine: Record "Purch. Rcpt. Line"; InvoicingTrackingSpecification: Record "Tracking Specification")
    var
        IsHandled: Boolean;
    begin
        IsHandled := false;
        ...
        if TrackingSpecificationExists then begin
            ...
            QtyToBeInvoicedAlt := InvoicingTrackingSpecification."Qty. to Invoice (Alt.)"; // P8001386
        end else begin
            ...
            QtyToBeInvoicedAlt := RemQtyToBeInvoicedAlt - PurchLine."Qty. to Receive (Alt.)"; // P8001386
        end;
        if Abs(QtyToBeInvoiced) > Abs(PurchRcptLine.Quantity - PurchRcptLine."Quantity Invoiced") then begin
            ...
            QtyToBeInvoicedAlt := PurchRcptLine."Quantity (Alt.)" - PurchRcptLine."Qty. Invoiced (Alt.)"; // P8001386
        end;
    end;

    local procedure UpdateQtyToBeInvoicedForReturnShipment(var QtyToBeInvoiced: Decimal; var QtyToBeInvoicedBase: Decimal; var QtyToBeInvoicedAlt: Decimal; TrackingSpecificationExists: Boolean; PurchLine: Record "Purchase Line"; ReturnShipmentLine: Record "Return Shipment Line"; InvoicingTrackingSpecification: Record "Tracking Specification")
    var
        IsHandled: Boolean;
    begin
        IsHandled := false;
        ...
        if TrackingSpecificationExists then begin
            ...
            QtyToBeInvoicedAlt := RemQtyToBeInvoicedAlt - PurchLine."Return Qty. to Ship (Alt.)"; // PR3.60   // P80050544
        end;
        if Abs(QtyToBeInvoiced) > Abs(ReturnShipmentLine.Quantity - ReturnShipmentLine."Quantity Invoiced") then begin
            ...
            QtyToBeInvoicedAlt := ReturnShipmentLine."Qty. Invoiced (Alt.)" - ReturnShipmentLine."Quantity (Alt.)"; // PR3.60   // P80050544
        end;
    end;
    ...
    begin
        ...
        with PurchHeader do begin
            if not IsHandled then begin
                ...
                PurchHeader."Pickup Load No." := ''; // P8004516
                ...
                if ProcessFns.FreshProInstalled then
                    ExtraChargeMgmt.MoveToDocumentHeader(DATABASE::"Purchase Header", // P8001032
                      PurchHeader."Document Type", PurchHeader."No.", PurchHeader."Posting Date", // P8000928
                      DATABASE::"Purch. Rcpt. Header", PurchRcptHeader."No.");                    // P8000928
                                                                                                  // P8004516
            ...
            if WhseReceive then begin
                ...
                if ProcessFns.ContainerTrackingInstalled then
                    if ContainerFns.UpdateContainerQtyToReceive(WhseRcptHeader, DATABASE::"Purchase Line", PurchHeader."Document Type", PurchHeader."No.") then // P8007748
                        RefreshTempLines(PurchHeader, TempPurchLineGlobal); // P8007748, P80073095
                                                                            // P8001323, P8004516
            end;
            if WhseShip then begin
                ...
                // P8001323, P8004516
                if ProcessFns.ContainerTrackingInstalled then
                    if ContainerFns.UpdateContainerQtyToShip(WhseShptHeader, DATABASE::"Purchase Line", PurchHeader."Document Type", PurchHeader."No.") then // P8007748
                        RefreshTempLines(PurchHeader, TempPurchLineGlobal); // P8007748, P80073095
                                                                            // P8001323, P8004516
            end;
        end;
    ...
    procedure InsertReceiptLine(PurchRcptHeader: Record "Purch. Rcpt. Header"; var PurchRcptLine: Record "Purch. Rcpt. Line"; PurchLine: Record "Purchase Line"; CostBaseAmount: Decimal; PostAsItem: Boolean)
    var
    ...
    begin
        ...
        if not IsHandled then
            if (PurchLine.Type = PurchLine.Type::Item) and (PurchLine."Qty. to Receive" <> 0) then begin
                ...
                if PurchLine."Quantity (Alt.)" <> 0 then // P80079756
                                                         // PR3.60
                    if PurchLine.CostInAlternateUnits() then
                        PurchRcptLine."Item Charge Base Amount" := Round(CostBaseAmount / PurchLine."Quantity (Alt.)" * PurchRcptLine."Quantity (Alt.)");
                // PR3.60
            end;
        PurchRcptLineInsert(PurchRcptLine, PurchRcptHeader, PurchLine);
        if xPurchLine."Alt. Qty. Transaction No." <> 0 then             // PR3.60
            AltQtyMgmt.PurchLineToReceiptLine(xPurchLine, PurchRcptLine); // PR3.60
        // PR3.70.01 Begin
        if (PurchRcptLine.Type = PurchRcptLine.Type::Item) and ProcessFns.FreshProInstalled then // PR3.70.03
            ExtraChargeMgmt.MoveToDocumentLine(
              DATABASE::"Purch. Rcpt. Line", PurchRcptLine."Document No.", PurchRcptLine."Line No.");
        // PR3.70.01 End

        UpdateCommManifestLine(xPurchLine, PurchRcptLine); // P8000891
    end;
    ...
    begin
        ...
        with PurchHeader do begin
            if not IsHandled then begin
                ...
                "Delivery Trip No." := DeliveryTripNo; // P8000549A, P8004516
                ...
                if ProcessFns.FreshProInstalled then
                    ExtraChargeMgmt.MoveToDocumentHeader(DATABASE::"Purchase Header", // P8001032
                      PurchHeader."Document Type", PurchHeader."No.", PurchHeader."Posting Date", // P8000928
                      DATABASE::"Return Shipment Header", ReturnShptHeader."No.");               // P8000928
                                                                                                 // P8004516
            ...
            if WhseShip then begin
                ...
                // P8001323, P8004516
                if ProcessFns.ContainerTrackingInstalled then
                    if ContainerFns.UpdateContainerQtyToShip(WhseShptHeader, DATABASE::"Purchase Line", PurchHeader."Document Type", PurchHeader."No.") then // P80046533
                        RefreshTempLines(PurchHeader, TempPurchLineGlobal); // P80046533, P80073095
                                                                            // P8001323, P8004516
            end;
            if WhseReceive then begin
                WhseRcptHeader.Get(TempWhseRcptHeader."No.");
                ...
                // P8001323, P8004516
                if ProcessFns.ContainerTrackingInstalled then
                    if ContainerFns.UpdateContainerQtyToReceive(WhseRcptHeader, DATABASE::"Purchase Line", PurchHeader."Document Type", PurchHeader."No.") then // P80046533
                        RefreshTempLines(PurchHeader, TempPurchLineGlobal); // P80046533, P80073095
                                                                            // P8001323, P8004516
            end;
        end;
    ...
    local procedure InsertReturnShipmentLine(ReturnShptHeader: Record "Return Shipment Header"; PurchLine: Record "Purchase Line"; CostBaseAmount: Decimal; PostAsItem: Boolean)
    var
        ReturnShptLine: Record "Return Shipment Line";
        IsHandled: Boolean;
    begin
    ...
    begin
        ...
        if (PurchLine.Type = PurchLine.Type::Item) and (PurchLine."Return Qty. to Ship" <> 0) then begin
            ...
            // PR3.60
            if PurchLine.CostInAlternateUnits() then
                ReturnShptLine."Item Charge Base Amount" := Round(CostBaseAmount / PurchLine."Quantity (Alt.)" * ReturnShptLine."Quantity (Alt.)");
            // PR3.60
        end;

        if xPurchLine."Alt. Qty. Transaction No." <> 0 then                 // PR3.60
            AltQtyMgmt.PurchLineToReturnShptLine(xPurchLine, ReturnShptLine); // PR3.60
        // PR3.70.01 Begin
        if (ReturnShptLine.Type = ReturnShptLine.Type::Item) and ProcessFns.FreshProInstalled then // PR3.70.03
            ExtraChargeMgmt.MoveToDocumentLine(
    ...
    begin
        ...
        with PurchHeader do begin
            ...
            if ProcessFns.FreshProInstalled then
                ExtraChargeMgmt.MoveToDocumentHeader(DATABASE::"Purchase Header", // P8001032
                  PurchHeader."Document Type", PurchHeader."No.", PurchHeader."Posting Date", // P8000928
                  DATABASE::"Purch. Inv. Header", PurchInvHeader."No.");                     // P8000928
                                                                                             // P8004516
    ...
    begin
        ...
        with PurchHeader do begin
            ...
            if ProcessFns.FreshProInstalled then
                ExtraChargeMgmt.MoveToDocumentHeader(DATABASE::"Purchase Header", // P8001032
                  PurchHeader."Document Type", PurchHeader."No.", PurchHeader."Posting Date", // P8000928
                  DATABASE::"Purch. Cr. Memo Hdr.", PurchCrMemoHeader."No.");                // P8000928
                                                                                             // P8004516
    ...
    begin
        with SalesShptLine do begin
            ...
            "Qty. Invoiced (Base)" := 0;
            // P8004516
            "Quantity (Alt.)" := DropShptPostBuffer."Quantity (Alt.)"; // P8000506A, P8004516
            "Qty. Invoiced (Alt.)" := 0;
            // P8004516
    ...
    begin
        with TempPurchLine do begin
            ...
            SetCommManifestFilters(PurchHeader, TempPurchLine); // P8000891, P8007748
            if PurchHeader."Document Type" = PurchHeader."Document Type"::Order then
                ...
                if TempDropShptPostBuffer.FindSet() then begin
                    repeat
                        ...
                        repeat
                            ...
                            // P8001077, P8004516
                            if ProcessFns.PricingInstalled then begin
                                ...
                                SalesContMgmt.InsertSalesContractHist(SalesShptLine."Document No.", SalesOrderLine, SalesOrderHeader);
                            end;
                            // P8001077, P8004516
                            ...
                            SalesOrderLine."Qty. to Ship (Alt.)" := SalesShptLine."Quantity (Alt.)"; // P800506A, P8004516
                            ...
                            if ProcessFns.AltQtyInstalled then                      // P8004516
                                AltQtyMgmt.DropShipShptToShipmentLine(SalesShptLine); // P8004516			    
    ...
    begin
        ...
        with GenJnlLine do begin
            ...
            if (InvoicePostBuffer.Type = InvoicePostBuffer.Type::FOODAccrualPlan) then
                AccrualFldMgmt.PurchPostPrepareGenJnlLine(
                  PurchHeader, InvoicePostBuffer."Accrual Line No.", GenJnlLine);
            // P8000241A
    ...
    begin
        ...
        if PostingDateExists and (ReplacePostingDate or (PurchaseHeader."Posting Date" = 0D)) then begin
            ...
            PurchaseHeader.UpdatePurchLines(PurchaseHeader.FieldCaption("Posting Date"), false); // P8001006, P8004516
            ModifyHeader := true;
        end;
    ...
    local procedure PostItemTracking(PurchHeader: Record "Purchase Header"; PurchLine: Record "Purchase Line"; var TempTrackingSpecification: Record "Tracking Specification" temporary; TrackingSpecificationExists: Boolean; PostAsItem: Boolean)
    var
    ...
    begin
        ...
        with PurchHeader do begin
            ...
            if ShouldProcessShipment then begin
                ...
                then
                    PostItemTrackingForShipment(PurchHeader, PurchLine, TrackingSpecificationExists, TempTrackingSpecification, PostAsItem); // P80073095
                ...
                if (Abs(RemQtyToBeInvoiced) > Abs(PurchLine."Qty. to Receive")) or
                   (Abs(RemQtyToBeInvoiced) >= Abs(QtyToInvoiceBaseInTrackingSpec)) and (QtyToInvoiceBaseInTrackingSpec <> 0)
                then
                    PostItemTrackingForReceipt(PurchHeader, PurchLine, TrackingSpecificationExists, TempTrackingSpecification, PostAsItem); // P80073095
    ...
    local procedure PostItemTrackingForReceipt(PurchHeader: Record "Purchase Header"; PurchLine: Record "Purchase Line"; TrackingSpecificationExists: Boolean; var TempTrackingSpecification: Record "Tracking Specification" temporary; PostAsItem: Boolean)
    var
        PurchRcptLine: Record "Purch. Rcpt. Line";
        ItemEntryRelation: Record "Item Entry Relation";
        EndLoop: Boolean;
            ...
            if PurchRcptLine.FindSet(true, false) then begin
                ...
                repeat
                    ...
                    UpdateQtyToBeInvoicedForReceipt(
                      QtyToBeInvoiced, QtyToBeInvoicedBase, QtyToBeInvoicedAlt, // P8001386
                    ...
                    UpdateInvoicedQtyOnPurchRcptLine(
                      PurchInvHeader, PurchRcptLine, PurchHeader, PurchLine, QtyToBeInvoiced, QtyToBeInvoicedBase, QtyToBeInvoicedAlt, TrackingSpecificationExists, TempTrackingSpecification); // P8001386
                    ...
                    if PostItemTrackingForReceiptCondition(PurchLine, PurchRcptLine) then
                        PostItemJnlLine(
                          PurchHeader, PurchLine, 0, 0, 0, QtyToBeInvoiced, QtyToBeInvoicedBase, QtyToBeInvoicedAlt, // PR3.60
                          ItemEntryRelation."Item Entry No.", '', TempTrackingSpecification, PostAsItem); // P8007748
                    if PurchLine."Alt. Qty. Transaction No." <> 0 then      // PR3.60
                        AltQtyMgmt.UpdateReceiptAltQtyEntries(PurchRcptLine); // PR3.60
                    EndLoop :=
    ...
    local procedure PostItemTrackingForShipment(PurchHeader: Record "Purchase Header"; PurchLine: Record "Purchase Line"; TrackingSpecificationExists: Boolean; var TempTrackingSpecification: Record "Tracking Specification" temporary; PostAsItem: Boolean)
    var
        ReturnShptLine: Record "Return Shipment Line";
        ItemEntryRelation: Record "Item Entry Relation";
        EndLoop: Boolean;
        ...
        with PurchHeader do begin
            ...
            if ReturnShptLine.FindSet(true, false) then begin
                ...
                repeat
                    ...
                    UpdateInvoicedQtyOnReturnShptLine(ReturnShptLine, QtyToBeInvoiced, QtyToBeInvoicedBase, QtyToBeInvoicedAlt); // P80073095
                    ...
                    // if PostItemTrackingForShipmentCondition(PurchLine, ReturnShptLine) then //P8001373
                    if PostAsItem then //P8001373
                        PostItemJnlLine(
                          PurchHeader, PurchLine, 0, 0, 0, QtyToBeInvoiced, QtyToBeInvoicedBase, QtyToBeInvoicedAlt, // PR3.60
                          ItemEntryRelation."Item Entry No.", '', TempTrackingSpecification, PostAsItem); // P8007748
                    if PurchLine."Alt. Qty. Transaction No." <> 0 then          // PR3.60
                        AltQtyMgmt.UpdateReturnShptAltQtyEntries(ReturnShptLine); // PR3.60
                    EndLoop :=
            ...
            if FindSet() then
                repeat
                    ...
                    if PurchHeader.Receive then begin
                        ...
                        "Qty. Received (Alt.)" += "Qty. to Receive (Alt.)"; // PR3.60
                    ...
                    if PurchHeader.Ship then begin
                        ...
                        "Return Qty. Shipped (Alt.)" += "Return Qty. to Ship (Alt.)"; // PR3.60
                                                                                      //"Qty. to Ship/Recv. (Cont.)" := 0; //P8001373, P80046533
                    end;
                    if PurchHeader.Invoice then begin
                        if "Document Type" = "Document Type"::Order then
                        ...
                        "Qty. Invoiced (Alt.)" := "Qty. Invoiced (Alt.)" + "Qty. to Invoice (Alt.)"; // PR3.60
                    ...
                    if WhseHandlingRequiredExternal(TempPurchLine) or SetDefaultQtyBlank or (Type = Type::FOODContainer) then begin // P80055951
                        if "Document Type" = "Document Type"::"Return Order" then begin
                            "Return Qty. to Ship" := 0;
                            "Return Qty. to Ship (Base)" := 0;
                            "Return Qty. to Ship (Alt.)" := 0; // P8000282A
                        end else begin
                            "Qty. to Receive" := 0;
                            "Qty. to Receive (Base)" := 0;
                            "Qty. to Receive (Alt.)" := 0; // P8000282A
                        end;
                    ...
                    if ("Commodity Manifest No." = '') then // P8000891
                        SetDefaultQuantity();
                    UpdateLotTracking(true); // P8000610A, P80077569
            ...
            if FindSet() then
                repeat
                    ...
                    if not IsHandled then begin
                        ...
                        // PR3.60
                        PurchOrderLine."Qty. Invoiced (Alt.)" += "Qty. to Invoice (Alt.)"; // PR3.60
                    ...
                    OnPostUpdateInvoiceLineOnBeforePurchOrderLineModify(PurchHeader, PurchOrderLine);
                ...
                if FindSet() then
                    repeat
                        ...
                        PurchOrderLine."Qty. Invoiced (Base)" :=
                          PurchOrderLine."Qty. Invoiced (Base)" + "Qty. to Invoice (Base)";
                        // PR3.60
                        PurchOrderLine."Qty. Invoiced (Alt.)" :=
                          PurchOrderLine."Qty. Invoiced (Alt.)" + "Qty. to Invoice (Alt.)";
                        // PR3.60
    ...
    local procedure InvoiceTransportCosts(var TransCostEntryNo: Integer)
    var
        PostedTransportCost: Record "N138 Posted Transport Cost";
    begin
        // TOM4524 - added parameter for TransCostEntryNo
        //N138F0000.sn
        if not ProcessFns.DistPlanningInstalled then exit;  // P8008464

        PostedTransportCost.SetRange("Entry No.", TransCostEntryNo); // TOM4524
        if PostedTransportCost.FindFirst then begin
            PostedTransportCost.Invoiced := true;
            PostedTransportCost.Modify;
        end;
        //N138F0000.en
    end;

    local procedure AssignPostingNo(PurchHeader: Record "Purchase Header"; PostingNoSeries: Code[10]; LastPostingNo: Code[20]): Code[20]
    var
        NoSeriesMgt: Codeunit NoSeriesManagement;
    begin
        // PR3.61.01
        // P8007748 - add parameter PurchHeader
        with PurchHeader do begin
    ...
    local procedure AssignShipmentPostingNo(var SalesHeader: Record "Sales Header"): Code[20]
    var
        NoSeriesMgt: Codeunit NoSeriesManagement;
    begin
        // PR3.60
        with SalesHeader do
            if ("Shipping No. Series" <> "No. Series") then
                "Shipping No." := NoSeriesMgt.GetNextNo("Shipping No. Series", "Posting Date", true)
            else
                if ("Last Shipping No." = '') then
                    "Shipping No." := "No."
                else
                    if ("Last Shipping No." = "No.") then
                        "Shipping No." := "Last Shipping No." + Text37002000
                    else
                        "Shipping No." := IncStr("Last Shipping No.");
        // PR3.60
    end;

    local procedure PostAccrualsToGL(PurchHeader: Record "Purchase Header")
    var
        GenJnlLine: Record "Gen. Journal Line";
        FirstLine: Boolean;
    begin
    ...
    procedure SetPickupLoad(LoadNo: Code[20])
    begin
        // P8000080A
        PickupLoadNo := LoadNo;
    end;
    ...
    local procedure PostPurchaseContainerLine(var ItemJnlLine: Record "Item Journal Line"; var PurchLine: Record "Purchase Line")
    var
        TempWhseJnlLine: Record "Warehouse Journal Line" temporary;
        TempWhseJnlLine2: Record "Warehouse Journal Line" temporary;
        PostWhseJnlLine: Boolean;
    begin
        ...
        with PurchLine do begin
            if ("Location Code" <> '') then begin
                GetLocation("Location Code");
                if not IsNonWarehouseItem() then
                    if (("Document Type" in ["Document Type"::Invoice, "Document Type"::"Credit Memo"]) and
                        Location."Directed Put-away and Pick") or
                       (Location."Bin Mandatory" and not (WhseReceive or WhseShip or InvtPickPutaway or "Drop Shipment"))
                    then begin
            ...
            if ItemJnlPostLine.CollectTrackingSpecification(TempHandlingSpecification) then
                if TempHandlingSpecification.FindSet then
                    repeat
                        ...
                        if TempTrackingSpecification.Insert then;
                    until TempHandlingSpecification.Next = 0;
            XferWhseRoundingAdjmts(PostWhseJnlLine);
            if PostWhseJnlLine then begin
                XferRegsToWhse;
    ...
    local procedure SetCommManifestFilters(PurchHeader: Record "Purchase Header"; var PurchLine3: Record "Purchase Line")
    begin
        // P8000891
        // P8007748 - add parameter PurchHeader
        if (CommManifestNo <> '') then begin
            PurchLine3.SetRange("Commodity Manifest No.", CommManifestNo);
            PurchLine3.SetRange("Commodity Received Date", PurchHeader."Posting Date");
        end;
    end;

    local procedure UpdateCommManifestLine(var PurchLine3: Record "Purchase Line"; var PurchRcptLine2: Record "Purch. Rcpt. Line")
    var
        CommManifestLine: Record "Commodity Manifest Line";
    begin
        // P8000891
        if (CommManifestNo <> '') then begin
            CommManifestLine.Get(PurchLine3."Commodity Manifest No.", PurchLine3."Commodity Manifest Line No.");
            CommManifestLine."Purch. Order Status" := CommManifestLine."Purch. Order Status"::Posted;
            CommManifestLine."Purch. Rcpt. No." := PurchRcptLine2."Document No.";
            CommManifestLine."Purch. Rcpt. Line No." := PurchRcptLine2."Line No.";
            CommManifestLine.Modify;
        end;
    end;

    local procedure WhseRqstUpdateCompletelyHandled(var PurchHeader: Record "Purchase Header")
    var
        ReleasePurchaseDocument: Codeunit "Release Purchase Document";
        WarehouseRequest: Record "Warehouse Request";
    begin
        ...
        ReleasePurchaseDocument.SetSkipCheckReleaseRestrictions; // P80097194
        ReleasePurchaseDocument.Run(PurchHeader);
    end;

    local procedure GetQtyToInvoiceAlt(PurchLine: Record "Purchase Line"; Receive: Boolean): Decimal
    begin
        // P8001366
        // P8007748 - add parameter Receive
        with PurchLine do begin
            "Qty. Rcd. Not Invoiced" := "Qty. Received (Alt.)" - "Qty. Invoiced (Alt.)";
            "Qty. to Receive" := "Qty. to Receive (Alt.)";
            "Qty. to Invoice" := "Qty. to Invoice (Alt.)";
            exit(GetQtyToInvoice(PurchLine, Receive)); // P8007748
        end;
    end;

    local procedure DeleteDocAccrualLine(PurchHeader: Record "Purchase Header")
    begin
        // P8007748
        DocAccrualLine.Reset;
        DocAccrualLine.SetRange("Accrual Plan Type", DocAccrualLine."Accrual Plan Type"::Purchase);
        DocAccrualLine.SetRange("Document Type", PurchHeader."Document Type");
        DocAccrualLine.SetRange("Document No.", PurchHeader."No.");
