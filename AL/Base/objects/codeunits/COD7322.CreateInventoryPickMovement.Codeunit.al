codeunit 7322 "Create Inventory Pick/Movement"
{
    ...
    // 
    // PRW16.00.01
    // P8000709, VerticalSoft, Don Bresee, 30 JUL 09
    //   Fix issue in routine to assign next line no.
    // 
    // PRW16.00.06
    ...
    begin
        ...
        if PurchHeader."Document Type" = PurchHeader."Document Type"::"Return Order" then
            LotStatusExclusionFilter := LotStatusMgmt.SetLotStatusExclusionFilter(LotStatus.FieldNo("Available for Purchase"));
        // P8001083
    ...
    var
        ...
        IsHandled: Boolean;
    begin
        LotStatusExclusionFilter := LotStatusMgmt.SetLotStatusExclusionFilter(LotStatus.FieldNo("Available for Transfer")); // P8001083
    ...
    var
        ...
        IsHandled: Boolean;
    begin
        LotStatusExclusionFilter := LotStatusMgmt.SetLotStatusExclusionFilter(LotStatus.FieldNo("Available for Consumption")); // P8001132
    ...
    begin
        ...
        if (RemQtyToPickBase >
            (QtyAvailToPickBase + RoundingAdjmtMgmt.GetNearZeroQtyForItem(NewWhseActivLine."Item No.")))
        then begin
            // P8001039
            ...
            if Find('-') then
                repeat
                    ...
                    if not IsHandled then begin
                        ...
                        // if RemQtyToPickBase < QtyAvailToPickBase then                                                        // P8001039
                        if (RemQtyToPickBase < (QtyAvailToPickBase + RoundingAdjmtMgmt.GetNearZeroQtyForItem("Item No."))) then // P8001039
    ...
    begin
        ...
        // P8001083
        LotStatusMgmt.QuantityAdjforItem(Item2, LotStatusExclusionFilter, Item2.Inventory, Item."Quantity on Hand (Alt.)");
        LotStatusMgmt.QuantityAdjForReservedOnInv(Item2, LotStatusExclusionFilter, Item2."Reserved Qty. on Inventory");
        // P8001083
    ...
    begin
        ...
        if WhseItemTrackingFEFO.FindFirstEntrySummaryFEFO(EntrySummary) then begin
            ...
            repeat
                ...
                if EntrySummary."Expiration Date" <> 0D then
                    if not LotStatusMgmt.ExcludeLot(WhseActivLine."Item No.", WhseActivLine."Variant Code", EntrySummary."Lot No.", LotStatusExclusionFilter) then begin // P8001352
                                                                                                                                                                         // P8001083
    ...
    begin
        ...
        if NewWhseActivLine.TrackAlternateUnits() then           // P8000282A
            NewWhseActivLine.Validate("Qty. to Handle (Alt.)", 0); // P8000282A
