codeunit 7304 "Whse. Jnl.-Register Batch"
{
    ...
    //   Use Lot No. from journal line instead of the tracking lines
    // 
    // PRW110.0
    // P8008178, To Increase, Jack Reynolds, 08 DEC 16
    //   Problem with lot number on reclass journal
    // 
    // PRW110.0.01
    ...
    //
    // PRW120.4
    // P800158495, To-Increase, Gangabhushan, 20 OCT 22
    //   CS00222758 | Not able to adjust Catch weight item alt. qty in Advanced warehouse location
    //   Fixed Container Qty(Alt) issue while Registering from Whse. Phy. Inv. Journals.
    ...
    var
        ...
        TempWhseJnlLineCont: Record "Warehouse Journal Line" temporary;
        ...
        with WhseJnlLine do begin
            ...
            repeat
                ...
                if WhseItemTrackingSetup.TrackingRequired() then begin
                    ...
                    UpdateItemTrackingLine;      // P80041932, P80092144
                ...
                // P8001323
                if "Phys. Inventory" and ("Entry Type" = "Entry Type"::"Negative Adjmt.") and ("From Container ID" <> '') then begin
                    TempWhseJnlLineCont := WhseJnlLine;
                    TempWhseJnlLineCont.Insert;
                end;
                // P8001323
    ...
    begin
        ...
        // P8008178
        if WhseItemTrkgLine.IsEmpty then begin
    ...
    local procedure RemoveFromContainer()
    var
        WhseJnlLine2: Record "Warehouse Journal Line";
        ...
        Qty: Decimal;
        QtyAlt: Decimal;
        InitialContQtyAlt: Decimal; // P800158495
    begin
        // P8001323
        if TempWhseJnlLineCont.FindFirst then
            repeat
                Item.Get(TempWhseJnlLineCont."Item No.");
                ContainerHeader.Get(TempWhseJnlLineCont."From Container ID");
                ContainerHeader.Mark(true);

                ContainerLine.SetRange("Container ID", ContainerHeader.ID);
                ContainerLine.SetRange("Item No.", TempWhseJnlLineCont."Item No.");
                ContainerLine.SetRange("Variant Code", TempWhseJnlLineCont."Variant Code");
                ContainerLine.SetRange("Lot No.", TempWhseJnlLineCont."Lot No.");
                ContainerLine.SetRange("Serial No.", TempWhseJnlLineCont."Serial No.");
                ContainerLine.SetRange("Unit of Measure Code", TempWhseJnlLineCont."Unit of Measure Code");
                if ContainerLine.FindSet(true) then
                    repeat
                        if Abs(TempWhseJnlLineCont.Quantity) < ContainerLine.Quantity then
                            Qty := Abs(TempWhseJnlLineCont.Quantity)
                        else
                            Qty := ContainerLine.Quantity;
                        if Abs(TempWhseJnlLineCont."Quantity (Alt.)") < ContainerLine."Quantity (Alt.)" then
                            QtyAlt := Abs(TempWhseJnlLineCont."Quantity (Alt.)")
                        else
                            QtyAlt := ContainerLine."Quantity (Alt.)";
                        TempWhseJnlLineCont.Quantity += Qty;
                        TempWhseJnlLineCont."Quantity (Alt.)" += QtyAlt;
                        ContainerLinePost := ContainerLine;
                        ContainerLine.Quantity -= Qty;
                        InitialContQtyAlt := ContainerLine."Quantity (Alt.)"; // P800158495
                        ContainerLine.Validate(Quantity);
                        ContainerLine."Quantity (Alt.)" := InitialContQtyAlt; // P800158495
                        if (0 < ContainerLine.Quantity) and Item."Catch Alternate Qtys." then
                        ...
                        if ContainerLine.Quantity = 0 then begin
                            ...
                            ContainerLinePost.PostContainerUse(ContainerLinePost.Quantity, ContainerLinePost."Quantity (Alt.)",
                              ContainerLine.Quantity, ContainerLine."Quantity (Alt.)");
                            ContainerLine.Modify;
                        end;
                    until (ContainerLine.Next = 0) or (TempWhseJnlLineCont.Quantity = 0);
                WhseJnlLine2.Get(TempWhseJnlLineCont."Journal Template Name", TempWhseJnlLineCont."Journal Batch Name",
                  TempWhseJnlLineCont."Location Code", TempWhseJnlLineCont."Line No.");
                WhseJnlLine2."Container License Plate" := '';
                WhseJnlLine2."From Container License Plate" := '';
                WhseJnlLine2."From Container ID" := '';
                WhseJnlLine2.Modify;
            until TempWhseJnlLineCont.Next = 0;

        WhseJnlLine2.Reset;
        WhseJnlLine2.SetRange("Journal Template Name", WhseJnlLine."Journal Template Name");
        WhseJnlLine2.SetRange("Journal Batch Name", WhseJnlLine."Journal Batch Name");
        WhseJnlLine2.SetRange("Entry Type", WhseJnlLine2."Entry Type"::"Positive Adjmt.");
        ContainerHeader.MarkedOnly(true);
        if ContainerHeader.FindFirst then
            repeat
                ...
                if ContainerLine.IsEmpty then begin
                    ContainerLine.SetRange(Quantity);
                    ContainerLine.DeleteAll;

                    WhseJnlLine2.SetRange("To Container License Plate", ContainerHeader.ID);
                    if WhseJnlLine2.FindFirst then begin
                        ContainerHeader.Validate("Bin Code", WhseJnlLine2."To Bin Code");
