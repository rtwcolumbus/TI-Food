page 6500 "Item Tracking Summary"
{
    ...
    begin
        LotStatusMgmt.GetLotStatus(ItemNo, VariantCode, "Lot No.", LotStatusExclusionFilter, LotStatusCode, LotExcluded); // P8001083
    end;
    ...
    begin
        // P80045713
        if (CloseAction <> ACTION::LookupOK) or (not LotExcluded) then
            exit(true);

        exit(Confirm(LotStatusWarning, false, "Lot No.", LotStatusMgmt.AvailableForFieldNoToActionText(LotStatusAvailFieldNo)));
        // P80045713
    end;
    ...
    var
        ...
        ItemNo: Code[20];
        VariantCode: Code[10];
        LotStatusWarning: Label 'Lot %1 is not available for %2.\Continue?';
    ...
    procedure SetLotStatus(ItemNo2: Code[20]; VariantCode2: Code[10]; AvailFieldNo: Integer)
    begin
        // P8001083
        ItemNo := ItemNo2;
        VariantCode := VariantCode2;
        LotStatusAvailFieldNo := AvailFieldNo; // P80045713
        LotStatusExclusionFilter := LotStatusMgmt.SetLotStatusExclusionFilter(AvailFieldNo); // P80045713
    end;
