page 26 "Vendor Card"
{
    ...
    //   Added choice to Vendor Menu Pickup Locations
    // 
    // PR3.70.07
    ...
    //   Add control on general tab for Total Maintenance Cost
    //   Add Vendor menu buttom item to run Vendor / Maintenance Trades
    // 
    // PRW15.00.01
    ...
    layout
    {
        area(content)
        {
            ...
            group(Receiving)
            {
                ...
                field("Commodity Vendor Type"; Rec."Commodity Vendor Type")
                {
                    ApplicationArea = FOODBasic;
                }
                field("Producer Zone Code"; Rec."Producer Zone Code")
                {
                    ApplicationArea = FOODBasic;
                }
                field("Commodity Invoicing Frequency"; Rec."Commodity Invoicing Frequency")
                {
                    ApplicationArea = FOODBasic;
                }
                field("Comm. Adv. Payment Type"; Rec."Comm. Adv. Payment Type")
                {
                    ApplicationArea = FOODBasic;
                }
                field("Comm. Adv. Payment Amount"; Rec."Comm. Adv. Payment Amount")
                {
                    ApplicationArea = FOODBasic;
                }
    ...
    actions
    {
        area(navigation)
        {
            group("Ven&dor")
            {
                ...
                action("Pickup Locations")
                {
                    ApplicationArea = FOODBasic;
                    Caption = 'Pickup Locations';
                    Image = CreateInventoryPickup;
                    RunObject = Page "Pickup Locations";
                    RunPageLink = "Vendor No." = FIELD("No.");
                }
                action(Trades)
                {
                ...
                action("Hauler Char&ges")
                {
                    ApplicationArea = FOODBasic;
                    Caption = 'Hauler Char&ges';
                    Image = ProjectExpense;
                    RunObject = Page "Hauler Charges";
                    RunPageLink = "Hauler No." = FIELD("No.");
                    RunPageView = SORTING("Hauler No.", "Receiving Location Code", "Producer Zone Code");
                }
                ...
                action(DataCollectionLines)
                {
                    ...
                    RunPageLink = "Source ID" = CONST(23),
                ...
                action("Accrual Groups")
                {
                    ...
                    RunPageView = SORTING("Accrual Group Type", "No.", "Accrual Group Code")
                                  WHERE("Accrual Group Type" = CONST(Vendor));
                }
