page 353 "Item Availability Lines"
{
    ...
    layout
    {
        area(content)
        {
            repeater(Control1)
            {
                ...
                field(ScheduledReceiptNotAvail; "Total Scheduled Receipt" - "Scheduled Receipt")
                {
                ...
                field(PlannedOrderReceiptNotAvail; "Total Planned Order Receipt" - "Planned Order Receipt")
                {
                    ApplicationArea = FOODBasic;
                ...
                field(InventoryNotAvail; Rec."Total Inventory" - Inventory)
                {
                    ApplicationArea = FOODBasic;
                ...
                field(PlannedOrderReleasesNotAvail; Rec."Total Planned Order Releases" - Rec."Planned Order Releases")
                {
                    ApplicationArea = FOODBasic;
    ...
    begin
        ...
        SetLines(
            NewItem,
             "Analysis Period Type".FromInteger(NewPeriodType), "Analysis Amount Type".FromInteger(NewAmountType), 0);

        OnAfterSet(Item, PeriodType.AsInteger(), AmountType);
    end;
#endif

    procedure Set(var NewItem: Record Item; NewPeriodType: Integer; NewAmountType: Option "Net Change","Balance at Date"; AvailableFor: Option " ",Sale,"Purchase Return",Transfer,Consumption,Adjustment,Planning)
    begin
        // P8001083 - add parameter for AvailableFor
        SetLines(
            NewItem,
             "Analysis Period Type".FromInteger(NewPeriodType), "Analysis Amount Type".FromInteger(NewAmountType), AvailableFor);

        OnAfterSet(Item, PeriodType.AsInteger(), AmountType);
    end;

    procedure SetLines(var NewItem: Record Item; NewPeriodType: Enum "Analysis Period Type"; NewAmountType: Enum "Analysis Amount Type"; AvailableFor: Option " ",Sale,"Purchase Return",Transfer,Consumption,Adjustment,Planning)
    var
    ...
    begin
        ...
        // P8001132
        ItemAvailFormsMgt.GetTotals(
            ItemTotal, "Total Planned Order Releases", "Total Planned Order Receipt", "Total Scheduled Receipt");
        "Total Inventory" := ItemTotal.Inventory;
        // P8001132
