page 6641 "Purchase Return Order Subform"
{
    ...
    // 
    // PRW16.00.02
    // P8000761, VerticalSoft, MMAS, 28 JAN 10
    //   Method SetLotFields() code changed in order to transform correctly into 2009.
    // 
    // PRW16.00.03
    ...
    layout
    {
        area(content)
        {
            repeater(Control1)
            {
                ...
                field("Return Qty. Shipped (Alt.)"; Rec."Return Qty. Shipped (Alt.)")
                {
                    ApplicationArea = FOODBasic;
                }
    ...
    begin
        LotEditable := ProcessFns.TrackingInstalled and ("Lot No." <> P800Globals.MultipleLotCode) and (Type = Type::Item);
    end;
    ...
    begin
        ...
        if ProcessFns.DistPlanningInstalled then  // P8008464
            if DeliveryTripMgt.GetWhseReqPUR(Rec, WarehouseRequest) then
    ...
    begin
        //N138F0000.sn
        if ProcessFns.DistPlanningInstalled then  // P8008464
            DeliveryTripMgt.LookupWhseReqPUR(Rec);
        //N138F0000.en
    end;
