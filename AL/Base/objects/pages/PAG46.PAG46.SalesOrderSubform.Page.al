page 46 "Sales Order Subform"
{
    ...
    // 
    // PR3.61.01
    //   Check for permission before calling Container Functions codeunit
    // 
    // PR3.70
    //   Item Availability
    // 
    // PR3.70.02
    //   Qunatity on Order was adjusted by quantity on current line - changed to outstanding quantity
    // 
    // PR3.70.03
    //   Accruals
    // 
    ...
    //   ADDED Exclude from sales forecast control to form
    //   made it default not visible
    // 
    // PR3.70.07
    // P8000153A, Myers Nissi, Jack Reynolds, 16 DEC 04
    //   Add support for checking lot preferences
    // 
    // PR3.70.09
    ...
    // 
    // PR3.70.10
    // P8000214A, Myers Nissi, Jack Reynolds, 18 MAY 05
    //   Clean up code when inserting container charges to avoid unnecessary saving of record
    // 
    ...
    //   Create function to run SalesHistory on sales line
    // 
    // PR4.00.04
    ...
    // 
    // PRW16.00.20
    // P8000664, VerticalSoft, Jimmy Abidi, 29 JUN 09
    //   Page modified after transformation
    // 
    ...
    // 
    // PRW16.00.04
    // P8000885, VerticalSoft, Ron Davidson, 28 DEC 10
    //   Added option to clear and reset Sales Contract information.
    // 
    // PRW16.00.05
    // P8000921, Columbus IT, Don Bresee, 08 APR 11
    //   Added Delivered Pricing fields
    // 
    // PRW16.00.06
    ...
    layout
    {
        area(content)
        {
            repeater(Control1)
            {
                ...
                field("Sales Sample"; Rec."Sales Sample")
                {
                    ApplicationArea = FOODBasic;
                    Visible = false;
                }
                ...
                field("Unit Price (Freight)"; Rec."Unit Price (Freight)")
                {
                    ApplicationArea = FOODBasic;
                    Visible = false;
                }
                ...
                field("Line Amount (Freight)"; Rec."Line Amount (Freight)")
                {
                    ApplicationArea = FOODBasic;
                    Visible = false;
                }
                ...
                field("Promo/Rebate Amount (LCY)"; Rec."Promo/Rebate Amount (LCY)")
                {
                    ApplicationArea = FOODBasic;
                    ...
                    begin
                        // P8000241A
                        CurrPage.Update;
                        AccrualMgmt.SalesPromoRebateDrillDown(Rec);
                        CurrPage.Update(false);
                        // P8000241A
                    end;
                }
                field("Commission Amount (LCY)"; Rec."Commission Amount (LCY)")
                {
                    ...
                    begin
                        // P8000241A
                        CurrPage.Update;
                        AccrualMgmt.SalesCommissionDrillDown(Rec);
                        CurrPage.Update(false);
                        // P8000241A
                    end;
                }
                ...
                field("Oldest Accept. Freshness Date"; Rec."Oldest Accept. Freshness Date")
                {
                    ApplicationArea = FOODBasic;
                    Editable = OldestAcceptableEditable;
                    Visible = false;
                }
                field("Staged Quantity"; Rec."Staged Quantity")
                {
                    ApplicationArea = FOODBasic;
                    Visible = false;
                }
        ...
        area(processing)
        {
            ...
            group("&Line")
            {
                ...
                group("F&unctions")
                {
                    ...
                    action("Reset Sales &Contract")
                    {
                        ApplicationArea = FOODBasic;
                        Caption = 'Reset Sales &Contract';
                        Image = ResetStatus;
                ...
                action("Change Sales Order Line")
                {
                    ...
                    begin
                        //ChangeQty;
                        ChangeSalesLineWizard.Init("No.", Quantity, 0, Rec, 1); // TOM4219, P8007827
                ...
                action("Post Settlement")
                {
                    ...
                    begin
                        TripSettlementMgt.PostSalesSettlementSlsLine(Rec, true, 0, 0, ''); // P8006787
                    end;
                }
                group("Item Availability by")
                {
                    ...
                    action(Lot)
                    {
                        ...
                        trigger OnAction()
                        begin
                            ItemAvailFormsMgt.ShowItemAvailFromSalesLine(Rec, ItemAvailFormsMgt.ByLot());
                        end;
                    }
                    action("BOM Level")
                    {
    ...
    begin
        ...
        if ProcessFns.DistPlanningInstalled then  // P8008464
            if DeliveryTripMgt.GetWhseReqSLS(Rec, WarehouseRequest) then
    ...
    begin
        //N138F0000.sn
        if ProcessFns.DistPlanningInstalled then  // P8008464
            DeliveryTripMgt.LookupWhseReqSls(Rec);
        //N138F0000.en
    end;

    local procedure OnOpenForm1()
    begin
    end;
    ...
    begin
        // PR3.61 Begin
        // CurrForm.SAVERECORD;                                                   // P8000214A
        if ProcessFns.ContainerTrackingInstalled then // PR3.61.01
            if (Type = Type::FOODContainer) or (xRec.Type = Type::FOODContainer) then begin // P8000214A
                CurrPage.SaveRecord;                                                  // P8000214A
    ...
    procedure _SalesHistory()
    begin
        SalesHistory; // P8000248B
    end;
    ...
    procedure SetLotFresh()
    begin
        // P8001062
        OldestAcceptableEditable := "Freshness Calc. Method" <> 0;
    end;

    local procedure ChangeQty()
    var
