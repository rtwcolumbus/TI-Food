page 6520 "Item Tracing"
{
    ...
    //   Change Navigate button to menu, add Related Entries to Line menu and Description AssistEdit
    //   Add Alternate Quantites
    //   Go to first record when form is opened or a new trace is performed
    //   Stay on same record for Expand All and Collapse All
    // 
    // PRW16.00.20
                ...
                field("Quantity (Alt.)"; Rec."Quantity (Alt.)")
                {
                    ...
                    begin
                        ...
                        if ItemLedgEntry.Get("Item Ledger Entry No.") then
                            AltQtyMgmt.ShowItemLedgAltQtyEntries(ItemLedgEntry, FieldNo("Quantity (Alt.)"));
                    end;
                }
    ...
    begin
        if not TempTrackEntry.FindFirst() then begin
            ...
            NavigateMenuEnable := false; // P8000466A
        end else begin
            FunctionsEnable := true;
            PrintEnable := true;
            NavigateEnable := true;
            NavigateMenuEnable := true; // P8000466A
        end;
    ...
    local procedure ShowRelatedEntries()
    var
        ItemLedgEntry: Record "Item Ledger Entry";
    begin
        // P8000466A
    ...
    local procedure GoToFirstTrackEntry()
    begin
        // P8000466A
        if not IsEmpty then
            FindFirst
