report 10070 "Sales Invoice (Pre-Printed)" // Version: FOODNA
{
    //  PR3.70
    ...
    // 
    // PRNA6.00
    // P8000646, VerticalSoft, Jack Reynolds, 17 DEC 08
    //   Add "Print Allowance Detail" to request page
    // 
    // PRNA6.00.01
    // P8000677, VerticalSoft, Jack Reynolds, 13 FEB 09
    //   Fix references to Document Typr for comments
    // 
    // PRW16.00.03
    ...
    //   Remove work around for AllowanceLine problem
    // 
    // PRNA7.10
    // P8001252, Columbus IT, Jack Reynolds, 08 JAN 14
    //   Reworked for alternate quantity detail and allowances
    
    DefaultLayout = RDLC;
    RDLCLayout = './layout/local/SalesInvoicePrePrinted.rdlc';
        ...
        dataitem("Sales Invoice Header"; "Sales Invoice Header")
        {
            ...
            dataitem("Sales Invoice Line"; "Sales Invoice Line")
            {
                ...
                dataitem(SalesLineComments; "Sales Comment Line")
                {
                    ...
                    DataItemTableView = SORTING("Document Type", "No.", "Document Line No.", "Line No.") WHERE("Document Type" = CONST(Shipment), "Print On Invoice" = CONST(true));
                ...
                begin
                    ...
                    if "Off-Invoice Allowance Code" <> '' then begin
                        ...
                        AllowanceAmount -= "Line Amount"; // P8001252
                        CurrReport.Skip;
                    end;
                    // PR3.70
            ...
            dataitem(AllowanceLine; "Integer")
            {
                ...
                trigger OnPreDataItem()
                begin
                    SetRange(Number, 1, TempAllowance.Count); // P8001252
                end;
            }
            dataitem("Sales Comment Line"; "Sales Comment Line")
            {
                DataItemLink = "No." = FIELD("No.");
                DataItemTableView = SORTING("Document Type", "No.", "Document Line No.", "Line No.") WHERE("Document Type" = CONST(Shipment), "Print On Invoice" = CONST(true), "Document Line No." = CONST(0));
            ...
            dataitem(CopyLoop; "Integer")
            {
                ...
                dataitem(PageLoop; "Integer")
                {
                    ...
                    dataitem(SalesInvLine; "Integer")
                    {
                        ...
                        column(TempSalesInvoiceLine__Amount_Including_VAT_; TempSalesInvoiceLine."Amount Including VAT" + AllowanceAmount)
                        {
                        }
                        ...
                        column(OffInvoiceAllowanceCode; TempSalesInvoiceLine."Off-Invoice Allowance Code")
                        {
                        }
                        dataitem(AltQtyEntry; "Integer")
                        {
                            DataItemTableView = SORTING(Number);
                            ...
                            column(AltQtys_1_; AltQtys[1])
                            {
                                DecimalPlaces = 0 : 5;
                            }
                            column(AltQtys_2_; AltQtys[2])
                            {
                                DecimalPlaces = 0 : 5;
                            }
                            column(AltQtys_3_; AltQtys[3])
                            {
                                DecimalPlaces = 0 : 5;
                            }
                            column(AltQtys_4_; AltQtys[4])
                            {
                                DecimalPlaces = 0 : 5;
                            }
                            column(AltQtys_5_; AltQtys[5])
                            {
                                DecimalPlaces = 0 : 5;
                            }
                            ...
                            begin
                                // PR3.60
                                if not AltQtyMgmt.GetEntryReportAltQtys(AltQtys, 5) then
                                    CurrReport.Break;
                                // PR3.60

                                // P8001252
                                if Number > -1000000000 then begin
                                    TaxLiable := 0;
                                    AmountExclInvDisc := 0;
                            ...
                            begin
                                ...
                                if ProcessFns.AltQtyInstalled then begin
                                    if not AltQtyMgmt.StartEntryReport(DATABASE::"Sales Invoice Line",
                                      TempSalesInvoiceLine."Document No.", TempSalesInvoiceLine."Line No.", TempSalesInvoiceLine."Quantity (Base)")
                                    then
                                        CurrReport.Break;
                                end else
                                    CurrReport.Break;
                        ...
                        begin
                            ...
                            with TempSalesInvoiceLine do begin
                                ...
                                if "Off-Invoice Allowance Code" <> '' then begin
                                    ...
                                    AllowanceAmount := -"Unit Price";
                                    if PrintAllowanceDetail then
                                        "Off-Invoice Allowance Code" := '';
                                end else
                                    // P8001252
                                    if Quantity = 0 then
            ...
            begin
                ...
                AllowanceAmount := 0;    // PR3.70, P8001252
            end;
        }
    }
    ...
    labels
    {
        AlternateQuantitiesCap = 'Alternate Quantities:';
    }
