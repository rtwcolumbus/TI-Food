report 86 "Adjust Add. Reporting Currency"
{
    ...
    //   Update "Charge (ACY)" and "Charge Posted to G/L (ACY)" for Extra Charges related to value entries
    // 
    // PR4.00.04
    // P8000375A, VerticalSoft, Jack Reynolds, 11 SEP 06
    //   Update Value Entry ABC Detail ACY fields
    // 
    // PRW16.00.02
    // P8000756, VerticalSoft, Jack Reynolds, 15 JAN 10
    //   Add Posting Date to calls to ExchangeAmLCYToFCY
    ...
    dataset
    {
        ...
        dataitem("Value Entry"; "Value Entry")
        {
            ...
            dataitem("Value Entry Extra Charge"; "Value Entry Extra Charge")
            {
                DataItemLink = "Entry No." = FIELD("Entry No.");
                DataItemTableView = SORTING("Entry No.", "Extra Charge Code");

                trigger OnAfterGetRecord()
                begin
                    // P8000062B
                    "Charge (ACY)" := ExchangeAmtLCYToFCY("Value Entry"."Posting Date", Charge, false);                               // P8000756
                    "Charge Posted to G/L (ACY)" := ExchangeAmtLCYToFCY("Value Entry"."Posting Date", "Charge Posted to G/L", false); // P8000756
                    "Expected Charge (ACY)" := ExchangeAmtLCYToFCY("Value Entry"."Posting Date", "Expected Charge", false);    // PR4.00 P8000756
                    "Exp. Chg. Posted to G/L (ACY)" := ExchangeAmtLCYToFCY("Value Entry"."Posting Date",
                      "Expected Charge Posted to G/L", false);                                                                // PR4.00 P8000756

                    Modify;
                end;
            }
            dataitem("Value Entry ABC Detail"; "Value Entry ABC Detail")
            {
                DataItemLink = "Entry No." = FIELD("Entry No.");
                DataItemTableView = SORTING("Entry No.", "Resource No.");

                trigger OnAfterGetRecord()
                begin
                    // P8000375A
                    "Cost (ACY)" := ExchangeAmtLCYToFCY("Value Entry"."Posting Date", Cost, false);                                       // P8000756
                    "Cost Posted to G/L (ACY)" := ExchangeAmtLCYToFCY("Value Entry"."Posting Date", "Cost Posted to G/L", false);         // P8000756
                    "Overhead (ACY)" := ExchangeAmtLCYToFCY("Value Entry"."Posting Date", Overhead, false);                               // P8000756
                    "Overhead Posted to G/L (ACY)" := ExchangeAmtLCYToFCY("Value Entry"."Posting Date", "Overhead Posted to G/L", false); // P8000756
