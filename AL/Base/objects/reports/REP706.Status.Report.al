report 706 Status
{
    ...
    //   Modified for alternate quantities
    // 
    // PR5.00
    // P8000466A, VerticalSoft, Don Bresee, 12 JUN 07
    //   Change GetCostingRemQty to GetCostingQty
    // 
    // PRW17.00
    // P8001132, Columbus IT, Jack Reynolds, 27 MAR 13
    //   Upgrade for NAV 2013
    DefaultLayout = RDLC;
    RDLCLayout = './layout/Status.rdlc';
    ...
    dataset
    {
        dataitem(Item; Item)
        {
            ...
            column(BaseUnitofMeasure_ItemCaption; UOMLabel)
            {
            }
            ...
            begin
                ...
                // P8000301A
                if "Costing Unit" = "Costing Unit"::Alternate then begin
                    UOMLabel := FieldCaption("Alternate Unit of Measure");
                    "Base Unit of Measure" := "Alternate Unit of Measure"
                end else
                    UOMLabel := FieldCaption("Base Unit of Measure");
                // P8000301A
            end;
    ...
    begin
        //RemainingQty := "Item Ledger Entry".Quantity;    // P8000301A
        RemainingQty := "Item Ledger Entry".GetCostingQty; // P8000301A, P8000466A
        ...
        with ItemApplnEntry do
            if "Item Ledger Entry".Positive then begin
                ...
                if Find('-') then
                    repeat
                        //SumQty(RemainingQty,"Outbound Item Entry No.",Quantity);    // P8000301A
                        SumQty(RemainingQty, "Outbound Item Entry No.", GetCostingQty); // P8000301A
                    until Next() = 0;
            end else begin
                Reset();
                ...
                if Find('-') then
                    repeat
                        //SumQty(RemainingQty,"Inbound Item Entry No.",-Quantity);    // P8000301A
                        SumQty(RemainingQty, "Inbound Item Entry No.", -GetCostingQty); // P8000301A
                    until Next() = 0;
            end;
    end;
    ...
    begin
        ...
        //IF (ItemLedgEntry.Quantity * AppliedQty < 0) OR    // P8000301A
        if (ItemLedgEntry.GetCostingQty * AppliedQty < 0) or // P8000301A
            ...
            if Find('-') then
                repeat
                    if "Partial Revaluation" then
                        ...
                        //SumUnitCost(UnitCost,"Cost Amount (Actual)" + "Cost Amount (Expected)","Item Ledger Entry".Quantity);    // P8000301A
                        SumUnitCost(UnitCost, "Cost Amount (Actual)" + "Cost Amount (Expected)", "Item Ledger Entry".GetCostingQty); // P8000301A
                until Next() = 0;
        end;
    end;
