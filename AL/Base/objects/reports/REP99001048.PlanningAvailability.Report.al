...
{
    // PRW111.00.02
    // P80070577, To-Increase, Gangabhushan, 01 MAR 19
    //   TI-12851 - Planning availability report and Lot-Status codes
    DefaultLayout = RDLC;
    RDLCLayout = './layout/PlanningAvailability.rdlc';
    ...
    dataset
    {
        ...
        dataitem("Purchase Line"; "Purchase Line")
        {
            ...
            begin
                ...
                LotStatusMgmt.SetInboundExclusions(RecItem, LotStatus.FieldNo("Available for Planning"), ExcludePurch, ExcludeSalesRet, ExcludeOutput);
                if not ExcludePurch then
                    // P80070577
                    if Selection then begin
        ...
        dataitem("Transfer Line"; "Transfer Line")
        {
            ...
            trigger OnAfterGetRecord()
            begin
                LotStatusMgmt.QuantityAdjForTransferLine("Transfer Line", LotStatusExclusionFilter); // P80070577
                if Selection then begin
        ...
        dataitem("Prod. Order Line"; "Prod. Order Line")
        {
            ...
            begin
                if not (Status in [Status::Simulated, Status::Finished]) then begin
                    ...
                    // P80070577
                    if RecItem.Get("Prod. Order Line"."Item No.") then;
                    LotStatusMgmt.SetInboundExclusions(RecItem, LotStatus.FieldNo("Available for Planning"), ExcludePurch, ExcludeSalesRet, ExcludeOutput);
                    if not ExcludeOutput then
                        // P80070577
                        if Selection then begin
        ...
        dataitem("Requisition Line"; "Requisition Line")
        {
            ...
            begin
                // P80070577
                if RecItem.Get("Requisition Line"."No.") then;
                LotStatusMgmt.SetInboundExclusions(RecItem, LotStatus.FieldNo("Available for Planning"),
                  ExcludePurch, ExcludeSalesRet, ExcludeOutput);
                if "Requisition Line"."Ref. Order Type" = "Requisition Line"."Ref. Order Type"::Purchase then
                    Exclude := ExcludePurch
                else
                    if "Requisition Line"."Ref. Order Type" = "Requisition Line"."Ref. Order Type"::"Prod. Order" then
            ...
            begin
                ...
                if Item.Get(TempPlanningBuffer."Item No.") then begin
                    ...
                    if TempPlanningBuffer."Item No." = OldItem then
                        ...
                        LotStatusMgmt.AdjustItemFlowFields(Item, LotStatusExclusionFilter, true, false, 0, false, false, false); // P80070577
    ...
    begin
        ...
        LotStatusExclusionFilter := LotStatusMgmt.SetLotStatusExclusionFilter(LotStatus.FieldNo("Available for Planning"));  // P80070577
    end;
    ...
    var
        ...
        ItemInventoryCalc: Decimal;
        LotStatus: Record "Lot Status Code";
        LotStatusExclusionFilter: Text;
