    ...
    dataset
    {
        dataitem(Customer; Customer)
        {
            ...
            begin
                RecordNo := RecordNo + 1;
                Clear(MakeReminder);
                MakeReminder.Set(Customer, CustLedgEntry, ReminderHeaderReq, OverdueEntriesOnly, IncludeEntriesOnHold, ExcludeDeductions, CustLedgEntryLineFeeOn); // P8002753
        ...
        layout
        {
            area(content)
            {
                group(Options)
                {
                    ...
                    field(ExcludeDeductions; ExcludeDeductions)
                    {
                        ApplicationArea = FOODBasic;
                        Caption = 'Exclude Unresolved Reductions';
