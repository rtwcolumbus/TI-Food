report 10077 "Sales Shipment NA" // Version: FOODNA
{
    // PRNA5.00.03
    // P8000624A, VerticalSoft, Jack Reynolds, 01 OCT 08
    //   Add option to display lot information with country of origin
    // 
    // PRNA6.00
    // P8000646, VerticalSoft, Jack Reynolds, 17 DEC 08
    //   Add "Show Lot Information" to request page
    // 
    // PRW16.00.03
    ...
    // 
    // PRNA7.00.01
    // P8001212, Columbus IT, Jack Reynolds, 25 SEP 13
    //   Fix caption and layout issues
    // 
    // PRNA7.10
    ...
    DefaultLayout = RDLC;
    RDLCLayout = './layout/local/SalesShipmentNA.rdlc';
            ...
            dataitem(CopyLoop; "Integer")
            {
                ...
                dataitem(PageLoop; "Integer")
                {
                    ...
                    dataitem(SalesShptLine; "Integer")
                    {
                        ...
                        dataitem(LotInformation; "Integer")
                        {
                            DataItemTableView = SORTING(Number);
                            column(LotNo; TempLotInfo."Lot No.")
                            {
                            }
                            column(CountryName; Country.Name)
                            {
                            }
                            column(LotQuantity; Round(TempLotInfo."Quantity to Use" / TempSalesShipmentLine."Qty. per Unit of Measure", 0.00001))
                            {
                                DecimalPlaces = 0 : 5;
                            }
                            column(LotInfo_HEADER; 'LotInfo')
                            {
                            }
                            column(LotInfo_REC; Format(Number))
                            {
                            }
                            ...
                            begin
                                ...
                                if not Country.Get(TempLotInfo."Country/Region of Origin Code") then
                                    Clear(Country);
                            end;
                            ...
                            begin
                                ...
                                TempLotInfo.Reset;
                                ;
                                SetRange(Number, 1, TempLotInfo.Count);
                            end;
                        }
                                ...
                                if ShowLotInfo then begin
                                    ...
                                    if ItemEntryRelation.FindSet then
                                        repeat
                                            ...
                                            TempLotInfo.SetRange("Lot No.", ItemLedgerEntry."Lot No.");
                                            if not TempLotInfo.FindFirst then begin
                                                LotInfo.Get("No.", "Variant Code", ItemLedgerEntry."Lot No.");
                                                TempLotInfo := LotInfo;
                                                TempLotInfo."Quantity to Use" := 0;
                                                TempLotInfo.Insert;
                                            end;
                                            TempLotInfo."Quantity to Use" -= ItemLedgerEntry.Quantity;
                                            TempLotInfo.Modify;
