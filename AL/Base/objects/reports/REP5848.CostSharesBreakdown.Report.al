report 5848 "Cost Shares Breakdown"
{
    ...
    DefaultLayout = RDLC;
    RDLCLayout = './layout/CostSharesBreakdown.rdlc';
    ...
    begin
        //RemainingQty := FromItemLedgEntry.Quantity;    // P8000466A
        RemainingQty := FromItemLedgEntry.GetCostingQty; // P8000466A
        ...
        repeat
            ...
            if (ToCostShareBuffer."Posting Date" <= EndDate) or (EndDate = 0D) then begin
                if (EndDate = 0D) or (ToItemLedgEntry."Posting Date" <= EndDate) and
                   //(FromCostShareBuffer.Quantity * ItemApplnEntry.Quantity < 0)    // P8000466A
                   (FromCostShareBuffer.Quantity * ItemApplnEntry.GetCostingQty < 0) // P8000466A
                then
                    //AppliedQty := AppliedQty + ItemApplnEntry.Quantity;    // P8000466A
                    AppliedQty := AppliedQty + ItemApplnEntry.GetCostingQty; // P8000466A

                with ToCostShareBuffer do begin
                    if Quantity < 0 then
                        //"New Quantity" := "New Quantity" - ItemApplnEntry.Quantity;    // P8000466A
                        "New Quantity" := "New Quantity" - ItemApplnEntry.GetCostingQty; // P8000466A

                    //CostShare := ItemApplnEntry.Quantity / FromCostShareBuffer.Quantity;    // P8000466A
                    CostShare := ItemApplnEntry.GetCostingQty / FromCostShareBuffer.Quantity; // P8000466A
    ...
    begin
        with TempCostShareBuffer do begin
            ...
            //Quantity := ItemLedgEntry.Quantity;    // P8000466A
            Quantity := ItemLedgEntry.GetCostingQty; // P8000466A
    ...
    begin
        ...
        with CostShareBuffer do begin
            ...
            if OutputQty <> 0 then begin
                // ShareOfCost := ItemLedgEntry.Quantity / OutputQty;   // P8000466A, P8001134
                ShareOfCost := ItemLedgEntry.GetCostingQty / OutputQty; // P8000466A, P8001134
