report 7315 "Calculate Whse. Adjustment"
{
    ...
    //   QuantityOnHandBuffer variable changed in SP1
    // 
    // PRW15.00.02
    // P8000615A, VerticalSoft, Jack Reynolds, 25 JUL 08
    //   Fix problem with descending key
    // 
    // PRW16.00
    // P8000646, VerticalSoft, Jack Reynolds, 02 DEC 08
    //   Additional parameters added to InsertItemJnlLine
    // 
    // PRW16.00.02
    ...
    // 
    // PRW16.00.06
    // P8000997, Columbus IT, Jack Reynolds, 15 NOV 11
    //   Fix problem with sign of quantity on negative adjustments
    // 
    // PRW17.00
    ...
    // 
    // PRW110.0.01
    // P80041998, To-Increase, Dayakar Battini, 26 JUN 17
    ...
    //   Upgrade to 13.00
    //
    // P80096266, To-Increase, Gangabhushan, 06 APR 20
    //   Warehouse Phy. inventory journal process error
    //
    // PRW118.00.05  
    // P800138349, To-Increase, Gangabhushan. 19 JAN 22
    //   CS00201715 | Item Journal not showing Lot number values    
    //
    // PRW118.00.05
    // P800160088, To increase, Gangabhushan, 22 NOV 22
    //   CS00223085 | Catch Weight item Adjustments on containers not allowed
        ...
        dataitem(Item; Item)
        {
            ...
            dataitem("Integer"; "Integer")
            {
                ...
                var
                    ...
                    SNLotNumbersByBin: Query "Lot Numbers by Bin and UOM 2"; // P800-MegaApp
                begin
                        ...
                        if Location.FindSet() then
                            repeat
                                ...
                                while SNLotNumbersByBin.Read() do begin
                                    if (SNLotNumbersByBin.Sum_Qty_Base <> 0) or (SNLotNumbersByBin.Sum_Qty_Alt <> 0) then begin // P800-<MegaApp, P800160088
                                        Init();
                                        ...
                                        "Qty. to Handle (Alt.)" := SNLotNumbersByBin.Sum_Qty_Alt; // P800-MegaApp
                    ...
                    with TempAdjmtBinContentBuffer do begin
                        ...
                        if FindSet() then
                            repeat
                                ...
                                if ("Qty. to Handle (Base)" <> 0) or ("Qty. to Handle (Alt.)" <> 0) then begin // P8000756
                                    if ("Qty. to Handle (Base)" * "Qty. to Handle (Alt.)" < 0) then begin        // P8000756
                                        if ("Qty. to Handle (Alt.)" < 0) then
                                            InsertItemJnlLine(TempAdjmtBinContentBuffer, 0, 0, -"Qty. to Handle (Alt.)", "Base Unit of Measure", 0)  // P8000646A
                                        else
                                            InsertItemJnlLine(TempAdjmtBinContentBuffer, 0, 0, -"Qty. to Handle (Alt.)", "Base Unit of Measure", 1); // P8000646A
                                        "Qty. to Handle (Alt.)" := 0;
                                    end;
                                    if ("Qty. to Handle (Base)" < 0) or ("Qty. to Handle (Alt.)" < 0) then // P8000756
                                        InsertItemJnlLine(TempAdjmtBinContentBuffer, QtyInUOM, -"Qty. to Handle (Base)", -"Qty. to Handle (Alt.)", "Base Unit of Measure", 0)   // P8000646A, P80096266
                                    else
                                        InsertItemJnlLine(TempAdjmtBinContentBuffer, QtyInUOM, -"Qty. to Handle (Base)", -"Qty. to Handle (Alt.)", "Base Unit of Measure", 1);  // P8000646A, P80096266
                                end;
                                // P8000591A
                ...
                begin
                    ...
                    Item.CopyFilter("Location Filter", WhseEntry."Location Code");
                    // P8000591A
    ...
    local procedure InsertItemJnlLine(var TempBinContentBuffer: Record "Bin Content Buffer" temporary; Quantity2: Decimal; QuantityBase2: Decimal; QuantityAlt2: Decimal; UOM2: Code[10]; EntryType2: Option "Negative Adjmt.","Positive Adjmt.")
    var
        IsHandled: Boolean;
    begin
        ...
        with ItemJnlLine do begin
            ...
            // if QuantityBase2 <> 0 then begin // P8000591A
            Init;
            "Line No." := NextLineNo;
            Validate("Posting Date", PostingDate);
            if (QuantityBase2 > 0) or (QuantityAlt2 > 0) then // P8000591A
                Validate("Entry Type", "Entry Type"::"Positive Adjmt.")
            else begin
                Validate("Entry Type", "Entry Type"::"Negative Adjmt.");
                Quantity2 := -Quantity2;
                QuantityBase2 := -QuantityBase2;
                QuantityAlt2 := -QuantityAlt2; // P8000591A
            end;
            ...
            if (QuantityAlt2 <> 0) then                    // P8000591A
                Validate("Quantity (Alt.)", QuantityAlt2); // P8000591A
            ...
            CreateReservationEntry2(ItemJnlLine, TempBinContentBuffer, EntryType2, UOM2, Quantity2, QuantityBase2, QuantityAlt2);
            // P800-MegaApp
            // P800138349
            ItemJnlLine.GetLotNo();
    ...
    local procedure CreateReservationEntry2(var ItemJournalLine: Record "Item Journal Line"; var TempBinContentBuffer: Record "Bin Content Buffer" temporary; EntryType: Option "Negative Adjmt.","Positive Adjmt."; UOMCode: Code[10]; Quantity: Decimal; QuantityBase: Decimal; QuantityAlt: Decimal)
    var
        Location: Record Location;
        WarehouseEntry: Record "Warehouse Entry";
        CreateReservEntry: Codeunit "Create Reserv. Entry";
    ...
    begin
        if TempBinContentBuffer."Location Code" = '' then
            exit;

        Location.Get(TempBinContentBuffer."Location Code");
        if not Location."Directed Put-away and Pick" then
            exit;

        // P8000591A, P800-MegaApp
        if ItemJournalLine."Order Type" = ItemJournalLine."Order Type"::Production then
            OrderLineNo := ItemJournalLine."Order Line No.";
        CreateReservEntry.CreateReservEntryFor(
          DATABASE::"Item Journal Line", ItemJournalLine."Entry Type", ItemJournalLine."Journal Template Name",
          ItemJournalLine."Journal Batch Name", OrderLineNo, ItemJournalLine."Line No.", ItemJournalLine."Qty. per Unit of Measure",
          Abs(Quantity), Abs(QuantityBase), TempBinContentBuffer."Serial No.", TempBinContentBuffer."Lot No.");
        if (QuantityAlt <> 0) then
            CreateReservEntry.AddAltQtyData(ItemJournalLine.Signed(QuantityAlt));
        // P80041998
        if TempBinContentBuffer."Lot No." <> '' then
            if LotNoInformation.Get(ItemJournalLine."Item No.", ItemJournalLine."Variant Code", TempBinContentBuffer."Lot No.") then
                CreateReservEntry.SetDates(0D, LotNoInformation."Expiration Date")
            else begin
                WarehouseEntry.SetCurrentKey(
                "Item No.", "Bin Code", "Location Code", "Variant Code", "Unit of Measure Code", "Lot No.", "Serial No.", "Entry Type");
                ...
                WarehouseEntry.SetRange("Unit of Measure Code", UOMCode);
                WarehouseEntry.SetFilter("Lot No.", TempBinContentBuffer."Lot No.");
                WarehouseEntry.SetFilter("Serial No.", TempBinContentBuffer."Serial No.");
                WarehouseEntry.SetFilter("Entry Type", '%1|%2', EntryType, WarehouseEntry."Entry Type"::Movement);
        ...
        if Item.TrackAlternateUnits() then
            CreateAltQtyLine(ItemJnlLine, TempBinContentBuffer."Lot No.", TempBinContentBuffer."Serial No.", Quantity, QuantityAlt);

        // P800-MegaApp
        // ItemJournalLine.GetLotNo;
        // ItemJournalLine.Modify(true);
        // P800-MegaApp
        // P8000591A            
    end;

    local procedure CreateAltQtyLine(var ItemJnlLine: Record "Item Journal Line"; LotNo: Code[50]; SerialNo: Code[50]; QtyBase: Decimal; QtyAlt: Decimal)
    var
    ...
    begin
        ...
        with ItemJnlLine do begin
            ...
            if AltQtyLine.Find('+') then
                AltQtyLine."Line No." := AltQtyLine."Line No." + 10000
            else
            ...
            AltQtyLine.Validate("Quantity (Base)", Abs(QtyBase));
            AltQtyLine.Validate("Quantity (Alt.)", Abs(QtyAlt));
            AltQtyLine.Insert;
        end;
    end;

    local procedure AdjustAltQty()
    var
    ...
    begin
        ...
        if Item.TrackAlternateUnits() then
            with TempAdjmtBinContentBuffer do begin
                ...
                if (ItemLedgEntry.Quantity = "Qty. to Handle (Base)") or // P8000756
                   (ItemLedgEntry."Quantity (Alt.)" < "Qty. to Handle (Alt.)")
                then
                    "Qty. to Handle (Alt.)" := ItemLedgEntry."Quantity (Alt.)"
                //ELSE
                //  IF ("Qty. to Handle (Base)" = 0) THEN // P8000756
                //    "Qty. to Handle (Alt.)" := 0;
            end;
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnAfterGetRecordItemOnAfterSNLotNumbersByBinSetFilters(var SNLotNumbersByBin: Query "Lot Numbers by Bin and UOM 2"; Item: Record Item)
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeAdjmtBinQuantityBufferInsert(var BinContentBuffer: Record "Bin Content Buffer"; WarehouseEntry: Record "Warehouse Entry"; var SNLotNumbersByBin: Query "Lot Numbers by Bin and UOM 2")
    begin
    end;
