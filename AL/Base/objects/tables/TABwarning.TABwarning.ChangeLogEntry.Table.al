...
{
    // PRW18.00.01
    // P8001388, Columbus IT, Jack Reynolds, 11 JUN 15
    //   Add additional field for large primary keys
    // 
    // PRW114.00
    ...
    procedure GetPrimaryKeyFriendlyName() FriendlyName: Text[250]
    begin
        // P80096141 - Original signature
        FriendlyName := GetPrimaryKeyFriendlyName2();
        if 250 < StrLen(FriendlyName) then
            FriendlyName := CopyStr(FriendlyName, 1, 250);
    end;

    [Obsolete('Replaced by GetFullPrimaryKeyFriendlyName procedure.', '18.0')]
    procedure GetPrimaryKeyFriendlyName2(): Text[500]
    var
        RecRef: RecordRef;
