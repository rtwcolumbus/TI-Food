table 99000853 "Inventory Profile"
{
    ...
    fields
    {
        ...
        field(37002001; "Lot Creation Date"; Date)
        {
    ...
    keys
    {
        ...
        key(Key2; "Item No.", "Variant Code", "Location Code", "Pre-Process", "Due Date", "Attribute Priority", "Order Priority")
        {
        }
    ...
    begin
        ...
        "Lot Creation Date" := ReservEntry."Lot Creation Date"; // P80076591
    ...
    begin
        ...
        // P8001082
        if ProdOrderComp."Pre-Process Type Code" <> '' then begin
            PreProcessType.Get(ProdOrderComp."Pre-Process Type Code");
            "Pre-Process" := PreProcessType.Blending in [PreProcessType.Blending::"Per Order", PreProcessType.Blending::"Per Item"];
            "Due Date" := "Due Date" - ProdOrderComp."Pre-Process Lead Time (Days)";
            if "Pre-Process" then begin
                ProdOrderComp.CalcFields("Pre-Process Quantity", "Qty. Pre-Processed");
                ProdOrderComp."Pre-Process Quantity" += ProdOrderComp."Qty. Pre-Processed" - ProdOrderComp."Act. Consumption (Qty)";
                ProdOrderComp."Reserved Qty. (Base)" +=
                  Round(ProdOrderComp."Pre-Process Quantity" * ProdOrderComp."Qty. per Unit of Measure", 0.00001);
            end;
        end;
        // P8001082
        ProdOrderComp.SetReservationFilters(ReservEntry);
        if not "Pre-Process" then // P8001082
            AutoReservedQty := -TransferBindings(ReservEntry, TrackingReservEntry);
    ...
    begin
        ...
        // P8001082
        if PlanningComponent."Pre-Process Type Code" <> '' then begin
            PreProcessType.Get(PlanningComponent."Pre-Process Type Code");
            "Pre-Process" := PreProcessType.Blending in [PreProcessType.Blending::"Per Order", PreProcessType.Blending::"Per Item"];
            "Due Date" := "Due Date" - PlanningComponent."Pre-Process Lead Time (Days)";
        end;
        // P8001082
    ...
    begin
        ...
        // P8001082
        PreProcessActivity.SetCurrentKey("Blending Order Status", "Blending Order No.");
        PreProcessActivity.SetRange("Blending Order Status", ProdOrderLine.Status);
        PreProcessActivity.SetRange("Blending Order No.", ProdOrderLine."Prod. Order No.");
        PreProcessActivity.CalcSums("Remaining Quantity");

        "Pre-Process" := PreProcessActivity."Remaining Quantity" > 0;
        if "Pre-Process" then
            ProdOrderLine."Reserved Qty. (Base)" +=
              Round(PreProcessActivity."Remaining Quantity" * ProdOrderLine."Qty. per Unit of Measure", 0.00001);
        // P8001082
    ...
    begin
        ...
        TransferFromOutboundTransfer(TransLine, TrackingEntry, '');
    end;

    procedure TransferFromOutboundTransfer(var TransLine: Record "Transfer Line"; var TrackingEntry: Record "Reservation Entry"; ExclusionFilter: Text[1024])
    var
        ReservEntry: Record "Reservation Entry";
    ...
    begin
        ...
        LotStatusMgmt.QuantityAdjForTransferLine(TransLine, ExclusionFilter); // P8001083
    ...
    begin
        // P80096141 - Original signature
        TransferFromInboundTransfer(TransLine, TrackingReservEntry, '');
    end;

    procedure TransferFromInboundTransfer(var TransLine: Record "Transfer Line"; var TrackingReservEntry: Record "Reservation Entry"; ExclusionFilter: Text[1024])
    var
        ReservEntry: Record "Reservation Entry";
        TempReservationEntry: Record "Reservation Entry" temporary;
        CrntInvProfile: Record "Inventory Profile";
    ...
    begin
        ...
        if ReservEntry.FindSet() then
            repeat
                // P8001083
                if not LotStatusMgmt.ExcludeLot(ReservEntry."Item No.", ReservEntry."Variant Code", ReservEntry."Lot No.",
    ...
    begin
        ...
        TrkgReservEntry."Lot Creation Date" := "Lot Creation Date"; // P80076591
