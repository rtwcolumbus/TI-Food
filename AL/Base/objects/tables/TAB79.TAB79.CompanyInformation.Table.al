...
{
    // PR3.60
    //   Rename MSDS State and ZIP Code to MSDS County and MSDS Post Code and use caption property
    //   for state and ZIP
    // 
    // PR3.70
    //   Modify Validation and Lookup logic for MSDS City and MSDS Post Code
    // 
    // PRW16.00
    // P8000640, VerticalSoft, Jack Reynolds, 17 NOV 08
    //   Add extended datatype property to MSDS fields
    // 
    // P8000641, VerticalSoft, Jack Reynolds, 17 NOV 08
    //   Increase MSDS phone number fields to Text30
    // 
    // PRW17.00
    ...
    fields
    {
        field(37002000; "GS1 Company Prefix"; Code[12])
        {
            ...
            begin
                // P80055555
                if "GS1 Company Prefix" = '' then begin
                    if Process800Fns.ContainerTrackingInstalled then
                        ContainerHeader.ModifyAll(SSCC, '');
                    exit;
                end;

                if StrLen("GS1 Company Prefix") < 4 then
                    Error(Text37002000, FieldCaption("GS1 Company Prefix"));

                if "GS1 Company Prefix" <> CopyStr(GLN, 1, StrLen("GS1 Company Prefix")) then
                    GLN := '';

                if ("GS1 Company Prefix" <> xRec."GS1 Company Prefix") and Process800Fns.ContainerTrackingInstalled then
                    if ContainerHeader.FindSet(true) then begin
                        InventorySetup.Get;
                        repeat
                            ContainerHeader.SSCC := ContainerFunctions.CreateSSCC("GS1 Company Prefix", InventorySetup."SSCC Extension Digit", ContainerHeader."Serial Reference");
    ...
    var
        ...
        Text37002000: Label '%1 must contain at least 4 digits.';
