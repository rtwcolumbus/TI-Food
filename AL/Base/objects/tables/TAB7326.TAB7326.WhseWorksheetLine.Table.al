table 7326 "Whse. Worksheet Line"
{
    ...
    //     Added function 'ProcessOverShip', called from 'Qty. to Handle - OnValidate()'
    // 
    // PR5.00
    ...
    //   Change calculation of Qty. to Handle (Base)
    // 
    // PRW15.00.03
    ...
    fields
    {
        ...
        field(37002100; "Lot No."; Code[50])
        {
            Caption = 'Lot No.';

            trigger OnValidate()
            begin
                ValidateLotNo; // P8000322A
            end;
        }
    }
    ...
    trigger OnInsert()
    begin
        SaveLotNo(true); // P8000322A
    end;

    trigger OnModify()
    begin
        SaveLotNo(false); // P8000322A
    end;
    ...
    begin
        ...
        case "Whse. Document Type" of
            ...
            "Whse. Document Type"::FOODStagedPick:
                WhseItemTrackingForm.SetSource(Rec, DATABASE::"Whse. Staged Pick Line");
            // P8000322A
            else
    ...
    begin
        ...
        case "Whse. Document Type" of
            ...
            // P8000322A
            "Whse. Document Type"::FOODStagedPick:
                WhseItemTrackingLines.SetSource(Rec, DATABASE::"Whse. Staged Pick Line");
            // P8000322A
            else
    ...
    procedure SaveLotNo(Inserting: Boolean)
    var
        WhseItemTrkgLine: Record "Whse. Item Tracking Line";
        WarehouseEntry: Record "Warehouse Entry";
    begin
        // P8000322A
        if Inserting or ("Lot No." <> xRec."Lot No.") or
           ("Qty. (Base)" <> xRec."Qty. (Base)") or
           ("Qty. to Handle (Base)" <> xRec."Qty. to Handle (Base)")
        then begin
            if not Inserting then
                ItemTrackingMgt.DeleteWhseItemTrkgLines(
                  DATABASE::"Whse. Worksheet Line", 0, Name, "Worksheet Template Name",
                  0, "Line No.", "Location Code", true);
            if ("Lot No." <> '') then begin
                WarehouseEntry."Lot No." := "Lot No."; // P800-MegaApp
                SetItemTrackingLines(WarehouseEntry, "Qty. (Base)"); // P8000466A, P800-MegaApp
                if ("Qty. to Handle (Base)" <> "Qty. (Base)") then begin
                    WhseItemTrkgLine.SetCurrentKey(
                      "Source ID", "Source Type", "Source Subtype", "Source Batch Name",
                      "Source Prod. Order Line", "Source Ref. No.", "Location Code");
                    WhseItemTrkgLine.SetRange("Source Type", DATABASE::"Whse. Worksheet Line");
                    WhseItemTrkgLine.SetRange("Source Subtype", 0);
                    WhseItemTrkgLine.SetRange("Source ID", Name);
                    WhseItemTrkgLine.SetRange("Source Batch Name", "Worksheet Template Name");
                    WhseItemTrkgLine.SetRange("Source Prod. Order Line", 0);
                    WhseItemTrkgLine.SetRange("Source Ref. No.", "Line No.");
                    WhseItemTrkgLine.SetRange("Location Code", "Location Code");
                    if WhseItemTrkgLine.Find('-') then begin
                        WhseItemTrkgLine.Validate("Qty. to Handle (Base)", "Qty. to Handle (Base)");
    ...
    begin
        ...
        if SalesLine.Get("Source Subtype", "Source No.", "Source Line No.") then begin
            ...
            SalesLine.Validate(Quantity, Quantity + "Qty. to Handle" - SalesLine."Outstanding Quantity");
            ...
            SalesHeader.Modify;
            //Shipment
            if ShipmentHeader.Get("Whse. Document No.") then begin
                ...
                ShipmentLine.SetRange("Source Subtype", 1);
