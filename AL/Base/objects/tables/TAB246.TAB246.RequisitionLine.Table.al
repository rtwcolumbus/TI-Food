...
{
    // PRW16.00.01
    // P8000686, VerticalSoft, Jack Reynolds, 09 APR 09
    //   Fix problem with bin codes and 1-document locations
    // 
    // PRW16.00.06
    // P8001030, Columbus IT, Jack Reynolds, 30 MAY 12
    //   Support for BOM/Routing on SKU
    // 
    // P8001092, Columbus IT, Don Bresee, 31 AUG 12
    //   Add logic to require produced items from Co-Product BOMs to be the Primary Co-Product
    // 
    // P8001097, Columbus IT, Jack Reynolds, 27 SEP 12
    //   Fix incorrect bin for priduciton orders
    // 
    ...
    //   Rollup 4 changes to default bin code
    // 
    // PRW110.0
    ...
    // 
    // PRW111.00.03
    // P80084216, To-Increase, Gangabhushan, 16 OCT 19
    //   CS00077710 - In Planning Worksheet the equipment is set to default when dates are changed
    ...
    fields
    {
                ...
                if (Type = Type::Item) and ("No." <> '') and ("Prod. Order No." = '') then begin
                    ...
                    if "Vendor No." <> '' then
                        if Item.VendorApprovalRequired then begin
                            ItemVend.Get("Vendor No.", "No.", "Variant Code");
                            ItemVend.TestField(Approved, true);
                        end
                        else
                            // P8001230
            ...
            begin
                ...
                if Type = Type::Item then begin
                    ...
                    if "Location Code" <> xRec."Location Code" then begin
                        ...
                        // end;
                        SetDefaultBin(IsDropShipment, FieldNo("Location Code")); // P8007748, P800-MegaApp
                        // P8000686
                        // P8001030
                        GetItem;
                        "Indirect Cost %" := Item.IndirectCostPct("Variant Code", "Location Code");
                        "Overhead Rate" :=
                          Round(Item.OverheadRate("Variant Code", "Location Code") * "Qty. per Unit of Measure", 0.00001);
                        // P8001030
                    end;
            ...
            begin
                ...
                if "Variant Code" <> '' then begin
                    ...
                    if "Variant Code" <> xRec."Variant Code" then begin
                        "Bin Code" := '';
                        // P8000686
                        SetDefaultBin(false, FieldNo("Variant Code")); // P800-MegaApp
        ...
        field(37002000; "Pre-Process"; Boolean)
        {
            Caption = 'Pre-Process';
        }
        field(37002001; "Equipment Code"; Code[20])
        {
            ...
            begin
                ...
                if "Equipment Code" <> '' then begin
                    if "Location Code" <> '' then begin
                        ...
                        Resource.TestField("Location Code", "Location Code");
                    end;
                    ProdBOMEquipment.Get("Production BOM No.", "Production BOM Version Code", "Equipment Code");
                    Validate("Routing No.", ProdBOMEquipment."Routing No.");
                end;
                if "Routing No." = '' then begin
                    Item.Get("No.");
                    Validate("Routing No.", P800BOMFns.GetRoutingAtSKU(Item, "Variant Code", "Location Code"));
                end;
            end;
        }
        ...
        field(99000885; "Production BOM Version Code"; Code[20])
        {
            ...
            var
                ...
                xProdBOMEquipment: Record "Prod. BOM Equipment";
            begin
                CheckActionMessageNew();
                ...
                // P8001283
                if P800Functions.ProcessInstalled and (xRec."Production BOM Version Code" <> "Production BOM Version Code") then begin // P80084216
                    P800BOMFns.GetPreferredEquipment("Production BOM No.", "Production BOM Version Code", "Location Code", ProdBOMEquipment);
                    // P80084216
                    P800BOMFns.GetPreferredEquipment("Production BOM No.", xRec."Production BOM Version Code", "Location Code", xProdBOMEquipment);
                    if xProdBOMEquipment."Resource No." <> ProdBOMEquipment."Resource No." then
                        // P80084216
                        Validate("Equipment Code", ProdBOMEquipment."Resource No.");
                end;
                // P8001283
        ...
        field(99000898; "Production BOM No."; Code[20])
        {
            ...
            begin
                ...
                if xRec."Production BOM No." <> "Production BOM No." then // P80084216
                    "Equipment Code" := ''; // P8001283
                ...
                // P8001092
                ProdBOMHeader.Get("Production BOM No."); // P8001132
                if (ProdBOMHeader."Mfg. BOM Type" = ProdBOMHeader."Mfg. BOM Type"::Process) then
                    case ProdBOMHeader."Output Type" of
                        ProdBOMHeader."Output Type"::Item:
                            if (ProdBOMHeader."Output Item No." <> "No.") then begin
                                if PlanningResiliency then
                                    TempPlanningErrorLog.SetError(
                                      StrSubstNo(
                                        Text37002001,
                                        Type, "No.", ProdBOMHeader.FieldCaption("Output Item No."),
                                        FieldCaption("Production BOM No."), "Production BOM No."),
                        ...
                        ProdBOMHeader."Output Type"::Family:
                            if not ProdBOMHeader.IsPrimaryCoProduct("No.") then begin
                                if PlanningResiliency then
                                    TempPlanningErrorLog.SetError(
                                      StrSubstNo(
                                        Text37002001,
                                        Type, "No.", FamilyLine.FieldCaption("Primary Co-Product"),
                                        FieldCaption("Production BOM No."), "Production BOM No."),
                                      DATABASE::"Production BOM Header", ProdBOMHeader.GetPosition);
                                Error(Text37002001,
                                  Type, "No.", FamilyLine.FieldCaption("Primary Co-Product"),
...
#pragma warning restore AL0603
        ...
        field(99000917; "Accept Action Message"; Boolean)
        {
            ...
            trigger OnValidate()
            begin
                if "Accept Action Message" then   // P8001082
                    TestField("Pre-Process", false); // P8001082
    ...
    var
        ...
        Text37002000: Label '%1 %2 must have %3 %4 as an output.';
        Text37002001: Label '%1 %2 must be the %3 on %4 %5.';
    ...
    begin
        ...
        if (Type = Type::Item) and ItemVend.ReadPermission then begin
            ...
            // P8001230
            GetItem;
            if Item.VendorApprovalRequired then
                ItemVend.SetRange(Approved, true);
            // P8001230
    ...
    procedure SetDefaultBin(DropShipment: Boolean; FldNo: Integer)
    var
        ShouldGetDefaultBin: Boolean;
    begin
        // P8000686
        // P8007748 - add parameter for Dropshipment
        if ("Location Code" <> '') and ("No." <> '') and not DropShipment then begin // P8007748
            if ("Bin Code" = '') and ("Ref. Order Type" = "Ref. Order Type"::"Prod. Order") then                                     // P8001301
                "Bin Code" := WMSManagement.GetLastOperationFromBinCode("Routing No.", "Routing Version Code", "Location Code", false, 0); // P8001301
            GetLocation("Location Code");
            if ("Bin Code" = '') and ("Ref. Order Type" = "Ref. Order Type"::"Prod. Order") then                                     // P8001301
                "Bin Code" := Location."From-Production Bin Code";                                                                     // P8001301  // P80050544
            if ("Bin Code" = '') and Location."Bin Mandatory" and not Location."Directed Put-away and Pick" then                     // P8001301
                if "Replenishment System" in ["Replenishment System"::Purchase, "Replenishment System"::Transfer] then // P8001097
                    ...
                    begin
                        ...
                        if FldNo = FieldNo("Location Code") then
                            OnValidateLocationCodeOnBeforeGetDefaultBin(Rec, ShouldGetDefaultBin, Location, CurrFieldNo)
                        else
                            OnBeforeGetDefaultBin(Rec, ShouldGetDefaultBin);
                        if ShouldGetDefaultBin then
