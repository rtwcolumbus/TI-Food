table 93 "Vendor Posting Group"
{
    ...
    fields
    {
        ...
        field(37002685; "Hauler Charge Account"; Code[20])
        {
            Caption = 'Hauler Charge Account';
            TableRelation = "G/L Account";

            trigger OnLookup()
            begin
                // P80053245
                GLAccountCategoryMgt.LookupGLAccount("Hauler Charge Account", GLAccountCategory."Account Category"::Expense, '');
            end;

            trigger OnValidate()
            begin
                // P80053245
                GLAccountCategoryMgt.CheckGLAccount("Hauler Charge Account", false, false, GLAccountCategory."Account Category"::Expense, '');
            end;
        }
    }
    ...
    procedure GetHaulerChargeAccount(): Code[20]
    begin
        // P80053245
        if "Hauler Charge Account" = '' then
            PostingSetupMgt.SendVendPostingGroupNotification(Rec, FieldCaption("Hauler Charge Account"));
        TestField("Hauler Charge Account");
        exit("Hauler Charge Account");
    end;
}
