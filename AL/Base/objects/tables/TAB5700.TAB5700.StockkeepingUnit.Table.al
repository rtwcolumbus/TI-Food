table 5700 "Stockkeeping Unit"
{
    ...
    //   Fix problem updating standard cost
    // 
    // PRW17.10
    ...
    fields
    {
        ...
        field(37002460; "Routing No."; Code[20])
        {
            Caption = 'Routing No.';
            ...
            begin
                // P8001030
                PlanningAssignment.SKURoutingReplace(Rec, xRec."Routing No.");

                // P8001196
                if "Routing No." <> xRec."Routing No." then begin
                    Item.Get("Item No.");
                    ItemCostMgt.UpdateUnitCostSKU(Item, Rec, 0, 0, true, true, 0, FieldNo("Routing No."));
                    if "Routing No." <> '' then
        ...
        field(37002461; "Production BOM No."; Code[20])
        {
            ...
            begin
                // P8001030
                PlanningAssignment.SKUBOMReplace(Rec, xRec."Production BOM No.");

                if "Production BOM No." <> xRec."Production BOM No." then
                    Item.Get("Item No.");                                                                  // P8001196
                ItemCostMgt.UpdateUnitCostSKU(Item, Rec, 0, 0, true, true, 0, FieldNo("Production BOM No.")); // P8001196
                if "Production BOM No." <> '' then begin
                    ...
                    if ProdBOMLine.Find('-') then
                        Error(Text37002000, FieldCaption("Production BOM No."), "Production BOM No.");
                    ...
                    ProdBOMHeader2.Get(Item."Production BOM No.");
                    ProdBOMHeader.TestField("Mfg. BOM Type", ProdBOMHeader2."Mfg. BOM Type");
                    ...
                    if ProdBOMHeader."Mfg. BOM Type" = ProdBOMHeader."Mfg. BOM Type"::Process then
                        case ProdBOMHeader."Output Type" of
                            ProdBOMHeader."Output Type"::Item:
                                begin // P8001092
                                    if ProdBOMHeader."Output Item No." <> "Item No." then
                                        Error(Text37002001, "Item No.", ProdBOMHeader.FieldCaption("Output Item No."));
                                    ProdBOMHeader.TestField("Output Variant Code", "Variant Code"); // P8001092
                                end; // P8001092
                            ProdBOMHeader."Output Type"::Family:
                                begin
                                    FamilyLine.SetRange("Family No.", ProdBOMHeader."No.");
                                    FamilyLine.SetRange("Item No.", "Item No.");
                                    FamilyLine.SetFilter("Variant Code", '%1', "Variant Code"); // P8001092
                                    FamilyLine.SetRange("By-Product", false);
                                    if not FamilyLine.Find('-') then
                                        Error(Text37002002, "Item No.");
                                end;
                        end;
                    ...
                    if "Routing No." = '' then
                        if ProcessFns.ProcessInstalled then begin
                            P800BOMFns.GetPreferredEquipment("Production BOM No.",
                              VersionMgt.GetBOMVersion("Production BOM No.", Today, true), "Location Code", ProdBOMEquipment);
        ...
        field(37002462; "Indirect Cost %"; Decimal)
        {
            Caption = 'Indirect Cost %';
            DecimalPlaces = 0 : 5;
            MinValue = 0;
        }
        field(37002463; "Overhead Rate"; Decimal)
        {
            AutoFormatType = 2;
            Caption = 'Overhead Rate';
        }
        field(37002470; "Single-Level Material Cost"; Decimal)
        {
            AutoFormatType = 2;
            Caption = 'Single-Level Material Cost';
            Editable = false;
        }
        field(37002471; "Single-Level Capacity Cost"; Decimal)
        {
            AutoFormatType = 2;
            Caption = 'Single-Level Capacity Cost';
            Editable = false;
        }
        field(37002472; "Single-Level Subcontrd. Cost"; Decimal)
        {
            AutoFormatType = 2;
            Caption = 'Single-Level Subcontrd. Cost';
            Editable = false;
        }
        field(37002473; "Single-Level Cap. Ovhd Cost"; Decimal)
        {
            AutoFormatType = 2;
            Caption = 'Single-Level Cap. Ovhd Cost';
            Editable = false;
        }
        field(37002474; "Single-Level Mfg. Ovhd Cost"; Decimal)
        {
            AutoFormatType = 2;
            Caption = 'Single-Level Mfg. Ovhd Cost';
            Editable = false;
        }
        field(37002475; "Rolled-up Material Cost"; Decimal)
        {
            AutoFormatType = 2;
        ...
        field(37002476; "Rolled-up Capacity Cost"; Decimal)
        {
            AutoFormatType = 2;
        ...
        field(37002477; "Rolled-up Subcontracted Cost"; Decimal)
        {
            AutoFormatType = 2;
            Caption = 'Rolled-up Subcontracted Cost';
            Editable = false;
        }
        field(37002478; "Rolled-up Cap. Overhead Cost"; Decimal)
        {
            AutoFormatType = 2;
            Caption = 'Rolled-up Cap. Overhead Cost';
            Editable = false;
        }
        field(37002479; "Rolled-up Mfg. Ovhd Cost"; Decimal)
        {
            AutoFormatType = 2;
            Caption = 'Rolled-up Mfg. Ovhd Cost';
            Editable = false;
        }
        field(37002480; "Last Unit Cost Calc. Date"; Date)
        {
        ...
        field(24; "Standard Cost"; Decimal)
        {
            ...
            begin
                ...
                ItemCostMgt.UpdateUnitCostSKU(Item, Rec, 0, 0, true, true, 0, FieldNo("Standard Cost")); // P8001030
            end;
        }
    ...
    var
        ...
        Text37002000: Label '%1 %2 contains variables.';
        Text37002001: Label '%1 must be the %2 for item process.';
        Text37002002: Label '%1 must be an Output Item for item process.';
        Text37002003: Label '%1 %2 must be the %3 on %4 %5 to have a %6 of %7.';
        Text37002004: Label '%1 %2, %3 %4, %5 %6  is not approved for %7 %8.';
#if not CLEAN21
    ...
    begin
        ...
        if "Production BOM No." <> '' then begin
            ...
            exit(Format(BOMHeader."Mfg. BOM Type"));
        end;
    end;
    ...
    begin
        if ("Location Code" = '') and ("Variant Code" = '') then begin
            GetSKUItem;
            exit(Item.IsAssemblyItem);
        end else
            exit("Replenishment System" = "Replenishment System"::Assembly);
    end;

    procedure IsTransItem(): Boolean
    begin
        // P801030
        if ("Location Code" = '') and ("Variant Code" = '') then
            exit(false)
        else
            exit(("Replenishment System" = "Replenishment System"::Transfer) and ("Transfer-from Code" <> ''));
    end;
    ...
    begin
        ...
        if Item.CostInAlternateUnits() then begin
            QtyPerBase := Item.CostingQtyPerBase();
    ...
    begin
        // P8001230
        GetSKUItem;
        exit(Item.VendorApprovalRequired);
    end;

    procedure VendorApproved(): Boolean
    var
    ...
    begin
        ...
        if not VendorApproved then
            Error(Text37002004, FieldCaption("Item No."), "Item No.", FieldCaption("Location Code"), "Location Code",
              FieldCaption("Variant Code"), "Variant Code", FieldCaption("Vendor No."), "Vendor No.");
    end;
    ...
    begin
        ...
        // P80053245
        if not Item.VendorApprovalRequired then
            SetVendor := true
        else
            if ItemVendor.Get(Item."Vendor No.", "Item No.", "Variant Code") then
                SetVendor := ItemVendor.Approved;
        if SetVendor then begin
            // P80053245
