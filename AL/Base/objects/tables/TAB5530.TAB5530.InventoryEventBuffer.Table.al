table 5530 "Inventory Event Buffer"
{
    ...
    begin
        ...
        "Reserved Quantity (Base)" := -SalesLine."Reserved Qty. (Base)";
        Positive := not ("Remaining Quantity (Base)" < 0);
        if ExcludeSalesRet then             // P8001144
            "Remaining Quantity (Base)" := 0; // P8001144
        "Derived from Blanket Order" := SalesLine."Blanket Order No." <> '';
        if "Derived from Blanket Order" then begin
            "Ref. Order No." := SalesLine."Blanket Order No.";
            "Ref. Order Line No." := SalesLine."Blanket Order Line No.";
    ...
    begin
        ...
        LotStatusMgmt.QuantityAdjForItemLedger(ItemLedgEntry, LotStatusExclusionFilter, "Remaining Quantity (Base)");
        // P8001144
    ...
    begin
        ...
        Positive := not ("Remaining Quantity (Base)" < 0);
        if ExcludeOutput then               // P8001144
            "Remaining Quantity (Base)" := 0; // P8001144
    ...
    begin
        ...
        LotStatusMgmt.QuantityAdjForTransferLine(TransLine, LotStatusExclusionFilter);
        "Remaining Quantity (Base)" := TransLine."Outstanding Qty. (Base)" + TransLine."Qty. in Transit (Base)";
        // P8001144
