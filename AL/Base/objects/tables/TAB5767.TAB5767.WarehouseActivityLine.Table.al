table 5767 "Warehouse Activity Line"
{
    ...
    //   P800 WMS Project - alternate quantities
    // 
    // PR4.00.04
    // P8000322A, VerticalSoft, Don Bresee, 08 AUG 06
    //   Hidden Document Handling
    //   Check for blocked lots on pick for sales
    //   Support for Staged Picks
    // 
    // P8000358A, VerticalSoft, Phyllis McGovern, 26 JUL 06
    //   Added field 'ADC Activity' and processing to accomodate
    // 
    ...
    //     Added local function 'OverShipUpdatePlaceRec'
    //     Added local function 'OverShipUpdateShipment'
    // 
    // PR5.00
    ...
    // 
    // PRW15.00.02
    // P8000622A, VerticalSoft, Don Bresee, 06 AUG 08
    //   Add key that was missed in WMS integration (P8000322A)
    // 
    // PRW15.00.03
    // P8000630A, VerticalSoft, Don Bresee, 17 SEP 08
    //   Add Delivery Trip Pick to Whse. Document Type
    //   Add logic for picks/put-aways related to delivery trips
    // 
    // P8000629A, VerticalSoft, Jack Reynolds, 21 SEP 08
    //   Set Alternate quantity to handle when quantity to handle is entered
    // 
    // PRW16.00.02
    ...
    // 
    // PRW110.0.01
    // P8008657, To-Increase, Dayakar Battini, 20 APR 17
    //   Fix issue with Replenishment Area bin code checking on place line
    // 
    // PRW110.0.02
    ...
    //
    // PRW121.4
    // P800164877, ToIncrease, Gangabhushan, 07 MAR 21
    //    Upgrade - Redesigned by adding our condition to MS
    ...
    fields
    {
        ...
        field(14; "Item No."; Code[20])
        {
            ...
            begin
                ...
                if "Item No." <> '' then begin
                    ...
                    "Pick Class Code" := Item."Pick Class Code";  // P80038975
                end else begin
                    Description := '';
                    "Description 2" := '';
                    "Variant Code" := '';
                    Validate("Unit of Measure Code", '');
                    "Pick Class Code" := '';  // P80038975
                end;
            end;
        }
        ...
        field(26; "Qty. to Handle"; Decimal)
        {
            ...
            begin
                ...
                if "Container ID" <> '' then
                    ContainerFns.ValidatePickQtyToHandleForProductionContainer(Rec, not SuppressProdContUpdate);
                // P80056710

                HandleOverPick; //P8000372A

                // IF "Qty. to Handle" > "Qty. Outstanding" THEN                                                        // P8001039
                if ("Qty. to Handle" > ("Qty. Outstanding" + RoundingAdjmtMgmt.GetNearZeroQtyForItem("Item No."))) then // P8001039
                    Error(
                      Text002,
                      "Qty. Outstanding");
                IsHandled := false;
                ...
                if not IsHandled then begin
                    ...
                    if (QuantityRounded > ("Qty. Outstanding" + RoundingAdjmtMgmt.GetNearZeroQtyForItem("Item No."))) then // P8001039, P800164877
                ...
                if P800Functions.AltQtyInstalled() then
                    AltQtyMgmt.SetWhseActLineAltQty(Rec);
                // P8000629A
            end;
        }
        field(27; "Qty. to Handle (Base)"; Decimal)
        {
            ...
            trigger OnValidate()
            begin
                TestField("Container License Plate", ''); // P80056710
                // P8000503A
                if ("Qty. to Handle (Base)" > "Qty. Outstanding (Base)") then
                    Error(Text002, StrSubstNo('%1 base', "Qty. Outstanding (Base)"));
                // P8000503A
        ...
        field(6500; "Serial No."; Code[50])
        {
            ...
            begin
                ...
                // P8000282A
                if P800Functions.AltQtyInstalled() then
                    AltQtyMgmt.UpdateAltQtyLotSerial(
                      "Alt. Qty. Transaction No.", xRec."Serial No.", xRec."Lot No.", "Serial No.", "Lot No.");
                // P8000282A
            end;
        }
        field(6501; "Lot No."; Code[50])
        {
            ...
            trigger OnLookup()
            begin
                SuppressPreferenceCheck := true; // P8001380
            ...
            begin
                ...
                if "Lot No." <> '' then begin
                    ...
                    ItemTrackingMgt.CheckWhsePickForSaleLot(Rec); // P8000322A

                    // P8001380
                    if (not SuppressPreferenceCheck) and (CurrFieldNo = FieldNo("Lot No.")) and P800Functions.TrackingInstalled then begin
                        SuppressPreferenceCheck := false;
                        if not CheckLotPreferences(true) then
                            Error(Text37002001, "Lot No.");
                    end;
                    // P8001380
                end;
                ...
                if "Lot No." <> xRec."Lot No." then
                    "Expiration Date" := 0D;
                TestField("Container License Plate", ''); // P8001323
        ...
        field(11028581; "Qty. to Container"; Decimal)
        {
            Caption = 'Qty. to Container';
            DecimalPlaces = 0 : 5;
        }
        field(11028582; "Include In Container"; Boolean)
        {
            Caption = 'Include';
        }
        ...
        field(37002082; "Qty. to Handle (Alt.)"; Decimal)
        {
            ...
            begin
                ...
                AltQtyMgmt.TestWhseActAltQtyInfo(Rec, false);
                ...
                if (CurrFieldNo = FieldNo("Qty. to Handle (Alt.)")) then begin
                    ...
                    AltQtyMgmt.CheckSummaryTolerance1("Alt. Qty. Transaction No.", "Item No.",
                                                      FieldCaption("Qty. to Handle (Alt.)"),
        ...
        field(37002400; "ADC Activity"; Option)
        {
            Caption = 'ADC Activity';
            OptionCaption = 'None,Pick ';
            OptionMembers = "None",Pick;
        }
        field(37002560; "Container Qty."; Decimal)
        {
            BlankZero = true;
        ...
        field(37002562; "First Container Line No."; Integer)
        {
            Caption = 'First Container Line No.';
            Editable = false;
            TableRelation = "Warehouse Activity Line"."Line No." WHERE("Activity Type" = FIELD("Activity Type"),
                                                                        "No." = FIELD("No."));
        }
        field(37002563; "Related Container Line No."; Integer)
        {
            Caption = 'Related Container Line No.';
            Editable = false;
            TableRelation = "Warehouse Activity Line"."Line No." WHERE("Activity Type" = FIELD("Activity Type"),
                                                                        "No." = FIELD("No."));
        }
        field(37002564; "Container License Plate"; Code[50])
        {
            ...
            begin
                // P8001323
                if xRec."Container License Plate" <> "Container License Plate" then
...
#pragma warning restore AS0009
        ...
        key(Key20; "From Staged Pick No.", "From Staged Pick Line No.", "Source Type", "Source Subtype", "Source No.", "Source Line No.", "Source Subline No.", "Unit of Measure Code", "Action Type", "Breakbulk No.", "Original Breakbulk")
        {
            SumIndexFields = "Qty. Outstanding (Base)", "Qty. Outstanding";
        }
        key(Key21; "Location Code", "Item No.", "Variant Code", "Activity Type", "Action Type", "Breakbulk No.", "From Staged Pick No.", "Bin Code", "Lot No.", "Serial No.")
        {
            SumIndexFields = "Qty. Outstanding (Base)";
        }
        key(Key22; "Location Code", "Bin Code", "Item No.", "Variant Code", "Activity Type", "Action Type", "Unit of Measure Code", "Lot No.", "Container Qty.")
        {
        }
        key(Key23; "Activity Type", "Action Type", "Container ID")
        {
        }
    }
    ...
    begin
        ...
        if "Container ID" <> '' then                       // P8001323
            ContainerFns.ClearPendingAssignment(Rec); // P8001323
    end;
    ...
    var
        ...
        Text37002080: Label 'Do you want to assign a %1?';
        HiddenDocMgmt: Codeunit "Process 800 Create Whse. Act.";
        AdjustSalesLineQty: Decimal;
        Consolidating: Boolean;
        RoundingAdjmtMgmt: Codeunit "Rounding Adjustment Mgmt.";
        Text37002000: Label 'The line is for a container. You can only handle the entire quantity of %1.';
        ContainerFns: Codeunit "Container Functions";
        Text37002001: Label 'Lot %1 fails to meet established lot preferences.';
        SuppressPreferenceCheck: Boolean;
        SuppressProdContUpdate: Boolean;
    ...
    begin
        ...
        with WhseActivLine do begin
            ...
            if Find('-') then
                repeat
                    SuppressProductionContainerUpdate; // P80056710
    ...
    begin
        ...
        with WhseActivLine do begin
            ...
            if not CalledFromHeader then
                if "Container ID" <> '' then begin
                    ...
                    if (ContainerHeader."Document Type" = DATABASE::"Prod. Order Component") and (not ContainerHeader."Pending Assignment") then begin
                        WhseActivLine2.Reset;
                        WhseActivLine2.SetRange("Activity Type", "Activity Type");
                        WhseActivLine2.SetRange("No.", "No.");
                        WhseActivLine2.SetFilter("Line No.", '<>%1', "Line No.");
                        WhseActivLine2.SetRange("Container ID", "Container ID");
                    end;
                end;
            // P80056710
            ...
            if WhseActivLine2.Find('-') then
                repeat
                    ...
                    WhseActivLine2.DeleteBinContent(WhseActivLine2."Action Type"::Place);    // P8001347  // P80050544
                    DeleteWhseActivLine2(WhseActivLine2, CalledFromHeader);
                    WhseActivLine2.DeleteAltQtys; // P8000282A
                    if WhseActivLine2."Container ID" <> '' then                   // P8001323
                        ContainerFns.ClearPendingAssignment(WhseActivLine2); // P8001323
        ...
        HiddenDocMgmt.UpdateHiddenDocOnDelete(Rec); // P8000322A
    end;
            ...
            then begin
                ...
                case WhseDocType2 of
                    ...
                    "Whse. Document Type"::Production:
                        begin
                            ...
                            if Location."Directed Put-away and Pick" then begin
                                ...
                                if ProdOrderCompLine."Bin Code" <> '' then   // P8008657
                    ...
                    "Whse. Document Type"::FOODStagedPick:
                        begin
                            WhseStagedPickLine.Get("Whse. Document No.", "Whse. Document Line No.");
                            TestField("Bin Code", WhseStagedPickLine."Bin Code");
                        end;
                    // P8000322A
        ...
        case "Source Type" of
            DATABASE::"Prod. Order Component":
                begin
                    ...
                    if ProdOrderComponentLine."Bin Code" <> '' then   // P8008657
    ...
    begin
        ...
        if Location."Directed Put-away and Pick" then begin
            ...
            if "ADC Activity" = "ADC Activity"::None then // P8000358A
                if not
        ...
        // P8000358A
        if "ADC Activity" <> "ADC Activity"::None then begin
            NewWhseActivLine."Qty. to Handle" := 0;
            NewWhseActivLine."Qty. to Handle (Base)" := 0;
            NewWhseActivLine."Qty. to Handle (Alt.)" := 0;
            NewWhseActivLine."ADC Activity" := "ADC Activity"::None;
        end;
        // P8000358A
        // P8000282A
        if TrackAlternateUnits() then begin
            NewWhseActivLine."Alt. Qty. Transaction No." := 0;
            NewWhseActivLine.Validate("Qty. to Handle (Alt.)", 0);
        end else
            NewWhseActivLine."Quantity (Alt.)" := 0;
        // P8000282A
        NewWhseActivLine."Container ID" := ''; // P8001323
        NewWhseActivLine."Container License Plate" := ''; // P8001323
        ...
        WhseActivLine."Qty. Outstanding" := WhseActivLine."Qty. to Handle";
        WhseActivLine."Qty. Outstanding (Base)" := WhseActivLine."Qty. to Handle (Base)";
        if TrackAlternateUnits() then                     // P8000282A
            WhseActivLine.Validate("Qty. to Handle (Alt.)") // P8000282A
        else
            WhseActivLine."Quantity (Alt.)" := 0;
        if Location."Directed Put-away and Pick" then
            WMSMgt.CalcCubageAndWeight(
        ...
        WhseActivLine."ADC Activity" := "ADC Activity"::None; // P8000358A
        ...
        if not IsHandled then begin
            ...
            if WhseActivLine."Activity Type" = WhseActivLine."Activity Type"::"Put-away" then begin
                if WhseActivLine."Breakbulk No." <> 0 then
                    Error(Text007);
                //WhseActivLine.TESTFIELD("Action Type", WhseActivLine."Action Type"::Place); // P8001323
            end;
    ...
    var
        ...
        WhseStdgPickHeader: Record "Whse. Staged Pick Header";
        WhseStgdPickCard: Page "Whse. Staged Pick";
        IsHandled: Boolean;
    begin
        IsHandled := false;
        ...
        case "Whse. Document Type" of
            ...
            "Whse. Document Type"::FOODStagedPick:
                begin
                    WhseStdgPickHeader.SetRange("No.", "Whse. Document No.");
                    WhseStgdPickCard.SetTableView(WhseStdgPickHeader);
                    WhseStgdPickCard.RunModal;
                end;
        // P8000322A
        end;
    end;
    ...
    begin
        if WhseActivLine.TrackingExists() then begin
            ...
            WhseItemTrkgLine.SetCurrentKey("Source ID", "Source Type");
            // P8000503A
            ...
            if WhseActivLine."Activity Type" = WhseActivLine."Activity Type"::"Invt. Movement" then
                case WhseActivLine."Source Type" of
                    ...
                    WhseActivLine."Whse. Document Type"::FOODStagedPick:
                        begin
                            WhseItemTrkgLine.SetRange("Source Type", DATABASE::"Whse. Staged Pick Line");
                            WhseItemTrkgLine.SetRange("Source ID", WhseActivLine."Whse. Document No.");
                            WhseItemTrkgLine.SetRange("Source Ref. No.", WhseActivLine."Whse. Document Line No.");
                        end;
                // P8000322A
    ...
    begin
        ...
        if (Item.Inventory - Abs(Item."Reserved Qty. on Inventory") +
            ...
            RoundingAdjmtMgmt.GetNearZeroQtyForItem("Item No.") + // P8001039
    ...
    procedure ShowItemAvailabilityByLot()
    var
        ItemAvailFormsMgt: Codeunit "Item Availability Forms Mgt";
    begin
        ItemAvailFormsMgt.ShowItemAvailFromWhseActivLine(Rec, ItemAvailFormsMgt.ByLot);
    end;
    ...
    procedure TransferFromStagedPickLine(WhseStagedPickLine: Record "Whse. Staged Pick Line")
    begin
        // P8000756
        "Activity Type" := "Activity Type"::Pick;
        "Shelf No." := WhseStagedPickLine."Shelf No.";
        "Item No." := WhseStagedPickLine."Item No.";
        "Variant Code" := WhseStagedPickLine."Variant Code";
        Description := WhseStagedPickLine.Description;
        "Description 2" := WhseStagedPickLine."Description 2";
        "Due Date" := WhseStagedPickLine."Due Date";
        "Whse. Document Type" := "Whse. Document Type"::FOODStagedPick;
        "Whse. Document No." := WhseStagedPickLine."No.";
        "Whse. Document Line No." := WhseStagedPickLine."Line No.";
    end;
    ...
    begin
        ...
        if ("Item No." = '') then
            exit(false);
        if ("Activity Type" = "Activity Type"::Pick) then begin
            // P8000322A - Entry of Alt. Qtys. on Prod. Order Component is not implemented
            //IF ("Whse. Document Type" <> "Whse. Document Type"::Shipment) AND
            //   ("Whse. Document Type" <> "Whse. Document Type"::Production)
            if "Whse. Document Type" <> "Whse. Document Type"::Shipment then         // P8000630A
                exit(false);
            // P8000322A
            if ("Action Type" <> "Action Type"::Take) then
                exit(false);
            GetLocation("Location Code");
            if not Location."Catch Alt. Qtys. On Whse. Pick" then
                exit(false);
        end else
            if not ("Activity Type" in ["Activity Type"::"Invt. Pick", "Activity Type"::"Invt. Put-away"]) then
                exit(false);
        GetItem;
        exit(Item.TrackAlternateUnits());
    end;

    procedure DeleteAltQtys()
    begin
        // P8000282A
    ...
    procedure DeleteAltQtys2(SuspendStatusCheck: Boolean)
    begin
        // P80095316
        if "Alt. Qty. Transaction No." <> 0 then
            AltQtyMgmt.DeleteAltQtyLines2("Alt. Qty. Transaction No.", SuspendStatusCheck);
    end;

    procedure PutAwayLookupLotSerialNo(LookupType: Enum "Item Tracking Type"; var ItemTrkgCode: Text[250]): Boolean
    begin
        // P8000282A
        if (ItemTrkgCode <> '') then
            ItemTrackingMgt.LookupTrackingNoInfo("Item No.", "Variant Code", LookupType, ItemTrkgCode) // P800144605
        else begin
            GetItem;
            case LookupType of
                LookupType::"Lot No.":
                    if not Confirm(Text37002080, false, FieldCaption("Lot No.")) then
                        LookUpTrackingSummary(Rec, "Action Type" <> "Action Type"::Place, -1, 1)
                    else begin
                        if P800Functions.TrackingInstalled() then
                            ItemTrkgCode := P800ItemTracking.AssignLotNo(Rec) // P8001234
                        else begin
                            Item.TestField("Lot Nos.");
                            ItemTrkgCode := NoSeriesMgt.GetNextNo(Item."Lot Nos.", WorkDate, true);
                        end;
                        exit(true);
                    end;
                LookupType::"Serial No.":
                    if not Confirm(Text37002080, false, FieldCaption("Serial No.")) then
                        LookUpTrackingSummary(Rec, "Action Type" <> "Action Type"::Place, -1, 0)
                    else begin
                        Item.TestField("Serial Nos.");
                        ItemTrkgCode := NoSeriesMgt.GetNextNo(Item."Serial Nos.", WorkDate, true);
                        exit(true);
                    end;
            end;
        end;
    ...
    local procedure ProcessOverShip()
    var
        ...
        WhseLineQuantityAdjust: Decimal;
        ...
        WhseActivity: Record "Warehouse Activity Line";
    ...
    begin
        ...
        if "Source Type" = DATABASE::"Sales Line" then begin
            WhseLineQuantityAdjust := 0;
            SalesHeader.Get("Source Subtype", "Source No.");
            if SalesLine.Get("Source Subtype", "Source No.", "Source Line No.") then begin
                SalesHeader."Allow Reopen" := true;
                SalesRelease.Reopen(SalesHeader);
                SalesHeader."Allow Reopen" := false;
                OriginalQty := SalesLine."Original Quantity";
                WhseLineQuantityAdjust := SalesLine.Quantity;
                SalesLine."Allow Quantity Change" := true;
                SalesLine.Validate(Quantity, SalesLine.Quantity + AdjustSalesLineQty);
                SalesLine.Validate("Original Quantity", OriginalQty);
                SalesLine."Allow Quantity Change" := false;
                SalesLine.Modify;
                WhseLineQuantityAdjust := SalesLine.Quantity - WhseLineQuantityAdjust;
                Validate(Quantity, Quantity + WhseLineQuantityAdjust);
                Modify;
                if not Consolidating then
                    OverShipUpdatePlaceRec(WhseLineQuantityAdjust);
                SalesRelease.Run(SalesHeader);
                SalesHeader.Modify;
                OverShipUpdateShipment("Whse. Document No.", WhseLineQuantityAdjust);
            end;
        end;
        ...
        if "Source Type" = DATABASE::"Transfer Line" then begin
            ...
            if TransferLine.Get("Source No.", "Source Line No.") then begin
                TransferRelease.Reopen(TransferHeader);
                WhseLineQuantityAdjust := TransferLine.Quantity;
                TransferLine."Allow Quantity Change" := true;
                TransferLine.Validate(Quantity, TransferLine.Quantity + AdjustSalesLineQty);
                TransferLine."Allow Quantity Change" := false;
                TransferLine.Modify;
                WhseLineQuantityAdjust := TransferLine.Quantity - WhseLineQuantityAdjust;
                Validate(Quantity, Quantity + WhseLineQuantityAdjust);
                Modify;
                if not Consolidating then
                    OverShipUpdatePlaceRec(WhseLineQuantityAdjust);
    ...
    procedure OverShipUpdatePlaceRec(WhseLineQuantityAdjust: Decimal)
    var
        WhseActivity: Record "Warehouse Activity Line";
    begin
        // P8000372A
        if "Action Type" = "Action Type"::Take then begin
            WhseActivity.SetFilter("No.", "No.");
            WhseActivity.SetRange("Source Type", "Source Type");
            WhseActivity.SetRange("Source Subtype", "Source Subtype");
            WhseActivity.SetFilter("Source No.", "Source No.");
            WhseActivity.SetRange("Source Line No.", "Source Line No.");
            WhseActivity.SetFilter("Item No.", "Item No.");
            WhseActivity.SetFilter("Unit of Measure Code", "Unit of Measure Code");
            WhseActivity.SetRange("Breakbulk No.", "Breakbulk No."); // P8000503A
            if "ADC Activity" = "ADC Activity"::None then begin  // P8000358A
                WhseActivity.SetRange("Lot No.", "Lot No.");
                WhseActivity.SetRange("Serial No.", "Serial No."); //P8000358A
            end;
            WhseActivity.SetRange("Action Type", "Action Type"::Place);
            if WhseActivity.Find('-') then begin
                WhseActivity.Validate(Quantity, WhseActivity.Quantity + WhseLineQuantityAdjust);
                if "ADC Activity" <> "ADC Activity"::None then // P8000358A
                    WhseActivity."Qty. to Handle" := 0;          // P8000358A
                WhseActivity.Modify;
                // P8000503A
            end else begin
                WhseActivity.SetRange("Unit of Measure Code");
                if WhseActivity.Find('-') then begin
                    WhseActivity.Validate(Quantity,
                      WhseActivity.Quantity +
                      Round(WhseLineQuantityAdjust *
                        "Qty. per Unit of Measure" / WhseActivity."Qty. per Unit of Measure", 0.00001));
                    if "ADC Activity" <> "ADC Activity"::None then
                        WhseActivity."Qty. to Handle" := 0;
                    WhseActivity.Modify;
                end;
                // P8000503A
            end;
    ...
    local procedure OverShipUpdateShipment(DocumentNO: Code[20]; WhseLineQuantityAdjust: Decimal)
    var
        ShipmentHeader: Record "Warehouse Shipment Header";
        ShipmentLine: Record "Warehouse Shipment Line";
        ReleaseWhseShptDoc: Codeunit "Whse.-Shipment Release";
    begin
        // P8000372A
        if ShipmentHeader.Get(DocumentNO) then begin
            ShipmentHeader."Allow Reopen" := true;
            ReleaseWhseShptDoc.Reopen(ShipmentHeader);
            ShipmentHeader."Allow Reopen" := false;
            ShipmentLine.SetCurrentKey("Source Type", "Source Subtype", "Source No.", "Source Line No.");
            ShipmentLine.SetRange("Source Type", "Source Type");
            ShipmentLine.SetRange("Source Subtype", "Source Subtype");
            ShipmentLine.SetFilter("Source No.", "Source No.");
            ShipmentLine.SetRange("Source Line No.", "Source Line No.");
            if ShipmentLine.Find('-') then begin
                if (ShipmentLine.Quantity + WhseLineQuantityAdjust) = 0 then begin
                    ShipmentLine.Delete(true);
                end else begin
                    ShipmentLine.Validate(Quantity, ShipmentLine.Quantity + WhseLineQuantityAdjust);
                    ShipmentLine.Modify;
                end;
            end;
            ReleaseWhseShptDoc.Release(ShipmentHeader);
            ShipmentHeader.Modify;
            ShipmentLine.Reset;
            ShipmentLine.SetFilter("No.", DocumentNO);
            if not ShipmentLine.Find('-') then
                ShipmentHeader.Delete;
        end;
    end;

    local procedure HandleOverPick()
    var
        WhseLineQuantityAdjust: Decimal;
    begin
        // P8000372A
        if ("Activity Type" = "Activity Type"::Pick) and ("Action Type" = "Action Type"::Take) and
           ("Source Type" in [DATABASE::"Sales Line", DATABASE::"Transfer Line"])
        then
            if ("Qty. to Handle" > "Qty. Outstanding") then begin
                WhseLineQuantityAdjust := Quantity;
                Validate(Quantity, "Qty. to Handle" + "Qty. Handled");
                WhseLineQuantityAdjust := Quantity - WhseLineQuantityAdjust; // P8000358A
                OverShipUpdatePlaceRec(WhseLineQuantityAdjust);              // P8000358A
            end;
    end;

    procedure AddBinContent()
    var
        BinContent: Record "Bin Content";
    begin
        // P8000503A
        // IF ("Action Type" = "Action Type"::Place) AND ("Bin Code" <> '') THEN // P8001347
        if ("Bin Code" <> '') then                                               // P8001347
            ...
            then begin
                ...
                BinContent.Dedicated := Bin.Dedicated; // P8001132
                BinContent.Insert;
            end;
    end;

    procedure GetSourceQtys(var Qty: Decimal; var QtyBase: Decimal; var QtyHandled: Decimal; var QtyHandledBase: Decimal): Boolean
    var
    ...
    begin
        // P8000503
        // P8000553A - added Boolean Return Value
        QtyHandled := 0;
        QtyHandledBase := 0;
        case "Whse. Document Type" of
            "Whse. Document Type"::Receipt:
                if WhsePostedRcptLine.Get("Whse. Document No.", "Whse. Document Line No.") then begin
                    Qty := WhsePostedRcptLine.Quantity;
                    QtyBase := WhsePostedRcptLine."Qty. (Base)";
                    QtyHandled := WhsePostedRcptLine."Qty. Put Away";
                    QtyHandledBase := WhsePostedRcptLine."Qty. Put Away (Base)";
                    exit(true); // P8000553A
                end;
            "Whse. Document Type"::Shipment:
                if WhseShipmentLine.Get("Whse. Document No.", "Whse. Document Line No.") then begin
                    Qty := WhseShipmentLine.Quantity;
                    QtyBase := WhseShipmentLine."Qty. (Base)";
                    QtyHandled := WhseShipmentLine."Qty. Picked";
                    QtyHandledBase := WhseShipmentLine."Qty. Picked (Base)";
                    exit(true); // P8000553A
                end;
            "Whse. Document Type"::"Internal Put-away":
                if WhseIntPutAwayLine.Get("Whse. Document No.", "Whse. Document Line No.") then begin
                    Qty := WhseIntPutAwayLine.Quantity;
                    QtyBase := WhseIntPutAwayLine."Qty. (Base)";
                    QtyHandled := WhseIntPutAwayLine."Qty. Put Away";
                    QtyHandledBase := WhseIntPutAwayLine."Qty. Put Away (Base)";
                    exit(true); // P8000553A
                end;
            "Whse. Document Type"::"Internal Pick":
                if WhseIntPickLine.Get("Whse. Document No.", "Whse. Document Line No.") then begin
                    Qty := WhseIntPickLine.Quantity;
                    QtyBase := WhseIntPickLine."Qty. (Base)";
                    QtyHandled := WhseIntPickLine."Qty. Picked";
                    QtyHandledBase := WhseIntPickLine."Qty. Picked (Base)";
                    exit(true); // P8000553A
                end;
            "Whse. Document Type"::Production:
                ...
                then begin
                    Qty := ProdOrderComponent."Expected Quantity";
                    QtyBase := ProdOrderComponent."Expected Qty. (Base)";
                    QtyHandled := ProdOrderComponent."Qty. Picked";
                    QtyHandledBase := ProdOrderComponent."Qty. Picked (Base)";
                    exit(true); // P8000553A
                end;
            "Whse. Document Type"::"Movement Worksheet":
                if WhseMovementWksh.Get(
                     "Source No.", "Whse. Document No.", "Location Code", "Source Line No.") // P8000553A
                then begin
                    Qty := WhseMovementWksh.Quantity;
                    QtyBase := WhseMovementWksh."Qty. (Base)";
                    QtyHandled := WhseMovementWksh."Qty. Handled" - WhseMovementWksh."Qty. to Handle";                   // P8000553A
                    QtyHandledBase := WhseMovementWksh."Qty. Handled (Base)" - WhseMovementWksh."Qty. to Handle (Base)"; // P8000553A
                    exit(true); // P8000553A
                end;
            "Whse. Document Type"::FOODStagedPick:
                if WhseStgdPickLine.Get("Whse. Document No.", "Whse. Document Line No.") then begin
                    Qty := WhseStgdPickLine."Qty. to Stage";
                    QtyBase := WhseStgdPickLine."Qty. to Stage (Base)";
                    QtyHandled := WhseStgdPickLine."Qty. Staged";
                    QtyHandledBase := WhseStgdPickLine."Qty. Staged (Base)";
                    exit(true); // P8000553A
                end;
        end;
    end;

    procedure UOMConversionNeeded(): Boolean
    var
        BinContent: Record "Bin Content";
    begin
        // P8001347
        if ("Activity Type" = "Activity Type"::Pick) and ("Action Type" = "Action Type"::Take) and
           ("Item No." <> '') and ("Unit of Measure Code" <> '') and ("Bin Code" <> '') and
           (Quantity <> 0) and ("Container Qty." = 0)
        then
            if BinContent.Get("Location Code", "Bin Code", "Item No.", "Variant Code", "Unit of Measure Code") then begin
                ...
                exit((BinContent.Quantity - BinContent."Pick Qty." - BinContent."Neg. Adjmt. Qty." - BinContent."ATO Components Pick Qty.") < 0);
            end;
    end;

    procedure GetAvailableContainers(var TempContainer: Record "Container Header" temporary; SingleLotOnly: Boolean): Boolean
    var
    ...
    begin
        // P8001347
        TempContainer.Reset;
        TempContainer.DeleteAll;
        if ContainerFns.GetAvailableContainers(
             "Location Code", "Bin Code", "Item No.", "Variant Code", "Unit of Measure Code",
             "Lot No.", "Container Qty.", SingleLotOnly, false, "Container ID", TempContainerLine)
        then begin
            repeat
                if not TempContainer.Get(TempContainerLine."Container ID") then begin
                    ContainerHeader.Get(TempContainerLine."Container ID");
                    TempContainer := ContainerHeader;
                    TempContainer.Insert;
                end;
            until (TempContainerLine.Next = 0);
            exit(TempContainer.FindSet);
        end;
    end;

    procedure GetAllocatedContainerCount(LocationCode: Code[10]; BinCode: Code[20]; ItemNo: Code[20]; VariantCode: Code[10]; UnitofMeasureCode: Code[10]; LotNo: Code[50]; ContainerQty: Decimal): Integer
    begin
        // P8001347
        SetCurrentKey(
          "Location Code", "Bin Code", "Item No.", "Variant Code",
          "Activity Type", "Action Type", "Unit of Measure Code", "Lot No.", "Container Qty.");
    ...
    procedure CheckLotPreferences(ShowWarning: Boolean): Boolean
    var
        SalesLine: Record "Sales Line";
        ProdOrderComponent: Record "Prod. Order Component";
    begin
            ...
            case "Source Type" of
                DATABASE::"Sales Line":
                    begin
                        ...
                        exit(SalesLine.CheckLotPreferences("Lot No.", ShowWarning));
                    end;
                DATABASE::"Prod. Order Component":
                    begin
                        ProdOrderComponent.Get("Source Subtype", "Source No.", "Source Line No.", "Source Subline No.");
                        exit(ProdOrderComponent.CheckLotPreferences("Lot No.", ShowWarning));
                    end;
                else
                    exit(true);
            end;
    ...
    procedure PickType(): Text
    var
        Container: Label 'Container';
        UOMConv: Label 'UOM Conversion';
    begin
        // P80039754
        if ("Container Qty." <> 0) then
            exit(Container)
        else
            if UOMConversionNeeded() then
                exit(UOMConv);
    end;
    ...
    procedure SuppressProductionContainerUpdate()
    begin
        // P80056710
        SuppressProdContUpdate := true;
    end;
