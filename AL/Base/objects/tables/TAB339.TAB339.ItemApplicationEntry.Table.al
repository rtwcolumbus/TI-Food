table 339 "Item Application Entry"
{
    ...
    //   Add Quantity (Alt.) as summ index field to support alternate quantities in revaluation journal
    // 
    // PR5.00
    // P8000466A, VerticalSoft, Jack Reynolds, 31 JUL 07
    //   Copy OutboundApplied, InboundApplied, and Returned functions and make alternate quantity versions of them
    // 
    // PRW16.00.04
    ...
    fields
    {
        ...
        {
            CalcFormula = Lookup ("Item Ledger Entry"."Commodity Class Code" WHERE("Entry No." = FIELD("Item Ledger Entry No.")));
            Caption = 'Commodity Class Code';
            Editable = false;
            FieldClass = FlowField;
            TableRelation = "Commodity Class";
        }
        field(37002681; "Commodity Cost Calculated"; Boolean)
        {
            Caption = 'Commodity Cost Calculated';
        }
        field(37002682; "Comm. Cost Adjmt."; Decimal)
        {
            AutoFormatType = 1;
            BlankZero = true;
            Caption = 'Comm. Cost Adjmt.';
        }
        field(37002683; "Comm. Cost Adjmt. (ACY)"; Decimal)
        {
            BlankZero = true;
            Caption = 'Comm. Cost Adjmt. (ACY)';
        }
    }
    ...
    begin
        exit(((Quantity > 0) or ("Quantity (Alt.)" > 0)) and ("Item Ledger Entry No." = "Inbound Item Entry No.")); // P8000466A
    end;
    ...
    procedure OutboundAppliedAlt(EntryNo: Integer; SameType: Boolean): Decimal
    var
        Applications: Record "Item Application Entry";
        ItemEntry: Record "Item Ledger Entry";
        QuantityAlt: Decimal;
    ...
    begin
        // P8000466A
        if not OriginalEntry.Get(EntryNo) then exit(0);
        if OriginalEntry."Entry Type" = OriginalEntry."Entry Type"::Transfer then exit(0);
        Applications.SetCurrentKey(Applications."Outbound Item Entry No.");
        Applications.SetRange(Applications."Outbound Item Entry No.", EntryNo);
        Applications.SetRange(Applications."Item Ledger Entry No.", EntryNo);
        QuantityAlt := 0;
        if Applications.FindFirst then
            repeat
                if ItemEntry.Get(Applications."Inbound Item Entry No.") then
                    if SameType then begin
                        if ItemEntry."Entry Type" = OriginalEntry."Entry Type" then
                            QuantityAlt := QuantityAlt + Applications."Quantity (Alt.)"
                    end
                    else
                        QuantityAlt := QuantityAlt + Applications."Quantity (Alt.)";
            until Applications.Next <= 0;
    ...
    procedure InboundAppliedAlt(EntryNo: Integer; SameType: Boolean): Decimal
    var
        Applications: Record "Item Application Entry";
        ItemEntry: Record "Item Ledger Entry";
        QuantityAlt: Decimal;
    ...
    begin
        ...
        if not OriginalEntry.Get(EntryNo) then exit(0);
        if OriginalEntry."Entry Type" = OriginalEntry."Entry Type"::Transfer then exit(0);
        Applications.SetCurrentKey(Applications."Inbound Item Entry No.");
        Applications.SetFilter("Outbound Item Entry No.", '<>%1', 0);
        Applications.SetRange("Inbound Item Entry No.", EntryNo);
        if OriginalEntry."Quantity (Alt.)" < 0 then
            Applications.SetRange("Item Ledger Entry No.", EntryNo)
        else
            ;
        //    Applications.SETFILTER("Item Ledger Entry No.",'<>%1',EntryNo);
        QuantityAlt := 0;
        if Applications.FindFirst then
            repeat
                if ItemEntry.Get(Applications."Outbound Item Entry No.") then
    ...
    procedure ReturnedAlt(EntryNo: Integer): Decimal
    var
        Applications: Record "Item Application Entry";
        ItemEntry: Record "Item Ledger Entry";
        Quantity: Integer;
    ...
    begin
        // P8000466A
        exit(-OutboundAppliedAlt(EntryNo, true) - InboundAppliedAlt(EntryNo, true));
    end;
