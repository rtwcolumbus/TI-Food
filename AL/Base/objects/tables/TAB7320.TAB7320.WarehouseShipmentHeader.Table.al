table 7320 "Warehouse Shipment Header"
{
    ...
    // 
    // PRW111.00.02
    // P80075174, To-Increase, Gangabhushan, 27 MAY 19
    // CS00061542 - Validation on delivery trip appears to not set proper filters for modify all
    //
    // PRW117.00.03
    ...
    fields
    {
        ...
        field(2; "Location Code"; Code[10])
        {
            ...
            begin
                if CurrFieldNo = FieldNo("Location Code") then // TOM4220
                ...
                "Container Pick and Ship" := Location."Container Pick and Ship";  // P800110480                 
            end;
        }
        field(3; "Assigned User ID"; Code[50])
        {
            ...
            begin
                ...
                UpdateAssignedUserOnLines(xRec."Assigned User ID", "Assigned User ID"); // P8001323
            end;
        }
        ...
        field(11028580; "Delivery Trip"; Code[20])
        {
            ...
            begin
                WarehouseRequest.SetRange("Warehouse Shipment No.", "No.");
                if "No." <> '' then // P80075174
                    WarehouseRequest.ModifyAll("Delivery Trip", "Delivery Trip", false);
            end;
        }
        field(37002000; "Shipment Time"; Time)
        {
        ...
        field(37002004; "Allow Reopen"; Boolean)
        {
            ...
            Description = 'P8000372A';
        }
        field(37002005; "Container Pick and Ship"; Boolean)
        {
    ...
    begin
        ...
        UpdateContainersOnDelete; // P80046533
    ...
    var
        ...
        Text37002700: Label 'One or more containers have been marked to be shipped for this shipment.';
    ...
    begin
        ...
        if WhseShptLine.Find('-') then
            repeat
                if not WhseShptLine.IsNonWarehouseItem() then // P8001290
    ...
    begin
        // P8001290
        OrderStatus := GetDocumentStatus(0);
        if (OrderStatus <> "Document Status") then begin
            Validate("Document Status", OrderStatus);
            Modify;
        end;
    end;

    procedure UpdateAssignedUserOnLines(xAssignedUser: Code[50]; AssignedUser: Code[50])
    var
        WarehouseShipmentLine: Record "Warehouse Shipment Line";
    begin
        // P8001323
        if xAssignedUser = AssignedUser then
            exit;

        WarehouseShipmentLine.SetRange("No.", "No.");
        if xAssignedUser = '' then
            WarehouseShipmentLine.SetRange("Assigned User ID", '')
        else
            WarehouseShipmentLine.SetFilter("Assigned User ID", '%1|%2', '', xAssignedUser);

        WarehouseShipmentLine.ModifyAll("Assigned User ID", AssignedUser);
    end;
    ...
    begin
        ...
        if P800Functions.ContainerTrackingInstalled then begin
            ContainerHeader.SetRange("Whse. Document Type", ContainerHeader."Whse. Document Type"::Shipment);
