table 5740 "Transfer Header"
{
    ...
    fields
    {
        field(37002060; "Delivery Route No."; Code[20])
        {
            ...
            TableRelation = "Delivery Route" WHERE("Location Code" = FIELD("Transfer-from Code"));
            ...
            begin
                ...
                if CurrFieldNo = FieldNo("Delivery Route No.") then begin
                    DeliveryRouteMgmt.CheckDeliveryRouteLocation("Delivery Route No.", "Transfer-from Code");
                    DeliveryRouteMgmt.CheckDeliveryRouteDOW("Delivery Route No.", "Shipment Date");
                end;
            end;
        }
                ...
                if xRec."Transfer-from Code" <> "Transfer-from Code" then begin
                    ...
                    if Confirmed then begin
                        ...
                        if ProcessFns.ProcessDataCollectionInstalled then
                            DataCollectionMgmt.CheckTransHeaderModify(xRec, Rec);
                        // P8001090
                        if Location.Get("Transfer-from Code") then begin
                            ...
                            if ProcessFns.DistPlanningInstalled then          // P8000954
                                DeliveryRouteMgmt.GetTransDeliveryRouting(Rec); // P8000954
                            TransLine.LockTable();
    ...
    begin
        ...
        DocExtraCharge.SetFilter("Table ID", '%1|%2', DATABASE::"Transfer Header", DATABASE::"Transfer Line"); // P8001032
        ...
        // P8001090
        if ProcessFns.ProcessDataCollectionInstalled then
            DataCollectionMgmt.DeleteDataSheet(DATABASE::"Transfer Header", 0, "No.");
        // P8001090
    ...
    begin
        ...
        TestField("No.");
        DocExtraCharge.Reset;
        DocExtraCharge.SetRange("Table ID", DATABASE::"Transfer Header"); // P8001032
        DocExtraCharge.SetRange("Document No.", "No.");
    ...
    begin
        ...
        if "No." <> '' then
            ExtraChargeMgt.ShowExtraCharge(DATABASE::"Transfer Header", 0, "No.", 0, ShortcutECCharge) // P8001032
        else
    ...
    begin
        ...
        if "No." <> '' then
            ExtraChargeMgt.ShowExtraVendor(DATABASE::"Transfer Header", 0, "No.", ShortcutECVendor) // P8001032
        else
    ...
    begin
        ...
        if "No." <> '' then
            ExtraChargeMgt.SaveExtraCharge(DATABASE::"Transfer Header", 0, "No.", 0, FieldNumber, Charge) // P8001032
        else
    ...
    begin
        ...
        if "No." <> '' then
            ExtraChargeMgt.SaveExtraVendor(DATABASE::"Transfer Header", 0, "No.", 0, FieldNumber, VendorNo) // P8001032
        else
    ...
    begin
        // P8000928
        if not ExtraChargeMgt.LookupExtraVendor(FieldNumber, VendorNo) then
            exit;
        if "No." <> '' then
            ExtraChargeMgt.SaveExtraVendor(DATABASE::"Transfer Header", 0, "No.", 0, FieldNumber, VendorNo) // P8001032
        else
    ...
    begin
        ...
        ReceivingLabelMgmt.WorksheetLinesForTransHdr(TransHeader, LabelWorksheetLine);
    ...
    begin
        ...
        ContainerHeader.SetRange("Document Subtype", 0);
        ...
        if TransLine.FindSet(true) then
            repeat
                if TransLine."Alt. Qty. Trans. No. (Ship)" <> 0 then
