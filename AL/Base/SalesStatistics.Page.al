page 160 "Sales Statistics"
{
    ...
    layout
    {
        area(content)
        {
            group(General)
            {
                ...
                field("Allowance[1] + Allowance[2]"; Allowance[1] + Allowance[2])
                {
                ...
                field("TotalSalesLineLCY.""Line Amount (Freight)"""; TotalSalesLineLCY."Line Amount (Freight)")
                {
                    ApplicationArea = FOODBasic;
                    AutoFormatType = 1;
                    Caption = 'Freight (LCY)';
    ...
    begin
        ...
        TotalAmount1 :=
          TotalSalesLine."Line Amount" - TotalSalesLine."Inv. Discount Amount" - // PR3.70
            Allowance[1] - Allowance[2];                                         // PR3.70
        ...
        TotalSalesLineLCY.Amount := TotalSalesLineLCY.Amount - TotalSalesLineLCY."Line Amount (Freight)"; // P8000921
    ...
    begin
        ...
        TotalSalesLine."Inv. Discount Amount" := TotalSalesLine."Line Amount" - TotalAmount1 - // PR3.70
          Allowance[1] - Allowance[2];                                                         // PR3.70
        UpdateInvDiscAmount();
    end;
    ...
    begin
        ...
        // P8000921
        if (TotalSalesLine."Line Amount (Freight)" <> 0) then begin
            TotalSalesLineLCY.Amount := TotalSalesLineLCY.Amount - TotalSalesLineLCY."Line Amount (Freight)";
            ProfitLCY := TotalSalesLineLCY.Amount - TotalSalesLineLCY."Unit Cost (LCY)";
            if (TotalSalesLineLCY.Amount = 0) then
                ProfitPct := 0
            else
        ...
        if Process800Fns.PricingInstalled then begin
            OffInvoiceFns.TotalAllowance(Rec, 'ORDER', Allowance[1], Allowance[2]);
            Allowance2LCY := Round(
              CurrExchRate.ExchangeAmtFCYToLCY("Posting Date", "Currency Code", Allowance[2], "Currency Factor"));
            TotalSalesLine."Line Amount" += Allowance[1];
            TotalAmount1 -= Allowance[2];
            TotalAmount2 -= Allowance[2];
            TotalSalesLineLCY.Amount -= Allowance2LCY;
            ProfitLCY -= Allowance2LCY;
            if TotalSalesLineLCY.Amount = 0 then
                ProfitPct := 0
            else
                ProfitPct := Round(100 * ProfitLCY / TotalSalesLineLCY.Amount, 0.01);
