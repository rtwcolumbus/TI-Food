page 5851 "Get Post.Doc - S.ShptLn Sbfrm"
{
    ...
    layout
    {
        area(content)
        {
            repeater(Control1)
            {
                ...
                field(QtyNotReturnedAlt; QtyNotReturnedAlt)
                {
                    ApplicationArea = FOODBasic;
                    CaptionClass = StrSubstNo('37002080,1,17,%1,%2', Type, "No.");
                    Caption = 'Qty. Not Returned (Alt.)';
                    DecimalPlaces = 0 : 5;
                    Visible = false;
                }
                field(CalcQtyReturnedAlt; CalcQtyReturnedAlt)
                {
                    ApplicationArea = FOODBasic;
                    CaptionClass = StrSubstNo('37002080,1,18,%1,%2', Type, "No.");
                    Caption = 'Qty. Returned (Alt.)';
                    DecimalPlaces = 0 : 5;
                    Visible = false;
                }
    ...
    begin
        ...
        with SalesShptLine2 do begin
            ...
            if Type <> Type::Item then
                exit(true);
            CalcShippedSaleNotReturned(QtyNotReturned, QtyNotReturnedAlt, RevUnitCostLCY, FillExactCostReverse); // P8000466A
            if not RevQtyFilter then
                exit(true);
            exit((QtyNotReturned > 0) or (QtyNotReturnedAlt > 0)); // P8000466A
        end;
    end;
    ...
    local procedure CalcQtyReturnedAlt(): Decimal
    begin
        // P8000466A
        if (Type = Type::Item) and ("Quantity (Alt.)" - QtyNotReturnedAlt > 0) then
            exit("Quantity (Alt.)" - QtyNotReturnedAlt);
        exit(0);
    end;
}
