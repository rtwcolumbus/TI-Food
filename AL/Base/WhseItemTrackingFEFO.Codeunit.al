codeunit 7326 "Whse. Item Tracking FEFO"
{
    ...
    var
        ...
        ItemTrackingPickingMethod: Option "None",,FIFO,LIFO;
        FEFOPickingForSale: Boolean;
    ...
    begin
        ...
        with ItemLedgEntry do begin
            ...
            if (ItemTrackingPickingMethod < ItemTrackingPickingMethod::FIFO) then // P8000621A
            ...
            repeat
                ...
                PostingDate := "Posting Date"; // P800-MegaApp
                SetTrackingFilterFromItemLedgEntry(ItemLedgEntry);
                ItemTrackingSetup.CopyTrackingFromItemLedgerEntry(ItemLedgEntry);
                FindSet();
                if not IsItemTrackingBlocked("Item No.", "Variant Code", ItemTrackingSetup) and
                    (not FEFOPickingForSale or ItemTrackingDataCollection.IsLotAvailableForSale2(ItemNo, VariantCode, "Lot No.")) // P800-MegaApp
                then begin
                    repeat
                    ...
                    then begin
                        // P800-MegaApp
                        if ItemTrackingPickingMethod >= ItemTrackingPickingMethod::FIFO then
                            "Expiration Date" := PostingDate;
                        // P800-MegaApp
    ...
    begin
        ...
        with TempGlobalEntrySummary do begin
            ...
            if ItemTrackingPickingMethod = ItemTrackingPickingMethod::LIFO then // P800-MegaApp
                SetAscending("Expiration Date", false);                         // P800-MegaApp
    ...
    procedure SetItemTrackingPickingMethod(NewItemTrackingPickingMethod: Integer)
    begin
        // P80053245
        ItemTrackingPickingMethod := NewItemTrackingPickingMethod; // P8000466A
    end;

    procedure SetFEFOPickingForSale(NewFEFOPickingForSale: Boolean)
    begin
        // P80053245
        FEFOPickingForSale := NewFEFOPickingForSale; // P8000466A
    end;

    local procedure UpdateEntrySummaryFEFO(LotNo: Code[50]; SerialNo: Code[50]; PostingDate: Date)
    begin
        // P80053245
        // P8000621A
        if not EntrySummaryFEFOExists(LotNo, SerialNo) then
            InsertEntrySummaryFEFO(LotNo, SerialNo, PostingDate)
        else
            with TempGlobalEntrySummary do begin
                FindSet;
                if (PostingDate < "Expiration Date") then begin
                    "Expiration Date" := PostingDate;
                    Modify;
                end;
            end;
