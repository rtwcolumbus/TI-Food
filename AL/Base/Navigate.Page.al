page 344 Navigate
{
    ...
    //   Support for maintenance work orders and ledger entries
    // 
    // PRW16.00.04
    ...
    //   Security filtering on the global record variables
    ...
    var
        ...
        Text37002685: Label 'Posted Commodity Manifest';
        [SecurityFiltering(SecurityFilter::Filtered)]
        PstdSalesPaymentHeader: Record "Posted Sales Payment Header";
        Text37002671: Label 'Posted Sales Payment';
        [SecurityFiltering(SecurityFilter::Filtered)]
        RegPreProcessActivity: Record "Reg. Pre-Process Activity";
        Text37002460: Label 'Pre-Process Activity';
        [SecurityFiltering(SecurityFilter::Filtered)]
        RegPreProcessBlendActivity: Record "Reg. Pre-Process Activity";
        Text37002461: Label 'Pre-Process Blending Activity';
    ...
    begin
        ...
        FindPstdSalesPaymentHeader(); // P800-MegaApp
        ...
        FindPstdCommManifestHeader(); // P800-MegaApp
        FindProdOrderHeader();
        FindPostedAssemblyHeader();
        FindRegPreProcessActivity(); // P800-MegaApp
        FindRegPreProcessBlendActivity(); // P800-MegaApp
        FindRepackOrder(); // P800-MegaApp
        FindTransShptHeader();
        FindTransRcptHeader();
    ...
    // P800-MegaApp
    local procedure FindPstdCommManifestHeader()
    begin
        // P8000891
        if PstdCommManifestHeader.ReadPermission then begin
            PstdCommManifestHeader.Reset;
            PstdCommManifestHeader.SetFilter("No.", DocNoFilter);
            PstdCommManifestHeader.SetFilter("Posting Date", PostingDateFilter);
            InsertIntoDocEntry(
              Rec, DATABASE::"Posted Comm. Manifest Header", 0, Text37002685, PstdCommManifestHeader.Count); // P80066030
        end;
    end;

    local procedure FindPstdSalesPaymentHeader()
    begin
        // P8000941
        if PstdSalesPaymentHeader.ReadPermission then begin
            PstdSalesPaymentHeader.Reset;
            PstdSalesPaymentHeader.SetFilter("No.", DocNoFilter);
            PstdSalesPaymentHeader.SetFilter("Posting Date", PostingDateFilter);
            InsertIntoDocEntry(
              Rec, DATABASE::"Posted Sales Payment Header", 0, Text37002671, PstdSalesPaymentHeader.Count); // P80066030
        end;
    end;

    local procedure FindRegPreProcessActivity()
    begin
        // P8001082
        if RegPreProcessActivity.ReadPermission then begin
            RegPreProcessActivity.Reset;
            RegPreProcessActivity.SetCurrentKey("Prod. Order Status", "Prod. Order No.");
            RegPreProcessActivity.SetRange(
              "Prod. Order Status",
              RegPreProcessActivity."Prod. Order Status"::Released, RegPreProcessActivity."Prod. Order Status"::Finished);
            RegPreProcessActivity.SetFilter("Prod. Order No.", DocNoFilter);
            InsertIntoDocEntry(
              Rec, DATABASE::"Reg. Pre-Process Activity", RegPreProcessActivity.Blending::" ",  // P80066030
              Text37002460, RegPreProcessActivity.Count);
        end;
    end;

    local procedure FindRegPreProcessBlendActivity()
    begin
        // P8001082
        if RegPreProcessBlendActivity.ReadPermission then begin
            RegPreProcessBlendActivity.Reset;
            RegPreProcessBlendActivity.SetCurrentKey("Blending Order Status", "Blending Order No.");
            RegPreProcessBlendActivity.SetRange(
              "Blending Order Status",
              RegPreProcessBlendActivity."Blending Order Status"::Released,
              RegPreProcessBlendActivity."Blending Order Status"::Finished);
            RegPreProcessBlendActivity.SetFilter("Blending Order No.", DocNoFilter);
            InsertIntoDocEntry(
              Rec, DATABASE::"Reg. Pre-Process Activity", RegPreProcessActivity.Blending::"Per Order", // P80066030
              Text37002461, RegPreProcessBlendActivity.Count);
        end;
    end;

    local procedure FindAccrualLedgEntry()
    begin
        // P8000119A
        if AccrualLedgEntry.ReadPermission then begin
            AccrualLedgEntry.Reset;
            AccrualLedgEntry.SetCurrentKey("Document No.", "Posting Date");
            AccrualLedgEntry.SetFilter("Document No.", DocNoFilter);
            AccrualLedgEntry.SetFilter("Posting Date", PostingDateFilter);
            InsertIntoDocEntry(
              Rec, DATABASE::"Accrual Ledger Entry", 0, AccrualLedgEntry.TableCaption, AccrualLedgEntry.Count); // P80066030
        end;
    end;

    local procedure FindContLedgEntry()
    begin
        // P8000140A
        if ContLedgEntry.ReadPermission then begin
            ContLedgEntry.Reset;
            ContLedgEntry.SetCurrentKey("Document No.", "Posting Date");
            ContLedgEntry.SetFilter("Document No.", DocNoFilter);
            ContLedgEntry.SetFilter("Posting Date", PostingDateFilter);
            InsertIntoDocEntry(
              Rec, DATABASE::"Container Ledger Entry", 0, ContLedgEntry.TableCaption, ContLedgEntry.Count); // P80066030
        end;
    end;

    local procedure FindWorkOrder()
    begin
        // P8000333A
        if WorkOrder.ReadPermission then begin
            WorkOrder.Reset;
            WorkOrder.SetFilter("No.", DocNoFilter);
            InsertIntoDocEntry(
              Rec, DATABASE::"Work Order", 0, WorkOrder.TableCaption, WorkOrder.Count); // P80066030
        end;
    end;

    local procedure FindMaintLedgEntry()
    begin
        // P8000333A
        if MaintLedgEntry.ReadPermission then begin
            MaintLedgEntry.Reset;
            MaintLedgEntry.SetCurrentKey("Document No.", "Posting Date");
            MaintLedgEntry.SetFilter("Document No.", DocNoFilter);
            MaintLedgEntry.SetFilter("Posting Date", PostingDateFilter);
            InsertIntoDocEntry(
              Rec, DATABASE::"Maintenance Ledger", 0, MaintLedgEntry.TableCaption, MaintLedgEntry.Count); // P80066030
        end;
    end;

    local procedure FindRepackOrder()
    begin
        // P8000496A
        if RepackOrder.ReadPermission then begin
            RepackOrder.Reset;
            RepackOrder.SetFilter("No.", DocNoFilter);
            InsertIntoDocEntry(
              Rec, DATABASE::"Repack Order", 0, RepackOrder.TableCaption, RepackOrder.Count); // P80066030
        end;
    end;
    // P800-MegaApp
    ...
    begin
        ...
        if ItemTrackingSearch then
            ...
            case Rec."Table ID" of
                ...
                // P8000891
                DATABASE::"Posted Comm. Manifest Header":
                    if "No. of Records" = 1 then
                        PAGE.Run(PAGE::"Posted Commodity Manifest", PstdCommManifestHeader)
                    else
                        PAGE.Run(0, PstdCommManifestHeader);
                // P8000891
                // P8000941
                DATABASE::"Posted Sales Payment Header":
                    if "No. of Records" = 1 then
                        PAGE.Run(PAGE::"Posted Sales Payment Card", PstdSalesPaymentHeader)
                    else
                        PAGE.Run(0, PstdSalesPaymentHeader);
                // P8000941
                // P8001082
                DATABASE::"Reg. Pre-Process Activity":
                    case "Document Type" of
                        RegPreProcessActivity.Blending::" ":
                            if "No. of Records" = 1 then
                                PAGE.Run(PAGE::"Reg. Pre-Process Activity", RegPreProcessActivity)
                            else
                                PAGE.Run(0, RegPreProcessActivity);
                        RegPreProcessActivity.Blending::"Per Order":
                            if "No. of Records" = 1 then
                                PAGE.Run(PAGE::"Reg. Pre-Process Activity", RegPreProcessBlendActivity)
                            else
                                PAGE.Run(0, RegPreProcessBlendActivity);
                    end;
                // P8001082
                DATABASE::"G/L Entry":
                    PAGE.Run(0, GLEntry);
                ...
                // P8000119A
                DATABASE::"Accrual Ledger Entry":
                    PAGE.Run(0, AccrualLedgEntry);
                // P8000119A
                // P8000140A
                DATABASE::"Container Ledger Entry":
                    PAGE.Run(0, ContLedgEntry);
                // P8000140A
                // P8000333A
                DATABASE::"Work Order":
                    PAGE.Run(0, WorkOrder);
                DATABASE::"Maintenance Ledger":
                    PAGE.Run(0, MaintLedgEntry);
                // P8000333A
                // P8000496A
                DATABASE::"Repack Order":
                    begin
                        RepackOrder.FindFirst;
                        case RepackOrder.Status of
                            RepackOrder.Status::Open:
                                PAGE.Run(PAGE::"Repack Order", RepackOrder);
                            RepackOrder.Status::Finished:
                                PAGE.Run(PAGE::"Finished Repack Order", RepackOrder);
                        end;
                    end;
                // P8000496A
    ...
    begin
        if (NewDocNo = '') and (NewPostingDate = 0D) and not NewItemTrackingSetup.TrackingExists() then begin
            ...
            if NewItemTrackingSetup.TrackingExists() then begin
                ...
                if NewPostingDate <> 0D then // P8000333A
