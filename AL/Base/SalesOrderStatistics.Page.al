page 402 "Sales Order Statistics"
{
    ...
    // 
    // P8000761, VerticalSoft, MMAS, 24 FEB 10
    //   Off-Invoice Allowance (tabs General,Invoicing,Shipping)
    //   Add Line by Line Profit Summary (SubForm added)
    //   New method SetSalesLines()
    // 
    // PRW16.00.05
    ...
    layout
    {
        area(content)
        {
            group(General)
            {
                ...
                field("Allowance1[1] + Allowance2[1]"; Allowance1[1] + Allowance2[1])
                {
                ...
                field("TotalSalesLineLCY[1].""Line Amount (Freight)"""; TotalSalesLineLCY[1]."Line Amount (Freight)")
                {
                    ApplicationArea = FOODBasic;
                    AutoFormatExpression = "Currency Code";
                    AutoFormatType = 1;
            ...
            group(Invoicing)
            {
                ...
                field("Allowance1[2] + Allowance2[2]"; Allowance1[2] + Allowance2[2])
                {
                    ApplicationArea = FOODBasic;
                    AutoFormatExpression = "Currency Code";
                    AutoFormatType = 1;
                ...
                field("TotalSalesLineLCY[2].""Line Amount (Freight)"""; TotalSalesLineLCY[2]."Line Amount (Freight)")
                {
                    ApplicationArea = FOODBasic;
                    AutoFormatExpression = "Currency Code";
                    AutoFormatType = 1;
            ...
            group(Shipping)
            {
                ...
                field("Allowance1[3] + Allowance2[3]"; Allowance1[3] + Allowance2[3])
                {
                    ApplicationArea = FOODBasic;
                    AutoFormatExpression = "Currency Code";
                    AutoFormatType = 1;
                ...
                field("TotalSalesLineLCY[3].""Line Amount (Freight)"""; TotalSalesLineLCY[3]."Line Amount (Freight)")
                {
                    ApplicationArea = FOODBasic;
                    AutoFormatExpression = "Currency Code";
                    AutoFormatType = 1;
    ...
    var
        ...
        Allowance1: array[3] of Decimal;
        Allowance2: array[3] of Decimal;
        Allowance2LCY: array[3] of Decimal;
    ...
    begin
        ...
        for i := 1 to 3 do begin
            ...
            // P8000921
            if (TotalSalesLine[i]."Line Amount (Freight)" <> 0) then begin
                TotalSalesLineLCY[i].Amount := TotalSalesLineLCY[i].Amount - TotalSalesLineLCY[i]."Line Amount (Freight)";
                ProfitLCY[i] := TotalSalesLineLCY[i].Amount - TotalSalesLineLCY[i]."Unit Cost (LCY)";
                if (TotalSalesLineLCY[i].Amount = 0) then
                    ProfitPct[i] := 0
                else
                    ProfitPct[i] := Round(ProfitLCY[i] / TotalSalesLineLCY[i].Amount * 100, 0.1);
            ...
            // P8000761
            if Process800Fns.PricingInstalled then begin
                case i of
                    1:
                        TransType := 'ORDER';
                    2:
                ...
                OffInvoiceFns.TotalAllowance(Rec, TransType, Allowance1[i], Allowance2[i]);
                Allowance2LCY[i] := Round(
                  CurrExchRate.ExchangeAmtFCYToLCY("Posting Date", "Currency Code", Allowance2[i], "Currency Factor"));
                TotalSalesLine[i]."Line Amount" += Allowance1[i];
                TotalAmount1[i] -= Allowance2[i];
                TotalAmount2[i] -= Allowance2[i];
                TotalSalesLineLCY[i].Amount -= Allowance2LCY[i];
                ProfitLCY[i] -= Allowance2LCY[i];
                if TotalSalesLineLCY[i].Amount = 0 then
                    ProfitPct[i] := 0
                else
                    ProfitPct[i] := Round(100 * ProfitLCY[i] / TotalSalesLineLCY[i].Amount, 0.01);
            end;
            // P8000761
        end;
    ...
    begin
        ...
        TotalAmount1[IndexNo] := TotalSalesLine[IndexNo]."Line Amount" - TotalSalesLine[IndexNo]."Inv. Discount Amount" - // P8000761
          Allowance1[IndexNo] - Allowance2[IndexNo];                                                                      // P8000761
        ...
        TotalSalesLineLCY[IndexNo].Amount :=                                                      // P8000921
          TotalSalesLineLCY[IndexNo].Amount - TotalSalesLineLCY[IndexNo]."Line Amount (Freight)"; // P8000921
    ...
    begin
        ...
        with TotalSalesLine[IndexNo] do
            "Inv. Discount Amount" := "Line Amount" - TotalAmount1[IndexNo] - // P8000761
            Allowance1[IndexNo] - Allowance2[IndexNo];                        // P8000761
    ...
    var
        TempSalesLine: Record "Sales Statistic Line" temporary;
        SalesStatFns: Codeunit "Sales Statistics Functions";
    begin
        // PR3.70
        SalesStatFns.GetTempSalesLine("Document Type", "No.", TempSalesLine);
        CurrPage.SalesLine.PAGE.SetTempSalesLine(TempSalesLine);
        // PR3.70
    end;
