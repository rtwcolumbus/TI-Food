table 7354 Bin
{
    ...
    fields
    {
        ...
        field(37002100; "Lot Combination Method"; Option)
        {
            Caption = 'Lot Combination Method';
            OptionCaption = 'Manual,Assign New Lot,Use Existing Lot';
            OptionMembers = Manual,"Assign New Lot","Use Existing Lot";

            trigger OnValidate()
            begin
                // P8000890
                if ("Lot Combination Method" <> xRec."Lot Combination Method") then begin
                    CheckEmptyBin(Text007);
                    TestField("Current Lot No.", '');
                end;
            end;
        }
        field(37002101; "Current Item No."; Code[20])
        {
            Caption = 'Current Item No.';
            Editable = false;
            TableRelation = Item;
        }
        field(37002102; "Current Variant Code"; Code[10])
        {
            Caption = 'Current Variant Code';
            Editable = false;
            TableRelation = "Item Variant".Code WHERE("Item No." = FIELD("Current Item No."));
        }
        field(37002103; "Current Lot No."; Code[50])
        {
            Caption = 'Current Lot No.';
            Editable = false;
            TableRelation = "Lot No. Information"."Lot No." WHERE("Item No." = FIELD("Current Item No."),
                                                                   "Variant Code" = FIELD("Current Variant Code"));
        }
    }
    ...
    begin
        ...
        // P8000494A
        FixedBinItem.Reset;
        FixedBinItem.SetCurrentKey("Location Code", "Bin Code");
        FixedBinItem.SetRange("Location Code", "Location Code");
        FixedBinItem.SetRange("Bin Code", Code);
