report 5801 "Invt. Valuation - Cost Spec."
{
    ...
    // 
    // PR4.00.05
    // P8000463A, VerticalSoft, Jack Reynolds, 28 MAR 07
    //   Change reference to item ledger quantity to costing quantity
    // 
    // PRW17.00
    // P8001132, Columbus IT, Jack Reynolds, 27 MAR 13
    //   Upgrade for NAV 2013
    DefaultLayout = RDLC;
    RDLCLayout = './layout/InvtValuationCostSpec.rdlc';
    ...
    begin
        RemainingQty := ItemLedgerEntry.GetCostingQty; // P8000301A
        if IsPositive then
            PosQty := ItemLedgerEntry.GetCostingQty; // P8000301A
        ...
        with ItemApplnEntry do
            if ItemLedgerEntry.Positive then begin
                ...
                if Find('-') then
                    repeat
                        SumQty(RemainingQty, PosQty, "Outbound Item Entry No.", GetCostingQty); // P8000301A
                    until Next() = 0;
            end else begin
                Reset;
                ...
                if Find('-') then
                    repeat
                        SumQty(RemainingQty, PosQty, "Inbound Item Entry No.", -GetCostingQty); // P8000301A
                    until Next() = 0;
            end;
            ...
            if Find('-') then
                repeat
                    if "Partial Revaluation" then
                        ...
                        SumUnitCost(TotalCostTotal["Entry Type".AsInteger() + 1],
                          "Cost Amount (Actual)" + "Cost Amount (Expected)", ItemLedgerEntry.GetCostingQty); // P800463
