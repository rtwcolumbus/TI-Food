        ...
        repeat
            ...
            if (ToCostShareBuf."Posting Date" <= EndDate) or (EndDate = 0D) then begin
                if (EndDate = 0D) or (ToItemLedgEntry."Posting Date" <= EndDate) and
                   //(FromCostShareBuf.Quantity * ItemApplnEntry.Quantity < 0)    // P8000466A
                   (FromCostShareBuf.Quantity * ItemApplnEntry.GetCostingQty < 0) // P8000466A
                then
                ...
                with ToCostShareBuf do begin
                    ...
                    //CostShare := ItemApplnEntry.Quantity / FromCostShareBuf.Quantity;    // P8000466A
                    CostShare := ItemApplnEntry.GetCostingQty / FromCostShareBuf.Quantity; // P8000466A
