...
{
    // PR1.00
    //   Maintain Production Order Xref when deleting
    //   New Process 800 fields
    //     Qty. in Batch (Base)
    // 
    // PR1.20
    //   Update "Remaining Quantity (Base)" when updating "Quantity (Base)"
    // 
    // PR2.00
    //   Deleteing Production Order Xref changed because of field changes in Production Order Xref
    // 
    // PR3.60
    ...
    // 
    // PR3.70.03
    //   Don't move BOM onto production order line for Process Family Orders
    // 
    // PR3.70.04
    ...
    //   Allow routing number to be overridden from BOM equipment
    // 
    // P8000260A, Myers Nissi, Jack Reynolds, 27 OCT 05
    //   Modify IsCompletelyOInvoiced to allow production output to be costed when order is finished and
    //     before all consumption is invoiced
    // 
    // PR4.00.01
    ...
    // 
    // PR5.00
    // P8000466A, VerticalSoft, Jack Reynolds, 20 JUN 07
    //   Helper functions for costing quantity
    // 
    ...
    //   Modify "Qty. in Batch (Base)" DecimalPlaces
    // 
    // P8000877, Columbus IT, Jack Reynolds, 02 MAR 11
    //   Support for Equipment Code on order lines
    // 
    // P8000889, Columbus IT, Jack Reynolds, 02 MAR 11
    //   Key on Equipment Code
    // 
    // P8000898, Columbus IT, Jack Reynolds, 14 FEB 11
    //   Support for Daily Production Plan report
    // 
    // PRW16.00.05
    // P8000925, Columbus IT, Jack Reynolds, 29 MAR 11
    //   Set routing number from equipment code on prod order line
    // 
    // PRW16.00.06
    // P8001030, Columbus IT, Jack Reynolds, 30 MAY 12
    //   Support for BOM/Routing on SKU
    // 
    // P8001073, Columbus IT, Jack Reynolds, 12 JUN 12
    //   Fix problem deleting orders that have been linked to batch orders
    // 
    // P8001092, Columbus IT, Don Bresee, 17 AUG 12
    //   Add "Co-Product Cost Share" & "Primary Co-Product" fields
    // 
    ...
    // 
    // P8001129, Columbus IT, Don Bresee, 17 JAN 13
    //   Fix incorrect base qty calculation
    // 
    // PRW17.00
    ...
    //   Problem with Batch Worksheet Quantity Planned
    // 
    // PRW110.0.03
    // P80056618, To-Increase, Jack Reynolds, 03 APR 18
    //   Fix problem setting unit cost for alternate quantity items
    // 
    // PRW111.00.01
    ...
    fields
    {
        ...
        field(11; "Item No."; Code[20])
        {
            ...
            begin
                ...
                if "Item No." = '' then
                    ...
                    "Variant Code" := ProdOrder."Variant Code"; // P8001030
                    "Bin Code" := ProdOrder."Bin Code";
                    "Replenishment Area Code" := ProdOrder."Replenishment Area Code"; // P8000494A
                    "Work Shift Code" := ProdOrder."Work Shift Code";       // P8000494A
                    ...
                    //"Production BOM No." := Item."Production BOM No.";                          // P8001030
                    "Production BOM No." := Item.ProductionBOMNo("Variant Code", "Location Code"); // P8001030
                                                                                                   //"Routing No." := Item."Routing No.";    // P8000219A
                    "Routing No." := GetRoutingNo(ProdOrder); // P8000219A, P8001030
                    ...
                    // PR3.60
                    //IF P800ProdOrderMgmt.IsProdFamilyProcess(Rec) THEN // PR3.70.03
                    //  Item.TESTFIELD("Production BOM No.", '');        // PR3.70.03
                    "Unit Cost (Costing Units)" := Item."Unit Cost";
                    "Unit Cost" := "Unit Cost" * Item.CostingQtyPerBase();
                    "Overhead Rate" := "Overhead Rate" * Item.CostingQtyPerBase();
                    // PR3.60
		    OnValidateItemNoOnAfterAssignItemValues(Rec, Item);

                    Validate("Label Unit of Measure Code", Item."Label Unit of Measure"); // P8001233

                    if "Item No." <> xRec."Item No." then begin
                        if not P800ProdOrderMgmt.IsProdFamilyProcess(Rec) then // PR3.70.03
                                                                               //VALIDATE("Production BOM No.",Item."Production BOM No.");                          // P8001030
                            Validate("Production BOM No.", Item.ProductionBOMNo("Variant Code", "Location Code")); // P8001030
                                                                                                                   //VALIDATE("Routing No.",Item."Routing No.");    // P8000219A
                        Validate("Routing No.", GetRoutingNo(ProdOrder)); // P8000219A, P8001030
                ...
                "Overhead Rate" := Item.OverheadRate("Variant Code", "Location Code") * "Qty. per Unit of Measure" * Item.CostingQtyPerBase;
                Validate("Production BOM No.", Item.ProductionBOMNo("Variant Code", "Location Code"));
                ProdOrder.Get(Status, "Prod. Order No.");
                Validate("Routing No.", GetRoutingNo(ProdOrder));
                // P80056618
        ...
        field(20; "Location Code"; Code[10])
        {
            ...
            trigger OnValidate()
            begin
                if ("Location Code" <> xRec."Location Code") then // P8000631A
                    "Replenishment Area Code" := '';                // P8000631A
        ...
        field(40; Quantity; Decimal)
        {
            ...
            begin
                ...
                P800ProdOrderMgmt.SetProdOrderQuantity(Rec, FieldNo(Quantity)); // PR3.60
            end;
        }
        ...
        field(60; "Production BOM No."; Code[20])
        {
            ...
            begin
                ...
                TestField("Production BOM Version Code"); // P8001030
        ...
        field(65; "Unit Cost"; Decimal)
        {
            ...
            begin
                ...
                if Item."Costing Method" = Item."Costing Method"::Standard then begin
                    ...
                    if GetSKU() then
                        "Unit Cost" := SKU."Unit Cost" * Item.CostingQtyPerBase   // PR3.70
                    else
                        "Unit Cost" := Item."Unit Cost" * Item.CostingQtyPerBase; // PR3.70
                end;

                "Cost Amount" := Round(Quantity * "Unit Cost");

                P800ProdOrderMgmt.SetProdOrderUnitCost(Rec, FieldNo("Unit Cost")); // PR3.60
            end;
        }
        ...
        field(80; "Unit of Measure Code"; Code[10])
        {
            ...
            begin
                ...
                "Unit Cost" :=
                  // ROUND(Item."Unit Cost" * "Qty. per Unit of Measure",                         // PR3.60
                  Round(Item."Unit Cost" * "Qty. per Unit of Measure" * Item.CostingQtyPerBase(), // PR3.60
                    GLSetup."Unit-Amount Rounding Precision");

                P800ProdOrderMgmt.SetProdOrderUnitCost(Rec, FieldNo("Unit of Measure Code")); // PR3.60
        ...
        field(81; "Quantity (Base)"; Decimal)
        {
            ...
            begin
                ...
                //TESTFIELD("Qty. per Unit of Measure",1); // PR1.00
                Quantity := "Quantity (Base)" / "Qty. per Unit of Measure"; // PR1.00
                "Remaining Quantity" := Quantity - "Finished Quantity";
                // "Remaining Qty. (Base)" := "Remaining Quantity" / "Qty. per Unit of Measure"; // PR1.20, P8001129
                "Remaining Qty. (Base)" := "Remaining Quantity" * "Qty. per Unit of Measure";    // P8001129

                Validate("Ending Time");

                "Cost Amount" := Round(Quantity * "Unit Cost");

                P800ProdOrderMgmt.SetProdOrderQuantity(Rec, FieldNo("Quantity (Base)")); // PR3.60
            end;
        }
        ...
        field(37002020; "Lot No."; Code[50])
        {
            ...
            begin
                ...
                if xRec."Lot No." = P800Globals.MultipleLotCode then
                    FieldError("Lot No.", Text37002000);
        ...
        field(37002081; "Quantity (Alt.)"; Decimal)
        {
            ...
            trigger OnValidate()
            begin
                P800ProdOrderMgmt.SetProdOrderQuantity(Rec, FieldNo("Quantity (Alt.)")); // PR3.60
            end;
        }
        field(37002091; "Finished Qty. (Alt.)"; Decimal)
        {
            AccessByPermission = TableData "Alternate Quantity Line" = R;
            BlankZero = true;
            CaptionClass = StrSubstNo('37002080,0,9,%1', "Item No.");
            Caption = 'Finished Qty. (Alt.)';
            DecimalPlaces = 0 : 5;
            Description = 'PR3.60';
            Editable = false;
        }
        field(37002093; "Unit Cost (Costing Units)"; Decimal)
        {
            ...
            begin
                ...
                if (Item."Costing Method" = Item."Costing Method"::Standard) then
                    if (CurrFieldNo = FieldNo("Unit Cost (Costing Units)")) then
                        Error(Text99000002,
                              FieldCaption("Unit Cost"),
                              Item.FieldCaption("Costing Method"), Item."Costing Method");

                P800ProdOrderMgmt.SetProdOrderUnitCost(Rec, FieldNo("Unit Cost (Costing Units)"));
                // PR3.60
            end;
        }
        field(37002460; "Qty. in Batch (Base)"; Decimal)
        {
            CalcFormula = Sum("Production Order XRef"."Quantity (Base)" WHERE("Source Table ID" = CONST(5406),
                                                                               "Source Type" = FIELD(Status),
                                                                               "Source No." = FIELD("Prod. Order No."),
                                                                               "Source Line No." = FIELD("Line No.")));
            Caption = 'Qty. in Batch (Base)';
            DecimalPlaces = 0 : 5;
            Description = 'PR1.00';
            Editable = false;
            FieldClass = FlowField;
        ...
        field(37002462; "Equipment Code"; Code[20])
        {
            ...
            begin
                ...
                ProdOrder.Get(Status, "Prod. Order No.");
                //GetItem;                                       // P8001030
                Validate("Routing No.", GetRoutingNo(ProdOrder)); // P8001030
                // P8000925
            end;
        }
        field(37002600; "By-Product"; Boolean)
        {
            ...
            trigger OnValidate()
            begin
                P800ProdOrderMgmt.SetProdOrderUnitCost(Rec, FieldNo("By-Product")); // PR3.60
            end;
        }
        field(37002601; "Unit Cost (By-Product)"; Decimal)
        {
            Caption = 'Unit Cost (By-Product)';
            Description = 'PR3.60';
        }
        ...
        field(37002870; "Actual Start Date"; Date)
        {
            ...
            begin
                // P8001090
                "Actual Start Time" := 0T;
            end;
        }
        field(37002871; "Actual Start Time"; Time)
        {
            Caption = 'Actual Start Time';
        }
        field(37002872; "Actual Stop Date"; Date)
        {
            Caption = 'Actual Stop Date';
        ...
        field(37002873; "Actual Stop Time"; Time)
        {
            Caption = 'Actual Stop Time';
        }
        field(99000750; "Production BOM Version Code"; Code[20])
        {
            ...
            begin
                ...
                // P80085384
                if (ProdOrder.Status <> Status) and (ProdOrder."No." <> "Prod. Order No.") then
                    ProdOrder.Get(Status, "Prod. Order No.");
                if not ProdOrder."Family Process Order" then
                    // P80085384
    ...
    keys
    {
        ...
        key(Key13; "Equipment Code", "Starting Date")
        {
    ...
    begin
        ...
        if ProcessFns.ProcessInstalled() then      // P8001082
            PreProcessMgmt.CancelBlendingOrder(Rec); // P8001082
    ...
    var
        ...
        Text37002000: Label 'may not be edited';
    ...
    begin
        ...
        if ("Line No." <> 0) then begin // PR3.60
        ...
        // PR1.00 Begin
        ProdXref.Reset();
        ProdXref.SetCurrentKey("Source Table ID", "Source Type", "Source No.", "Source Line No.");
        ProdXref.SetRange("Source Table ID", DATABASE::"Prod. Order Line");
        ProdXref.SetRange("Source Type", Status);
        ProdXref.SetRange("Source No.", "Prod. Order No.");
        ProdXref.SetRange("Source Line No.", "Line No.");
        ProdXref.DeleteAll(); // P8001073

        ProdXref.Reset();
        ProdXref.SetCurrentKey("Prod. Order Status", "Prod. Order No.");
        ProdXref.SetRange("Prod. Order Status", Status);
        ProdXref.SetRange("Prod. Order No.", "Prod. Order No.");
        ProdXref.SetRange("Prod. Order Line No.", "Line No.");
    ...
    begin
        ...
        if "Variant Code" <> '' then
            if ItemVariant.Get("Item No.", "Variant Code") then
                exit(ItemVariant."Production BOM No.");
        // PR2.00.05 End
    end;

    procedure PkgBOMVersion(): Code[20]
    var
        BOMNo: Code[20];
    begin
        // PR2.00.05 Begin
        BOMNo := PkgBOMNo;
        if BOMNo <> '' then
            exit(VersionMgt.GetBOMVersion(BOMNo, "Due Date", true));
        // PR2.00.05 End
    end;

    procedure PkgBOMUOM(): Code[10]
    var
        ItemVariant: Record "Item Variant";
    begin
        // PR2.00.05 Begin
        if "Variant Code" <> '' then
            if ItemVariant.Get("Item No.", "Variant Code") then
                exit(ItemVariant."Unit of Measure Code");
        // PR2.00.05 End
    end;
    ...
    begin
        ...
        EasyLotTracking.TestProdOrderLine(Rec);
        if "Line No." = 0 then
            exit;

        QtyToHandle := "Remaining Qty. (Base)";
        //QtyToHandleAlt := "remaining qty. (Alt.)";
        EasyLotTracking.SetProdOrderLine(Rec);
        EasyLotTracking.ReplaceTracking(xRec."Lot No.", "Lot No.", 0,
          "Quantity (Base)", QtyToHandle, 0, "Quantity (Base)");
    end;

    procedure GetRoutingNo(ProdOrder: Record "Production Order") RoutingNo: Code[20]
    var
        ProdBOMEquipment: Record "Prod. BOM Equipment";
    begin
        // P8000219A
        // P8001030 -  modified to return result
        GetItem; // P8001030
        RoutingNo := Item.RoutingNo("Variant Code", "Location Code"); // P8001030
        if ProcessFns.ProcessInstalled then
            // P8000925
            if ProdBOMEquipment.Get("Production BOM No.", "Production BOM Version Code", "Equipment Code") then begin
                if ProdBOMEquipment."Routing No." <> '' then
                    RoutingNo := ProdBOMEquipment."Routing No."; // P8001030
            end else
                // P8000925
                if ProdBOMEquipment.Get("Production BOM No.", "Production BOM Version Code", ProdOrder."Equipment Code") then
                    if ProdBOMEquipment."Routing No." <> '' then
                        RoutingNo := ProdBOMEquipment."Routing No."; // P8001030
    end;
    ...
    local procedure UpdateReplFieldOnComps()
    var
        ProdOrderComp: Record "Prod. Order Component";
    begin
        // P8000631A
        ...
        if ProdOrderComp.FindSet then
            repeat
                ...
                if (ProdOrderComp."Location Code" = "Location Code") then begin
                    ProdOrderComp.Validate("Replenishment Area Code", "Replenishment Area Code");
                    ProdOrderComp.Modify(true);
                end;
            until (ProdOrderComp.Next = 0);
    end;

    procedure GetEquipmentCode()
    var
        ProdOrder: Record "Production Order";
    begin
        // P8000877
        if "Equipment Code" = '' then begin
            ProdOrder.Get(Status, "Prod. Order No.");
            "Equipment Code" := ProdOrder."Equipment Code";
        end;
    end;
    ...
    procedure BatchQuantityBase(): Decimal
    var
        ProductionOrderXRef: Record "Production Order XRef";
    begin
        // P80046223
        ProductionOrderXRef.SetRange("Prod. Order Status", Status);
        ProductionOrderXRef.SetRange("Prod. Order No.", "Prod. Order No.");
        ProductionOrderXRef.SetRange("Prod. Order Line No.", "Line No.");
        ProductionOrderXRef.CalcSums("Quantity (Base)");
        exit(ProductionOrderXRef."Quantity (Base)");
    end;
