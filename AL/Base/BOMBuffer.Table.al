table 5870 "BOM Buffer"
{
    ...
    fields
    {
        ...
        field(37002460; "Qty. per Unit of Measure"; Decimal)
        {
    ...
    begin
        ...
        InitFromItem(BOMItem, AsmHeader."Variant Code", AsmHeader."Location Code", AsmHeader."Unit of Measure Code"); // P8001144
    ...
    begin
        ...
        InitFromItem(BOMItem, AsmLine."Variant Code", AsmLine."Location Code", AsmLine."Unit of Measure Code"); // P8001144
        ...
        case BOMComp.Type of
            BOMComp.Type::Item:
                begin
                    ...
                    InitFromItem(BOMItem, BOMComp."Variant Code", ParentLocationCode, BOMComp."Unit of Measure Code"); // P8001144
                end;
    ...
    begin
        ...
        TransferFromProdComp(EntryNo, ProdBOMLine, NewIndentation, NewIndentation, ParentQtyPer, 1, 1, ParentScrapQtyPer, NeedByDate, ParentLocationCode, ParentItem, BOMQtyPerUOM);
    end;

    procedure TransferFromProdComp(var EntryNo: Integer; ProdBOMLine: Record "Production BOM Line"; NewIndentation: Integer; ParentQtyPer: Decimal; ParentQtyPerUOM: Decimal; ParentCoProdBOMFactor: Decimal; ParentScrapQtyPer: Decimal; ParentScrapPct: Decimal; NeedByDate: Date; ParentLocationCode: Code[10]; ParentItem: Record Item; BOMQtyPerUOM: Decimal)
    var
        BOMItem: Record Item;
        UOMMgt: Codeunit "Unit of Measure Management";
        CostCalculationMgt: Codeunit "Cost Calculation Management";
    begin
        // P8001144 - Add ParentQtyPerUOM and ParentCoProdBOMFactor parameters
        Init;
        EntryNo += 1;
        "Entry No." := EntryNo;
        Type := Type::Item;
        ...
        InitFromItem(BOMItem, ProdBOMLine."Variant Code", ParentLocationCode, ProdBOMLine."Unit of Measure Code"); // P8001144
        ...
        "Qty. per Parent" :=
          ...
          UOMMgt.GetQtyPerUnitOfMeasure(BOMItem, ProdBOMLine."Unit of Measure Code") /
          BOMQtyPerUOM / ParentItem."Lot Size";
        "Qty. per Parent" := "Qty. per Parent" * ParentCoProdBOMFactor;
        "Qty. per Top Item" := Round(ParentQtyPer * "Qty. per Parent", UOMMgt.QtyRndPrecision);
        "Qty. per Parent" := Round("Qty. per Parent" * ParentQtyPerUOM, UOMMgt.QtyRndPrecision);

        "Scrap Qty. per Parent" := "Qty. per Parent" - ProdBOMLine."Quantity per" * ParentCoProdBOMFactor * ParentQtyPerUOM;
        "Scrap Qty. per Top Item" :=
          "Qty. per Top Item" -
          Round(((ParentQtyPer - ParentScrapQtyPer) / ParentQtyPerUOM) * ("Qty. per Parent" - "Scrap Qty. per Parent"), UOMMgt.QtyRndPrecision);
    ...
    begin
        ...
        InitFromItem(BOMItem, ProdOrderLine."Variant Code", ProdOrderLine."Location Code", ProdOrderLine."Unit of Measure Code"); // P8001144
    ...
    begin
        ...
        InitFromItem(BOMItem, ProdOrderComp."Variant Code", ProdOrderComp."Location Code", ProdOrderComp."Unit of Measure Code"); // P8001144
        ...
        if ("Needed by Date" <> 0D) and (ProdOrderComp."Pre-Process Lead Time (Days)" <> 0) then // P8001144
            "Needed by Date" := "Needed by Date" - ProdOrderComp."Pre-Process Lead Time (Days)";   // P8001144
        Indentation := 1;
    ...
    begin
        InitFromItem(Item, '', '', ''); // P80096165
    end;

    local procedure InitFromItem(Item: Record Item; VariantCode: Code[10]; LocationCode: Code[10]; UOMCode: Code[10])
    var
    ...
    begin
        ...
        if (UOMCode = '') then
            ...
            "Qty. per Unit of Measure" := UOMMgt.GetQtyPerUnitOfMeasure(Item, UOMCode);
        end;
        Item.InitFromSKU(VariantCode, LocationCode);
        // P8001144
    ...
    begin
        ...
        Item.Get("No.");
        Item.InitFromSKU("Variant Code", "Location Code"); // P8001144
