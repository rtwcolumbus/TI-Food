table 99000829 "Planning Component"
{
    ...
    //   Add fields: Step Code, Production BOM No., Production BOM Version Code, Production BOM Line No.
    // 
    // PRW16.00.06
    ...
    fields
    {
        ...
        field(13; "Unit of Measure Code"; Code[10])
        {
            ...
            begin
                ...
                //"Indirect Cost %" := Round(Item."Indirect Cost %" * "Qty. per Unit of Measure", UOMMgt.QtyRndPrecision); // P8001030
        ...
        field(21; "Variant Code"; Code[10])
        {
            ...
            begin
                ...
                "Overhead Rate" :=
                  Round(Item.OverheadRate("Variant Code", "Location Code") * "Qty. per Unit of Measure",
                    GLSetup."Unit-Amount Rounding Precision");
                // P8001030
            end;
        }
        ...
        field(25; "Expected Quantity"; Decimal)
        {
            ...
            begin
                if Item.Get("Item No.") and ("Ref. Order Type" <> "Ref. Order Type"::Assembly) then
                    if Item.GetItemUOMRndgPrecision("Unit of Measure Code", false) then   // PR3.70.03
        ...
        field(50; "Unit Cost"; Decimal)
        {
            ...
            begin
                ...
                if Item."Costing Method" = Item."Costing Method"::Standard then begin
                    ...
                    "Indirect Cost %" :=
                      //   Round(Item."Indirect Cost %" * "Qty. per Unit of Measure", UOMMgt.QtyRndPrecision); // P8001030
                      Item.IndirectCostPct("Variant Code", "Location Code");                  // P8001030
                    "Overhead Rate" := Item.OverheadRate("Variant Code", "Location Code");    // P8001030
                    "Direct Unit Cost" :=
        ...
        field(56; "Indirect Cost %"; Decimal)
        {
            ...
            begin
                "Direct Unit Cost" :=
                  ...
                  Round(("Unit Cost" - "Overhead Rate" * "Qty. per Unit of Measure") / (1 + "Indirect Cost %" / 100));  // PR3.10
            end;
        }
    ...
    begin
        ...
        // P8000386A
        "Step Code" := ProdOrderComp."Step Code";
        "Production BOM No." := ProdOrderComp."Production BOM No.";
        "Production BOM Version Code" := ProdOrderComp."Production BOM Version Code";
        "Production BOM Line No." := ProdOrderComp."Production BOM Line No.";
        // P8000386A
        UpdateDatetime;
