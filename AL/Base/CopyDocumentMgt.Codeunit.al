codeunit 6620 "Copy Document Mgt."
{
    ...
    // 
    // PR3.70.01
    //   Fix problem with copy lines not clearing alternate quantity and container data
    //   Extra Charges
    // 
    // PR3.70.02
    //   Fix problem copying sales lines
    // 
    // PR3.70.07
    ...
    //   OnRun - change call to CopyLotPrefCustomerToSalesLine
    // 
    // PR5.00
    // P8000466A, VerticalSoft, Jack Reynolds, 31 JUL 07
    //   Run GetLotNo on sales and purchase lines after copy tracking to set Lot No. field on these lines
    // 
    // PRW16.00.02
    // P8000753, VerticalSoft, Jack Reynolds, 10 DEC 09
    //   Problem with Lot No. being copied to sales and purchase lines
    // 
    // PRW16.00.03
    // P8000818, VerticalSoft, Jack Reynolds, 29 APR 10
    //   Fix problem with alternate quantities for purchase and sales invoices and credit memos
    // 
    // PRW16.00.04
    // P8000885, VerticalSoft, Ron Davidson, 19 JAN 11
    //   Sales Contracts were added to the Copy Sales Documents functionality
    //   Added logic to check contract's validity and prompt user to select contract if check fails.
    // 
    // PRW16.00.05
    // P8000921, Columbus IT, Don Bresee, 08 APR 11
    //   Add FOB/Freight fields to Sales Line
    // 
    // P8000942, Columbus IT, Jack Reynolds, 05 MAY 11
    //   Set Salesperson Code and Purchaser Code from User ID
    // 
    // P8000943, Columbus IT, Jack Reynolds, 09 MAY 11
    //   Support for Receipt Date on Sales Return Orders
    // 
    // PRW17.00
    ...
    // 
    // PRW111.00.02
    // P80069190, To-Increase, Gangabhushan, 21 FEB 19
    //   TI-12625 - Return Sales Shipment can throw error
    // 
    // PRW113.00
    ...
    var
        ...
        TempSalesInvLine2: Record "Sales Invoice Line" temporary;
        TempReturnRcptLine: Record "Return Receipt Line" temporary;
        TempSalesCrMemoLine: Record "Sales Cr.Memo Line" temporary;
        AllowUnapprovedItems: Boolean;
        // FOOD
    ...
    procedure CopyFromPstdPurchECToHeader(var ToPurchHeader: Record "Purchase Header"; FromDocType: Option; var FromPurchRcptHeader: Record "Purch. Rcpt. Header"; var FromPurchInvHeader: Record "Purch. Inv. Header"; var FromReturnShptHeader: Record "Return Shipment Header"; var FromPurchCrMemoHeader: Record "Purch. Cr. Memo Hdr.")
    var
        TableID: Integer;
        DocNo: Code[20];
        LineNo: Integer;
    ...
    procedure CopyFromPstdPurchECToLine(var ToPurchLine: Record "Purchase Line"; FromDocType: Option; var FromPurchRcptLine: Record "Purch. Rcpt. Line"; var FromPurchInvLine: Record "Purch. Inv. Line"; var FromReturnShptLine: Record "Return Shipment Line"; var FromPurchCrMemoLine: Record "Purch. Cr. Memo Line")
    var
        TableID: Integer;
        DocNo: Code[20];
        LineNo: Integer;
    ...
    procedure GetSalesContract(ToSalesHeader: Record "Sales Header"; var ToSalesLine: Record "Sales Line"; FromDocType: Enum "Sales Document Type From"; FromDocNo: Code[20]; FromDocLineNo: Integer)
    var
        SalesContMgmt: Codeunit "Sales Contract Management";
        PriceCalcMgt: Codeunit "Sales Price Calc. Mgt.";
    begin
        // P8000885
        if (ToSalesLine."Contract No." = '') or RecalculateLines or
           (ToSalesHeader."Document Type" in [ToSalesHeader."Document Type"::"Credit Memo", ToSalesHeader."Document Type"::"Return Order"])
        then
            exit;
        ToSalesLine."Document Type" := ToSalesHeader."Document Type";
        ToSalesLine.Type := ToSalesLine.Type::Item;
        if not SalesContMgmt.VerifyContract(ToSalesHeader, ToSalesLine) then begin
            ToSalesLine."Contract No." := '';
            ToSalesLine."Outstanding Qty. (Contract)" := 0;
            ToSalesLine."Outstanding Qty. (Cont. Line)" := 0;
            PriceCalcMgt.FindSalesLinePrice(ToSalesHeader, ToSalesLine, 0);
            case FromDocType of
                "Sales Document Type From"::Quote,
                "Sales Document Type From"::"Blanket Order",
                "Sales Document Type From"::FOODStandingOrder, // PR3.60
                ...
                "Sales Document Type From"::"Credit Memo":
                    begin
                        TempSalesLine."Document Type" := GetSalesDocumentType(FromDocType);
                        TempSalesLine."Document No." := FromDocNo;
                        TempSalesLine."Line No." := FromDocLineNo;
                        TempSalesLine."Contract No." := ToSalesLine."Contract No.";
                        TempSalesLine.Insert;
                    end;
                "Sales Document Type From"::"Posted Shipment":
                    begin
                        TempSalesShptLine."Document No." := FromDocNo;
                        TempSalesShptLine."Line No." := FromDocLineNo;
                        TempSalesShptLine."Contract No." := ToSalesLine."Contract No.";
                        TempSalesShptLine.Insert;
                    end;
                "Sales Document Type From"::"Posted Invoice":
                    begin
                        TempSalesInvLine2."Document No." := FromDocNo;
                        TempSalesInvLine2."Line No." := FromDocLineNo;
                        TempSalesInvLine2."Contract No." := ToSalesLine."Contract No.";
                        TempSalesInvLine2.Insert;
                    end;
                "Sales Document Type From"::"Posted Credit Memo":
                    begin
                        TempSalesCrMemoLine."Document No." := FromDocNo;
                        TempSalesCrMemoLine."Line No." := FromDocLineNo;
                        TempSalesCrMemoLine."Contract No." := ToSalesLine."Contract No.";
                        TempSalesCrMemoLine.Insert;
                    end;
                "Sales Document Type From"::"Posted Return Receipt":
                    begin
                        TempReturnRcptLine."Document No." := FromDocNo;
                        TempReturnRcptLine."Line No." := FromDocLineNo;
                        TempReturnRcptLine."Contract No." := ToSalesLine."Contract No.";
    ...
    begin
        // P8001230
        AllowUnapprovedItems := AllowUnapproved;
    end;

    procedure ApprovedItem(PurchHeader: Record "Purchase Header"; PurchLine: Record "Purchase Line"): Boolean
    var
        Item: Record Item;
    ...
    begin
        ...
        if PurchHeader."Allow Unapproved Items" then
            exit(true);

        Item.Get(PurchLine."No.");
        if Item.VendorApprovalRequired then begin
            ItemVendor.SetRange("Vendor No.", PurchHeader."Buy-from Vendor No.");
            ItemVendor.SetRange("Item No.", PurchLine."No.");
            ItemVendor.SetRange("Variant Code", PurchLine."Variant Code");
    ...
    begin
        case FromDocType of
            ...
            FromDocType::"Arch. Return Order":
                exit("Sales Document Type"::"Return Order");
            // PR3.60
            FromDocType::FOODStandingOrder:
                exit("Sales Document Type"::FOODStandingOrder);
            else
    ...
    begin
        case SalesDocType of
            ...
            // P80096165
            SalesDocType::FOODStandingOrder:
                exit("Sales Document Type From"::FOODStandingOrder);
        end;
    end;
    ...
    begin
        with ToSalesHeader do begin
            ...
            "Applies-to ID" := '';
            "Opportunity No." := '';
            "Salesperson Code" := SetSalesperson("Salesperson Code"); // P8000942
            "Quote No." := '';
    ...
    begin
        ...
        CopyPurchDoc("Purchase Document Type From"::"Posted Credit Memo", FromDocNo, ToPurchaseHeader);
        InsertCancellationLine := false;
    end;
    ...
    begin
        with ToPurchHeader do begin
            ...
            // P8001230
            "Allow Unapproved Items" := AllowUnapprovedItems;
            Modify;
            // P8001230
            ...
            if FromPurchLine.Find('-') then
                repeat
                    if not ExtTxtAttachedToPosPurchLine(FromPurchHeader, MoveNegLines, FromPurchLine."Attached to Line No.") then
                        ...
                        then begin
                            CopyFromPurchECToLine(ToPurchLine, FromPurchLine); // PR3.70.01
    ...
    begin
        with ToPurchHeader do begin
            ...
            case FromDocType of
                ...
                "Purchase Document Type From"::"Credit Memo":
                    begin
                        ...
                        CopyFromPurchECToHeader(ToPurchHeader, FromPurchHeader); // PR3.70.01
                    end;
                "Purchase Document Type From"::"Posted Receipt":
                    begin
                        CopyPurchHeaderFromPostedReceipt(FromPurchRcptHeader, ToPurchHeader, OldPurchHeader);
                        // PR3.70.01
                        CopyFromPstdPurchECToHeader(
                            ToPurchHeader, FromDocType, FromPurchRcptHeader, FromPurchInvHeader, FromReturnShptHeader, FromPurchCrMemoHeader);
                        // PR3.70.01 
                    end;
                "Purchase Document Type From"::"Posted Invoice":
                    begin
            ...
            "Applies-to Doc. No." := '';
            "Applies-to ID" := '';
            "Purchaser Code" := ToPurchHeader.SetPurchaser("Purchaser Code"); // P8000942
            "Quote No." := '';
    ...
    begin
        ...
        // PR3.70.01 Begin
        if (FromSalesLine.Type = FromSalesLine.Type::FOODContainer) or
          (FromSalesLine."Container Line No." <> 0) // PR3.70.02
        then
            exit(false);
        // PR3.70.01 End
        ...
        // P8000885
        if TempSalesLine.Get(FromSalesLine."Document Type", FromSalesLine."Document No.", FromSalesLine."Line No.") then begin
            ToSalesLine."Contract No." := TempSalesLine."Contract No.";
            ToSalesLine.InitOutstandingQtyCont;
        end;
        // P8000885
        ...
        ToSalesLine."Alt. Qty. Transaction No." := 0; // PR3.70.01
        ...
        if ToSalesLine."Document Type" = ToSalesLine."Document Type"::"Return Order" then
            ToSalesLine."Receipt Date" := ToSalesLine."Shipment Date";
        // P8000943
    ...
    begin
        ...
        ToSalesLine.SetUnitPrice; // P8006632
        ToSalesLine.Validate("Unit Price (Freight)", FromSalesLine."Unit Price (Freight)"); // P8000921
        ToSalesLine.Validate("Unit Price", FromSalesLine."Unit Price");
        ToSalesLine.Validate("Line Discount %", FromSalesLine."Line Discount %");
    ...
    begin
        ...
        if RecalculateLines and not FromSalesLine."System-Created Entry" then begin
            ...
            ToSalesLine."Appl.-from Item Entry" := FromSalesLine."Appl.-from Item Entry";   // P8007924
            if IsDeferralToBeCopied(
                "Deferral Document Type"::Sales, ToSalesLine."Document Type".AsInteger(), FromSalesCommentDocTypeInt)
            then
            ...
            ToSalesLine.SetAltQtyWithNoHandling; // P8000818
    ...
    begin
        ...
        if (FromSalesLine.Type <> FromSalesLine.Type::" ") and (FromSalesLine."No." <> '') then begin
            ...
            if not (FromSalesLine.Type in [FromSalesLine.Type::Item, FromSalesLine.Type::Resource]) then begin
                ...
                then begin
                    ToSalesLine."Unit Price (FOB)" := 0;     // P8000921
                    ToSalesLine."Unit Price (Freight)" := 0; // P8000921
    ...
    var
        ...
        IsHandled: Boolean;
    begin
        if not ApprovedItem(ToPurchHeader, FromPurchLine) then // P8001230
            exit(false);                                        // P8001230
        ...
        ToPurchLine."Alt. Qty. Transaction No." := 0; // PR3.70.01
        ...
        then begin
            if RecalculateAmount then begin
                ToPurchLine.SetUnitPrice; // P8006632
    ...
    begin
        ...
        if RecalculateLines and not FromPurchLine."System-Created Entry" then begin
            ...
            ToPurchLine.SetAltQtyWithNoHandling; // P8000818
                ...
                if FromSalesLine.FindSet then
                    repeat
                        if not IsItemBlocked(FromSalesLine."No.") then begin
                            ...
                            // P8000885
                            ToSalesLine."Contract No." := FromSalesLine."Contract No.";
                            GetSalesContract(ToSalesHeader, ToSalesLine,
                                             FromSalesLine."Document Type", FromSalesLine."Document No.", FromSalesLine."Line No.");
                            // P8000885
            ...
            if FromSalesShptLine.FindSet then
                repeat
                    if not IsItemBlocked(FromSalesShptLine."No.") then begin
                        ...
                        // P8000885
                        "Contract No." := FromSalesShptLine."Contract No.";
                        GetSalesContract(ToSalesHeader, ToSalesLine,
                                         "Sales Document Type From"::"Posted Shipment", FromSalesShptLine."Document No.", FromSalesShptLine."Line No."); // P8001132
                        // P8000885
                        CheckItemAvailability(ToSalesHeader, ToSalesLine);
            ...
            if FromSalesInvLine.FindSet then
                repeat
                    if not IsItemBlocked(FromSalesInvLine."No.") then begin
                        ...
                        // P8000885
                        "Contract No." := FromSalesInvLine."Contract No.";
                        GetSalesContract(ToSalesHeader, ToSalesLine,
                                         "Sales Document Type From"::"Posted Invoice", FromSalesInvLine."Document No.", FromSalesInvLine."Line No."); // P8001132
                        // P8000885
            ...
            if FromReturnRcptLine.FindSet then
                repeat
                    if not IsItemBlocked(FromReturnRcptLine."No.") then begin
                        ...
                        // P8000885
                        "Contract No." := FromReturnRcptLine."Contract No.";
                        GetSalesContract(ToSalesHeader, ToSalesLine,
                                         "Sales Document Type From"::"Posted Return Receipt", FromReturnRcptLine."Document No.", FromReturnRcptLine."Line No."); // P8001132
                        // P8000885
                        CheckItemAvailability(ToSalesHeader, ToSalesLine);
            ...
            if FromSalesCrMemoLine.FindSet then
                repeat
                    if not IsItemBlocked(FromSalesCrMemoLine."No.") then begin
                        ...
                        // P8000885
                        "Contract No." := FromSalesCrMemoLine."Contract No.";
                        GetSalesContract(ToSalesHeader, ToSalesLine,
                            "Sales Document Type From"::"Posted Credit Memo", FromSalesCrMemoLine."Document No.", FromSalesCrMemoLine."Line No."); // P8001132
                        // P8000885 
        ...
        with FromSalesShptLine do
            if FindSet then
                repeat
                    ...
                    // P8000885
                    if TempSalesShptLine.Get(FromSalesShptLine."Document No.", FromSalesShptLine."Line No.") then begin
                        FromSalesLine."Contract No." := TempSalesShptLine."Contract No.";
                        FromSalesLine.InitOutstandingQtyCont;
                    end;
                    // P8000885
                        ...
                        repeat
                            ...
                            then begin
                                if CopyItemTrkg then begin
                                    ...
                                    ToSalesLine.GetLotNo; // P8000466A
                                    ToSalesLine.Modify;   // P8000466A
                                end;
        ...
        with FromSalesInvLine do
            if FindSet then
                repeat
                    ...
                    // P8000885
                    if TempSalesInvLine2.Get(FromSalesInvLine."Document No.", FromSalesInvLine."Line No.") then begin
                        FromSalesLine."Contract No." := TempSalesInvLine2."Contract No.";
                        FromSalesLine.InitOutstandingQtyCont;
                    end;
                    // P8000885
                    FromSalesLine."Appl.-from Item Entry" := 0;
        ...
        with FromSalesCrMemoLine do
            if FindSet then
                repeat
                    ...
                    // P8000885
                    if TempSalesCrMemoLine.Get(FromSalesCrMemoLine."Document No.", FromSalesCrMemoLine."Line No.") then begin
                        FromSalesLine."Contract No." := TempSalesCrMemoLine."Contract No.";
                        FromSalesLine.InitOutstandingQtyCont;
                    end;
                    // P8000885
                    FromSalesLine."Appl.-from Item Entry" := 0;
        ...
        with FromReturnRcptLine do
            if FindSet then
                repeat
                    ...
                    // P8000885
                    if TempReturnRcptLine.Get(FromReturnRcptLine."Document No.", FromReturnRcptLine."Line No.") then begin
                        FromSalesLine."Contract No." := TempReturnRcptLine."Contract No.";
                        FromSalesLine.InitOutstandingQtyCont;
                    end;
                    // P8000885
                    FromSalesLine."Appl.-from Item Entry" := 0;
                        ...
                        repeat
                            ...
                            then begin
                                if CopyItemTrkg then begin
                                    ...
                                    ToPurchLine.GetLotNo; // P8000466A
                                    ToPurchLine.Modify;   // P8000466A
                                end;
    ...
    begin
        ...
        // PR3.70.01 Begin
        ToSalesLine."Quantity (Alt.)" := 0;
        ToSalesLine."Qty. to Ship (Alt.)" := 0;
        ToSalesLine."Qty. Shipped (Alt.)" := 0;
        ToSalesLine."Qty. to Invoice (Alt.)" := 0;
        ToSalesLine."Qty. Invoiced (Alt.)" := 0;
        ToSalesLine."Return Qty. to Receive (Alt.)" := 0;
        ToSalesLine."Return Qty. Received (Alt.)" := 0;
        ToSalesLine."Net Weight to Ship" := 0;
        ToSalesLine."Amount to Ship (LCY)" := 0;
        //ToSalesLine."Qty. to Ship/Recv. (Cont.)" := 0; // P80046533
        // PR3.70.01 End;
    ...
    begin
        ...
        // PR3.70.01 Begin
        ToPurchLine."Quantity (Alt.)" := 0;
        ToPurchLine."Qty. to Receive (Alt.)" := 0;
        ToPurchLine."Qty. Received (Alt.)" := 0;
        ToPurchLine."Qty. to Invoice (Alt.)" := 0;
        ToPurchLine."Qty. Invoiced (Alt.)" := 0;
        ToPurchLine."Return Qty. to Ship (Alt.)" := 0;
        ToPurchLine."Return Qty. Shipped (Alt.)" := 0;
        // PR3.70.01 End
