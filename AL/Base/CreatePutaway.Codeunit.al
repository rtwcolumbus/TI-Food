codeunit 7313 "Create Put-away"
{
    ...
    begin
        ...
        with PostedWhseRcptLine do begin
            ...
            if Location."Directed Put-away and Pick" then begin
                ...
                GetFirstTemplLine("Location Code"); // P8000494A
            ...
            if Location."Directed Put-away and Pick" then begin
                ...
                repeat
                                ...
                                if Bin.Code <> "Bin Code" then begin
                                    ...
                                    then begin
                                        ...
                                        if BinContent."Max. Qty." <> 0 then begin
                                            ...
                                            if (QtyToPutAwayBase < 0) then // P8000494A
                                                QtyToPutAwayBase := 0;       // P8000494A
                                            if QtyToPutAwayBase > RemQtyToPutAwayBase then
                                                QtyToPutAwayBase := RemQtyToPutAwayBase;
                                        end;
                                        // P8000494A
                    ...
                    // until (PutAwayTemplLine.Next() = 0) or EverythingHandled; // P8000494A
                until (not GetNextTemplLine()) or EverythingHandled;       // P8000494A
            end else begin
                Clear(Bin);
    ...
    begin
        with Bin do begin
            ...
            // P8000494A
            if (FixedProdBinCode <> '') then begin
                SetRange(Code, FixedProdBinCode);
                SetRange("Warehouse Class Code");
                SetRange("Bin Type Code");
                SetRange("Cross-Dock Bin");
            end;
            // P8000494A
            if Find('+') then begin
    ...
    begin
        ...
        if QtyToPutAwayBase > 0 then begin
            ...
            // P8001323
            if (not Location."Bin Mandatory") and (P800Functions.ContainerTrackingInstalled) then begin
                CreateNewWhseActivity(
                  PostedWhseRcptLine, WhseActivLine, 1, LineNo,
                  0, QtyToPutAwayBase, true, false, EmptyZoneBin, false);
                LineNo += 10000;
                CreateNewWhseActivity(
                  PostedWhseRcptLine, WhseActivLine, 2, LineNo,
                  0, QtyToPutAwayBase, true, false, EmptyZoneBin, false);
            end else
                // P8001323
                CreateNewWhseActivity(
    ...
    local procedure GetFirstTemplLine(LocationCode: Code[10])
    begin
        // P8000494A
        Clear(FixedProdBinCode);
        if Item.GetFixedBinItem(LocationCode, FixedProdBinCode) then
            PutAwayTemplLine.Init;
    end;

    local procedure GetNextTemplLine(): Boolean
    begin
        // P8000494A
        if (FixedProdBinCode <> '') then begin
            Clear(FixedProdBinCode);
            exit(PutAwayTemplLine.Find('-'));
        end;
        exit(PutAwayTemplLine.Next <> 0);
    end;
