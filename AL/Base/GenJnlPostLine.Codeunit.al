codeunit 12 "Gen. Jnl.-Post Line"
{
    ...
    //   Add call to Process 800 codeunit to transfer P800 fields to customer ledger
    // 
    // PR4.00.04
    // P8000405A, VerticalSoft, Jack Reynolds, 05 OCT 06
    //   Fix problem with G/L Entry No. being used twice
    // 
    // PR4.00.05
    // P8000413A, VerticalSoft, Jack Reynolds, 02 APR 07
    //   Previous project P8000405A which fixed a Navision bug has been fixed in SP3 so P8000405A has been removed
    // 
    // PRW16.00.02
    // P8000762, VerticalSoft, Jack Reynolds, 28 JAN 10
    //   Move Accrual Entry field to vendor ledger
    // 
    // PRW16.00.04
    ...
    // 
    // PRW16.00.06
    // P8001012, Columbus IT, Jack Reynolds, 06 JAN 12
    //   Fix problem applying deduction management payments with discounts
    // 
    // PRW17.00
    ...
    // 
    // PRW17.10.02
    // P8001293, Columbus IT, Jack Reynolds, 20 FEB 14
    //   Fix problem applying payment to a payment with deduction management
    // 
    // PRW18.00.02
    ...
    // 
    // PRW110.0.01
    // P80037382, to-Increase, Dayakar Battini, 16 MAY 17
    //   Fix issue when applying deduction management payments
    // 
    // P80041422, to-Increase, Jack Reynolds, 14 JUN 17
    //   Fix issue when applying deduction management payments
    //   Remove changes from P8001293
    // 
    // PRW111.00.02
    // P80068489,To-Increase, Gangabhushan, 31 DEC 18
    //   TI-12522 - VAT issues for accruals process, Modified PostAccrual fnc
    // 
    // PRW111.00.03
    // P80083834,To-Increase, Gangabhushan, 03 OCT 19
    //   CS00076798 - Can't post Commodity Manifest for multiple Vendors
    ...
    var
        ...
        GLSetupRead: Boolean;
        PostDeductionApplication: Boolean;
    ...
    begin
        ...
        with GenJnlLine do begin
            ...
            if "Account No." <> '' then begin
                ...
                "Accrual Entry" := "Accrual Entry" or                                                                               // P8000762
                ("Account Type" = "Account Type"::FOODAccrualPlan) or ("Bal. Account Type" = "Bal. Account Type"::FOODAccrualPlan); // P8000762
        ...
        with GenJnlLine do
            case "Account Type" of
                ...
                "Account Type"::FOODAccrualPlan:
                    PostAccrual(GenJnlLine); // P8001213
                                             // P8000241A
            end;
    ...
    begin
        ...
        then begin
            ...
            // P8001012
            if PostDeductionApplication then begin
                OldCVLedgEntryBuf."Amount to Apply" -= PmtDisc;
                OldCVLedgEntryBuf2."Amount to Apply" -= PmtDisc;
            end;
            // P8001012
    ...
    begin
        ...
        repeat
            ...
            ClearUnresolvedDeduction(OldCustLedgEntry); // P8002752
    ...
    begin
        ...
        with GenJnlLine do begin
            ...
            if PostDeductionApplication then begin
                ...
                ClearUnresolvedDeduction(CustLedgEntry); // P8002752
                CustLedgEntry.Modify;
            end;
            // P8000202A End
        ...
        PostDeductionApplication := false; // P8000202A
    end;
    ...
    begin
        ...
        if GenJnlLine."Applies-to Doc. No." <> '' then begin
            ...
            if TempOldCustLedgEntry.Find('-') then begin
                ...
                if PostDeductionApplication then                           // P8000202A, P8001213
                    NewCVLedgEntryBuf."Remaining Amount" -= RemainingAmount; // P8000202A, P8001213
    ...
    procedure SetPostDeductionApplication()
    begin
        // P8000202A
        PostDeductionApplication := true;
    end;

    procedure PostAccrual(GenJnlLine: Record "Gen. Journal Line")
    var
        GLEntry: Record "G/L Entry";
        AccrualAccountNo: Code[20];
        AccrualLedgEntryNo: Integer;
        VATPostingSetup: Record "VAT Posting Setup";
    begin
        // P8001213 - add parameter for GenJnlLine
        // P8000241A
        AccrualPostLine.PostGenJnlLine(GenJnlLine, AccrualAccountNo, AccrualLedgEntryNo);
        with GenJnlLine do begin
            InitGLEntry(GenJnlLine, GLEntry, AccrualAccountNo, "Amount (LCY)", "Source Currency Amount", true, true); // P8001213
            GLEntry."Bal. Account Type" := "Bal. Account Type";
            GLEntry."Bal. Account No." := "Bal. Account No.";
            InitVAT(GenJnlLine, GLEntry, VATPostingSetup); // P80068489
            InsertGLEntry(GenJnlLine, GLEntry, true); // P8001213
            PostVAT(GenJnlLine, GLEntry, VATPostingSetup); // P80068489
        end;
        // P8000241A
    end;
    ...
    begin
        // P8000888
        GLReg2 := GLReg;
        NextVATEntryNo2 := NextVATEntryNo;
        NextTransactionNo2 := NextTransactionNo;
    end;

    procedure SetGLRegister(var GLReg2: Record "G/L Register"; NextVATEntryNo2: Integer; NextTransactionNo2: Integer)
    begin
        // P8000888
        if (GLReg2."No." <> 0) then begin
            GLReg := GLReg2;
            IsGLRegInserted := GLReg.Find; // P80083834
            NextEntryNo := GLReg."To Entry No." + 1;
            NextVATEntryNo := NextVATEntryNo2;
            NextTransactionNo := NextTransactionNo2;
        end;
    end;

    local procedure ClearUnresolvedDeduction(var CustLedgerEntry: Record "Cust. Ledger Entry")
    begin
        // P8002752
        if not CustLedgerEntry.Open then
            CustLedgerEntry."Unresolved Deduction" := false;
    end;
