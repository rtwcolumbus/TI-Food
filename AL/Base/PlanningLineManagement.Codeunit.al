codeunit 99000809 "Planning Line Management"
{
    ...
    //   Transfer Step Code and BOM No. to planning component
    // 
    // PRW16.00.06
    ...
    //   New parameters for TransferBOM
    // 
    // PRW19.00.01
    ...
    // 
    // PRW110.0.02
    // P80056311, To-Increase, Dayakar Battini, 28 MAR 18
    //   Process Mfg check.
    ...
    begin
        with PlanningRoutingLine do begin
            ...
            // P8000219A, P800-MegaApp
            Item.Get(ReqLine."No.");
            PlanningRoutingLine."Use ABC Cost" := RoutingLine."Use ABC Cost";
            RoutingRec.GetTable(PlanningRoutingLine);
            if not CostCalcMgt.CalcABCRtngCostPerUnit(
                RoutingRec, Item, ReqLine."Starting Date", ReqLine."Location Code", // P8001030
                ReqLine."Production BOM No.", ReqLine."Production BOM Version Code",
                "Direct Unit Cost", "Indirect Cost %",
                "Overhead Rate", "Unit Cost per", "Unit Cost Calculation")
            then begin
                // P8000219A
    ...
    local procedure TransferBOM(ProdBOMNo: Code[20]; Level: Integer; LineQtyPerUOM: Decimal; ItemQtyPerUOM: Decimal; StepCode: Code[10])
    var
    ...
    begin
        // P8000386A - add parameter for step code
        ...
        if (Level = 1) then                                                                                                   // P8001092
            LineQtyPerUOM := LineQtyPerUOM * BOMHeader.GetCoProdBOMFactor(ReqLine."Production BOM Version Code", ReqLine."No."); // P8001092, P8001301
        ...
        if ProdBOMLine[Level].Find('-') then
            repeat
                ...
                if not IsHandled then begin
                    if StepCode <> '' then                         // P8000386A
                        ProdBOMLine[Level]."Step Code" := StepCode; // P8000386A
                    ...
                    case ProdBOMLine[Level].Type of
                        ...
                        ProdBOMLine[Level].Type::"Production BOM":
                            begin
                                ...
                                TransferBOM(ProdBOMLine[Level]."No.", Level + 1, ReqQty, 1, ProdBOMLine[Level]."Step Code"); // P8000386A
                                ProdBOMLine[Level].SetRange("Production BOM No.", ProdBOMNo);
    ...
    begin
        ...
        if not PlanningRtngLine.FindFirst then begin
            ...
            if ReqLine."Ref. Order Type" = ReqLine."Ref. Order Type"::"Prod. Order" then
                if ProcessFns.ProcessInstalled then // P80056311
                    if P800ProdOrderMgt.CalculatePlanningLineDates(ReqLine, Direction) then begin
                        CalculatePlanningLineDates(ReqLine);
                        ReqLine.VALIDATE("Order Date", ReqLine."Starting Date");
                        exit;
                    end;
            // P8008029
            if Direction = Direction::Forward then
                    ...
                    if not IsHandled then
                        TransferBOM(
                          ...
                          UOMMgt.GetQtyPerUnitOfMeasure(
                            Item, VersionMgt.GetBOMUnitOfMeasure(ReqLine."Production BOM No.", ReqLine."Production BOM Version Code")), ''); // P8000386A
                end;
            end;
        end;
    ...
    begin
        with PlanningComponent do begin
            ...
            // P8000386A, P800-MegaApp
            "Step Code" := ProdBOMLine."Step Code"; // PR2.00.03
            "Production BOM No." := ProdBOMLine."Production BOM No.";
            "Production BOM Version Code" := ProdBOMLine."Version Code";
            "Production BOM Line No." := ProdBOMLine."Line No.";
            // P8000386A
            // P8001082, P800-MegaApp
            "Pre-Process Type Code" := ProdBOMLine."Pre-Process Type Code";
            "Pre-Process Lead Time (Days)" := ProdBOMLine."Pre-Process Lead Time (Days)";
            // P8001082
    ...
    begin
        with PlanningComp do begin
            IsFound :=
              ...
              and ("Step Code" = ProdBOMLine."Step Code"); // P8000386A, P800-MegaApp
