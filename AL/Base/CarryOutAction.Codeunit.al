codeunit 99000813 "Carry Out Action"
{
    ...
    // 
    // PR4.00.04
    // P8000386A, VerticalSoft, Jack Reynolds, 26 SEP 06
    //   Transfer Step Code and BOM No. from planning component
    // 
    // PR6.00.04
    // P8000903, VerticalSoft, Jack Reynolds, 09 FEB 11
    //   Set replenishment area when creating production orders
    // 
    // PRW16.00.06
    // P8001092, Columbus IT, Don Bresee, 28 AUG 12
    ...
    // 
    // PRW17.10.02
    // P8001283, Columbus IT, Jack Reynolds, 10 FEB 14
    //   Add Equipment code to Planning Worksheet
    // 
    // PRW17.10.03
    // P8001327, Columbus IT, Jack Reynolds, 12 JUN 14
    //   Fix problem setting equipment code on production order lines
    // 
    // PRW18.00.01
    // P8001392, Columbus IT, Jack Reynolds, 06 AUG 15
    //   Fix problem with bin codes and replenishment areas
    // 
    // PRW110.0.02
    // P80046473, To-Increase, Dayakar Battini, 08 SEP 17
    //  Correct Family prod order line details
    // 
    // P80048888, To-Increase, Dayakar Battini, 15 NOV 17
    //   Fix problem when finding the BOM
    // 
    // PRW113.00
    ...
    begin
        with ReqLine do begin
            ...
            if ProdOrderLine.Get("Ref. Order Status", "Ref. Order No.", "Ref. Line No.") then begin
                ...
                if (ProdOrderLine.Quantity <> 0) then                   // P8001092
                    CoProdQtyFactor := Quantity / ProdOrderLine.Quantity; // P8001092
                ...
                CoProdOrderChgAndReshedule(ReqLine, ProdOrderLine, CoProdQtyFactor); // P8001092
                ...
                if PlanningComponent.Find('-') then
                    repeat
                        if ProdOrderComp.Get(
                          // ProdOrderLine.Status,ProdOrderLine."Prod. Order No.",ProdOrderLine."Line No.",PlanningComponent."Line No.")             // P8001092
                          ProdOrderLine.Status, ProdOrderLine."Prod. Order No.", PlanningComponent."Ref. Order Line No.", PlanningComponent."Line No.") // P8001092
                        then begin
                ...
                if "Planning Level" = 0 then
                    if ProdOrder.Get("Ref. Order Status", "Ref. Order No.") then begin
                        if not ProdOrder."Family Process Order" then // P8001092
    ...
    begin
        ...
        if not HeaderExist then begin
            ...
            ProdOrder."Equipment Code" := ReqLine."Equipment Code"; // P8001283
                                                                    // P8000903
            ProdOrder.Validate("Replenishment Area Code", ProdOrder.GetDefaultReplArea);
            if (ProdOrder."Bin Code" = '') and (ProdOrder."Replenishment Area Code" = '') then // P8001392
                                                                                               // P8000903
        ...
        if (ProdOrder."Source Type" = ProdOrder."Source Type"::Family) then
            InsertFamilyOrderLines(ReqLine, ProdOrder)
        else
            // P8001092
    ...
    begin
        ...
        ProdOrderLine."Due Date" := ReqLine."Due Date";  // P80048888
        ProdOrderLine.Validate("Equipment Code", ReqLine."Equipment Code"); // P8001327
        ProdOrderLine.Validate("Replenishment Area Code", ProdOrder."Replenishment Area Code"); // P8000903
        if ProdOrderLine."Replenishment Area Code" = '' then // P8001392
    ...
    local procedure InsertFamilyOrderLines(var ReqLine: Record "Requisition Line"; var ProdOrder: Record "Production Order")
    var
        ProdOrderLine: Record "Prod. Order Line";
        ...
        Item2: Record Item;
        MainProdOrderLine: Record "Prod. Order Line";
        LotNo: Code[50];
    begin
        // P8001092
        ProdOrderLine.SetRange("Prod. Order No.", ProdOrder."No.");
        ...
        if ProdOrderLine.Find('+') then
            LastLineNo := ProdOrderLine."Line No.";

        Family.Get(ReqLine."Production BOM No.");
        FamilyLine.FindOutputItem(ReqLine."Production BOM No.", ReqLine."No.");
        FamilyLine.FindFirst;
        FamilyLine.SetFilter("Item No.", '<>%1', '');
        FamilyLine.FindSet;
        repeat
            ProdOrderLine.Init;
            ProdOrderLine.BlockDynamicTracking(true);
            ProdOrderLine.Status := ProdOrder.Status;
            ProdOrderLine."Prod. Order No." := ProdOrder."No.";
            ProdOrderLine."Line No." := FamilyLine."Line No." + LastLineNo;
            ProdOrderLine."Item No." := FamilyLine."Item No.";
            if (FamilyLine."Item No." <> ReqLine."No.") then
                ProdOrderLine.Validate("Unit of Measure Code", FamilyLine."Unit of Measure Code")
            else begin
                ProdOrderLine.Validate("Unit of Measure Code", ReqLine."Unit of Measure Code");
                MainProdOrderLine := ProdOrderLine;
            end;
            ProdOrderLine."Production BOM Version Code" := ReqLine."Production BOM Version Code";
            ProdOrderLine."Routing Version Code" := ReqLine."Routing Version Code";
            ProdOrderLine."Routing Type" := ReqLine."Routing Type";
            ...
            ProdOrderLine."Variant Code" := FamilyLine."Variant Code";
            ProdOrderLine."Location Code" := ReqLine."Location Code";
            ProdOrderLine.Validate("Equipment Code", ReqLine."Equipment Code"); // P8001327
            ProdOrderLine.Validate("Replenishment Area Code", ProdOrder."Replenishment Area Code");
            if ProdOrderLine."Bin Code" = '' then
                ProdOrderLine."Bin Code" := ReqLine."Bin Code";
            ProdOrderLine."Scrap %" := ReqLine."Scrap %";
            ProdOrderLine."Production BOM No." := ReqLine."Production BOM No.";
            ProdOrderLine."By-Product" := FamilyLine."By-Product";
            ProdOrderLine."Primary Co-Product" := FamilyLine.IsPrimaryCoProduct();
            ProdOrderLine."Co-Product Cost Share" := FamilyLine."Co-Product Cost Share";
            Item2.Get(FamilyLine."Item No.");
            ProdOrderLine."Inventory Posting Group" := Item2."Inventory Posting Group";
            ProdOrderLine.Validate("Unit Cost", FamilyLine."Unit Cost");
            ProdOrderLine."Routing No." := ReqLine."Routing No.";
            ProdOrderLine."Starting Time" := ReqLine."Starting Time";
            ProdOrderLine."Starting Date" := ReqLine."Starting Date";
            ProdOrderLine."Ending Time" := ReqLine."Ending Time";
            ...
            ProdOrderLine.Status := ProdOrder.Status;
            ProdOrderLine."Planning Level Code" := ReqLine."Planning Level";
            ProdOrderLine."Indirect Cost %" := Item2."Indirect Cost %";   // P80046473
            ProdOrderLine."Overhead Rate" := Item2."Overhead Rate";  // P80046473
            ProdOrderLine.Validate(Quantity, ProdOrder.Quantity * FamilyLine.Quantity);
            if not (ProdOrder.Status = ProdOrder.Status::Planned) then
                ProdOrderLine."Planning Flexibility" := ReqLine."Planning Flexibility";
            ProdOrderLine.Validate("Unit Cost (Costing Units)", FamilyLine."Unit Cost");
            ProdOrderLine."Routing No." := Family."Routing No.";
            ProdOrderLine."Routing Reference No." := 0;
            ProdOrderLine.UpdateDatetime;
            ProdOrderLine."Shortcut Dimension 1 Code" := ReqLine."Shortcut Dimension 1 Code";
            ProdOrderLine."Shortcut Dimension 2 Code" := ReqLine."Shortcut Dimension 2 Code";
            ProdOrderLine."Dimension Set ID" := ReqLine."Dimension Set ID"; // P8001133
            ProdOrderLine.Insert;
        until (FamilyLine.Next = 0);

        ProdOrderLine := MainProdOrderLine;
        ProdOrderLine.Find;

        //CalcProdOrder.TransferTaskInfo(ProdOrderLine); // P8001213
        ...
        TransferBOM(ReqLine, ProdOrder, 0);
        TransferCapNeed(ReqLine, ProdOrder, ProdOrderLine."Routing No.", ProdOrderLine."Routing Reference No.");
    ...
    local procedure UpdateFamilyOrderComponent(var PlanningComponent: Record "Planning Component"; var ProdOrderComp: Record "Prod. Order Component")
    var
        ProdBOMComponent: Record "Production BOM Line";
    begin
        // P8001092
        with PlanningComponent do
            if not ProdBOMComponent.Get("Production BOM No.", "Production BOM Version Code", "Production BOM Line No.") then
                exit;
        ProdOrderComp.Validate("Quantity per", ProdBOMComponent."Batch Quantity");
    end;

    local procedure CoProdOrderChgAndReshedule(var ReqLine: Record "Requisition Line"; var ProdOrderLine: Record "Prod. Order Line"; CoProdQtyFactor: Decimal)
    var
        ProdOrder: Record "Production Order";
    ...
    begin
        ...
        if ProdOrder.Get(ReqLine."Ref. Order Status", ReqLine."Ref. Order No.") then
            if ProdOrder."Family Process Order" then begin
                ProdOrder.Quantity := ProdOrder.Quantity * CoProdQtyFactor;
                ProdOrder.Modify;
                ProdOrderLine2.SetRange(Status, ProdOrderLine.Status);
                ProdOrderLine2.SetRange("Prod. Order No.", ProdOrderLine."Prod. Order No.");
                ProdOrderLine2.SetFilter("Line No.", '<>%1', ProdOrderLine."Line No.");
                if ProdOrderLine2.FindSet then
                    repeat
                        ProdOrderLine2.Validate(Quantity, ProdOrderLine2.Quantity * CoProdQtyFactor);
                        ProdOrderLine2.Modify(true);
                    until (ProdOrderLine2.Next = 0);
                ProdOrderComp.SetRange(Status, ProdOrderLine.Status);
                ProdOrderComp.SetRange("Prod. Order No.", ProdOrderLine."Prod. Order No.");
                ProdOrderComp.SetFilter("Prod. Order Line No.", '%1|%2', 0, ProdOrderLine."Line No.");
