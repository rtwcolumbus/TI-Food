codeunit 7312 "Create Pick"
{
            ...
            if (TotalAvailQtyToPickBase > 0) then  // P8001347
                repeat
                    ...
                    if FromBinContent."Bin Code" <> ToBinCode then
                        ...
                        CalcBinAvailQtyToPickForUOM(FromBinContent, TempWhseActivLine2, AvailableQtyBase); // P8001347
                ...
                repeat
                    if IsPickableBin(FromBinContent, ToBinCode) and BinNotExcluded(FromBinContent, false, LotNo) then
                        if ContainerLinesExist(LocationCode, ItemNo, VariantCode, FromBinContent."Bin Code", UnitofMeasureCode, LotNo) then begin
                            CalcBinAvailQtyToPickForUOM(FromBinContent, TempWhseActivLine, AvailableQtyBase);
        ...
        if FromBinContent.FindSet then begin
            ...
            if (TotalAvailQtyToPickBase > 0) then
                repeat
                    ...
                    then begin
                        CalcBinAvailQtyToPickAllUOMs(FromBinContent, TempWhseActivLine, AvailableQtyBase);
                        ApplyMaximumQty(AvailableQtyBase, TotalQtytoPickBase);
                        ApplyMaximumQty(AvailableQtyBase, TotalAvailQtyToPickBase);
                        if AvailableQtyBase > 0 then begin
    ...
    local procedure CalcBinAvailQtyToPickForUOM(var BinContent: Record "Bin Content"; var TempWhseActivLine2: Record "Warehouse Activity Line" temporary; var QtyToPickBase: Decimal)
    var
        QtyToPickBaseAllUOMs: Decimal;
    begin
        // P8001347
        CalcBinAvailQtyToPick2(BinContent, TempWhseActivLine2, QtyToPickBase, QtyToPickBaseAllUOMs);
        ApplyMaximumQty(QtyToPickBase, QtyToPickBaseAllUOMs);
    end;

    local procedure CalcBinAvailQtyToPickAllUOMs(var BinContent: Record "Bin Content"; var TempWhseActivLine2: Record "Warehouse Activity Line" temporary; var QtyToPickBaseAllUOMs: Decimal)
    var
        QtyToPickBase: Decimal;
    begin
        CalcBinAvailQtyToPick2(BinContent, TempWhseActivLine2, QtyToPickBase, QtyToPickBaseAllUOMs); // P8001347
    end;

    local procedure CalcBinAvailQtyToPick2(var BinContent: Record "Bin Content"; var TempWhseActivLine2: Record "Warehouse Activity Line" temporary; var QtyToPickBase: Decimal; var QtyToPickBaseAllUOMs: Decimal)
    var
        ...
        if FindAllUOMBinContents(AllBinContent, BinContent) then
            repeat
                if BinNotExcluded(AllBinContent, false, TempWhseItemTrackingLine."Lot No.") then begin
                    CalcBinAvailQtyToPick(AvailableQtyBase, AllBinContent, TempWhseActivLine2);
                    CalcBinAvailQtyToPickExclusions(
                      AvailableQtyBase, AllBinContent, TempWhseItemTrackingLine."Lot No.", TempWhseActivLine2);
                end else
                    AvailableQtyBase := 0;
    ...
    local procedure CalcBinAvailQtyToPickExclusions(var QtyBase: Decimal; var BinContent: Record "Bin Content"; LotNo: Code[50]; var TempWhseActivLine2: Record "Warehouse Activity Line" temporary)
    var
        BinContent2: Record "Bin Content";
        LotQtyBase: Decimal;
            ...
            with TotalTempExclusion do
                if FindBinLotExclusions(BinContent2) then begin
                    ...
                    repeat
                        ...
                        CalcBinAvailQtyToPick(LotQtyBase, BinContent2, TempWhseActivLine2);
