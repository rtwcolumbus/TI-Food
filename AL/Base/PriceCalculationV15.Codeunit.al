...
codeunit 7003 "Price Calculation - V15" implements "Price Calculation"
{
    ...
    begin
        ...
        case CurrLineWithPrice.GetTableNo() of
            ...
            Database::"Sales Line":
                begin
                    OnGetShortSubstituteItem(ShortSubstituteItem);
                    if ShortSubstituteItem then
                        SalesPriceCalcMgt.SetShortSubstituteItem();
                    SalesLine := Line;
                    SalesPriceCalcMgt.FindSalesLinePrice(Header, SalesLine, CalledByFieldNo);
                    OnSetContractItem(SalesPriceCalcMgt.IsContractItem());
    ...
    [IntegrationEvent(false, false)]
    local procedure OnGetShortSubstituteItem(var Short: Boolean)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnSetContractItem(Contract: Boolean)
    begin
    end;
