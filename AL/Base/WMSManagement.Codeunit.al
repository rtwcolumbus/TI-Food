codeunit 7302 "WMS Management"
{
    ...
    //   Allow pick to by bypassed for consumption entries
    // 
    // PR5.00
    ...
    //   Fix problem when testing for alternate quantity
    // 
    // PRW16.00.06
    ...
    begin
        ...
        with ItemJnlLine do begin
            if ((not "Phys. Inventory") and (Quantity = 0) and ("Quantity (Alt.)" = 0) and ("Invoiced Quantity" = 0)) or // P8000591A
            ...
            // P8000591A
            if ((WhseJnlLine."To Bin Code" = Location."Adjustment Bin Code") or
                (WhseJnlLine."From Bin Code" = Location."Adjustment Bin Code")) and ("Quantity (Alt.)" <> 0)
            then begin
                WhseJnlLine."Quantity (Alt.)" := "Quantity (Alt.)";
                WhseJnlLine."Quantity (Absolute, Alt.)" := Abs("Quantity (Alt.)");
            end;
            // P8000591A
    ...
    begin
        ...
        with WhseJnlLine do begin
            ...
            RoundingAdjmtMgmt.AdjustWhseMovementQtys(WhseJnlLine, false); // P8001039, P8001127
            ...
            // P8001323
            if ("Entry Type" = "Entry Type"::Movement) and (Item."Alternate Unit of Measure" <> '') and (("From Container ID" <> '') or ("To Container ID" <> '')) then
            ...
            // P8000591A
            if (SourceJnl <> SourceJnl::ItemJnl) and
               (Location.LocationType = 3) and // P8000662
               (Quantity <> 0) and Item.TrackAlternateUnits() and
               (("From Bin Code" = Location."Adjustment Bin Code") or
                ("To Bin Code" = Location."Adjustment Bin Code"))
            then begin
                TestField("Quantity (Alt.)");
                if ("Qty. (Base)" * "Quantity (Alt.)" < 0) then
                    Error(Text37002000, FieldCaption("Qty. (Base)"), FieldCaption("Quantity (Alt.)"));
            end;
            // P8000591A
        end;
        ...
        with WhseJnlLine do
            if ItemTrackingCode.Get(Item."Item Tracking Code") then begin
                ...
                if ("Lot No." <> '') and
                   ...
                   ("From Bin Code" <> Location."Adjustment Bin Code") and
                   ((Location."Adjustment Bin Code" = '') and ((Quantity <> 0) or ("Qty. (Base)" <> 0))) and // P8001110
                   (((Location."Adjustment Bin Code" <> '') and
                     ("Entry Type" = "Entry Type"::Movement)) or
                    (("Entry Type" <> "Entry Type"::Movement) or
                     ("Source Document" = "Source Document"::"Reclass. Jnl.")))
    ...
    begin
        with WhseJnlLine do begin
            ...
            if Location."Directed Put-away and Pick" then begin
                ...
                Quantity := Round(QuantityBase, UOMMgt.QtyRndPrecision); // P800122712
                "Unit of Measure Code" := GetBaseUOM(ItemJnlLine."Item No.");
                "Qty. per Unit of Measure" := 1;
            end;
            "Qty. (Base)" := Round(QuantityBase, UOMMgt.QtyRndPrecision); // P800122712
            "Qty. (Absolute)" := Abs(Quantity);
            "Qty. (Absolute, Base)" := Abs(Round(QuantityBase, UOMMgt.QtyRndPrecision)); // P800122712 
    ...
    procedure ShowWhseDocLine(WhseDocType: Option Receipt,"Posted Receipt",Shipment,"Internal Put-away","Internal Pick",Production,,Assembly,,"Staged Pick"; WhseDocNo: Code[20]; WhseDocLineNo: Integer)
    var
        IsHandled: Boolean;
    begin
        IsHandled := false;
        ...
        case WhseDocType of
            ...
            // P8000322A
            WhseDocType::"Staged Pick":
                ShowStagedPickLine(WhseDocNo, WhseDocLineNo);
            // P8000322A
            else
    ...
    // P800131478
    procedure ShowStagedPickLine(WhseDocNo: Code[20]; WhseDocLineNo: Integer)
    var
        WhseStagedPickLine: Record "Whse. Staged Pick Line";
    begin
        WhseStagedPickLine.SetRange("No.", WhseDocNo);
        WhseStagedPickLine.SetRange("Line No.", WhseDocLineNo);
        PAGE.RunModal(PAGE::"Whse. Staged Pick Lines", WhseStagedPickLine);
    end;
    ...
    begin
        case WhseActivityDocType of
            ...
            WhseActivityDocType::Assembly:
                ShowAssemblyLine(WhseDocNo, WhseDocLineNo);
            // P800131478
            WhseActivityDocType::FOODStagedPick:
                ShowStagedPickLine(WhseDocNo, WhseDocLineNo);
        // P800131478
        end;
    end;
    ...
    begin
        ...
        with ItemJnlLine do
            if (("Entry Type" in
                ...
                ((Quantity > 0) or ("Quantity (Alt.)" > 0))) or // P8000591A
               (("Entry Type" in
    ...
    begin
        ...
        if ProdOrderCompLine."Qty. Picked (Base)" < ItemJnlLine."Quantity (Base)" then
            if Location."Require Production Picking" then // P8000494A
                if not ProdOrderCompLine.ReplenishmentNotRequired() then // P8000322A
    ...
    begin
        ...
        // P8001039
        // IF BinContent."Quantity (Base)" < ABS(QuantityBase) THEN
        if ((BinContent."Quantity (Base)" + RoundingAdjmtMgmt.GetNearZeroQtyForItem(ItemNo)) < Abs(QuantityBase)) then
            // P8001039
            BinContent.FieldError(
