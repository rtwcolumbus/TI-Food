...
{
    // PR3.70.01
    //   Delete extra charge posting setup
    // 
    // PR3.70.03
    //   New Fields
    //     Sales Account (Accrual)
    //     Purch. Account (Accrual)
    //     Sales Plan Account (Accrual)
    //     Purch. Plan Account (Accrual)
    // 
    // PR4.00.04
    // P8000375A, VerticalSoft, Jack Reynolds, 07 SEP 06
    //   New fields for ABC cost detail
    // 
    // PRW16.00.05
    // P8000921, Columbus IT, Don Bresee, 07 APR 11
    //   Add "Sales Account (Freight)" field
    // 
    // PRW111.00
    ...
    fields
    {
        ...
        field(37002049; "Sales Account (Freight)"; Code[20])
        {
            Caption = 'Sales Account (Freight)';
            TableRelation = "G/L Account";

            trigger OnLookup()
            begin
                // P80053245
                GLAccountCategoryMgt.LookupGLAccount("Sales Account (Freight)", GLAccountCategory."Account Category"::Income, '');
            end;

            trigger OnValidate()
            begin
                CheckGLAcc("Sales Account (Freight)"); // P8000921
            end;
        }
        field(37002120; "Sales Account (Accrual)"; Code[20])
        {
            ...
            trigger OnValidate()
            begin
                CheckGLAcc("Sales Account (Accrual)"); // PR3.61AC
            end;
        }
        field(37002121; "Purch. Account (Accrual)"; Code[20])
        {
            ...
            trigger OnValidate()
            begin
                CheckGLAcc("Purch. Account (Accrual)"); // PR3.61AC
            end;
        }
        field(37002122; "Sales Plan Account (Accrual)"; Code[20])
        {
            Caption = 'Sales Plan Account (Accrual)';
            Description = 'PR3.70.03';
            TableRelation = "G/L Account";
            ...
            begin
                // P80053245
                GLAccountCategoryMgt.LookupGLAccount("Sales Plan Account (Accrual)", GLAccountCategory."Account Category"::Expense, '');
            end;

            trigger OnValidate()
            begin
                CheckGLAcc("Sales Plan Account (Accrual)"); // PR3.61AC
            end;
        }
        field(37002123; "Purch. Plan Account (Accrual)"; Code[20])
        {
            Caption = 'Purch. Plan Account (Accrual)';
            Description = 'PR3.70.03';
            TableRelation = "G/L Account";
            ...
            begin
                // P80053245
                GLAccountCategoryMgt.LookupGLAccount("Purch. Plan Account (Accrual)", GLAccountCategory."Account Category"::Income, '');
            end;

            trigger OnValidate()
            begin
                CheckGLAcc("Purch. Plan Account (Accrual)"); // PR3.61AC
            end;
        }
        field(37002460; "ABC Direct Account"; Code[20])
        {
            Caption = 'ABC Direct Account';
            TableRelation = "G/L Account";

            trigger OnValidate()
            begin
                CheckGLAcc("ABC Direct Account"); // P8000375A
            end;
        }
        field(37002461; "ABC Overhead Account"; Code[20])
        {
            Caption = 'ABC Overhead Account';
            TableRelation = "G/L Account";

            trigger OnValidate()
            begin
                CheckGLAcc("ABC Overhead Account"); // P8000375A
            end;
        }
    ...
    begin
        // P80073095
        ExtraChargePostingSetup.SetRange("Gen. Bus. Posting Group", "Gen. Bus. Posting Group");
        ExtraChargePostingSetup.SetRange("Gen. Prod. Posting Group", "Gen. Prod. Posting Group");
        if not ExtraChargePostingSetup.IsEmpty then
    ...
    begin
        ...
        // P80053245
        if "Sales Account (Freight)" = '' then
            SuggestAccount(RecRef, FieldNo("Sales Account (Freight)"));
        if "Sales Account (Accrual)" = '' then
            SuggestAccount(RecRef, FieldNo("Sales Account (Accrual)"));
        if "Sales Plan Account (Accrual)" = '' then
            SuggestAccount(RecRef, FieldNo("Sales Plan Account (Accrual)"));
        // P80053245
    ...
    begin
        ...
        if "Purch. Account (Accrual)" = '' then
            SuggestAccount(RecRef, FieldNo("Purch. Account (Accrual)"));
        if "Purch. Plan Account (Accrual)" = '' then
            SuggestAccount(RecRef, FieldNo("Purch. Plan Account (Accrual)"));
        // P80053245
    ...
    begin
        ...
        // P80053245
        if "ABC Direct Account" = '' then
            SuggestAccount(RecRef, FieldNo("ABC Direct Account"));
        if "ABC Overhead Account" = '' then
            SuggestAccount(RecRef, FieldNo("ABC Overhead Account"));
        // P80053245
    ...
    procedure GetSalesAccountFreight(): Code[20]
    begin
        // P80053245
        if "Sales Account (Freight)" = '' then
            PostingSetupMgt.SendGenPostingSetupNotification(Rec, FieldCaption("Sales Account (Freight)"));
        TestField("Sales Account (Freight)");
        exit("Sales Account (Freight)");
    end;
    ...
    begin
        // P80053245
        if "Sales Account (Accrual)" = '' then
            PostingSetupMgt.SendGenPostingSetupNotification(Rec, FieldCaption("Sales Account (Accrual)"));
        TestField("Sales Account (Accrual)");
        exit("Sales Account (Accrual)");
    end;

    procedure GetPurchAccountAccrual(): Code[20]
    begin
        // P80053245
        if "Purch. Account (Accrual)" = '' then
            PostingSetupMgt.SendGenPostingSetupNotification(Rec, FieldCaption("Purch. Account (Accrual)"));
        TestField("Purch. Account (Accrual)");
        exit("Purch. Account (Accrual)");
    end;

    procedure GetSalesPlanAccountAccrual(): Code[20]
    begin
        // P80053245
        if "Sales Plan Account (Accrual)" = '' then
            PostingSetupMgt.SendGenPostingSetupNotification(Rec, FieldCaption("Sales Plan Account (Accrual)"));
        TestField("Sales Plan Account (Accrual)");
        exit("Sales Plan Account (Accrual)");
    end;

    procedure GetPurchPlanAccountAccrual(): Code[20]
    begin
        // P80053245
        if "Purch. Plan Account (Accrual)" = '' then
            PostingSetupMgt.SendGenPostingSetupNotification(Rec, FieldCaption("Purch. Plan Account (Accrual)"));
        TestField("Purch. Plan Account (Accrual)");
        exit("Purch. Plan Account (Accrual)");
    end;

    procedure GetABCDirectAccount(): Code[20]
    begin
        // P80053245
        if "ABC Direct Account" = '' then
            PostingSetupMgt.SendGenPostingSetupNotification(Rec, FieldCaption("ABC Direct Account"));
        TestField("ABC Direct Account");
        exit("ABC Direct Account");
    end;

    procedure GetABCOverheadAccount(): Code[20]
    begin
        // P80053245
        if "ABC Overhead Account" = '' then
            PostingSetupMgt.SendGenPostingSetupNotification(Rec, FieldCaption("ABC Overhead Account"));
        TestField("ABC Overhead Account");
        exit("ABC Overhead Account");
    end;
}
