codeunit 99000830 "Create Reserv. Entry"
{
    ...
    // 
    // PR3.70.05
    // P8000060A, Myers Nissi, Jack Reynolds, 09 JUN 04
    //   CreateEntry - when creating the "reservation from" entry, reserve the sign the alternate quantites
    // 
    // PRW16.00.01
    // P8000673, VerticalSoft, Don Bresee, 31 JAN 09
    //   Add default value for New Expiration Date
    // 
    // PRW16.00.02
    // P8000747, VerticalSoft, Don Bresee, 24 NOV 09
    //   Change handling of expiration dates
    // 
    // P8000756, VerticalSoft, Jack Reynolds, 14 JAN 10
    //   New code added that requires parallel code for alternate quantity
    // 
    // PRW16.00.06
    ...
    begin
        ...
        SetDefNewExpirationDate(ItemNo, VariantCode); // P8000673
        ...
        if not QtyToHandleAndInvoiceIsSet then begin
            ...
            // PR3.60.03
            if ReservEntry.TrackAlternateUnits and (not ReservEntry.CatchAlternateQtys) then begin
                AltQtyMgmt.InitAlternateQty(ReservEntry."Item No.", 0,
                  ReservEntry."Qty. to Handle (Base)", ReservEntry."Quantity (Alt.)");
        ...
        if (Status = Status::Reservation) or (Status = Status::Tracking) then begin
            ...
            ReservEntry2."Quantity (Alt.)" := -ReservEntry."Quantity (Alt.)"; // P8000060A
            ReservEntry2."Qty. to Handle (Alt.)" := -ReservEntry."Qty. to Handle (Alt.)";   // P8000756
            ReservEntry2."Qty. to Invoice (Alt.)" := -ReservEntry."Qty. to Invoice (Alt.)"; // P8000756
            ...
            if not QtyToHandleAndInvoiceIsSet then begin
                ...
                ReservEntry2."Qty. to Handle (Alt.)" := ReservEntry2."Quantity (Alt.)";  // P8000060A
                ReservEntry2."Qty. to Invoice (Alt.)" := ReservEntry2."Quantity (Alt.)"; // P8000060A
            end;
   ...
   // P800131478 - Microsoft has removed this overload, but I use it a lot so will retain it
    ...
    procedure AddAltQtyData(AltQty: Decimal)
    begin
        // PR3.61
        InsertReservEntry."Quantity (Alt.)" := AltQty; // P8001035
        InsertReservEntry."Qty. to Handle (Alt.)" := AltQty;
        InsertReservEntry."Qty. to Invoice (Alt.)" := AltQty;
    end;
    ...
    begin
        // P8000673, P8000747
        with InsertReservEntry do
            if ("Source Type" = DATABASE::"Item Journal Line") and
               ("Source Subtype" = 4) and ("New Expiration Date" = 0D)
            then
                "New Expiration Date" :=
                  ItemTrkgMgt.ExistingExpirationDate(
                    ItemNo, VariantCode, "New Lot No.", "New Serial No.", false, EntriesExist);
    end;

    procedure SetNewLotStatus(NewLotStatusCode: Code[10])
    begin
        // P8001083
        InsertReservEntry."New Lot Status Code" := NewLotStatusCode;
    end;

    procedure SetSupplierLotNo(SupplierLotNo: Code[50])
    begin
        InsertReservEntry."Supplier Lot No." := SupplierLotNo; // P8001106
    end;

    procedure SetLotCreationDate(LotCreationDate: Date)
    begin
        // P8008351
        InsertReservEntry."Lot Creation Date" := LotCreationDate;
    end;

    procedure SetCountryOfOrigin(CountryOfOrigin: Code[10])
    begin
        // P8008351
        InsertReservEntry."Country/Region of Origin Code" := CountryOfOrigin;
    end;
