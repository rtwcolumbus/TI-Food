codeunit 6501 "Item Tracking Data Collection"
{
    ...
    //   Moved IsLotAvailableForSale from codeunit 6500 - re-implement the modification, add to FEFO, Kitting
    // 
    // PRW15.00.01
    // P8000538A, VerticalSoft, Jack Reynolds, 22 OCT 07
    //  RefreshLotSNAvailability - fix problem where this function is changing the current record
    // 
    // P8000548A, VerticalSoft, Don Bresee, 05 MAR 08
    //   Use "Near-Zero Qty. Value" from Invt. Setup in availability check
    // 
    // P8000599A, VerticalSoft, Don Bresee, 14 MAY 08
    //   Remove P8000538A - fixed in SP1
    // 
    // PRW15.00.02
    // P8000621A, VerticalSoft, Don Bresee, 05 AUG 08
    //   Rework implementation of Lot Picking Method for FIFO & LIFO (P8000466A)
    // 
    // PRW16.00.04
    ...
    //   Add "Supplier Lot No." field to Entry Summary
    // 
    // PRW111.00
    ...
    var
        ...
        Text37002000: Label 'Lot %1 fails to meet established lot preferences.';
        RequiredCountryOfOrigin: Code[10];
    ...
    begin
        ...
        RequiredCountryOfOrigin := GetCountryOfOrigin(TempTrackingSpecification); // P8000946
        IsHandled := false;
        ...
        Clear(RequiredCountryOfOrigin); // P8000946
        ...
        if ItemTrackingSummaryForm.RunModal = ACTION::LookupOK then begin
            ...
            if LookupMode = LookupMode::"Lot No." then
                if not TempTrackingSpecification.CheckLotPreferences(TempGlobalEntrySummary."Lot No.", true) then
                    Error(Text37002000, TempGlobalEntrySummary."Lot No."); // P8001070
                                                                           // P8000153A End
    ...
    begin
        ...
        ItemTrackingSummaryPage.SetLotStatus(TempTrackingSpecification."Item No.", TempTrackingSpecification."Variant Code", // P8001083
            LotStatusMgmt.TrackingSpecToFieldNo(TempTrackingSpecification)); // P8001083, P80045713
    end;
    ...
    begin
        ...
        ItemTrackingSummaryForm.SetCurrentBinAndItemTrkgCode(CurrBinCode, CurrItemTrackingCode);
        ItemTrackingSummaryForm.SetLotStatus(TempTrackingSpecification."Item No.", TempTrackingSpecification."Variant Code", // P8001083
          LotStatusMgmt.TrackingSpecToFieldNo(TempTrackingSpecification)); // P8001083, P80045713
            ...
            repeat
                ...
                then begin
                    ...
                    if not IsHandled then
                        if IsLotAvailableForSale(TrackingSpecification, TempGlobalReservEntry."Lot No.") and // P800466A
                           IsLotFresh(TrackingSpecification, TempGlobalReservEntry) // P8000899
                        then begin
            ...
            repeat
                if not TempGlobalReservEntry.Get(TempReservEntry."Entry No.", TempReservEntry.Positive) then begin
                    ...
                    if not IsHandled then begin
                        if IsLotAvailableForSale(TrackingSpecification, TempGlobalReservEntry."Lot No.") and // P8000466A
                            IsLotFresh(TrackingSpecification, TempGlobalReservEntry) // P8000899
                        then begin
                            TempGlobalReservEntry.Insert();
                            CreateEntrySummary(TrackingSpecification, TempGlobalReservEntry);
    ...
    begin
        ...
        if TempReservEntry.Positive then begin
            ...
            TempGlobalEntrySummary."Freshness Date" := GetLotFreshDate(TempReservEntry); // P8000899
            TempGlobalEntrySummary."Supplier Lot No." := GetSupplierLotNo(TempReservEntry); // P8001106
    ...
    begin
        ...
        TempGlobalEntrySummary.SetTrackingKey();
        // P8001083
        if LotStatusMgmt.TrackingSpecBlocked(TempTrackingSpecification) then
            exit(false);
        // P8001083
        ...
        InvtSetup.Get;
        exit(TempGlobalEntrySummary."Total Available Quantity" >= -InvtSetup."Near-Zero Qty. Value");
        // P8000548A
    end;
    ...
    local procedure IsLotAvailableForSale(var TrackingSpecification2: Record "Tracking Specification"; LotNo: Code[50]): Boolean
    begin
        // P8000322A, P8000466A
        with TrackingSpecification2 do
            if ("Source Type" = DATABASE::"Sales Line") then
                exit(IsLotAvailableForSale2("Item No.", "Variant Code", LotNo));
        exit(true);
        // P8000322A
    end;

    procedure IsLotAvailableForSale2(ItemNo: Code[20]; VariantCode: Code[10]; LotNo: Code[50]): Boolean
    var
    ...
    begin
        ...
        if LotNoInfo.Get(ItemNo, VariantCode, LotNo) then
            // P8000946
            if (RequiredCountryOfOrigin <> '') and (RequiredCountryOfOrigin <> LotNoInfo."Country/Region of Origin Code") then
                exit(false)
            else
                // P8000946
                exit(not LotNoInfo.Blocked);
        exit(true);
        // P8000466A
    end;

    procedure IsLotFresh(TrackingSpecification: Record "Tracking Specification" temporary; ReservEntry: Record "Reservation Entry"): Boolean
    var
        P800ItemTracking: Codeunit "Process 800 Item Tracking";
        P800Func: Codeunit "Process 800 Functions";
    begin
        // P8000899
        if not P800Func.TrackingInstalled then
            exit(true);
        if P800ItemTracking.VerifyReservLotIsFresh(TrackingSpecification, ReservEntry) then
            exit(true);
    end;
    ...
    begin
        ...
        if not P800Func.TrackingInstalled then
            exit(0D);
        exit(P800ItemTracking.GetLotFreshDate(ReservEntry))
    end;

    procedure GetCountryOfOrigin(TrackingSpec: Record "Tracking Specification") CountryOfOrigin: Code[10]
    var
        SalesLine: Record "Sales Line";
    begin
        // P8000946
        if TrackingSpec."Source Type" = DATABASE::"Sales Line" then begin
            ...
            CountryOfOrigin := SalesLine."Country/Region of Origin Code";
        end;
    end;

    local procedure GetSupplierLotNo(var ReservEntry: Record "Reservation Entry"): Code[50]
    var
        LotNoInfo: Record "Lot No. Information";
    begin
        // P8001106
        with ReservEntry do begin
            if ("Supplier Lot No." <> '') then
                exit("Supplier Lot No.");
            if LotNoInfo.Get("Item No.", "Variant Code", "Lot No.") then
                exit(LotNoInfo."Supplier Lot No.");
        end;
    end;
