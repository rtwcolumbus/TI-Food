...
{
    // PRW16.00.03
    // P8000790, VerticalSoft, Don Bresee, 18 MAR 10
    //   Set CAPTION property based on Production BOM type (1.20)
    // 
    // PRW16.00.06
    ...
    actions
    {
        area(processing)
        {
            action("&Show Matrix")
            {
                ...
                begin
                    Clear(MatrixForm);
                    MatrixForm.Load(MATRIX_CaptionSet, MatrixRecords, MATRIX_CurrSetLength, ProdBOM, ShowLevel, LocationCode); // P8001030
                    MatrixForm.RunModal;
                end;
            }
            action("Previous Set")
    ...
    trigger OnOpenPage()
    begin
        CurrPage.Caption(StrSubstNo(Text37002000, Format(ProdBOM."Mfg. BOM Type"))); // PR1.20
        BuildMatrix;

        EnableLocation := ShowLevel = ShowLevel::Multi; // P8001030
    end;
    ...
    var
        ...
        Text37002000: Label 'Prod. %1 Matrix per Version';
    ...
    begin
        // P8001030
        if ShowLevel = ShowLevel::Single then
            LocationCode := '';
        EnableLocation := ShowLevel = ShowLevel::Multi; // P8001030
        // P8001030
        BuildMatrix;
