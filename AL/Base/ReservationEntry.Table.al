table 337 "Reservation Entry"
{
    ...
    // 
    // PR3.70.01
    //   Add "Qty. to Handle (Alt.)" as sum index field to key
    //     Reservation Status,Item No.,Variant Code,Location Code,Source Type,Source Subtype,Lot No.,Serial No.
    // 
    // PR3.70.08
    // P8000183A, Myers Nissi, Jack Reynolds, 14 FEB 05
    //   Add Quantity (Alt.) as sum index field
    // 
    // PRW16.00.06
    ...
    fields
    {
        ...
        field(37002023; "Qty. (Phys. Inventory)"; Decimal)
        {
            ...
            begin
                if "Serial No." <> '' then
                    if not ("Qty. (Phys. Inventory)" in [0, 1]) then
                        Error(Text37002000);

                Validate("Quantity (Base)", "Qty. (Phys. Inventory)" - "Qty. (Calculated)"); // PR3.60
                Positive := "Quantity (Base)" >= 0;
                //IF Positive <> xRec.Positive THEN BEGIN // PR3.61
                //  IF xRec.DELETE THEN;                  // PR3.61
                //    INSERT;                             // PR3.61
                //END;                                    // PR3.61
                if Positive then
                ...
                // P800ItemTracking.UpdateItemJnlPhysQty(Rec); // PR3.61
            end;
        }
        ...
        field(37002081; "Quantity (Alt.)"; Decimal)
        {
            ...
            var
                P800ItemTracking: Codeunit "Process 800 Item Tracking";
            begin
                "Qty. to Handle (Alt.)" := "Quantity (Alt.)";  // PR3.60
                "Qty. to Invoice (Alt.)" := "Quantity (Alt.)"; // PR3.60
            end;
        }
        ...
        field(4; "Quantity (Base)"; Decimal)
        {
            ...
            begin
                ...
                // PR3.60.03
                if TrackAlternateUnits and (not CatchAlternateQtys) then begin
                    AltQtyMgmt.InitAlternateQty("Item No.", 0, "Quantity (Base)", "Quantity (Alt.)");
                    "Qty. to Handle (Alt.)" := "Quantity (Alt.)";
    ...
    keys
    {
        ...
        key(Key2; "Source ID", "Source Ref. No.", "Source Type", "Source Subtype", "Source Batch Name", "Source Prod. Order Line", "Reservation Status", "Shipment Date", "Expected Receipt Date")
        {
            ...
            SumIndexFields = "Quantity (Base)", Quantity, "Qty. to Handle (Base)", "Qty. to Invoice (Base)", "Quantity (Alt.)", "Qty. to Handle (Alt.)", "Qty. to Invoice (Alt.)";
        }
...
#pragma warning restore AS0009
        {
            MaintainSIFTIndex = false;
            SumIndexFields = "Quantity (Base)", Quantity, "Quantity (Alt.)", "Qty. to Invoice (Alt.)";
        }
#pragma warning disable AS0009
...
#pragma warning restore AS0009
        ...
        key(Key37002000; "Source Type", "Source Subtype", "Source ID", "Source Batch Name", "Source Prod. Order Line", "Source Ref. No.", "Reservation Status", "Expected Receipt Date")
        {
        }
        key(Key37002001; "Reservation Status", "Item No.", "Variant Code", "Location Code", "Source Type", "Source Subtype", "Shipment Date")
        {
            SumIndexFields = "Quantity (Base)", Quantity;
        }
        key(Key37002002; "Reservation Status", "Item No.", "Variant Code", "Location Code", "Source Type", "Source Subtype", "Lot No.", "Serial No.")
        {
            SumIndexFields = "Quantity (Base)", Quantity, "Qty. to Handle (Alt.)";
        }
        key(Key37002003; "Source Type", "Source ID", "Source Batch Name", "Source Ref. No.", "Lot No.", "Serial No.")
        {
            SumIndexFields = "Quantity (Base)", Quantity;
        }
        key(Key37002004; "Source Type", "Item No.", "Variant Code", "Lot No.")
        {
            SumIndexFields = "Quantity (Base)", "Qty. (Phys. Inventory)", "Qty. (Alt.) (Phys. Inventory)";
        }
    }
    ...
    var
        ...
        Text37002000: Label 'Serialized items may only have counts of 0 or 1.';
    ...
    begin
        ...
        IsSameTracking := IsSameTracking and ("New Lot Status Code" = ReservEntry."New Lot Status Code"); // P8001083
    ...
    begin
        ...
        "Supplier Lot No." := TrackingSpecification."Supplier Lot No."; // P8001167
        "Lot Creation Date" := TrackingSpecification."Lot Creation Date";                         // P8008351
        "Country/Region of Origin Code" := TrackingSpecification."Country/Region of Origin Code"; // P8008351
    ...
    begin
        ...
        "New Lot Status Code" := TrackingSpecification."New Lot Status Code"; // P8001083
    ...
    begin
        ...
        "New Lot Status Code" := ReservEntry."New Lot Status Code"; // Upgrade18.0
    ...
    [Obsolete('FOOD-Replaced by AutoIncrement property', '18.1')]
    procedure AssignEntryNo()
    var
    ...
    begin
        // PR3.60
        if "Source Type" in [DATABASE::"Sales Line", DATABASE::"Purchase Line"] then
            exit("Source Subtype");
        // PR3.60
    end;
