page 37002173 "Purchase Order Guide"
{
    ...
    begin
        ...
        while SignalFns.EventPending(2) do begin
            ...
            if CopyStr(EventText, 1, 5) = 'CLOSE' then begin
                AlwaysClose := true;
                CurrPage.Close;
                // P8001230
            end else
                if CopyStr(EventText, 1, 11) = 'ALLOWUNAPPR' then begin
                    OrderGuideMgmt.SetAllowUnapproved(CopyStr(EventText, 13) = Format(true));
                    CurrPage.Update(false);
                end;
            // P8001230
        end;
