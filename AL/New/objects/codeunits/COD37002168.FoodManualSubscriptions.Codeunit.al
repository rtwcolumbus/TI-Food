codeunit 37002168 "Food Manual Subscriptions"
{
    ...
    // P800162917
    procedure SetPostingLocation(PostingLocation: Code[10])
    begin
        PostingLocationCode := PostingLocation;
    end;

    // P800162917
    [EventSubscriber(ObjectType::Codeunit, Codeunit::"Sales-Post", 'OnCodeGetPostingLocation', '', false, false)]
    local procedure SalesPost_OnCodeGetPostingLocation(var PostingLocation: Code[10])
    begin
        if PostingLocation = '' then
            PostingLocation := PostingLocationCode;
    end;

    // P800162917
    [EventSubscriber(ObjectType::Codeunit, Codeunit::"Purch.-Post", 'OnCodeGetPostingLocation', '', false, false)]
    local procedure PurchPost_OnCodeGetPostingLocation(var PostingLocation: Code[10])
    begin
        if PostingLocation = '' then
            PostingLocation := PostingLocationCode;
    end;
