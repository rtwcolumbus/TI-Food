codeunit 37002166 "Event Subscribers (Core)"
{
    ...
    //
    // P800164877, To Increase, Jack Reynolds, 13 MAR 23
    //   Prevent Rounding in 'Qty per Unit of Measure' field of 'Item Unit of Measure' table
    ...
    // P800164877
    [EventSubscriber(ObjectType::Codeunit, Codeunit::"Config. Validate Management", 'OnBeforeRoundDecimalValue', '', true, false)]
    local procedure ConfigValidateManagement_OnBeforeRoundDecimalValue(var DecimalFieldRef: FieldRef; var DecimalValue: Decimal; var ShouldRound: Boolean)
    var
        RecordRef: RecordRef;
        ItemUnitofMeasure: Record "Item Unit of Measure";
    begin
        RecordRef := DecimalFieldRef.Record();
        if (RecordRef.Number = Database::"Item Unit of Measure") and (DecimalFieldRef.Number = ItemUnitofMeasure.FieldNo("Qty. per Unit of Measure")) then
            ShouldRound := false;
    end;
