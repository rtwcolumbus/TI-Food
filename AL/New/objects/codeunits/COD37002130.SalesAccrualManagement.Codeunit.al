codeunit 37002130 "Sales Accrual Management"
{
    ...
    //
    // PRW121.2
    // P800138545, To-Increase, JAck Reynolds 22 FEB 23
    //   Event in AccumSalesLineAmounts for PriceAmount
    ...
    begin
        // P8000274A - add parameter SalesHeader
        with AccrualPlan do begin
            GetSalesQtys(AccrualPlan, SalesLine, PricingQty, QtyBase, QtyAlt);
            ...
            AccumSalesLineAmountsOnBeforeSetPriceAmount(AccrualPlan, SalesHeader, SalesLine, PricingQty, PriceAmount, IsHandled); // P800138545
            if not IsHandled then begin // P800138545
                PriceAmount := PriceAmount +
                  (SalesLine."Unit Price" * PricingQty) -
                ...
                PriceAmount := PriceAmount - SalesLine."Accrual Amount (Price)" * PricingQty;
                // P8000767
    ...
    procedure GetSalesQtys(var AccrualPlan: Record "Accrual Plan"; var SalesLine: Record "Sales Line"; var PricingQty: Decimal; var QtyBase: Decimal; var QtyAlt: Decimal)
    begin
        with SalesLine do begin
            PricingQty := GetPricingQty();
    ...
    begin
        ...
        with AccrualSearchLine do begin
            ...
            IF (SalesHeader."Document Type" in [SalesHeader."Document Type"::Invoice, SalesHeader."Document Type"::"Credit Memo"]) and // P8005495
             (SalesHeader."Document Date" <> 0D)  // P800149934 // P8005495
            then begin
                SetRange("Date Type", "Date Type"::"Order Date");
                SetFilter("Start Date", '..%1', SalesHeader."Document Date"); // P800149934
                SetFilter("End Date", '%1|%2..', 0D, SalesHeader."Document Date"); // P800149934
                SalesInsertCalcLines(AccrualSearchLine, SalesHeader, SalesLine);
            end;
            // P800138545            
            // END;                   // P8000767
    ...
    local procedure AccumSalesLineAmountsOnBeforeSetPriceAmount(var AccrualPlan: Record "Accrual Plan"; var SalesHeader: Record "Sales Header"; var SalesLine: Record "Sales Line"; PricingQty: Decimal; var PriceAmount: Decimal; var IsHandled: Boolean)
    begin
    end;
}
