report 37002461 "Production Ticket"
{
        ...
        dataitem("Production Order"; "Production Order")
        {
            ...
            dataitem(PageLoop; "Integer")
            {
                ...
                dataitem(ProdOrderLine; "Integer")
                {
                    ...
                    column(OrderLabel1; OrderLabel[1])
                    {
                    }
                    column(OrderLabel4; OrderLabel[4])
                    {
                    }
                    column(OrderLabel2; OrderLabel[2])
                    {
                    }
                    column(OrderSource; OrderSource)
                    {
                    }
                    column(OrderLabel3; OrderLabel[3])
                    {
                    }
                    column(OrderShipDate; OrderShipDate)
                    {
                    }
                    ...
                    begin
                        ...
                        if "Production Order"."Order Type" = "Production Order"."Order Type"::Package then // PR1.20
                            GetSourceOrderinfo(TempProdOrderLine);                                         // PR1.20, PR3.70.03, P800150458
                ...
                dataitem(PkgOrder; "Production Order")
                {
                    ...
                    dataitem(PkgLine; "Prod. Order Line")
                    {
                        ...
                        column(PkgLineOrderSource; OrderSource)
                        {
                        }
                        ...
                        column(PkgLineOrderShipDate; OrderShipDate)
                        {
                        }
                        column(PkgLineOrderLabel1; OrderLabel[1])
                        {
                        }
                        column(PkgLineOrderLabel2; OrderLabel[2])
                        {
                        }
                        column(PkgLineOrderLabel3; OrderLabel[3])
                        {
                        }
                        column(PkgLineOrderLabel4; OrderLabel[4])
                        {
                        }
                        ...
                        var
                            ...
                            VersionNo: Code[20];
                        begin
                            GetSourceOrderinfo(PkgLine); // PR1.20, P800150458
    ...
    rendering
    {
        layout(StandardRDLCLayout)
        {
            ...
            Type = RDLC;
            LayoutFile = './layout/ProductionTicket.rdlc';
        }
    }
    ...
    // P800150458
    local procedure GetSourceOrderinfo(ProdOrderLine: Record "Prod. Order Line")
    var
        ProdXref: Record "Production Order XRef";
        LabelStockOrder: Label 'STOCK ORDER';
    begin
        Clear(OrderNo);
        Clear(OrderSource);
        Clear(OrderShipDate);
        ...
        ProdXref.SetRange("Source Table ID", DATABASE::"Sales Line");
        if ProdXref.FindFirst() then
            GetSalesOrderInfo(ProdXref, OrderNo, OrderSource, OrderShipDate, OrderLabel)
        else begin
            ProdXref.SetRange("Source Table ID", DATABASE::"Transfer Line");
            if ProdXref.FindFirst() then
                GetTransferOrderInfo(ProdXref, OrderNo, OrderSource, OrderShipDate, OrderLabel)
            else
                OrderLabel[4] := LabelStockOrder;
        end;
    end;

    // P800150458
    local procedure GetSalesOrderInfo(ProdXref: Record "Production Order XRef"; var OrderNo: Code[20]; var OrderSource: Text; var OrderShipDate: Date; var OrderLabel: array[4] of Text[30])
    var
        SalesHeader: Record "Sales Header";
        LabelOrderNo: Label 'Sales Order:';
        LabelOrderSource: Label 'Customer:';
        LabelOrderShipDate: Label 'Ship:';
    begin
        if SalesHeader.Get(ProdXref."Source Type", ProdXref."Source No.") then begin
            OrderNo := SalesHeader."No.";
            OrderLabel[1] := LabelOrderNo;
            OrderSource := SalesHeader."Sell-to Customer Name";
            OrderLabel[2] := LabelOrderSource;
            OrderShipDate := SalesHeader."Shipment Date";
            OrderLabel[3] := LabelOrderShipDate;
        end;
    end;

    // P800150458
    local procedure GetTransferOrderInfo(ProdXref: Record "Production Order XRef"; var OrderNo: Code[20]; var OrderSource: Text; var OrderShipDate: Date; var OrderLabel: array[4] of Text[30])
    var
        TransferHeader: Record "Transfer Header";
        Location: Record Location;
        LabelOrderNo: Label 'Trans. Order:';
        LabelOrderSource: Label 'To:';
        LabelOrderShipDate: Label 'Ship:';
    begin
        if TransferHeader.Get(ProdXref."Source No.") then begin
            OrderNo := TransferHeader."No.";
            OrderLabel[1] := LabelOrderNo;
            Location.Get(TransferHeader."Transfer-to Code");
            if Location.Name <> '' then
                OrderSource := Location.Name
            else
                OrderSource := Location.Code;
            OrderLabel[2] := LabelOrderSource;
            OrderShipDate := TransferHeader."Shipment Date";
            OrderLabel[3] := LabelOrderShipDate;
        end;
    end;
