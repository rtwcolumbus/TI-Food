table 37002801 Asset
{
    ...
    begin
        ...
        if 0 < StrLen("Asset Hierarchy") then begin
            // P800163700
            Hierarchy := xHierarchy.Split(Delimiter());
            xNo := Hierarchy.Get(2);
            if Hierarchy.Count() > 2 then
                xParent := Hierarchy.Get(3);
            "Asset Hierarchy" := CopyStr("Asset Hierarchy", 2 + StrLen(xNo));
            // P800163700
        end else
