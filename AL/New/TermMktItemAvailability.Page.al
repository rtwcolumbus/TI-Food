page 37002662 "Term. Mkt. Item Availability"
{
    ...
    begin
        ...
        SignalFns.ClearControl(2); // P80059471
    end;

    trigger OnFindRecord(Which: Text): Boolean
    begin
        // ItemCategory[1] := GETFILTER("Item Category Code");
        // ItemCategory[2] := ItemCategory[1];
        // ItemNo[1] := GETFILTER("Item No.");
        // ItemNo[2] := ItemNo[1];
        // VariantCode[1] := GETFILTER("Variant Code");
        // VariantCode[2] := VariantCode[1];
    ...
    var
        ...
        UpdatePage: Boolean;
    begin
        while SignalFns.EventPending(2) do begin
            EventText := SignalFns.GetNextEvent(2);
            if CopyStr(EventText, 1, 5) = 'CLEAR' then begin
                ClearFilters;
                LocationCode := '';
                CustomerNo := '';
                ShipDate := 0D;
                ...
                if CopyStr(EventText, 1, 6) = 'ORDER,' then begin
                    SalesHeader.Get(SalesHeader."Document Type"::Order, CopyStr(EventText, 7));
                    if (OrderNo <> SalesHeader."No.") or
                       (CustomerNo <> SalesHeader."Sell-to Customer No.") or
                       (LocationCode <> SalesHeader."Location Code") or
                    ...
                    if CopyStr(EventText, 1, 5) = 'ITEM,' then begin
                        ItemLotAvail.Copy(Rec);
                        SharedItemLotAvail.Reset;
                        SharedItemLotAvail.SetRange("Item No.", CopyStr(EventText, 6));
                        if SharedItemLotAvail.Find('-') then
                            repeat
                                Rec := SharedItemLotAvail;
                                Modify;
                            until SharedItemLotAvail.Next = 0;
                        Rec.Copy(ItemLotAvail);
                        CurrPage.Update(true);
                    end else
                        if CopyStr(EventText, 1, 5) = 'CLOSE' then
                            CurrPage.Close; // P80059471
        end;
