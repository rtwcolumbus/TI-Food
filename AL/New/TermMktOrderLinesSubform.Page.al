page 37002661 "Term. Mkt. Order Lines Subform"
{
    ...
    begin
        ...
        if Type = Type::Item then
            UpdateAvail := UpdateAvailability("No.", "Variant Code", "Lot No.", "Country/Region of Origin Code",
              -RepackLine."Target Quantity", -"Quantity (Base)", 0, SalesSetup."Terminal Market Item Level");

        if Repack then
            UpdateAvail := UpdateAvail or
    ...
    begin
        ...
        if SharedItemLotAvail.Get(ItemNo, VariantCode, LotNo, Country) then begin
            ...
            // SignalFns.AddEvent(2, StrSubstNo('ITEM,%1', ItemNo));
            exit(true);
        end;
    end;
