...
page 99000919 "Demand Forecast"
{
    ...
    // 
    // PRW17.00.01
    // P8001171, Columbus IT, Jack Reynolds, 13 JUN 13
    //   Change to varaible name
    ...
    layout
    {
        area(content)
        {
            group(General)
            {
                ...
                field("<VarinatFilter>"; VariantFilter)
                {
                    ...
                    trigger OnLookup(var Text: Text): Boolean
                    var
                        VarList: Page Variants;
                        VarRec: Record Variant;
                    begin
                        // P8000869
                        VarList.LookupMode(true);
                        VarList.SetTableView(VarRec); // P8001171
                        if not (VarList.RunModal = ACTION::LookupOK) then
                            exit(false)
                        else begin
                            VarList.GetRecord(VarRec);  // P8001171
                            Text := VarRec.Code;        // P8001171
                        end;
    ...
    begin
        CurrPage.Matrix.PAGE.LoadMatrix(
          MatrixColumnCaptions, MatrixRecords, ProductionForecastName, DateFilter, LocationFilter, VariantFilter, ForecastType, // P8000869
