page 42 "Sales Order" // Version: FOODNA
{
    // --------------------------------------------------------------------------------
    ...
    // 
    // PR3.61.01
    //   Add menu button items for container specification
    // 
    // PR3.70
    ...
    //  Added Exclude from sales Forcast control to invoice tab
    // 
    // PR3.70.07
    // P8000126A, Myers Nissi, Steve Post, 17 NOV 04
    //   Added field Allow Order Below Minimum to Invoicing tab
    //   Message i below minimum when displaying statistics
    // 
    ...
    //   Incorporate SP1 changes - move controls
    // 
    // PRW16.00.01
    ...
    //   Added "FOB Pricing" field and "Calculate Freight" action
    // 
    // PRW16.00.06
    // P8000999, Columbus IT, Jack Reynolds, 09 DEC 11
    //   Support for non-modal operation of order guide
    // 
    ...
    // 
    // PRW17.00.01
    // P8001156, Columbus IT, Jack Reynolds, 20 MAY 13
    //   Fix problem running form web client
    // 
    // PRW17.10.01
    // P8001252, Columbus IT, Jack Reynolds, 06 JAN 14
    //   Bring NA modification to print pick tickets into W1 version
    // 
    // PRW17.10.02
    // P8001265, Columbus IT, Jack Reynolds, 21 JAN 14
    //   Change Order Guide shortcut
    // 
    // PRW18.00
    ...
    //   CS00076976 - Quantity in alternate quantity enry is incorrect
    ...
    layout
    {
        area(content)
        {
            group(General)
            {
                ...
                field(Control11028580; Settlement)
                {
            ...
            group("Invoice Details")
            {
                ...
                field("Allow Order Below Minimum"; "Allow Order Below Minimum")
                {
                    ApplicationArea = FOODBasic;
                }
                field("FOB Pricing"; "FOB Pricing")
                {
                    ApplicationArea = FOODBasic;
                    Importance = Additional;
        ...
        area(navigation)
        {
            group("O&rder")
            {
                ...
                action("Order &Guide")
                {
                    ...
                    trigger OnAction()
                    var
                        OrderGuide: Page "Sales Order Guide";
                    begin
                        // P8000999
                        // if SignalFns.GetControlID(2) <> '' then
                        //     exit;

                        TestField(Status, Status::Open);
                        TestField("Bill-to Customer No.");

                        OrderGuide.SetSignalFns(SignalFns);
                        OrderGuide.SetDocument(Rec);
                        OrderGuide.Run;
                    end;
                }
                action("&Alternate Quantities")
                {
                    ApplicationArea = FOODBasic;
                    AccessByPermission = TableData "Alternate Quantity Line" = R;
                    Caption = '&Alternate Quantities';
                    ...
                    begin
                        // PR3.60 Begin
                        SalesOrder.Copy(Rec);
                        SalesOrder.FilterGroup(3);
                        SalesOrder.SetRecFilter;
                        SalesOrder.FilterGroup(0);
                        PAGE.RunModal(PAGE::"Sales Order Alt. Qty. Entry", SalesOrder);
                ...
                action("Con&tainers")
                {
                    ...
                    trigger OnAction()
                    begin
                        ContainerSpecification; // PR3.61.01
                    end;
                }
                ...
                action("Data Sheets")
                {
                    ...
                    begin
                        // P8001090
                        DataCollectionMgmt.DataSheetsForSalesHeader(Rec);
                    end;
                }
                action(DocAttach)
                {
...
#pragma warning restore
        ...
        area(processing)
        {
            ...
            group("F&unctions")
            {
                ...
                action("Calculate &Freight")
                {
                    ...
                    trigger OnAction()
                    begin
                        CalculateFreight(true); // P8000921
                    end;
                }
                ...
                action(Settlement)
                {
                    ...
                    begin
                        TripSettlementMgt.SettlementSlsOrder(Rec);
                    end;
                }
    ...
    begin
        if xRec."No." <> "No." then // P8000999
            CloseOrderGuide;          // P8000999
    ...
    var
        ...
        Usage: Option "Order Confirmation","Work Order","Pick Instruction","Pick Ticket";
        [InDataSet]
        JobQueueVisible: Boolean;
        Text001: Label 'Do you want to change %1 in all related records in the warehouse?';
        Text002: Label 'The update has been interrupted to respect the warning.';
        DynamicEditable: Boolean;
        Text37002000: Label 'Order is below minimum %1.';
        SignalFns: Codeunit "Process 800 Signal Functions";
        Text37002001: Label 'Order guide must not be open.';
    ...
    procedure CloseOrderGuide()
    begin
        // // P8000999
        // if SignalFns.GetControlID(2) <> '' then begin
        //     SignalFns.AddEvent(2, 'CLOSE');
