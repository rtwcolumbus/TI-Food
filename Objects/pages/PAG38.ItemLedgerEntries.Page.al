page 38 "Item Ledger Entries"
{
    ...
    //   Made form editable and made all displayed controls
    //   non editable except Exclude from Sales Forecast so
    //   individual ILE lines can be excluded
    //   Added call to OnModifyRecord trigger
    // 
    // PR3.70.07
    // P8000156A, Myers Nissi, Steve Post, 17 DEC 04
    //   Change InsertAllowed and DeleteAllowed
    // 
    // PR4.00.04
    // P8000370A, VerticalSoft, Jack Reynolds, 08 SEP 06
    //   Add menu item to run cost trace
    // 
    // PR5.00
    // P8000466A, VerticalSoft, Don Bresee, 26 JUN 07
    //   Add Item Tracing to Functions menu
    //   Add Document No. and Lot No. to captioning
    //   Added controls for Shipped Qty. Not Ret. (Alt.)
    // 
    // PRW15.00.01
    ...
    // 
    // P8001019, Columbus IT, Jack Reynolds, 25 SEP 12
    //   Drill down for Account Schedule Item Units
    // 
    // PRW17.00
    ...
    layout
    {
        area(content)
        {
            repeater(Control1)
            {
                ...
                field(LotStatusCode; LotStatusCode)
                {
                    ApplicationArea = FOODBasic;
                    Caption = 'Lot Status Code';
                    Style = Attention;
                    StyleExpr = LotExcluded;
                    Visible = false;
                }
                ...
                field("Quantity (Alt.)"; Rec."Quantity (Alt.)")
                {
                    ...
                    trigger OnDrillDown()
                    begin
                        AltQtyMgmt.ShowItemLedgAltQtyEntries(Rec, FieldNo("Quantity (Alt.)")); // PR3.60
                    end;
                }
                field("Invoiced Quantity (Alt.)"; Rec."Invoiced Quantity (Alt.)")
                {
                    ApplicationArea = FOODBasic;

                    trigger OnDrillDown()
                    begin
                        AltQtyMgmt.ShowItemLedgAltQtyEntries(Rec, FieldNo("Invoiced Quantity (Alt.)")); // PR3.60
                    end;
                }
                field("Remaining Quantity (Alt.)"; Rec."Remaining Quantity (Alt.)")
                {
                    ApplicationArea = FOODBasic;
                }
                ...
                field("Sales Amount (FOB)"; Rec."Sales Amount (FOB)")
                {
                    ApplicationArea = FOODBasic;
                    Visible = false;
                }
                field("Sales Amount (Freight)"; Rec."Sales Amount (Freight)")
                {
                    ApplicationArea = FOODBasic;
                    Visible = false;
                }
    ...
    actions
    {
        area(navigation)
        {
            group("Ent&ry")
            {
                ...
                action("Alternate Quantity Entries")
                {
                    ApplicationArea = FOODBasic;
                    Caption = 'Alternate Quantity Entries';
                    Image = Entries;
                    RunObject = Page "Alternate Quantity Entries";
                    RunPageLink = "Table No." = CONST(32),
                                  "Source Line No." = FIELD("Entry No.");
                    RunPageView = SORTING("Table No.", "Document No.", "Source Line No.", "Line No.");
                }
            }
            group("&Application")
            {
        ...
        area(processing)
        {
            group("F&unctions")
            {
                ...
                action("&Lot Tracing")
                {
                    ...
                    trigger OnAction()
                    var
                        SelectedEntry: Record "Item Ledger Entry";
                    begin
                        CurrPage.SetSelectionFilter(SelectedEntry); // P8000984
                        OpenLotTracing(SelectedEntry); // P8000979, P8000984
                    end;
                }
            }
            action("&Navigate")
    ...
    begin
        LotStatusMgmt.GetLotStatus("Item No.", "Variant Code", "Lot No.", LotStatusExclusionFilter, LotStatusCode, LotExcluded); // P8001083
    end;
    ...
    begin
        // P8001019
        if not (UseDimFilter[3] or UseDimFilter[4] or UseDimFilter[5] or UseDimFilter[6]) then
            exit(Find(Which));
        ...
        for i := 1 to StrLen(Which) do begin
            ...
            while (not EOF) and (not ShowRecord("Dimension Set ID")) do // P8001133
    ...
    var
        NextRec: Record "Item Ledger Entry";
        Direction: Integer;
        NoSteps: Integer;
        StepsTaken: Integer;
        EOF: Boolean;
    begin
        ...
        while (StepsTaken < NoSteps) and (not EOF) do begin
            EOF := Next(Direction) = 0;
            if (not EOF) and ShowRecord("Dimension Set ID") then begin // P8001133
                NextRec := Rec;
                StepsTaken += 1;
            end;
        end;
    ...
    begin
        ...
        case true of
            ...
            // P8000466A
            GetFilter("Document No.") <> '':
                begin
                    SourceTableName := FieldCaption("Document No.");
                    SourceFilter := GetFilter("Document No.");
                end;
            GetFilter("Lot No.") <> '':
                begin
                    SourceTableName := FieldCaption("Lot No.");
                    SourceFilter := GetFilter("Lot No.");
                end;
        // P8000466A
        end;
        if (Description = '') then                                  // P8000466A
            exit(StrSubstNo('%1 %2', SourceTableName, SourceFilter)); // P8000466A
    ...
    procedure SetDimFilter(UseFilter: array[6] of Boolean; var Value: array[6] of Record "Dimension Value"; var ValueTemp: Record "Dimension Value" temporary)
    var
        Index: Integer;
    begin
        // P8001019
        for Index := 4 to 6 do begin
            UseDimFilter[Index] := UseFilter[Index];
            DimValue[Index].Copy(Value[Index]);
        end;

        DimValueTemp.Copy(ValueTemp, true);
    end;

    procedure ShowRecord(DimensionSetID: Integer) EntryMatch: Boolean
    var
    ...
    begin
        // P8001019
        // P8001133 - EntryNo changed to DimensionSetID
        EntryMatch := true;
        Index := 3;
        while EntryMatch and (Index <= 6) do begin
            if UseDimFilter[Index] then begin
                DimCode := DimValue[Index].GetFilter("Dimension Code");
                if DimensionSetEntry.Get(DimensionSetID, DimCode) then // P8001133
                    EntryMatch := DimValueTemp.Get(DimCode, DimensionSetEntry."Dimension Value Code")
                else
                    EntryMatch := false;
            end;
            Index += 1;
