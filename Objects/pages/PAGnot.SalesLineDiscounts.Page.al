...
{
    // PRW16.00.20
    // P8000664, VerticalSoft, Jack Reynolds, 26 JAN 09
    //   Replace Type by "Item Type" and "Item Code" by "Item Code 1"
    // 
    // PRW16.00.06
        ...
        area(content)
        {
            group(General)
            {
                ...
                field(CodeFilterCtrl; CodeFilter)
                {
                    ...
                    begin
                        Result := true;
                        case "Item Type" of                // PR3.60
                            "Item Type"::Item:               // PR3.60
                                begin
                            ...
                            "Item Type"::"Item Disc. Group": // PR3.60
                                begin
            ...
            repeater(Control1)
            {
                ...
                field("Starting Date"; "Starting Date")
                {
                    ...
                    trigger OnDrillDown()
                    begin
                        if GetFilter("Item Type") <> '' then // PR3.60
                            ItemTypeFilter := "Item Type"      // PR3.60
                    end;
                }
    ...
    begin
        ...
        if ItemTypeFilter <> ItemTypeFilter::None then
            SetRange("Item Type", ItemTypeFilter) // PR3.60
        else
            SetRange("Item Type"); // PR3.60
        ...
        if CodeFilter <> '' then begin
            SetFilter("Item Code", CodeFilter); // PR3.60
        end else
            SetRange("Item Code"); // PR3.60
    ...
    begin
        case GetFilter("Item Type") of // P8001213
            Format(Type::Item):
                exit(0);
    ...
    begin
        ...
        if GetFilter("Item Type") = '' then
            FilterPageBuilder.AddFieldNo(TableCaption, FieldNo("Item Type"));
        if GetFilter("Item Code") = '' then
            FilterPageBuilder.AddFieldNo(TableCaption, FieldNo("Item Code"));
        // P8007748
    ...
    begin
        ...
        if GetFilter("Item Type") <> '' then // PR3.60
            ItemTypeFilter := GetTypeFilter    // PR3.60, P8001213
        else
            ItemTypeFilter := ItemTypeFilter::None;

        SalesCodeFilter := GetFilter("Sales Code");
        CodeFilter := GetFilter("Item Code"); // PR3.60
