page 5856 "Get Post.Doc - P.RcptLn Sbfrm"
{
    ...
    layout
    {
        area(content)
        {
            repeater(Control1)
            {
                ...
                field(RemainingQtyAlt; RemainingQtyAlt)
                {
                    ApplicationArea = FOODBasic;
                    CaptionClass = StrSubstNo('37002080,1,7,%1,%2', Type, "No.");
                    Caption = 'Remaining Quantity (Alt.)';
                    DecimalPlaces = 0 : 5;
                    Visible = false;
                }
                field(CalcAppliedQtyAlt; CalcAppliedQtyAlt)
                {
                    ApplicationArea = FOODBasic;
                    CaptionClass = StrSubstNo('37002080,1,19,%1,%2', Type, "No.");
    ...
    begin
        ...
        with PurchRcptLine2 do begin
            ...
            CalcReceivedPurchNotReturned(RemainingQty, RemainingQtyAlt, RevUnitCostLCY, FillExactCostReverse); // P8000466A
            if not RevQtyFilter then
                exit(true);
            exit((RemainingQty > 0) or (RemainingQtyAlt > 0)); // P8000466A
        end;
    end;
    ...
    local procedure CalcAppliedQtyAlt(): Decimal
    begin
        // P8000466A
        if (Type = Type::Item) and ("Quantity (Alt.)" - RemainingQtyAlt > 0) then
            exit("Quantity (Alt.)" - RemainingQtyAlt);
        exit(0);
    end;
