page 9001 "Accounting Manager Role Center"
{
    ...
    //   Add P800 Ded. Mgmt. and Accrual elements
    // 
    // PRW19.00
    ...
    actions
    {
        ...
        area(sections)
        {
            ...
            {
                Caption = 'Accruals';
                action("Customer Rebates/Promos")
                {
                ...
                action("Vendor Rebates/Promos")
                {
                    ApplicationArea = FOODBasic;
                    Caption = 'Vendor Rebates/Promos';
                    RunObject = Page "Vendor Accrual Plan List";
                }
                action("Accrual Journals")
                {
                    ApplicationArea = FOODBasic;
                    Caption = 'Accrual Journals';
                    RunObject = Page "Accrual Journal Batches";
                    RunPageView = WHERE("Template Type" = CONST(Accrual),
                                        Recurring = CONST(false));
                }
                action("Scheduled Accrual Journals")
                {
                    ApplicationArea = FOODBasic;
                    Caption = 'Scheduled Accrual Journals';
                    RunObject = Page "Accrual Journal Batches";
                    RunPageView = WHERE("Template Type" = CONST("Scheduled Accrual"),
                                        Recurring = CONST(false));
                }
            }
        ...
        area(processing)
        {
            ...
            action(Action37002007)
            {
                ApplicationArea = FOODBasic;
                Caption = 'Accrual Journals';
                Image = Journals;
                RunObject = Page "Accrual Journal";
            }
            separator(Separator37002009)
            {
            }
            action("Payment Application")
            {
                ApplicationArea = FOODBasic;
            ...
            action("A&ccrual Setup")
            {
                ApplicationArea = FOODBasic;
                Caption = 'A&ccrual Setup';
                Image = Setup;
