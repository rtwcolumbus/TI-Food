...
{ 
                ...
                if (InvoicePostingBuffer.Amount <> 0) or (InvoicePostingBuffer."Amount (ACY)" <> 0) then begin
                    ...
                    if InvoicePostingBuffer.Type = InvoicePostingBuffer.Type::"Fixed Asset" then begin
                        PrepareLineFADiscount(
                          ...
                          TotalVAT, TotalVATACY, TotalAmount, TotalAmountACY, TotalVATBase, TotalVATBaseACY, PurchLine); // P800133109
                        ...
                        UpdateInvoicePostingBuffer(InvoicePostingBuffer, true, PurchLine); // P800133109
            ...
            if not IsHandled then begin
                ...
                if (InvoicePostingBuffer.Amount <> 0) or (InvoicePostingBuffer."Amount (ACY)" <> 0) then begin
                    ...
                    if InvoicePostingBuffer.Type = InvoicePostingBuffer.Type::"Fixed Asset" then begin
                        ...
                        InvoicePostingBuffer.UpdateVATBase(TotalVATBase, TotalVATBaseACY);
                        InvoicePostingBuffer.Type := InvoicePostingBuffer.Type::"G/L Account";
                        UpdateInvoicePostingBuffer(InvoicePostingBuffer, false, PurchLine); // P800133109
                        InvoicePostingBuffer.Type := InvoicePostingBuffer.Type::"Fixed Asset";
                    end else begin
                        IsHandled := false;
                        LineDiscAccount := '';
    ...
    begin
        // if (PurchLine.Type = PurchLine.Type::"G/L Account") or (PurchLine.Type = PurchLine.Type::"Fixed Asset") then             // P800133109
        if (PurchLine.Type in [PurchLine.Type::"G/L Account", PurchLine.Type::"Fixed Asset", PurchLine.Type::FOODAccrualPlan]) then // P800133109
    ...
    local procedure PrepareLineFADiscount(var InvoicePostingBuffer: Record "Invoice Posting Buffer"; GenPostingSetup: Record "General Posting Setup"; AccountNo: Code[20]; TotalVAT: Decimal; TotalVATACY: Decimal; TotalAmount: Decimal; TotalAmountACY: Decimal; TotalVATBase: Decimal; TotalVATBaseACY: Decimal; PurchLine: Record "Purchase Line")
    var
    ...
    local procedure UpdateInvoicePostingBuffer(InvoicePostingBuffer: Record "Invoice Posting Buffer"; ForceGLAccountType: Boolean; PurchLine: Record "Purchase Line")
    var
    ...
    begin
        ...
        if (InvoicePostingBuffer.Type = InvoicePostingBuffer.Type::FOODAccrualPlan) then begin
            AccrualFldMgmt.PurchPostUpdatePostingBuffer(InvoicePostingBuffer, PurchLine);
        ...
        with GenJnlLine do begin
            ...
            if (InvoicePostingBuffer.Type = InvoicePostingBuffer.Type::FOODAccrualPlan) then
                AccrualFldMgmt.PurchPostPrepareGenJnlLine(
                  PurchHeader, InvoicePostingBuffer."FOOD Accrual Line No.", GenJnlLine);
            // P800133109
        end;
