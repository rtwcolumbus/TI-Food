codeunit 6503 "Item Tracking Doc. Management"
{
    ...
    begin
        if SignFactor <> 1 then begin
            ...
            // P80004516
            TempItemLedgEntry."Quantity (Alt.)" *= SignFactor;
            TempItemLedgEntry."Remaining Quantity (Alt.)" *= SignFactor;
            TempItemLedgEntry."Invoiced Quantity (Alt.)" *= SignFactor;
            // P80004516
        ...
        if TempItemLedgEntry.FindFirst() then begin
            ...
            // P8000466A
            TempItemLedgEntry."Quantity (Alt.)" += TempItemLedgEntry2."Quantity (Alt.)";
            TempItemLedgEntry."Remaining Quantity (Alt.)" += TempItemLedgEntry2."Remaining Quantity (Alt.)";
            TempItemLedgEntry."Invoiced Quantity (Alt.)" += TempItemLedgEntry2."Invoiced Quantity (Alt.)";
            // P8000466A
    ...
    begin
        ...
        if ItemTrackingExistsInBuffer(TempTrackingSpecBuffer, Type, Subtype, ID, BatchName, ProdOrderLine, RefNo, ItemTrackingSetup) then begin
            ...
            TempTrackingSpecBuffer."Quantity (Alt.)" += Abs(QtyAlt); // P8004516
            ...
            repeat
                if TempItemLedgEntry.TrackingExists() then begin
                    ...
                    FillTrackingSpecBuffer(
                        ...
                        ItemTrackingSetup, TempItemLedgEntry.Quantity, TempItemLedgEntry."Quantity (Alt.)", TempItemLedgEntry.Correction); // P8004516
            ...
            repeat
                if ReservEntry.TrackingExists() then begin
                    ...
                    FillTrackingSpecBuffer(
                        ...
                        ItemTrackingSetup, ReservEntry."Quantity (Base)", ReservEntry."Quantity (Alt.)", ReservEntry.Correction); // P8004516
            ...
            repeat
                if TrackingSpec.TrackingExists() then begin
                    ...
                    FillTrackingSpecBuffer(
                        ...
                        ItemTrackingSetup, TrackingSpec."Quantity (Base)", TrackingSpec."Quantity (Alt.)", TrackingSpec.Correction); // P8004516
            ...
            repeat
                ...
                then begin
                    ...
                    if ValueEntry."Invoiced Quantity" = 0 then begin
                        TempItemLedgEntry.Quantity := 0;
                        TempItemLedgEntry."Quantity (Alt.)" := 0;
                    end else begin
                        if ItemLedgEntry.CostInAlternateUnits then begin
                            ...
                            TempItemLedgEntry.Quantity := ValueEntry."Invoiced Quantity";
                            TempItemLedgEntry."Quantity (Alt.)" := Round(
                              TempItemLedgEntry."Quantity (Alt.)" * ValueEntry."Invoiced Quantity" / TempItemLedgEntry.Quantity, 0.00001);
                        end;
                    end;
                    //IF TempItemLedgEntry.Quantity <> 0 THEN
                    if (TempItemLedgEntry.Quantity <> 0) or (TempItemLedgEntry."Quantity (Alt.)" <> 0) then
                        // P8000466A
                        AddTempRecordToSet(TempItemLedgEntry, SignFactor);
                end;
