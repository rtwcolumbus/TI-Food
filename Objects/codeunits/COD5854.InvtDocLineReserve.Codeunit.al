codeunit 5854 "Invt. Doc. Line-Reserve"
{
    ...
    var
        ...
        Outbound, ShowError, HasError : Boolean;
    begin
        if Blocked then
            exit;
        ...
        if NewInvtDocLine."Document Type" = NewInvtDocLine."Document Type"::Receipt then begin
            NewInvtDocLine.CalcFields("Reserved Qty. Inbnd. (Base)");
            ShowError := (NewInvtDocLine."Reserved Qty. Inbnd. (Base)" <> 0);
        end else begin
            NewInvtDocLine.CalcFields("Reserved Qty. Outbnd. (Base)");
            ShowError := (NewInvtDocLine."Reserved Qty. Outbnd. (Base)" <> 0);
            Outbound := true;
        end;
        ...
        if NewInvtDocLine."Item No." <> OldInvtDocLine."Item No." then
            if ShowError then
                NewInvtDocLine.FieldError("Item No.", MustNotBeChangedErr)
            else
                HasError := true;
        ...
        if Outbound and (NewInvtDocLine."Location Code" <> OldInvtDocLine."Location Code") then
            if ShowError then
                NewInvtDocLine.FieldError("Location Code", MustNotBeChangedErr)
            else
                HasError := true;
        ...
        if Outbound and (NewInvtDocLine."Bin Code" <> OldInvtDocLine."Bin Code") then
            if ShowError then
                NewInvtDocLine.FieldError("Bin Code", MustNotBeChangedErr)
            else
                HasError := true;
        ...
        if NewInvtDocLine."Variant Code" <> OldInvtDocLine."Variant Code" then
            if ShowError then
                NewInvtDocLine.FieldError("Variant Code", MustNotBeChangedErr)
            else
                HasError := true;
        ...
        if NewInvtDocLine."Line No." <> OldInvtDocLine."Line No." then
            HasError := true;
        ...
        if HasError then begin
            ...
            if (NewInvtDocLine."Item No." <> OldInvtDocLine."Item No.") or
               ...
               ((not Outbound) and (NewInvtDocLine."Location Code" <> OldInvtDocLine."Location Code"))
            then
                AssignForPlanning(OldInvtDocLine);
        end;
        // P800127049
    ...
    // P800127049
    procedure SetDeleteItemTracking(Flag: Boolean)
    begin
        DeleteItemTracking := Flag;
    end;

    // P800127049
    procedure DeleteLineConfirm(var InvtDocLine: Record "Invt. Document Line"): Boolean
    begin
        if not InvtDocLine.ReservEntryExist() then
            exit(true);
        ...
        with InvtDocLine do begin
            ...
            case "Document Type" of
                "Document Type"::Receipt:
                    begin
                        ReservMgt.SetReservSource(InvtDocLine);
                        if RedStorno or DeleteItemTracking then // P800127049
    ...
    begin
        ...
        ItemTrackingLines.SetSourceSpec(TrackingSpecification, InvtDocLine."Posting Date"); // P800127049
