codeunit 13 "Gen. Jnl.-Post Batch"
{
    ...
    // 
    // PRW15.00.01
    // P8000535A, VerticalSoft, Jack Reynolds, 15 OCT 07
    //   Fix problem posting deductions through the cash receipts journal
    // 
    // PRW16.00.05
    ...
    var
        ...
        PaymentEntry: Record "Cust. Ledger Entry" temporary;
        DedGenJnlLine: Record "Gen. Journal Line" temporary;
    ...
    begin
        ...
        with GenJnlLine do begin
            ...
            if P800Fns.AccrualsInstalled() then
                AccrualPostLine.PrePostGenJnlBatch(GenJnlLine);
            // P8000241A    // P80050544
            ...
            PostDeductions; // P80000535A - moved up from below    // P80050544
    ...
    procedure MarkDeductions(GenJnlLine: Record "Gen. Journal Line")
    var
        CustLedger: Record "Cust. Ledger Entry";
    begin
        // P8000170A
        if GenJnlLine.DeductionsExist then begin
            CustLedger.Find('+');
            PaymentEntry := CustLedger;
            PaymentEntry.Insert;
            DedGenJnlLine := GenJnlLine;
            DedGenJnlLine.Insert;
        end;
    end;

    procedure PostDeductions()
    var
        DeductionLine: Record "Deduction Line";
        DedMgt: Codeunit "Deduction Management";
    begin
        // P8000170A
        if PaymentEntry.Find('-') then begin
            DedGenJnlLine.Find('-');
            DedMgt.LockTables;
            repeat
                DeductionLine.SetRange("Source Table No.", DATABASE::"Gen. Journal Line");
                DeductionLine.SetRange("Source ID", DedGenJnlLine."Journal Template Name");
                DeductionLine.SetRange("Source Batch Name", DedGenJnlLine."Journal Batch Name");
                DeductionLine.SetRange("Source Ref. No.", DedGenJnlLine."Line No.");
                DedMgt.PostDeductions(PaymentEntry, DeductionLine, DedGenJnlLine."Posting Date"); // P8000920
                DedGenJnlLine.Next;
            until PaymentEntry.Next = 0;
            if GLReg.FindLast then    // P80000535A
                GLRegNo := GLReg."No."; // P80000535A
        end;
    end;
    ...
    begin
        with GenJournalLine do begin
            ...
            MarkDeductions(GenJnlLine5); // P8000170A, P80041198
