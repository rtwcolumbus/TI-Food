codeunit 5815 "Undo Sales Shipment Line"
{
    ...
    begin
        with SalesShptLine do begin
            if SalesHeader.Get(SalesHeader."Document Type"::Order, "Order No.") then // PR3.61.02
                ReleaseSalesDoc.Reopen(SalesHeader);                                  // PR3.61.02
            ...
            if ProcessFns.PricingInstalled then                      // PR3.70
                OffInvoiceFns.UndoAllowanceSalesLines("Document No."); // PR3.70
                ...
                if Type = Type::Item then begin
                    ...
                    if not SkipUndoPosting then begin
                        ...
                        UndoPostingMgt.CollectItemLedgEntries(
                            TempItemLedgEntry, DATABASE::"Sales Shipment Line", "Document No.", "Line No.", "Quantity (Base)", "Quantity (Alt.)", "Item Shpt. Entry No."); // P8000267B
    ...
    begin
        ...
        with SalesShptLine do begin
            ...
            UndoPostingMgt.CollectItemLedgEntries(
                TempApplyToEntryList, DATABASE::"Sales Shipment Line", "Document No.", "Line No.", "Quantity (Base)", "Quantity (Alt.)", "Item Shpt. Entry No."); // P8000267B
            ...
            if GetInvoicedShptEntries(SalesShptLine, ItemLedgEntryNotInvoiced) then begin
                ...
                repeat
                    ...
                    ItemJnlLine."Quantity (Alt.)" := ItemLedgEntryNotInvoiced."Quantity (Alt.)"; // PR3.60, P8001132
    ...
    begin
        with OldSalesShptLine do begin
            ...
            NewSalesShptLine."Quantity (Alt.)" := -"Quantity (Alt.)"; // PR3.60
            ...
            NewSalesShptLine."Qty. Invoiced (Alt.)" := NewSalesShptLine."Quantity (Alt.)"; // PR3.70
            ...
            // PR3.60 Begin
            if OldSalesShptLine.AltQtyEntriesExist then
                AltQtyMgmt.UndoAltQtyEntries(
                  DATABASE::"Sales Shipment Line", OldSalesShptLine."Document No.", OldSalesShptLine."Line No.",
                  DATABASE::"Sales Shipment Line", NewSalesShptLine."Document No.", NewSalesShptLine."Line No.",
                  false);
            // PR3.60 End
        end;
    end;
    ...
    begin
        ...
        with SalesShptLine do begin
            ...
            UndoPostingMgt.UpdateSalesLine(SalesLine, Quantity - "Quantity Invoiced", "Quantity (Base)" - "Qty. Invoiced (Base)", "Quantity (Alt.)" - "Qty. Invoiced (Alt.)", TempGlobalItemLedgEntry); // P8000267B
