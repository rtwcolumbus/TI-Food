...
{
    // PR1.00
    //   Copy Auto Plan to components
    // 
    // PR2.00.03
    //   Copy Step Code to components
    //   Use Step Code to keep similar lines from different steps from combining
    // 
    // PR2.00.05
    //   TransferBOM - map variable type to items
    //   Calculate - for packaging variants transfer package BOM
    // 
    // PR3.70.07
    // P8000153A, Myers Nissi, Jack Reynolds, 16 DEC 04
    //   Add BOM line source identification when creating components
    // 
    // PR4.00
    // P8000219A, Myers Nissi, Jack Reynolds, 24 JUN 05
    //   TransferRouting - call function to set costs from ABC
    // 
    // P8000197A, Myers Nissi, Jack Reynolds, 22 SEP 05
    //   Calculate production order date using production times from BOM equipment
    // 
    // PRW15.00.01
    // P8000520A, VerticalSoft, Jack Reynolds, 14 SEP 07
    //   Generate error if phantom BOM has no active version
    // 
    // PRW15.00.03
    ...
    //   Fix problem calculating Starting Date
    // 
    // PRW16.00.06
    ...
    //    Added support for Prod. Order Pre-Processing
    // 
    // PRW17.00
    ...
    // 
    // PRW17.00.01
    // P8001208, Columbus IT, Jack Reynolds, 11 SEP 13
    //   Resore missing code for variables and step code
    // 
    // PRW17.10.02
    // P8001301, Columbus IT, Jack Reynolds, 03 MAR 14
    //   New parameters for TransferBOM and CalculateProdOrderDates
    // 
    // PRW18.00
    ...
    var
        ...
        Text37002000: Label 'Production BOM %1 has no active version.';
    ...
    begin
        ...
        Item.Get(ProdOrderLine."Item No."); // P8000219A
    ...
    begin
        ...
        // P8000219A Begin
        ProdOrderRoutingLine."Unit Cost per" := 0; // P80073095
        ProdOrderRoutingLine."Use ABC Cost" := RoutingLine."Use ABC Cost";
        RoutingRec.GetTable(ProdOrderRoutingLine);
        if not CostCalcMgt.CalcABCRtngCostPerUnit(
          RoutingRec, Item, ProdOrderLine."Due Date", ProdOrderLine."Location Code", // P8001030
          ProdOrderLine."Production BOM No.", ProdOrderLine."Production BOM Version Code",
          ProdOrderRoutingLine."Direct Unit Cost", ProdOrderRoutingLine."Indirect Cost %",
          ProdOrderRoutingLine."Overhead Rate", ProdOrderRoutingLine."Unit Cost per", ProdOrderRoutingLine."Unit Cost Calculation")
        then begin
            ProdOrderRoutingLine."Unit Cost per" := RoutingLine."Unit Cost per"; // P80073095
                                                                                 // P8000219A End
    ...
    local procedure TransferBOM(ProdBOMNo: Code[20]; Level: Integer; LineQtyPerUOM: Decimal; ItemQtyPerUOM: Decimal; StepCode: Code[10]): Boolean
    var
        ...
        if ProdBOMLine[Level].Find('-') then
            repeat
                ...
                if not IsHandled then begin
                    // P8001208
                    // PR2.00.05 Begin
                    if ProdBOMLine[Level].Type = ProdBOMLine[Level].Type::FOODVariable then begin
                        ItemVariantVariable.Get(ProdOrderLine."Item No.", ProdOrderLine."Variant Code", ProdBOMLine[Level]."No.");
                        ProdBOMLine[Level].Type := ProdBOMLine[Level].Type::Item;
                        ProdBOMLine[Level]."No." := ItemVariantVariable."Variable Item No.";
                        Item.Get(ItemVariantVariable."Variable Item No.");
                        ProdBOMLine[Level].Description := Item.Description;
                        ProdBOMLine[Level].Quantity := ProdBOMLine[Level].Quantity * ItemVariantVariable.Quantity;
                        ProdBOMLine[Level]."Unit of Measure Code" := ItemVariantVariable."UOM Code";
                    end;
                    // PR2.00.05 End
                    if StepCode <> '' then                         // PR2.00.03
                        ProdBOMLine[Level]."Step Code" := StepCode; // PR2.00.03
                                                                    // P8001208
                    ...
                    case ProdBOMLine[Level].Type of
                        ...
                        ProdBOMLine[Level].Type::"Production BOM":
                            begin
                                ...
                                TransferBOM(ProdBOMLine[Level]."No.", Level + 1, ReqQty, 1, ProdBOMLine[Level]."Step Code"); // PR2.00.03
                                ProdBOMLine[Level].SetRange("Production BOM No.", ProdBOMNo);
                                if Level > 1 then
    ...
    begin
        ...
        if not ProdOrderRoutingLine.FindFirst() then begin
            // P8000197A
            if P800ProdOrderMgt.CalculateProdOrderLineDates(ProdOrderLine, Direction) then begin
                CalculateProdOrderDates(ProdOrderLine, LetDueDateDecrease); // P8001301
                exit;
            end;
            // P8000197A
    ...
    begin
        ...
        if CalcComponents then
            if ProdOrderLine."Production BOM No." <> '' then begin
                ...
                // PR2.00.05 Begin
                PkgBOMNo := ProdOrderLine.PkgBOMNo;
                ProdOrder.Get(ProdOrderLine.Status, ProdOrderLine."Prod. Order No.");
                ...
                then begin
                    ...
                    ProdOrderComp.Validate("Unit of Measure Code", Item."Base Unit of Measure");
                    ...
                    if not TransferBOM(
                         ...
                         UOMMgt.GetQtyPerUnitOfMeasure(
                           ...
                           VersionMgt.GetBOMUnitOfMeasure(
                             ProdOrderLine."Production BOM No.",
                             ProdOrderLine."Production BOM Version Code")), // PR2.00.03
                             '')                                            // PR2.00.03
                    then
                        ErrorOccured := true;
                end;
                ...
                if PkgBOMNo <> '' then
                    TransferBOM(
                      PkgBOMNo,
                      1,
                      ProdOrderLine."Qty. per Unit of Measure", // P8001301
                      UOMMgt.GetQtyPerUnitOfMeasure(
                        Item, ProdOrderLine.PkgBOMUOM),
                      '');
                // PR2.00.05 End
            end;
        ...
        with ProdOrderComp do
            if "Location Code" <> '' then begin
                ...
                if Location."Bin Mandatory" and (not Location."Directed Put-away and Pick") then
                    if UseDefaultBin() then // P8001142
