codeunit 5845 "Get Inventory Report"
{
    ...
    //   Take into account writeoff, ABC, and extra charge accounts
        ...
        with InvtPostingSetup do begin
            if Find('-') then
                repeat
                    ...
                    // P8000466A
                    TempInvtPostingSetup.Reset();
                    TempInvtPostingSetup.SetRange("Writeoff Account (Company)", "Writeoff Account (Company)");
                    if not TempInvtPostingSetup.Find('-') then begin
                        UpDateWindow(WindowType, WindowNo, FieldCaption("Writeoff Account (Company)"));
                        InsertGLInvtReportEntry(InventoryReportLine, "Writeoff Account (Company)", InventoryReportLine."Inventory Adjmt.");
                    end;

                    TempInvtPostingSetup.Reset();
                    TempInvtPostingSetup.SetRange("Writeoff Account (Vendor)", "Writeoff Account (Vendor)");
                    if not TempInvtPostingSetup.Find('-') then begin
                        UpDateWindow(WindowType, WindowNo, FieldCaption("Writeoff Account (Vendor)"));
                        InsertGLInvtReportEntry(InventoryReportLine, "Writeoff Account (Vendor)", InventoryReportLine."Inventory Adjmt.");
                    end;
                    // P8000466A
    ...
    var
        ...
        TempECPostingSetup: Record "Extra Charge Posting Setup" temporary;
    begin
        with GenPostingSetup do begin
            if Find('-') then
                repeat
                    ...
                    // P8000466A
                    TempGenPostingSetup.Reset();
                    TempGenPostingSetup.SetRange("ABC Direct Account", "ABC Direct Account");
                    if not TempGenPostingSetup.Find('-') then begin
                        UpDateWindow(WindowType, WindowNo, FieldCaption("ABC Direct Account"));
                        InsertGLInvtReportEntry(
                          InventoryReportLine, "ABC Direct Account", InventoryReportLine."Overhead Applied");
                    end;

                    TempGenPostingSetup.Reset();
                    TempGenPostingSetup.SetRange("ABC Direct Account", "ABC Overhead Account");
                    if not TempGenPostingSetup.Find('-') then begin
                        UpDateWindow(WindowType, WindowNo, FieldCaption("ABC Overhead Account"));
                        InsertGLInvtReportEntry(
                          InventoryReportLine, "ABC Overhead Account", InventoryReportLine."Overhead Applied");
                    end;
                    // P8000466A
        ...
        // P8000466A
        with ECPostingSetup do begin
            if Find('-') then
                repeat
                    TempECPostingSetup.Reset;
                    TempECPostingSetup.SetRange("Invt. Accrual Acc. (Interim)", "Invt. Accrual Acc. (Interim)");
                    if not TempECPostingSetup.Find('-') then begin
                        UpDateWindow(WindowType, WindowNo, FieldCaption("Invt. Accrual Acc. (Interim)"));
                        InsertGLInvtReportEntry(
                          InventoryReportLine, "Invt. Accrual Acc. (Interim)", InventoryReportLine."Invt. Accrual (Interim)");
                    end;
                    ...
                    TempECPostingSetup.Reset;
                    TempECPostingSetup.SetRange("Direct Cost Applied Account", "Direct Cost Applied Account");
                    if not TempECPostingSetup.Find('-') then begin
                        UpDateWindow(WindowType, WindowNo, FieldCaption("Direct Cost Applied Account"));
                        InsertGLInvtReportEntry(
                          InventoryReportLine, "Direct Cost Applied Account", InventoryReportLine."Direct Cost Applied");
                    ...
                    TempECPostingSetup := ECPostingSetup;
                    TempECPostingSetup.Insert;
                until Next = 0;
        end;
        // P8000466A
    end;
