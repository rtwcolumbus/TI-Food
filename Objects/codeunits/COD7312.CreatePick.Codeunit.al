codeunit 7312 "Create Pick"
{
    ...
    //   Add old Item Tracking picking features to base Navision FEFO picking method
    // 
    // P8000503A, VerticalSoft, Don Bresee, 13 FEB 07
    //   Picking of Intermediates, Non-Pick Items
    //   Bin Content Issues
    //   Rounding re-work
    // 
    // PRW15.00.03
    ...
    // 
    // PRW16.00.01
    // P8000710, VerticalSoft, Don Bresee, 31 JUL 09
    //   Fix rounding issue when picking specific lots
    // 
    // P8000756, VerticalSoft, Don Bresee, 22 JAN 10
    //   P800 code removed - problem corrected in base NAV
    // 
    // P8000899, Columbus IT, Jack Reynolds, 14 MAR 11
    //   Added Lot Freshness logic
    // 
    // PRW16.00.06
    // P8001039, Columbus IT, Don Bresee, 01 MAR 12
    ...
    // 
    // PRW17.00
    // P8001132, Columbus IT, Jack Reynolds, 01 APR 13
    //   Lot Status for Assembly
    // 
    // PRW17.00.01
    // P8001190, Columbus IT, Jack Reynolds, 07 AUG 13
    //   fix problem with no bins on directed picks
    // P8001347, Columbus IT, Don Bresee, 12 SEP 14
    //   Add new picking features for containers, change to UOM and Breakbulk handling
    // 
    // PRW18.00
    ...
    // 
    // PRW19.00.01
    // P8007470, To-Increase, Jack Reynolds, 02 AUG 16
    //   Fix problems with staged picking
    // 
    // P8008143, To-Increase, Dayakar Battini, 01 DEC 16
    //   Fix problem with staged pick, misplaced code from NAV5 to NAV6 migration.
    // 
    // PRW110.0
    // P8007748, To-Increase, Jack Reynolds, 03 NOV 16
    //   General changes and refactoring for NAV 2017
    // 
    // P80040341, To-Increase, Dayakar Battini, 01 JUN 17
    //   Fix problem with staged pick, reverted 8143
    // 
    // P80041428, To-Increase, Jack Reynolds, 13 JUN 17
    //   Permission error in CalcStageBin if advanced warehouse not enabled
    // 
    // PRW110.0.02
    // P80038975, To-Increase, Dayakar Battini, 14 DEC 17
    ...
    // 
    // PRW111.00.02
    // P80064711, To-Increase, Jack Reynolds, 17 SEP 18
    //   Fix problem with staged pick
    // 
    // PRW113.00
    ...
    var
        ...
        StagingFromPickLine: Record "Whse. Staged Pick Line";
        StagingFromSourceLine: Record "Whse. Staged Pick Source Line";
        ItemTrackingPickingMethod: Integer;
        BreakBulkQtyToPick: Decimal;
        PlaceLinePickQty: Decimal;
        PickDate: Date;
        ...
        PickUOM: Record "Item Unit of Measure";
        StagingPickMode: Option Loose,Containers,"Other UOMs";
        ContainerPickMode: Option "Entire Qty.","Full Containers","Remaining Qty.";
        PickingContainers: Boolean;
        TempPickExclusion: Record "Directed Pick Exclusion" temporary;
    ...
    begin
        ...
        if not IsHandled then begin
            // P80056710
            CreateTempLineForProductionContainer(LocationCode, ToBinCode, TotalQtytoPick, TotalQtytoPickBase);
            if TotalQtytoPick <= 0 then
                exit;
            // P80056710
        ...
        */
        ...
        if TotalQtytoPickBase <> 0 then begin
            SetPickUOM(ItemNo, UnitofMeasureCode); // P8001347
            ...
            if TempWhseItemTrackingLine.Find('-') then begin
                repeat
                    if TempWhseItemTrackingLine."Qty. to Handle (Base)" <> 0 then begin
                        ...
                        CreateAllTempLines(
                          LocationCode, ItemNo, VariantCode, UnitofMeasureCode, FromBinCode, ToBinCode,
                          QtyPerUnitofMeasure, QtyRoundingPrecision, QtyRoundingPrecisionBase, QtyToPick, TempWhseItemTrackingLine, QtyToPickBase, true, true);
                        // P8001347
            ...
            if TotalQtytoPickBase <> 0 then
                if PickAccordingToFEFO(LocationCode, WhseItemTrackingSetup, ItemNo) or
                   PickStrictExpirationPosting(ItemNo, WhseItemTrackingSetup)
                then begin
                    CreateFEFOLines(
                      LocationCode, ItemNo, VariantCode, UnitofMeasureCode, FromBinCode, ToBinCode,
                      QtyPerUnitofMeasure, QtyRoundingPrecision, QtyRoundingPrecisionBase, TotalQtytoPick, TotalQtytoPickBase);
                    CreateTempRemQtyLine(
                      LocationCode, ItemNo, VariantCode, UnitofMeasureCode, ToBinCode,
                      QtyPerUnitofMeasure, QtyRoundingPrecision, QtyRoundingPrecisionBase, TotalQtytoPick, TempWhseItemTrackingLine, TotalQtytoPickBase);
                end;
            // P8001347
                    ...
                    if not HasExpiredItems then begin
                        if WhseItemTrackingSetup."Serial No. Required" then begin
                            ...
                            for i := 1 to TotalQtytoPick do begin
                                ...
                                CreateTempLooseLines(
                                    LocationCode, ItemNo, VariantCode, UnitofMeasureCode,
                                    FromBinCode, ToBinCode, QtyPerUnitofMeasure, QtyRoundingPrecision, QtyRoundingPrecisionBase, QtyToPick, TempWhseItemTrackingLine, QtyToPickBase, false, true);
                                // P8001347
                            end;
                            TotalQtytoPick := 0;
                            ...
                            CreateAllTempLines(LocationCode, ItemNo, VariantCode, UnitofMeasureCode,
                            FromBinCode, ToBinCode, QtyPerUnitofMeasure, QtyRoundingPrecision, QtyRoundingPrecisionBase, TotalQtytoPick, TempWhseItemTrackingLine, TotalQtytoPickBase, true, true);
                        // P8001347
                    end;
        end;
    ...
    begin
        ...
        if Location."Bin Mandatory" then begin
            if not Location."Directed Put-away and Pick" then begin
                ...
                if QtyAvailableBase > 0 then begin
                    // IF TotalQtytoPickBase > QtyAvailableBase THEN                                                    // P8001039
                    if (TotalQtytoPickBase > (QtyAvailableBase + RoundingAdjmtMgmt.GetNearZeroQtyForItem(ItemNo))) then // P8001039
            ...
            if (ReservationExists and ReservedForItemLedgEntry) or not ReservationExists then begin
                ...
                CalcStageBin(
                  LocationCode, ItemNo, VariantCode, UnitofMeasureCode,
                  ToBinCode, QtyPerUnitofMeasure, QtyRoundingPrecision, QtyRoundingPrecisionBase, TempWhseItemTrackingLine,
            ...
            IF (TotalQtytoPickBase > 0) AND
               (Location."Always Create Pick Line" OR (WhseSource = WhseSource::"Delivery Trip Pick"))
            THEN BEGIN
            // P8000630A
        ...
        if QtyAvailableBase > 0 then begin
            ...
            if P800Functions.ContainerTrackingInstalled then begin
                CreateTempActivityLine(LocationCode, '', UnitofMeasureCode, QtyPerUnitofMeasure, QtytoPick, QtytoPickBase, 1, 0, QtyRoundingPrecision, QtyRoundingPrecisionBase);
                CreateTempActivityLine(LocationCode, '', UnitofMeasureCode, QtyPerUnitofMeasure, QtytoPick, QtytoPickBase, 2, 0, QtyRoundingPrecision, QtyRoundingPrecisionBase);
            end else
                // P8001323
    ...
    begin
        ...
        with TempWhseActivLine do begin
            ...
            ClearFilterStagingFromFields(TempWhseActivLine); // P8000322A
    ...
    begin
        ...
        with FromBinContent do begin
            ...
            if not IsHandled then
                if FindSet() then
                    repeat
                        FromBinContent.SetLotStatusExclusion(LotStatusExclusionFilter); // P8001083
                        QtyAvailableBase :=
                          CalcQtyAvailToPick(0) -
                          CalcQtyInProdContainerBase(LocationCode, "Bin Code", ItemNo, VariantCode, '') - // P80056710
                        ...
                        if QtyAvailableBase > 0 then begin
                            ...
                            if (QtytoPick > 0) then begin // P8001347
                                CreateTempActivityLine(
    ...
    begin
        ...
        if GetBinContent(
            ...
            IsMovementWorksheet, WhseItemTrkgExists, false, false, WhseItemTrackingSetup, TotalQtytoPick, TotalQtytoPickBase, StagingFromPickLine) // P80096141
        then begin
            ...
            if (TotalAvailQtyToPickBase > 0) then  // P8001347
                repeat
                    ...
                    if FromBinContent."Bin Code" <> ToBinCode then
                        // CalcBinAvailQtyToPick(AvailableQtyBase,FromBinContent,TempWhseActivLine2, WhseItemTrackingSetup);    // P8001347
                        CalcBinAvailQtyToPickForUOM(AvailableQtyBase, FromBinContent, TempWhseActivLine2, WhseItemTrackingSetup); // P8001347, P800144605
                    if BinNotExcluded(FromBinContent, false, TempWhseItemTrackingLine."Lot No.") and (BinIsForPick or BinIsForReplenishment) then begin // P8001347
                        ...
                        if AvailableQtyBase > 0 then begin
                            // ToQtyToPickBase := CalcQtyToPickBase(FromBinContent,TempWhseActivLine); // P8001347
                            CalcQtyToPickBaseForUOM(FromBinContent, ToQtyToPickBase); // P8001347
                            ...
                            if not IsHandled then begin
                                ...
                                if (ToQtyToPick > 0) then begin // P8001347
                                    CreateTempActivityLine(
                ...
                // UNTIL (NEXT = 0) OR (TotalQtytoPickBase = 0);                                // P8001347
                until (FromBinContent.Next() = 0) or (TotalQtytoPickBase = 0) or (TotalAvailQtyToPickBase <= 0); // P8001347
        end;
    end;
    ...
    local procedure GetBinContent(
        ...
        WhseItemTrackingSetup: Record "Item Tracking Setup"; TotalQtytoPick: Decimal; TotalQtytoPickBase: Decimal; StagingFromPickLine: Record "Whse. Staged Pick Line"): Boolean
    var
        IsHandled: Boolean;
        Result: Boolean;
    begin
        ...
        exit(
            FromBinContent.GetBinContent(
                ...
                IsMovementWorksheet, WhseItemTrkgExists, WhseItemTrackingSetup, StagingFromPickLine));
    end;
        ...
        if FromItemUOM.Find('-') then
            repeat
                if GetBinContent(
                    ...
                    IsMovementWorksheet, WhseItemTrkgExists, true, false, WhseItemTrackingSetup, TotalQtytoPick, TotalQtytoPickBase, StagingFromPickLine) // P80096141
                then
                    repeat
        ...
        if ItemUOM.Find('-') then
            repeat
                if GetBinContent(
                    ...
                    IsMovementWorksheet, WhseItemTrkgExists, false, true, WhseItemTrackingSetup, TotalQtytoPick, TotalQtytoPickBase, StagingFromPickLine) // P80096141
                then
                    repeat
                        if (FromBinContent."Bin Code" <> ToBinCode) and
                            ((UseForPick(FromBinContent) and (not IsMovementWorksheet)) or
    ...
    begin
        ...
        if Location."Bin Mandatory" or P800Functions.ContainerTrackingInstalled then // P8001323
    ...
    begin
        ...
        BinContent.SetLotStatusExclusion(LotStatusExclusionFilter); // P8001083
        QtyToPickBase := BinContent.CalcQtyAvailToPick(AvailableQtyBase - TempWhseActivLine."Qty. (Base)");
        QtyToPickBase -= CalcQtyInProdContainerBase(BinContent."Location Code", BinContent."Bin Code", BinContent."Item No.", // P80056710
          BinContent."Variant Code", BinContent."Unit of Measure Code"); // P80056710
    end;
    ...
    begin
        with BinContent do begin
            ...
            if (LotStatusExclusionFilter <> '') and (GetFilter("Lot No. Filter") = '') then                   // P8001083
                LotStatusMgmt.QuantityAdjForBinContent(BinContent, LotStatusExclusionFilter, "Quantity (Base)");  // P8001083
    ...
    var
        ...
        PerPickClassCode: Boolean;
        OldPickClassCode: Code[20];
        DeliveryRouteManagement: Codeunit "Delivery Route Management";
        IsHandled: Boolean;
    begin
        if P800Functions.DistPlanningInstalled then // P80058367
            PerPickClassCode := DeliveryRouteManagement.IsPickClassCodeEnabled;  // P80038975
        TempWhseActivLine.Reset();
        if PerPickClassCode then
            if InStagingMode() then
                TempWhseActivLine.SetCurrentKey("No.", "Pick Class Code", "Source No.")
            else
                TempWhseActivLine.SetCurrentKey("Pick Class Code", "Source No.");  // P80038975
        if not TempWhseActivLine.Find('-') then begin
        ...
        repeat
            ...
            // P80038975
            if PerPickClassCode then
                TempWhseActivLine.SetRange("Pick Class Code", TempWhseActivLine."Pick Class Code");
            // P80038975
            ...
            repeat
                ...
                if IsHandled then begin
                    ...
                    // P80038975
                    if PerPickClassCode then begin
                        if TempWhseActivLine."Pick Class Code" <> '' then begin
                            if TempWhseActivLine."Pick Class Code" <> OldPickClassCode then begin
                                CreateWhseActivHeader(
                                  TempWhseActivLine."Location Code", FirstWhseDocNo, LastWhseDocNo,
                                  NoOfSourceDoc, NoOfLines, WhseDocCreated);
                                CreateWhseDocLine;
                            ...
                            if CreatePickParameters."Create Pick Per Source Doc." and (TempWhseActivLine."Source No." <> OldSourceNo) then begin // P800131478
                                CreateWhseActivHeader(
                                TempWhseActivLine."Location Code", FirstWhseDocNo, LastWhseDocNo,
                                NoOfSourceDoc, NoOfLines, WhseDocCreated);
                                CreateWhseDocLine;
                ...
                OldPickClassCode := TempWhseActivLine."Pick Class Code";    // P80038975
            ...
            TempWhseActivLine.SetRange("Pick Class Code");    // P80038975
    ...
    begin
        ...
        if not (WhseActivLine."Whse. Document Type" in [
                                                        ...
                                                        WhseActivLine."Whse. Document Type"::FOODStagedPick, // P8000322A
        ...
        if not IsHandled then
            if (Location."Bin Mandatory" or P800Functions.ContainerTrackingInstalled) and (not WhseItemTrackingSetup."Serial No. Required") then
    ...
    begin
        ...
        // P8001347
        if (TempWhseActivLine2."Container Qty." <> 0) then begin
            WhseActivLine."First Container Line No." := LineNo;
            WhseActivLine."Related Container Line No." := LineNo + 10000;
        end else
            // P8001347
    ...
    begin
        ...
        if TempWhseActivLine.Find('-') then
            repeat
                ...
                TempWhseActivLine.Delete();
                // P8001347
                if (TempWhseActivLine."Container Qty." <> 0) then begin
                    WhseActivLine."First Container Line No." := LineNo - 10000;
                    WhseActivLine."Related Container Line No." := LineNo - 10000;
                end else
                    // P8001347
                ...
                if WhseActivLine.Quantity > 0 then begin
                    ...
                    WhseActivLine.SuppressProductionContainerUpdate; // P80056710
    ...
    begin
        with WhseActivLine do begin
            ...
            // P8001352
            QtyAssignedToPick := "Qty. Outstanding (Base)" + CalcBreakbulkOutstdQty(WhseActivLine, WhseItemTrackingSetup);
            if not InStagingMode() then
                SubtractStagedBinPickQtyBase(
                  WhseActivLine, LocationCode, ItemNo, VariantCode, WhseItemTrackingSetup."Lot No.", WhseItemTrackingSetup."Serial No.", QtyAssignedToPick);
            //exit("Qty. Outstanding (Base)" + CalcBreakbulkOutstdQty(WhseActivLine, WhseItemTrackingSetup));
            exit(QtyAssignedToPick);
            // P8001352
        end;
    end;
    ...
    begin
        ...
        PickDate := WhseWkshLine2."Due Date"; // P8000899
    end;
    ...
    begin
        ...
        PickDate := WhseShptLine2."Due Date"; // P8000899
    end;
    ...
    begin
        ...
        TempNo := TempNo2;
        PickDate := WhseInternalPickLine2."Due Date"; // P8000899
    ...
    begin
        ...
        if not WhseItemTrkgExists then // P8001347
            exit;                        // P8001347
    ...
    begin
        ...
        // P8000466A
        WhseItemTrackingFEFO.SetFEFOPickingForSale(SourceType = DATABASE::"Sales Line"); // P80053245
        WhseItemTrackingFEFO.SetItemTrackingPickingMethod(ItemTrackingPickingMethod);    // P80053245
        // P8000466A
        ...
        if WhseItemTrackingFEFO.FindFirstEntrySummaryFEFO(EntrySummary) then begin
            ...
            repeat
                ...
                // P8001083
                then
                    if not LotStatusMgmt.ExcludeLot(ItemNo, VariantCode, EntrySummary."Lot No.", LotStatusExclusionFilter) then
                        if VerifyLotFreshenss(SourceNo, SourceType, SourceSubType, SourceLineNo, EntrySummary."Lot No.") // P8000899
                                                                                                                         // P8001083
                        then begin
    ...
    begin
        with TempWhseItemTrackingLine do begin
            ...
            "Qty. to Handle" := CalcUOMQtyFromBase(QuantityBase); // P8001347
    ...
    begin
        ...
        case SourceType of
            DATABASE::"Sales Line":
                if SourceSubType = SalesLine."Document Type"::Order then
                    AvailFor := LotStatus.FieldNo("Available for Sale");
            DATABASE::"Purchase Line":
                if SourceSubType = PurchLine."Document Type"::"Return Order" then
                    AvailFor := LotStatus.FieldNo("Available for Purchase");
            DATABASE::"Transfer Line":
                AvailFor := LotStatus.FieldNo("Available for Transfer");
            DATABASE::"Prod. Order Component":
                AvailFor := LotStatus.FieldNo("Available for Consumption");
            DATABASE::"Assembly Line":                                   // P8001132
                AvailFor := LotStatus.FieldNo("Available for Consumption"); // P8001132
        end;

        if AvailFor <> AvailableFor then begin
    ...
    begin
        ...
        if not InStagingMode() then
            SubtractStagedBinPickQtyBase(
              WhseActivLine, LocationCode, ItemNo, VariantCode, WhseItemTrackingLine."Lot No.", WhseItemTrackingLine."Serial No.", QtyAssignedToPick);
        // P8000322A
        ...
        if not InStagingMode() then
            SubtractStagedBinPickQtyBase(
              TempWhseActivLine, LocationCode, ItemNo, VariantCode, WhseItemTrackingLine."Lot No.", WhseItemTrackingLine."Serial No.", QtyAssignedPick);
        // P8000322A

        // if Location."Always Create Pick Line" or CrossDock then begin // P8001347
        if AlwaysCreatePickLine() or CrossDock then begin                // P8001347            
    ...
    local procedure PickAccordingToFEFO(LocationCode: Code[10]; WhseItemTrackingSetup: Record "Item Tracking Setup"; ItemNo: Code[20]) PickAccordingToFEFO: Boolean
    begin
        // P8007748 - add parameter for ItemNo
        GetLocation(LocationCode);
        // P8000466A
        if WhseItemTrackingSetup.TrackingRequired() then
            if PickAccordingToItemTracking(ItemNo) then
                exit(true);
        // P8000466A
    ...
    begin
        ...
        exit(GetFromBinContentQty(LocCode, FromBinCode, ItemNo, Variant, UoMCode, WhseItemTrackingLine));
    end;
    ...
    begin
        ...
        with TempWhseActivLine do begin
            ...
            case CreatePickParameters."Whse. Document" of
                ...
                // P8000756
                CreatePickParameters."Whse. Document"::FOODStagedPick:
                    TransferFromStagedPickLine(WhseStagedPickLine);
            // P8000756
            end;
                ...
                case WhseSource2 of
                    ...
                    CreatePickParameters."Whse. Document"::Production:
                        if ("Action Type" <> "Action Type"::Take) or (ProdOrderCompLine."Unit of Measure Code" = TempWhseActivLine."Unit of Measure Code") then begin
                            // ProdOrderCompLine.CalcFields("Pick Qty.","Pick Qty. (Base)"); // P8000503A
                            ProdOrderCompLine.CalcPickQtys;                                  // P8000503A
                            CalcMaxQty(
                              QtyToPick,
                    ...
                    CreatePickParameters."Whse. Document"::FOODStagedPick:
                        begin
                            WhseStagedPickLine.CalcFields("Pick to Stage Qty.", "Pick to Stage Qty. (Base)");
                            CalcMaxQty(
                              QtyToPick,
                              WhseStagedPickLine."Qty. to Stage" -
                              WhseStagedPickLine."Qty. Staged" -
                              WhseStagedPickLine."Pick to Stage Qty.",
                              QtyToPickBase,
                              WhseStagedPickLine."Qty. to Stage (Base)" -
                              WhseStagedPickLine."Qty. Staged (Base)" -
                              WhseStagedPickLine."Pick to Stage Qty. (Base)",
                              "Action Type");
                        end;
                // P8000756
                end;
            ...
            // P8001347
            if PickingContainers then begin
                "Container Qty." := QtyToPick;
            end;
            // P8001347
            ...
            // P8000756
            if CreatePickParameters."Whse. Document" in
                [CreatePickParameters."Whse. Document"::Shipment, CreatePickParameters."Whse. Document"::FOODDeliveryTripPick]
             then
                SetStagingFromFields;
            // P8000756
            ...
            // P80038975
            if P800Functions.DistPlanningInstalled then // P80058367
                if DeliveryRouteManagement.IsPickClassCodeEnabled then begin
                    GetItem("Item No.");
                    "Pick Class Code" := Item."Pick Class Code";
                end;
            // P80038975
            Insert();

            AddBinContent; // P8000756
        end;
    end;
    ...
    begin
        // UpdateToQtyToPick(QtyAvailableBase,ToQtyPerUOM,ToQtyToPick,ToQtyToPickBase,TotalQtyToPick,TotalQtyToPickBase); // P8001347
        UpdateToQtyToPick2(QtyAvailableBase, ToQtyPerUOM, ToQtyToPick, ToQtyToPickBase, TotalQtyToPick, TotalQtyToPickBase);   // P8001347
    ...
    begin
        ...
        if (TotalQtyToPickBase > (QtyAvailableBase + RoundingAdjmtMgmt.GetNearZeroQtyForItem(Item."No."))) then
            ToQtyToPickBase := QtyAvailableBase
        else
            ToQtyToPickBase := TotalQtyToPickBase;
        // P8001039
    ...
    procedure SetWhseStagedPickLine(WhseStagedPickLine2: Record "Whse. Staged Pick Line"; TempNo2: Integer)
    begin
        // P8000322A
        WhseStagedPickLine := WhseStagedPickLine2;
        TempNo := TempNo2;
        PickDate := WhseStagedPickLine2."Due Date"; // P8000899
        // P8000322A
    end;

    procedure RestrictToStagedFromPick(WhseStagedPickSourceLine: Record "Whse. Staged Pick Source Line")
    begin
        // P8000322A
        with WhseStagedPickSourceLine do
            if not StagingFromPickLine.Get("No.", "Line No.") then
                Clear(StagingFromPickLine);
        // P8000322A
    end;

    local procedure CalcStageBin(
        LocationCode: Code[10]; ItemNo: Code[20]; VariantCode: Code[10]; UnitofMeasureCode: Code[10]; ToBinCode: Code[20]; QtyPerUnitofMeasure: Decimal;
        QtyRoundingPrecision: Decimal; QtyRoundingPrecisionBase: Decimal; var TempWhseItemTrackingLine: Record "Whse. Item Tracking Line" temporary;
        var TotalQtytoPick: Decimal; var TotalQtytoPickBase: Decimal)
    var
        QtyToPick: Decimal;
        QtyToPickBase: Decimal;
        QtyAttempted: Decimal;
        QtyAttemptedBase: Decimal;
        QtyInBin: Decimal;
    begin
        // P8000322A
        // P80041428
        if not P800Functions.AdvWhseInstalled then begin
            Clear(StagingPickMode);
            exit;
        end;
        // P80041428
        with StagingFromSourceLine do begin
            if (StagingFromPickLine."No." <> '') then begin
                SetRange("No.", StagingFromPickLine."No.");
                SetRange("Line No.", StagingFromPickLine."Line No.");
            end;
            SetRange("Source Type", SourceType);
            SetRange("Source Subtype", SourceSubType);
            ...
            if Find('-') then
                repeat
                    ...
                    QtyToPick := "Qty. Outstanding" - "Pick Qty.";
                    QtyToPickBase := "Qty. Outstanding (Base)" - "Pick Qty. (Base)";
                    ...
                    //      TotalQtytoPick := TotalQtytoPick - QtyToPick;             // P8007470
                    //      TotalQtytoPickBase := TotalQtytoPickBase - QtyToPickBase; // P8007470
                    if (QtyToPick > 0) or (QtyToPickBase > 0) then begin
                        QtyAttempted := QtyToPick;         // P8007470
                        QtyAttemptedBase := QtyToPickBase; // P8007470
                        if ("Bin Code" <> '') then
                            // P8001347
                            // CalcPickBin(
                            //   LocationCode, ItemNo, VariantCode, UnitofMeasureCode, ToBinCode,
                            //   QtyPerUnitofMeasure, QtyToPick, TempWhseItemTrackingLine, FALSE, QtyToPickBase);
                            case StagingPickMode of
                                StagingPickMode::Loose:
                                    CalcPickBin(
                                      LocationCode, ItemNo, VariantCode, UnitofMeasureCode, ToBinCode,
                                      QtyPerUnitofMeasure, QtyRoundingPrecision, QtyRoundingPrecisionBase, QtyToPick, TempWhseItemTrackingLine, false, QtyToPickBase,
                                      QtyRoundingPrecision, QtyRoundingPrecisionBase);
                                StagingPickMode::Containers:
                                    CalcContainerPickBin(
                                      LocationCode, ItemNo, VariantCode, UnitofMeasureCode, ToBinCode,
                                      QtyPerUnitofMeasure, QtyRoundingPrecision, QtyRoundingPrecisionBase, TempWhseItemTrackingLine."Lot No.", QtyToPick, false, QtyToPickBase);
                                StagingPickMode::"Other UOMs":
                                    CalcOtherUOMsPickBin(
                                      LocationCode, ItemNo, VariantCode, UnitofMeasureCode, ToBinCode,
                                      QtyPerUnitofMeasure, QtyRoundingPrecision, QtyRoundingPrecisionBase, QtyToPick, TempWhseItemTrackingLine, false, QtyToPickBase);
                            end;
                        // P8001347
                    end;
                    TotalQtytoPick -= QtyAttempted - QtyToPick;             // P8007470
                    TotalQtytoPickBase -= QtyAttemptedBase - QtyToPickBase; // P8007470
                until (Next = 0) or ((TotalQtytoPick = 0) and (TotalQtytoPickBase = 0));
        end;
        Clear(StagingFromSourceLine);
        // P8000322A
        Clear(StagingPickMode); // P8001347
    end;

    local procedure InStagingMode(): Boolean
    begin
        exit(StagingFromPickLine."No." <> ''); // P8007470
    end;

    local procedure SetStagingFromFields()
    begin
        // P8000322A
        with TempWhseActivLine do begin
            "From Staged Pick No." := StagingFromPickLine."No.";           // P8007470
            "From Staged Pick Line No." := StagingFromPickLine."Line No."; // P8007470
        end;
        // P8000322A
    end;

    local procedure ClearFilterStagingFromFields(var WhseActivLine: Record "Warehouse Activity Line")
    begin
        // P8000322A
        with WhseActivLine do begin
            SetRange("From Staged Pick No.");
            SetRange("From Staged Pick Line No.");
        end;
        // P8000322A
    end;

    local procedure PickAccordingToItemTracking(ItemNo: Code[20]): Boolean
    var
        Item: Record Item;
        ItemTrackingCode: Record "Item Tracking Code";
    begin
        // P8000466A
        ItemTrackingPickingMethod := 0;
        Item.Get(ItemNo);
        if (Item."Item Tracking Code" <> '') then
            with ItemTrackingCode do begin
                ...
                then begin
                    ItemTrackingPickingMethod := "Lot Warehouse Picking Method";
                    exit(true);
                end;
            end;
        exit(false);
    ...
    local procedure DoNotPickItem(ItemNo: Code[20]): Boolean
    var
    ...
    begin
        ...
        exit(ProdCompLine.ReplenishmentNotRequired());
        // P8000322A
    end;

    local procedure SubtractStagedBinPickQtyBase(var WhseActivLine: Record "Warehouse Activity Line"; LocationCode: Code[10]; ItemNo: Code[20]; VariantCode: Code[10]; LotNo: Code[50]; SerialNo: Code[50]; var QtyOnPicks: Decimal)
    var
        WhseActivLine2: Record "Warehouse Activity Line";
    begin
        // P8000322A
        ...
        with WhseActivLine do begin
            Reset;
            SetCurrentKey(
              "Location Code", "Item No.", "Variant Code", "Activity Type", "Action Type",
              "Breakbulk No.", "From Staged Pick No.", "Bin Code", "Lot No.", "Serial No.");
            SetRange("Location Code", LocationCode);
            SetRange("Item No.", ItemNo);
            SetRange("Variant Code", VariantCode);
            SetRange("Activity Type", "Activity Type"::Pick);
            SetRange("Action Type", "Action Type"::Take);
            SetRange("Breakbulk No.", 0);
            SetFilter("From Staged Pick No.", '<>%1', '');
            SetFilter("Bin Code", '<>%1', '');
            if (LotNo <> '') then
            ...
            CalcSums("Qty. Outstanding (Base)");
            QtyOnPicks := QtyOnPicks - "Qty. Outstanding (Base)";
            Copy(WhseActivLine2);
        end;
        // P8000322A
    end;

    procedure VerifyLotFreshenss(SourceNo: Code[20]; SourceType: Integer; SourceSubtype: Integer; SourceLineNo: Integer; LotNo: Code[50]) Result: Boolean
    var
    ...
    begin
        ...
        if (SourceType <> DATABASE::"Sales Line") or
           (SourceSubtype <> SalesLine."Document Type"::Order) or
           (LotNo = '') or
           not SalesLine.Get(SourceSubtype, SourceNo, SourceLineNo)
        then
            exit(true);

        if PickDate = 0D then
            PickDate2 := SalesLine."Shipment Date"
        else
            PickDate2 := PickDate;
        exit(P800ItemTrack.VerifySalesLotIsFresh(SalesLine, LotNo, PickDate2));
    end;

    local procedure SetPickUOM(ItemNo: Code[20]; UnitofMeasureCode: Code[10])
    begin
        // P8001347
        if not PickUOM.Get(ItemNo, UnitofMeasureCode) then begin
            PickUOM.Init;
            PickUOM."Qty. per Unit of Measure" := 1;
        end;
        if (PickUOM."Rounding Precision" = 0) then
            PickUOM."Rounding Precision" := 0.00001;
    end;

    local procedure UpdateToQtyToPick2(QtyAvailableBase: Decimal; ToQtyPerUOM: Decimal; var ToQtyToPick: Decimal; var ToQtyToPickBase: Decimal; TotalQtyToPick: Decimal; TotalQtyToPickBase: Decimal)
    begin
        // P8001347
        ToQtyToPick := CalcUOMQtyFromBase(QtyAvailableBase);
        if (ToQtyToPick > TotalQtyToPick) then
            ToQtyToPick := Round(TotalQtyToPick, PickUOM."Rounding Precision", '<');
        if (ToQtyToPick < 0) then
            ToQtyToPick := 0;
        ToQtyToPickBase := Round(ToQtyToPick * PickUOM."Qty. per Unit of Measure", 0.00001);
        ApplyMaximumQty(ToQtyToPickBase, TotalQtyToPickBase);
        if (ToQtyToPickBase < 0) then
            ToQtyToPickBase := 0;
    end;

    local procedure CalcUOMQtyFromBase(QtyAvailBase: Decimal) Qty: Decimal
    begin
        // P8001347
        Qty := Round(QtyAvailBase / PickUOM."Qty. per Unit of Measure", PickUOM."Rounding Precision");
        if (Qty > 0) then
            if (Round(Qty * PickUOM."Qty. per Unit of Measure", 0.00001) >
                (QtyAvailBase + RoundingAdjmtMgmt.GetNearZeroQtyForItem(Item."No.")))
            then
                Qty := Qty - PickUOM."Rounding Precision";
    end;

    local procedure ApplyMaximumQty(var Qty: Decimal; MaxQty: Decimal)
    begin
        // P8001347
        if (Qty > MaxQty) then
            Qty := MaxQty;
    ...
    local procedure CreateFEFOLines(
        LocationCode: Code[10]; ItemNo: Code[20]; VariantCode: Code[10]; UnitofMeasureCode: Code[10]; FromBinCode: Code[20]; ToBinCode: Code[20]; QtyPerUnitofMeasure: Decimal;
        QtyRoundingPrecision: Decimal; QtyRoundingPrecisionBase: Decimal; var TotalQtytoPick: Decimal; var TotalQtytoPickBase: Decimal)
    var
        HasExpiryDate: Boolean;
        FindOtherUOMs: Boolean;
        EntrySummary: Record "Entry Summary";
        WhseItemTrackingLine: Record "Whse. Item Tracking Line";
        WhseItemTrackingFEFO: Codeunit "Whse. Item Tracking FEFO";
        TotalAvailQtyToPickBase: Decimal;
    ...
    begin
        // P8001347
        if (TotalQtytoPickBase > 0) and Location."Bin Mandatory" and Location."Directed Put-away and Pick" then
            for FindOtherUOMs := false to true do
                if (not FindOtherUOMs) or Location."Allow Breakbulk" then
                    for HasExpiryDate := true downto false do begin
                        if (TotalQtytoPickBase <= 0) then
                            exit;
                        GetBin(Location.Code, WhseWkshLine."To Bin Code");
                        if FindFirstFEFOLot(ItemNo, VariantCode, HasExpiryDate, EntrySummary, WhseItemTrackingFEFO) then begin // P80053254
                            ReqFEFOPick := true;
                            repeat
                                if IsLotNoPickable(ItemNo, VariantCode, EntrySummary, HasExpiryDate) then begin
                                    WhseItemTrackingLine."Lot No." := EntrySummary."Lot No."; // P800144605
                                    WhseItemTrackingLine."Serial No." := EntrySummary."Serial No."; // P800144605
                                    TotalAvailQtyToPickBase :=
                                      CalcTotalAvailQtyToPick(
                                        Location.Code, ItemNo, VariantCode, WhseItemTrackingLine, // P800144605
                                        SourceType, SourceSubType, SourceNo, SourceLineNo, SourceSubLineNo, 0, false); // P8001386
                                    if (TotalAvailQtyToPickBase > 0) and CalledFromWksh and (WhseWkshLine."From Bin Code" <> '') then
                                        ApplyMaximumQty(
                                          TotalAvailQtyToPickBase,
                                          GetFromBinContentQty(
                                            WhseWkshLine."Location Code", WhseWkshLine."From Bin Code", WhseWkshLine."Item No.",
                                    ...
                                    if (TotalAvailQtyToPickBase > 0) then begin
                                        QtyToPickBase := TotalQtytoPickBase;
                                        ApplyMaximumQty(QtyToPickBase, TotalAvailQtyToPickBase);
                                        InsertTempItemTrkgLine(Location.Code, ItemNo, VariantCode, EntrySummary, QtyToPickBase);
                                        QtyToPick := TempWhseItemTrackingLine."Qty. to Handle";
                                        TotalQtytoPick -= QtyToPick;
                                        TotalQtytoPickBase -= QtyToPickBase;
                                        if FindOtherUOMs then
                                            CreateTempOtherUOMLines(
                                              LocationCode, ItemNo, VariantCode, UnitofMeasureCode, FromBinCode, ToBinCode,
                                              QtyPerUnitofMeasure, QtyRoundingPrecision, QtyRoundingPrecisionBase, QtyToPick, TempWhseItemTrackingLine, QtyToPickBase, false)
                                        else
                                            CreateAllTempLines(
                                              LocationCode, ItemNo, VariantCode, UnitofMeasureCode, FromBinCode, ToBinCode,
                                              QtyPerUnitofMeasure, QtyRoundingPrecision, QtyRoundingPrecisionBase, QtyToPick, TempWhseItemTrackingLine, QtyToPickBase, false, false);
                                        TotalQtytoPick += QtyToPick;
                                        TotalQtytoPickBase += QtyToPickBase;
                                        TempWhseItemTrackingLine.Delete;
                                        Clear(TempWhseItemTrackingLine);
                                        WhseItemTrkgExists := false;
                                    end;
                                end;
                            until not FindNextFEFOLot(EntrySummary, WhseItemTrackingFEFO) or (TotalQtytoPickBase = 0); // P80053245
                        end;
                    end;
    end;

    local procedure IsLotNoPickable(ItemNo: Code[20]; VariantCode: Code[10]; var EntrySummary: Record "Entry Summary"; HasExpiryDate: Boolean): Boolean
    begin
        // P8001347
        if ((EntrySummary."Expiration Date" <> 0D) and HasExpiryDate) or
           ((EntrySummary."Expiration Date" = 0D) and (not HasExpiryDate))
        then
            if not LotStatusMgmt.ExcludeLot(ItemNo, VariantCode, EntrySummary."Lot No.", LotStatusExclusionFilter) then
                if VerifyLotPreferences(EntrySummary."Lot No.") then
                    exit(VerifyLotFreshenss(SourceNo, SourceType, SourceSubType, SourceLineNo, EntrySummary."Lot No."));
    end;

    local procedure VerifyLotPreferences(LotNo: Code[50]): Boolean
    var
        SalesLine: Record "Sales Line";
    begin
        // P8001347
        if (SourceType = DATABASE::"Sales Line") then
            if SalesLine.Get(SourceSubType, SourceNo, SourceLineNo) then
                exit(SalesLine.CheckLotPreferences(LotNo, false));
        exit(true);
    end;

    local procedure FindFirstFEFOLot(ItemNo: Code[20]; VariantCode: Code[10]; HasExpiryDate: Boolean; var EntrySummary: Record "Entry Summary"; var WhseItemTrackingFEFO: Codeunit "Whse. Item Tracking FEFO"): Boolean
    begin
        // P8001347
        // P80053245 - Change parameter for CU6501 (ItemTrackingDataCollection) to CU7326 (WhseItemTrackingFEFO)
        WhseItemTrackingFEFO.SetFEFOPickingForSale(SourceType = DATABASE::"Sales Line");
        WhseItemTrackingFEFO.SetItemTrackingPickingMethod(ItemTrackingPickingMethod);
        WhseItemTrackingFEFO.CreateEntrySummaryFEFO(Location, ItemNo, VariantCode, HasExpiryDate);
        if WhseItemTrackingFEFO.FindFirstEntrySummaryFEFO(EntrySummary) then
            exit(true);
        SetHasExpiredItems(WhseItemTrackingFEFO);
    end;

    local procedure FindNextFEFOLot(var EntrySummary: Record "Entry Summary"; var WhseItemTrackingFEFO: Codeunit "Whse. Item Tracking FEFO"): Boolean
    begin
        // P8001347
        // P80053245 - Change parameter for CU6501 (ItemTrackingDataCollection) to CU7326 (WhseItemTrackingFEFO)
        if WhseItemTrackingFEFO.FindNextEntrySummaryFEFO(EntrySummary) then
            exit(true);
        SetHasExpiredItems(WhseItemTrackingFEFO);
    end;

    local procedure SetHasExpiredItems(var WhseItemTrackingFEFO: Codeunit "Whse. Item Tracking FEFO")
    begin
        // P8001347
        // P80053245 - Change parameter for CU6501 (ItemTrackingDataCollection) to CU7326 (WhseItemTrackingFEFO)
        if not HasExpiredItems then begin
            HasExpiredItems := WhseItemTrackingFEFO.GetHasExpiredItems;
            EnqueueCannotBeHandledReason(WhseItemTrackingFEFO.GetResultMessageForExpiredItem); // P80066030
        end;
    end;

    local procedure CreateAllTempLines(
        LocationCode: Code[10]; ItemNo: Code[20]; VariantCode: Code[10]; UnitofMeasureCode: Code[10]; FromBinCode: Code[20]; ToBinCode: Code[20]; QtyPerUnitofMeasure: Decimal;
        QtyRoundingPrecision: Decimal; QtyRoundingPrecisionBase: Decimal; var TotalQtytoPick: Decimal; var TempWhseItemTrackingLine: Record "Whse. Item Tracking Line" temporary;
        var TotalQtytoPickBase: Decimal; IncludeOtherUOMLines: Boolean; IncludeRemQtyLine: Boolean)
    begin
        // P8001347
        if (TotalQtytoPickBase > 0) then begin
            CreateTempContainerLines(
              LocationCode, ItemNo, VariantCode, UnitofMeasureCode, FromBinCode, ToBinCode,
              QtyPerUnitofMeasure, QtyRoundingPrecision, QtyRoundingPrecisionBase, TotalQtytoPick, TempWhseItemTrackingLine, TotalQtytoPickBase);
            CreateTempLooseLines(
              LocationCode, ItemNo, VariantCode, UnitofMeasureCode, FromBinCode, ToBinCode,
              QtyPerUnitofMeasure, QtyRoundingPrecision, QtyRoundingPrecisionBase, TotalQtytoPick, TempWhseItemTrackingLine, TotalQtytoPickBase,
              IncludeOtherUOMLines, IncludeRemQtyLine);
        end;
    end;

    local procedure CreateTempContainerLines(
        LocationCode: Code[10]; ItemNo: Code[20]; VariantCode: Code[10]; UnitofMeasureCode: Code[10]; FromBinCode: Code[20]; ToBinCode: Code[20]; QtyPerUnitofMeasure: Decimal;
        QtyRoundingPrecision: Decimal; QtyRoundingPrecisionBase: Decimal; var TotalQtytoPick: Decimal; var TempWhseItemTrackingLine: Record "Whse. Item Tracking Line" temporary;
    ...
    begin
        ...
        if (TotalQtytoPickBase > 0) then begin
            GetLocation(LocationCode);
            if Location."Bin Mandatory" and Location."Directed Put-away and Pick" then
                if ContainerLinesExist(
                     LocationCode, ItemNo, VariantCode, '', UnitofMeasureCode, TempWhseItemTrackingLine."Lot No.")
                then begin
                    Calc1ContainerMode(
                      LocationCode, ItemNo, VariantCode, UnitofMeasureCode, ToBinCode, QtyPerUnitofMeasure, QtyRoundingPrecision, QtyRoundingPrecisionBase,
                      TempWhseItemTrackingLine."Lot No.", TotalQtytoPick, TotalQtytoPickBase, ContainerPickMode::"Entire Qty.");
                    if (TotalQtytoPickBase > 0) then begin
                        QtyAttempted := TotalQtytoPickBase;
                        Calc1ContainerMode(
                          LocationCode, ItemNo, VariantCode, UnitofMeasureCode, ToBinCode, QtyPerUnitofMeasure, QtyRoundingPrecision, QtyRoundingPrecisionBase,
                          TempWhseItemTrackingLine."Lot No.", TotalQtytoPick, TotalQtytoPickBase, ContainerPickMode::"Full Containers");
                        if (TotalQtytoPickBase > 0) and (TotalQtytoPickBase <> QtyAttempted) then
                            Calc1ContainerMode(
                              LocationCode, ItemNo, VariantCode, UnitofMeasureCode, ToBinCode, QtyPerUnitofMeasure, QtyRoundingPrecision, QtyRoundingPrecisionBase,
                              TempWhseItemTrackingLine."Lot No.", TotalQtytoPick, TotalQtytoPickBase, ContainerPickMode::"Remaining Qty.");
                    end;
                end;
        end;
    end;

    local procedure Calc1ContainerMode(
        LocationCode: Code[10]; ItemNo: Code[20]; VariantCode: Code[10]; UnitofMeasureCode: Code[10]; ToBinCode: Code[20];
        QtyPerUnitofMeasure: Decimal; QtyRoundingPrecision: Decimal; QtyRoundingPrecisionBase: Decimal; LotNo: Code[50];
        var TotalQtytoPick: Decimal; var TotalQtytoPickBase: Decimal; PickMode: Option)
    begin
        // P8001347
        ContainerPickMode := PickMode;
        StagingPickMode := StagingPickMode::Containers;
        CalcStageBin(
          LocationCode, ItemNo, VariantCode, UnitofMeasureCode, ToBinCode,
          QtyPerUnitofMeasure, QtyRoundingPrecision, QtyRoundingPrecisionBase, TempWhseItemTrackingLine, TotalQtytoPick, TotalQtytoPickBase);
        if Location."Use Cross-Docking" and (TotalQtytoPickBase > 0) then
            CalcContainerPickBin(
              LocationCode, ItemNo, VariantCode, UnitofMeasureCode,
              ToBinCode, QtyPerUnitofMeasure, QtyRoundingPrecision, QtyRoundingPrecisionBase, LotNo, TotalQtytoPick, true, TotalQtytoPickBase);
        if (TotalQtytoPickBase > 0) then
            CalcContainerPickBin(
              LocationCode, ItemNo, VariantCode, UnitofMeasureCode,
              ToBinCode, QtyPerUnitofMeasure, QtyRoundingPrecision, QtyRoundingPrecisionBase, LotNo, TotalQtytoPick, false, TotalQtytoPickBase);
    end;

    local procedure CalcContainerPickBin(
        LocationCode: Code[10]; ItemNo: Code[20]; VariantCode: Code[10]; UnitofMeasureCode: Code[10]; ToBinCode: Code[20]; QtyPerUnitofMeasure: Decimal;
        QtyRoundingPrecision: Decimal; QtyRoundingPrecisionBase: Decimal; LotNo: Code[50]; var TotalQtytoPick: Decimal; CrossDock: Boolean; var TotalQtytoPickBase: Decimal)
    var
        WhseItemTrackingSetup: Record "Item Tracking Setup";
        FromBinContent: Record "Bin Content";
    ...
    begin
        ...
        then begin
            TotalAvailQtyToPickBase :=
                CalcTotalAvailQtyToPick(
                LocationCode, ItemNo, VariantCode, TempWhseItemTrackingLine, // P800144605
                SourceType, SourceSubType, SourceNo, SourceLineNo, SourceSubLineNo, TotalQtytoPickBase, false); // P8001386
            if (TotalAvailQtyToPickBase > 0) then
                repeat
                    if IsPickableBin(FromBinContent, ToBinCode) and BinNotExcluded(FromBinContent, false, LotNo) then
                        if ContainerLinesExist(LocationCode, ItemNo, VariantCode, FromBinContent."Bin Code", UnitofMeasureCode, LotNo) then begin
                            CalcBinAvailQtyToPickForUOM(AvailableQtyBase, FromBinContent, TempWhseActivLine, WhseItemTrackingSetup); // P800144605
                            ApplyMaximumQty(AvailableQtyBase, TotalAvailQtyToPickBase);
                            ApplyMaximumQty(AvailableQtyBase, TotalQtytoPickBase);
                            if AvailableQtyBase > 0 then begin
                                CalcQtyToPickBaseForUOM(FromBinContent, QtyToPickBase);
                                ApplyMaximumQty(AvailableQtyBase, QtyToPickBase);
                                UpdateToQtyToPick2(
                                    AvailableQtyBase, QtyPerUnitofMeasure, QtyAvailToPick, QtyToPickBase, TotalQtytoPick, TotalQtytoPickBase);
                                if (QtyAvailToPick > 0) then
                                    AddContainerPickLines(
                                        LocationCode, FromBinContent."Bin Code", ToBinCode, ItemNo, VariantCode, UnitofMeasureCode, QtyPerUnitofMeasure,
                                        LotNo, QtyAvailToPick, TotalQtytoPick, TotalQtytoPickBase, TotalAvailQtyToPickBase);
                            end;
                        end;
                until (FromBinContent.Next() = 0) or (TotalQtytoPickBase = 0) or (TotalAvailQtyToPickBase <= 0);
        end;
    end;

    local procedure AddContainerPickLines(LocationCode: Code[10]; FromBinCode: Code[20]; ToBinCode: Code[20]; ItemNo: Code[20]; VariantCode: Code[10]; UnitofMeasureCode: Code[10]; QtyPerUnitofMeasure: Decimal; LotNo: Code[50]; QtyAvailtoPick: Decimal; var TotalQtytoPick: Decimal; var TotalQtytoPickBase: Decimal; var TotalAvailQtyToPickBase: Decimal)
    var
        TempContainerTotal: Record "Warehouse Activity Line" temporary;
        ContQty: Decimal;
    begin
        // P8001347
        if (QtyAvailtoPick < TotalQtytoPick) then
            ContQty := QtyAvailtoPick
        else
            ContQty := TotalQtytoPick;
        ContainerFns.BuildContainerTotals(
          LocationCode, FromBinCode, ItemNo, VariantCode, UnitofMeasureCode, LotNo, ContQty,
          ContainerPickMode <> ContainerPickMode::"Full Containers", TempWhseActivLine, TempContainerTotal);
        with TempContainerTotal do
            if FindSet then begin
                PickingContainers := true;
                repeat
                    if ContQtyNotExcluded(LocationCode, FromBinCode, ItemNo, VariantCode, UnitofMeasureCode, LotNo, "Container Qty.") then
                        while (Quantity > 0) and (TotalQtytoPick >= "Container Qty.") and
                              (TotalQtytoPickBase > 0) and (TotalAvailQtyToPickBase > 0)
                        do begin
                            ApplyMaximumQty("Qty. (Base)", TotalQtytoPickBase);
                            CreateTempActivityLine(
                              LocationCode, FromBinCode, UnitofMeasureCode, QtyPerUnitofMeasure, "Container Qty.", "Qty. (Base)", 1, 0);
                            CreateTempActivityLine(
                              LocationCode, ToBinCode, UnitofMeasureCode, QtyPerUnitofMeasure, "Container Qty.", "Qty. (Base)", 2, 0);
                            TotalQtytoPick := TotalQtytoPick - "Container Qty.";
                            TotalQtytoPickBase := TotalQtytoPickBase - "Qty. (Base)";
                            TotalAvailQtyToPickBase := TotalAvailQtyToPickBase - "Qty. (Base)";
                            Quantity := Quantity - 1;
                        end;
                until (Next = 0) or (TotalQtytoPickBase = 0) or (TotalAvailQtyToPickBase <= 0);
                PickingContainers := false;
            end;
    end;

    local procedure ContainerLinesExist(LocationCode: Code[10]; ItemNo: Code[20]; VariantCode: Code[10]; BinCode: Code[20]; UnitofMeasureCode: Code[10]; LotNo: Code[50]): Boolean
    var
    ...
    begin
        ...
        with ContainerLine do begin
            SetCurrentKey(
              "Item No.", "Variant Code", "Location Code", "Bin Code", "Unit of Measure Code", "Lot No.");
            SetRange("Item No.", ItemNo);
            SetRange("Variant Code", VariantCode);
            SetRange("Location Code", LocationCode);
            if (BinCode <> '') then
    ...
    local procedure IsPickableBin(var FromBinContent: Record "Bin Content"; ToBinCode: Code[20]): Boolean
    begin
        // P8001347
        exit((FromBinContent."Bin Code" <> ToBinCode) and
             ((UseForPick(FromBinContent) and (not IsMovementWorksheet)) or
              (UseForReplenishment(FromBinContent) and IsMovementWorksheet)));
    end;

    local procedure CreateTempLooseLines(
        LocationCode: Code[10]; ItemNo: Code[20]; VariantCode: Code[10]; UnitofMeasureCode: Code[10]; FromBinCode: Code[20]; ToBinCode: Code[20]; QtyPerUnitofMeasure: Decimal;
        QtyRoundingPrecision: Decimal; QtyRoundingPrecisionBase: Decimal; var TotalQtytoPick: Decimal; var TempWhseItemTrackingLine: Record "Whse. Item Tracking Line" temporary;
        var TotalQtytoPickBase: Decimal; IncludeOtherUOMLines: Boolean; IncludeRemQtyLine: Boolean)
    var
    ...
    begin
        ...
        if (TotalQtytoPickBase > 0) then begin
            CreateTempLine(
              LocationCode, ItemNo, VariantCode, UnitofMeasureCode, FromBinCode, ToBinCode,
              QtyPerUnitofMeasure, QtyRoundingPrecision, QtyRoundingPrecisionBase, TotalQtytoPick, TempWhseItemTrackingLine, TotalQtytoPickBase, WhseItemTrackingSetup);
            if (TotalQtytoPickBase > 0) then
                if IncludeOtherUOMLines then
                    CreateTempOtherUOMLines(
                      LocationCode, ItemNo, VariantCode, UnitofMeasureCode, FromBinCode, ToBinCode,
                      QtyPerUnitofMeasure, QtyRoundingPrecision, QtyRoundingPrecisionBase, TotalQtytoPick, TempWhseItemTrackingLine, TotalQtytoPickBase, IncludeRemQtyLine)
                else
                    if IncludeRemQtyLine then
                        CreateTempRemQtyLine(
                          LocationCode, ItemNo, VariantCode, UnitofMeasureCode, ToBinCode,
                          QtyPerUnitofMeasure, QtyRoundingPrecision, QtyRoundingPrecisionBase, TotalQtytoPick, TempWhseItemTrackingLine, TotalQtytoPickBase);
        end;
    ...
    local procedure CreateTempOtherUOMLines(
        LocationCode: Code[10]; ItemNo: Code[20]; VariantCode: Code[10]; UnitofMeasureCode: Code[10]; FromBinCode: Code[20]; ToBinCode: Code[20]; QtyPerUnitofMeasure: Decimal;
        QtyRoundingPrecision: Decimal; QtyRoundingPrecisionBase: Decimal; var TotalQtytoPick: Decimal; var TempWhseItemTrackingLine: Record "Whse. Item Tracking Line" temporary;
        var TotalQtytoPickBase: Decimal; IncludeRemQtyLine: Boolean)
    begin
        // P8001347
        if (TotalQtytoPickBase > 0) then begin
            if Location."Bin Mandatory" and Location."Directed Put-away and Pick" and Location."Allow Breakbulk" then begin
                StagingPickMode := StagingPickMode::"Other UOMs";
                CalcStageBin(
                  LocationCode, ItemNo, VariantCode, UnitofMeasureCode, ToBinCode,
                  QtyPerUnitofMeasure, QtyRoundingPrecision, QtyRoundingPrecisionBase, TempWhseItemTrackingLine, TotalQtytoPick, TotalQtytoPickBase);
                if Location."Use Cross-Docking" and (TotalQtytoPickBase > 0) then
                    CalcOtherUOMsPickBin(
                      LocationCode, ItemNo, VariantCode, UnitofMeasureCode,
                      ToBinCode, QtyPerUnitofMeasure, QtyRoundingPrecision, QtyRoundingPrecisionBase, TotalQtytoPick, TempWhseItemTrackingLine, true, TotalQtytoPickBase);
                if (TotalQtytoPickBase > 0) then
                    CalcOtherUOMsPickBin(
                      LocationCode, ItemNo, VariantCode, UnitofMeasureCode,
                      ToBinCode, QtyPerUnitofMeasure, QtyRoundingPrecision, QtyRoundingPrecisionBase, TotalQtytoPick, TempWhseItemTrackingLine, false, TotalQtytoPickBase);
            end;
            if IncludeRemQtyLine then
                CreateTempRemQtyLine(
                  LocationCode, ItemNo, VariantCode, UnitofMeasureCode, ToBinCode,
    ...
    local procedure CalcOtherUOMsPickBin(
        LocationCode: Code[10]; ItemNo: Code[20]; VariantCode: Code[10]; UnitofMeasureCode: Code[10]; ToBinCode: Code[20]; QtyPerUnitofMeasure: Decimal;
        QtyRoundingPrecision: Decimal; QtyRoundingPrecisionBase: Decimal; var TotalQtytoPick: Decimal; var TempWhseItemTrackingLine: Record "Whse. Item Tracking Line";
    ...
    var
        ...
        TotalAvailQtyToPickBase: Decimal;
        AvailableQtyBase: Decimal;
        AvailableQtyBaseAllUOMs: Decimal;
    begin
        // P8001347
        if (TotalQtytoPickBase > 0) then
            ItemTrackingMgt.GetWhseItemTrkgSetup(ItemNo, WhseItemTrackingSetup);
        FromBinContent.GetBinContent(
            ItemNo, VariantCode, UnitofMeasureCode, LocationCode, ToBinCode, CrossDock,
            IsMovementWorksheet, WhseItemTrkgExists, WhseItemTrackingSetup, StagingFromPickLine);
        FromBinContent.SetCurrentKey("Location Code", "Item No.", "Variant Code", "Cross-Dock Bin", "Bin Ranking");
        FromBinContent.SetRange("Unit of Measure Code");
        if FromBinContent.FindSet then begin
            ...
            if (TotalAvailQtyToPickBase > 0) then
                repeat
                    if IsPickableBin(FromBinContent, ToBinCode) and
                        BinNotExcluded(FromBinContent, true, TempWhseItemTrackingLine."Lot No.")
                    then begin
                        CalcBinAvailQtyToPickAllUOMs(AvailableQtyBase, FromBinContent, TempWhseActivLine, WhseItemTrackingSetup); // P800144605
                        ApplyMaximumQty(AvailableQtyBase, TotalQtytoPickBase);
                        ApplyMaximumQty(AvailableQtyBase, TotalAvailQtyToPickBase);
                        if AvailableQtyBase > 0 then begin
                            CalcQtyToPickBaseAllUOMs(FromBinContent, AvailableQtyBaseAllUOMs);
                            ApplyMaximumQty(AvailableQtyBase, AvailableQtyBaseAllUOMs);
                            UpdateQuantitiesToPick(
                                AvailableQtyBase,
                                QtyPerUnitofMeasure, FromQtyToPick, FromQtyToPickBase,
                                QtyPerUnitofMeasure, ToQtyToPick, ToQtyToPickBase,
                                TotalQtytoPick, TotalQtytoPickBase);
                            if (ToQtyToPick > 0) then begin
                                CreateTempActivityLine(
                                    LocationCode, FromBinContent."Bin Code", UnitofMeasureCode, QtyPerUnitofMeasure, FromQtyToPick, FromQtyToPickBase, 1, 0);
                                CreateTempActivityLine(
                                    LocationCode, ToBinCode, UnitofMeasureCode, QtyPerUnitofMeasure, ToQtyToPick, ToQtyToPickBase, 2, 0);
                            end;
                            TotalAvailQtyToPickBase := TotalAvailQtyToPickBase - ToQtyToPickBase;
                        end;
                    end;
                    FromBinContent.SetRange("Bin Code", FromBinContent."Bin Code");
                    FromBinContent.FindLast;
                    FromBinContent.SETRANGE("Bin Code");
                    FromBinContent.SetStagingFromBin(LocationCode, StagingFromPickLine); // P80064711
                until (FromBinContent.Next = 0) or (TotalQtytoPickBase = 0) or (TotalAvailQtyToPickBase <= 0);
        end;
    end;

    local procedure CalcBinAvailQtyToPickForUOM(var QtyToPickBase: Decimal; var BinContent: Record "Bin Content"; var TempWhseActivLine2: Record "Warehouse Activity Line" temporary; WhseItemTrackingSetup: Record "Item Tracking Setup")
    var
        QtyToPickBaseAllUOMs: Decimal;
    begin
        // P8001347
        CalcBinAvailQtyToPick2(QtyToPickBase, QtyToPickBaseAllUOMs, BinContent, TempWhseActivLine2, WhseItemTrackingSetup); // P800144605
        ApplyMaximumQty(QtyToPickBase, QtyToPickBaseAllUOMs);
    end;

    local procedure CalcBinAvailQtyToPickAllUOMs(var QtyToPickBaseAllUOMs: Decimal; var BinContent: Record "Bin Content"; var TempWhseActivLine2: Record "Warehouse Activity Line" temporary; WhseItemTrackingSetup: Record "Item Tracking Setup")
    var
        QtyToPickBase: Decimal;
    begin
        CalcBinAvailQtyToPick2(QtyToPickBase, QtyToPickBaseAllUOMs, BinContent, TempWhseActivLine2, WhseItemTrackingSetup); // P8001347, P800144605
    end;

    local procedure CalcBinAvailQtyToPick2(var QtyToPickBase: Decimal; var QtyToPickBaseAllUOMs: Decimal; var BinContent: Record "Bin Content"; var TempWhseActivLine2: Record "Warehouse Activity Line" temporary; WhseItemTrackingSetup: Record "Item Tracking Setup")
    var
        AllBinContent: Record "Bin Content";
        AvailableQtyBase: Decimal;
    begin
        // P8001347
        QtyToPickBase := 0;
        QtyToPickBaseAllUOMs := 0;
        if FindAllUOMBinContents(AllBinContent, BinContent) then
            repeat
                if BinNotExcluded(AllBinContent, false, TempWhseItemTrackingLine."Lot No.") then begin
                    CalcBinAvailQtyToPick(AvailableQtyBase, AllBinContent, TempWhseActivLine2, WhseItemTrackingSetup);
                    CalcBinAvailQtyToPickExclusions(
                      AvailableQtyBase, AllBinContent, TempWhseItemTrackingLine."Lot No.", TempWhseActivLine2, WhseItemTrackingSetup); // P800144605
                end else
                    AvailableQtyBase := 0;
                QtyToPickBaseAllUOMs += AvailableQtyBase;
                if (AllBinContent."Unit of Measure Code" = BinContent."Unit of Measure Code") then
                    QtyToPickBase := AvailableQtyBase;
            until (AllBinContent.Next = 0);
    end;

    local procedure CalcBinAvailQtyToPickExclusions(var QtyBase: Decimal; var BinContent: Record "Bin Content"; LotNo: Code[50]; var TempWhseActivLine2: Record "Warehouse Activity Line" temporary; WhseItemTrackingSetup: Record "Item Tracking Setup")
    var
        BinContent2: Record "Bin Content";
        LotQtyBase: Decimal;
        TotalTempExclusion: Record "Directed Pick Exclusion" temporary;
        OldLotNo: Code[50];
    begin
        // P8001347
        AddContQtyExclusions(QtyBase, BinContent, LotNo);
        if (QtyBase > 0) and (LotNo = '') then begin
            BinContent2.Copy(BinContent);
            with BinContent2 do
                TotalTempExclusion.BuildTotalExclusions(
                  "Location Code", "Bin Code", "Item No.", "Variant Code", "Unit of Measure Code", '', true, TempPickExclusion);
            with TotalTempExclusion do
                if FindBinLotExclusions(BinContent2) then begin
                    OldLotNo := TempWhseItemTrackingLine."Lot No.";
                    repeat
                        TempWhseItemTrackingLine."Lot No." := "Lot No.";
                        BinContent2.SetRange("Lot No. Filter", "Lot No.");
                        CalcBinAvailQtyToPick(LotQtyBase, BinContent2, TempWhseActivLine2, WhseItemTrackingSetup); // P800144605
                        QtyBase := QtyBase - LotQtyBase;
                    until (QtyBase <= 0) or (Next = 0);
                    if (QtyBase < 0) then
                        QtyBase := 0;
                    TempWhseItemTrackingLine."Lot No." := OldLotNo;
                end;
        end;
    end;

    local procedure CalcQtyToPickBaseForUOM(var BinContent: Record "Bin Content"; var QtyToPickBase: Decimal)
    var
        QtyToPickBaseAllUOMs: Decimal;
    begin
        // P8001347
        CalcQtyToPickBase2(BinContent, QtyToPickBase, QtyToPickBaseAllUOMs);
        ApplyMaximumQty(QtyToPickBase, QtyToPickBaseAllUOMs);
    end;

    local procedure CalcQtyToPickBaseAllUOMs(var BinContent: Record "Bin Content"; var QtyToPickBaseAllUOMs: Decimal)
    var
        QtyToPickBase: Decimal;
    begin
        CalcQtyToPickBase2(BinContent, QtyToPickBase, QtyToPickBaseAllUOMs); // P8001347
    end;

    local procedure CalcQtyToPickBase2(var BinContent: Record "Bin Content"; var QtyToPickBase: Decimal; var QtyToPickBaseAllUOMs: Decimal)
    var
        AllBinContent: Record "Bin Content";
        QtyBase: Decimal;
    begin
        ...
        if FindAllUOMBinContents(AllBinContent, BinContent) then
            repeat
                if BinNotExcluded(AllBinContent, false, TempWhseItemTrackingLine."Lot No.") then begin
                    QtyBase := CalcQtyToPickBase(AllBinContent, TempWhseActivLine); // P800-MegaApp
                    CalcQtyToPickBaseExclusions(QtyBase, AllBinContent, TempWhseItemTrackingLine."Lot No.");
                end else
                    QtyBase := 0;
                QtyToPickBaseAllUOMs += QtyBase;
                if (AllBinContent."Unit of Measure Code" = BinContent."Unit of Measure Code") then
                    QtyToPickBase := QtyBase;
            until (AllBinContent.Next = 0);
    end;

    local procedure CalcQtyToPickBaseExclusions(var QtyBase: Decimal; var BinContent: Record "Bin Content"; LotNo: Code[50])
    var
        BinContent2: Record "Bin Content";
        LotQtyBase: Decimal;
        TotalTempExclusion: Record "Directed Pick Exclusion" temporary;
            ...
            with TotalTempExclusion do
                if FindBinLotExclusions(BinContent2) then begin
                    ...
                    repeat
                        TempWhseItemTrackingLine."Lot No." := "Lot No.";
                        BinContent2.SetRange("Lot No. Filter", "Lot No.");
                        LotQtyBase := CalcQtyToPickBase(BinContent2, TempWhseActivLine); // P800-MegaApp
                        QtyBase := QtyBase - LotQtyBase;
                    until (QtyBase <= 0) or (Next = 0);
                    if (QtyBase < 0) then
                        QtyBase := 0;
    ...
    local procedure FindAllUOMBinContents(var AllBinContent: Record "Bin Content"; var BinContent: Record "Bin Content"): Boolean
    begin
        // P8001347
        with AllBinContent do begin
            SetRange("Location Code", BinContent."Location Code");
            SetRange("Bin Code", BinContent."Bin Code");
            SetRange("Item No.", BinContent."Item No.");
            SetRange("Variant Code", BinContent."Variant Code");
            BinContent.CopyFilter("Lot No. Filter", "Lot No. Filter");
            BinContent.CopyFilter("Serial No. Filter", "Serial No. Filter");
            exit(FindSet);
        end;
    end;

    local procedure CreateTempRemQtyLine(
        LocationCode: Code[10]; ItemNo: Code[20]; VariantCode: Code[10]; UnitofMeasureCode: Code[10]; ToBinCode: Code[20]; QtyPerUnitofMeasure: Decimal;
        QtyRoundingPrecision: Decimal; QtyRoundingPrecisionBase: Decimal; var TotalQtytoPick: Decimal; var TempWhseItemTrackingLine: Record "Whse. Item Tracking Line" temporary;
        var TotalQtytoPickBase: Decimal)
    var
    ...
    begin
        // P8001347
        if (TotalQtytoPickBase > 0) and
           Location."Bin Mandatory" and Location."Directed Put-away and Pick" and
           (AlwaysCreatePickLine() or (CreatePickParameters."Whse. Document" = CreatePickParameters."Whse. Document"::FOODDeliveryTripPick))
        then begin
            UpdateQuantitiesToPick(
              TotalQtytoPickBase,
              QtyPerUnitofMeasure, QtytoPick, QtytoPickBase,
    ...
    local procedure AlwaysCreatePickLine(): Boolean
    begin
        exit((not CreatePickParameters."Disable Remaining Qty. Line") and Location."Always Create Pick Line"); // P8001347, P800131478
    end;

    // [Obsolete('Replaced by procedure SetParameters().', '19.0')]
    procedure SetDisableRemQtyLine(NewDisableRemQtyLine: Boolean)
    begin
        CreatePickParameters."Disable Remaining Qty. Line" := NewDisableRemQtyLine; // P800131478
    end;

    procedure ClearTempPickExclusions()
    begin
        // P8001347
        TempPickExclusion.Reset;
        TempPickExclusion.DeleteAll;
    end;

    procedure AddTempBinExclusion(LocationCode: Code[10]; BinCode: Code[20]; ItemNo: Code[20]; VariantCode: Code[10]; UnitOfMeasureCode: Code[10]; LotNo: Code[50])
    begin
        // P8001347
        TempPickExclusion.AddBinExclusion(
          LocationCode, BinCode, ItemNo, VariantCode, UnitOfMeasureCode, LotNo);
    end;

    procedure AddTempContQtyExclusion(LocationCode: Code[10]; BinCode: Code[20]; ItemNo: Code[20]; VariantCode: Code[10]; UnitOfMeasureCode: Code[10]; LotNo: Code[50]; ContainerQty: Decimal)
    begin
        // P8001347
        TempPickExclusion.AddContQtyExclusion(
          LocationCode, BinCode, ItemNo, VariantCode, UnitOfMeasureCode, LotNo, ContainerQty);
    end;

    procedure AddTempExclusion(LocationCode: Code[10]; BinCode: Code[20]; ItemNo: Code[20]; VariantCode: Code[10]; UnitOfMeasureCode: Code[10]; LotNo: Code[50]; ContainerQty: Decimal)
    begin
        // P8001347
        TempPickExclusion.AddExclusion(
          LocationCode, BinCode, ItemNo, VariantCode, UnitOfMeasureCode, LotNo, ContainerQty);
    end;

    local procedure BinNotExcluded(var BinContent: Record "Bin Content"; AllUOMs: Boolean; LotNo: Code[50]): Boolean
    var
        PickExclusion: Record "Directed Pick Exclusion";
    begin
        // P8001347
        if PickExclusion.BinNotExcluded(BinContent, AllUOMs, LotNo) then
            exit(TempPickExclusion.BinNotExcluded(BinContent, AllUOMs, LotNo));
    end;

    local procedure AddContQtyExclusions(var QtyBase: Decimal; var BinContent: Record "Bin Content"; LotNo: Code[50])
    var
        TotalTempExclusion: Record "Directed Pick Exclusion" temporary;
    begin
        // P8001347
        with BinContent do
            TotalTempExclusion.BuildTotalExclusions(
              "Location Code", "Bin Code", "Item No.", "Variant Code", "Unit of Measure Code", LotNo, false, TempPickExclusion);
        TotalTempExclusion.AddContQtyExclusions(QtyBase, BinContent, LotNo, TempWhseActivLine);
    end;

    local procedure ContQtyNotExcluded(LocationCode: Code[10]; BinCode: Code[20]; ItemNo: Code[20]; VariantCode: Code[10]; UnitOfMeasureCode: Code[10]; LotNo: Code[50]; ContainerQty: Decimal): Boolean
    var
        PickExclusion: Record "Directed Pick Exclusion";
    begin
        // P8001347
        if PickExclusion.ContQtyNotExcluded(
             LocationCode, BinCode, ItemNo, VariantCode, UnitOfMeasureCode, LotNo, ContainerQty)
        then
            exit(TempPickExclusion.ContQtyNotExcluded(
                   LocationCode, BinCode, ItemNo, VariantCode, UnitOfMeasureCode, LotNo, ContainerQty));
    end;

    procedure AddToWhsePick(var WhsePickHeader: Record "Warehouse Activity Header"; ShowError: Boolean)
    var
    ...
    begin
        ...
        with TempWhseActivLine do begin
            Reset;
            SetRange("Activity Type", "Activity Type"::Pick);
            SetRange("Location Code", WhsePickHeader."Location Code");
            if Location."Bin Mandatory" then
                SetRange("Action Type", "Action Type"::Take)
            else
                SetRange("Action Type", "Action Type"::" ");
            if CreatePickParameters."Per Bin" or CreatePickParameters."Per Zone" then begin // P800131478
                WhsePickLine.SetRange("Activity Type", WhsePickHeader.Type);
                WhsePickLine.SetRange("No.", WhsePickHeader."No.");
                CopyFilter("Action Type", WhsePickLine."Action Type");
                if CreatePickParameters."Per Bin" then begin // P800131478
                    WhsePickLine.SetFilter("Bin Code", '<>%1', '');
                    if not WhsePickLine.FindFirst then
                        SetFilter("Bin Code", '%1', '')
                    else
                        SetRange("Bin Code", WhsePickLine."Bin Code");
                end;
                if CreatePickParameters."Per Zone" then begin // P800131478
                    WhsePickLine.SetFilter("Zone Code", '<>%1', '');
                    if not WhsePickLine.FindFirst then
                        SetFilter("Zone Code", '%1', '')
                    else
                        SetRange("Zone Code", WhsePickLine."Zone Code");
                end;
            end;
            if not FindSet then begin
                if ShowError then
                    Error(Text000, DequeueCannotBeHandledReason); // P80066030
                exit;
            end;
            WhseActivHeader.LockTable;
            WhseActivHeader.Get(WhsePickHeader.Type, WhsePickHeader."No.");
            WhsePickLine.LockTable;
            repeat
                CreateWhseDocLine;
            until IsEmpty;
        end;
    ...
    [Obsolete('Replaced by procedure SetParameters().', '19.0')]
    procedure SetValues2(CreatePickperSourceDoc2: Boolean)
    begin
        CreatePickParameters."Create Pick Per Source Doc." := CreatePickperSourceDoc2; // P800131478
    end;
    ...
    local procedure CreateTempLineForProductionContainer(LocationCode: Code[10]; ToBinCode: Code[20]; var TotalQtyToPick: Decimal; var TotalQtyToPickBase: Decimal)
    var
        ContainerHeader: Record "Container Header";
        TempContainerLine: Record "Container Line" temporary;
    begin
        // P80056710
        if (SourceType = DATABASE::"Prod. Order Component") and P800Functions.ContainerTrackingInstalled then
            if CreatePickParameters."Whse. Document" = CreatePickParameters."Whse. Document"::Production then
                ContainerFns.CreatePickFromProductionContainer(ProdOrderCompLine, LocationCode, ToBinCode, TotalQtyToPick, TempContainerLine)
            else
                ContainerFns.CreatePickFromProductionContainer(WhseWkshLine, LocationCode, ToBinCode, TotalQtyToPick, TempContainerLine);

        if TempContainerLine.FindSet then
            repeat
                ContainerHeader.Get(TempContainerLine."Container ID");

                CreateTempActivityLine(LocationCode, TempContainerLine."Bin Code", TempContainerLine."Unit of Measure Code",
                  TempContainerLine."Qty. per Unit of Measure", TempContainerLine.Quantity, TempContainerLine."Quantity (Base)", 1, 0);
                TempWhseActivLine."Lot No." := TempContainerLine."Lot No.";
                TempWhseActivLine."Container ID" := TempContainerLine."Container ID";
                TempWhseActivLine.Modify;

                CreateTempActivityLine(LocationCode, ToBinCode, TempContainerLine."Unit of Measure Code",
                  TempContainerLine."Qty. per Unit of Measure", TempContainerLine.Quantity, TempContainerLine."Quantity (Base)", 2, 0);
                TempWhseActivLine."Lot No." := TempContainerLine."Lot No.";
                TempWhseActivLine."Container ID" := TempContainerLine."Container ID";
                TempWhseActivLine.Modify;

                TotalQtyToPick -= TempContainerLine.Quantity;
                TotalQtyToPickBase -= TempContainerLine."Quantity (Base)";
                TotalQtyPickedBase += TempContainerLine."Quantity (Base)";
            until TempContainerLine.Next = 0;
    end;

    local procedure CalcQtyInProdContainerBase(LocationCode: Code[10]; BinCode: Code[20]; ItemNo: Code[20]; VariantCode: Code[10]; UOM: Code[10]) QtyBaseToExclude: Decimal
    var
        ContainerHeader: Record "Container Header";
        ContainerLine: Record "Container Line";
        WarehouseActivityLine: Record "Warehouse Activity Line";
        WarehouseActivityLine2: Record "Warehouse Activity Line";
        QtyBaseExcluded: Decimal;
    begin
            ...
            repeat
                ...
                if ContainerLine.FindSet then
                    repeat
                        ...
                        ContainerLine.CalcSums("Quantity (Base)");
                        ContainerLine.SetRange("Lot No.");

                        // Figure out what portion of the container content has already been excluded from consideration for picking
                        WarehouseActivityLine.SetRange("Action Type", WarehouseActivityLine."Action Type"::Take);
                        WarehouseActivityLine.SetRange("Container ID", ContainerLine."Container ID");
                        WarehouseActivityLine.SetRange("Item No.", ItemNo);
                        WarehouseActivityLine.SetRange("Variant Code", VariantCode);
                        if UOM <> '' then
                            WarehouseActivityLine.SetRange("Unit of Measure Code", UOM);
                        WarehouseActivityLine.SetRange("Lot No.", ContainerLine."Lot No.");
                        WarehouseActivityLine.CalcSums("Qty. (Base)");
                        QtyBaseExcluded := WarehouseActivityLine."Qty. (Base)";

                        WarehouseActivityLine2.Copy(TempWhseActivLine);
                        TempWhseActivLine.Reset;
                        TempWhseActivLine.SetRange("Action Type", TempWhseActivLine."Action Type"::Take);
                        TempWhseActivLine.SetRange("Container ID", ContainerLine."Container ID");
                        TempWhseActivLine.SetRange("Item No.", ItemNo);
                        TempWhseActivLine.SetRange("Variant Code", VariantCode);
                        if UOM <> '' then
                            TempWhseActivLine.SetRange("Unit of Measure Code", UOM);
                        TempWhseActivLine.SetRange("Lot No.", ContainerLine."Lot No.");
                        TempWhseActivLine.CalcSums("Qty. (Base)");
                        QtyBaseExcluded += TempWhseActivLine."Qty. (Base)";
                        TempWhseActivLine.Copy(WarehouseActivityLine2);

                        QtyBaseToExclude += ContainerLine."Quantity (Base)" - QtyBaseExcluded;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeCreateWhseDocument(var TempWhseActivLine: Record "Warehouse Activity Line" temporary; WhseSource: Option "Pick Worksheet",Shipment,"Movement Worksheet","Internal Pick",Production,Assembly,,"Staged Pick","Delivery Trip Pick"; var IsHandled: Boolean)
    begin
    end;
