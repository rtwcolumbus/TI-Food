codeunit 5896 "Calc. Inventory Adjmt. - Order"
{
    ...
    // 
    // P8001200, Columbus IT, Don Bresee, 27 AUG 13
    //   Always use Actual Cost for Lot Combinations
    // 
    // PRW17.10.01
    // P8001260, Columbus IT, Jack Reynolds, 14 JAN 14
    //   Fix problems using quantity instead of costing quantity
    // 
    // PRW110.0.02
    // P80050651, To-Increase, Jack Reynolds, 05 FEB 18
    //   Resource costs for Repack
    // 
    // PRW111.00.03
    // P80075205, To-Increase, Jack Reynolds, 29 MAY 19
    //   Fix Issue with repack resource cost, missing line from upgrade
    ...
    begin
        ...
        CalcActualResourceCosts(InvtAdjmtEntryOrder); // P80050651, P80075205
                ...
                if Find('-') then begin
                    ...
                    repeat
                        if Positive = IsPositiveOutputs then begin
                            ...
                            //IF RemOutputQty * (Quantity + ReversedQty) <> 0 THEN BEGIN    // P8001260
                            if RemOutputQty * (GetCostingQty + ReversedQty) <> 0 then begin // P8001260
                                                                                            // Calculate cost for gross output
                                                                                            //NewActInvtAdjmtEntryOrder.RoundCosts((Quantity + ReversedQty) / RemOutputQty);    // P8001260
                                NewActInvtAdjmtEntryOrder.RoundCosts((GetCostingQty + ReversedQty) / RemOutputQty); // P8001260

                                //RemOutputQty -= (Quantity + ReversedQty);    // P8001260
                                RemOutputQty -= (GetCostingQty + ReversedQty); // P8001260
    ...
    begin
        if not InvtAdjmtEntryOrder.Find or not InvtAdjmtEntryOrder."Completely Invoiced" then
            ...
            InvtAdjmtEntryOrder.AdjustCostsForOrderType(); // P80050651
            InvtAdjmtEntryOrder.CalcDirectCostFromCostShares();
            InvtAdjmtEntryOrder.CalcIndirectCostFromCostShares();
            InvtAdjmtEntryOrder.CalcUnitCost();
        end;
    ...
    begin
        ...
        InvtAdjmtEntryOrder.SetBOMILEFilters(CalcActualMaterialCostQuery, false); // P8001134
        ...
        with InvtAdjmtEntryOrder do
            if ("Order Type" = "Order Type"::Production) then begin
                ...
                P800ProdOrderMgmt.AddProdLineActCost(ProdOrderLine, "Single-Level Material Cost", "Single-Lvl Material Cost (ACY)"); // PR3.60
            end;
        // P8001132
    end;
    ...
    begin
        ...
        if InvtAdjmtEntryOrder.IsBOMOrderType() then // P8001134
            exit(1);                                   // P8001134
    ...
    begin
        with FromItemLedgEntry do begin
            ...
            SourceInvtAdjmtEntryOrder.SetBOMILEFilters(FromItemLedgEntry, true); // P8001134
            ...
            if FindSet() then
                repeat
                    ...
                    if OutbndItemLedgEntry."Applies-to Entry" <> 0 then
                        //Qty += OutbndItemLedgEntry.Quantity;    // P8001260
                        Qty += OutbndItemLedgEntry.GetCostingQty; // P8001260
                until Next() = 0;
        end;
    end;
    ...
    local procedure CalcActualResourceCosts(var InvtAdjmtEntryOrder: Record "Inventory Adjmt. Entry (Order)")
    var
        ResLedgerEntry: Record "Res. Ledger Entry";
        ACYMgt: Codeunit "Additional-Currency Management";
        DirectCost: Decimal;
        TotalCostACY: Decimal;
        DirectCostACY: Decimal;
        DirectUnitCostACY: Decimal;
    begin
        // P80050651
        if InvtAdjmtEntryOrder."Order Type" <> InvtAdjmtEntryOrder."Order Type"::FOODRepack then
            exit;

        ResLedgerEntry.SetRange("Entry Type", ResLedgerEntry."Entry Type"::Usage);
        ResLedgerEntry.SetRange("Order Type", InvtAdjmtEntryOrder."Order Type");
        ResLedgerEntry.SetRange("Order No.", InvtAdjmtEntryOrder."Order No.");
        if ResLedgerEntry.FindSet then
            repeat
                DirectCost := ResLedgerEntry.Quantity * ResLedgerEntry."Direct Unit Cost";

                TotalCostACY := ACYMgt.CalcACYAmt(ResLedgerEntry."Total Cost", ResLedgerEntry."Posting Date", false);
                DirectUnitCostACY := ACYMgt.CalcACYAmt(ResLedgerEntry."Direct Unit Cost", ResLedgerEntry."Posting Date", true);
                DirectCostACY := ResLedgerEntry.Quantity * DirectUnitCostACY;

                InvtAdjmtEntryOrder.AddSingleLvlCapacityCost(DirectCost, DirectCostACY);
                InvtAdjmtEntryOrder.AddSingleLvlCapOvhdCost(ResLedgerEntry."Total Cost" - DirectCost, TotalCostACY - DirectCostACY);
