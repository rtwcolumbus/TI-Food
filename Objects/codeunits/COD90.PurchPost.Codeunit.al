codeunit 90 "Purch.-Post" // Version: FOODNA
{
    ...
    begin
        ...
        with PurchLine do begin
            ...
            if Quantity <> 0 then begin
                ...
                // DivideAmount(PurchHeader, PurchLine, 1 ,"Qty. to Invoice", TempVATAmountLine, TempVATAmountLineRemainder);                        // PR3.60
            ...
            if (QtyToBeInvoiced <> 0) or (QtyToBeInvoicedAlt <> 0) then begin // P8000971
                ...
                if ItemChargeNo <> '' then
                    ...
                    if PurchLine.CostInAlternateUnits() then begin
                        if (QtyToBeInvoicedAlt <> 0) and (PurchLine.Type = PurchLine.Type::Item) then // P8007748
                            Factor := QtyToBeInvoicedAlt / PurchLine."Qty. to Invoice (Alt.)"
                        else
                ...
                if PurchHeader."Prices Including VAT" then
                    Amount :=
                      (CostingQty * PurchLine."Direct Unit Cost" * (1 - PurchLine."Line Discount %" / 100) / // P8000382A
                       (1 + PurchLine."VAT %" / 100)) + RemAmt
                else
                    Amount :=
                      (CostingQty * PurchLine."Direct Unit Cost" * (1 - PurchLine."Line Discount %" / 100)) + RemAmt; // P8000382A
                RemAmt := Amount - Round(Amount);
    ...
    local procedure DivideAmount(PurchHeader: Record "Purchase Header"; var PurchLine: Record "Purchase Line"; QtyType: Option General,Invoicing,Shipping; PurchLineQty: Decimal; PurchLineQtyAlt: Decimal; var TempVATAmountLine: Record "VAT Amount Line" temporary; var TempVATAmountLineRemainder: Record "VAT Amount Line" temporary)
    var
    ...
    local procedure InsertReceiptLine(PurchRcptHeader: Record "Purch. Rcpt. Header"; var PurchRcptLine: Record "Purch. Rcpt. Line"; PurchLine: Record "Purchase Line"; CostBaseAmount: Decimal; PostAsItem: Boolean)
    var
            ...
            if PurchRcptLine.FindSet(true, false) then begin
                ...
                repeat
                    ...
                    UpdateInvoicedQtyOnPurchRcptLine(
                      PurchInvHeader, PurchRcptLine, PurchHeader, PurchLine, QtyToBeInvoiced, QtyToBeInvoicedBase, QtyToBeInvoicedAlt, TrackingSpecificationExists); // P8001386
