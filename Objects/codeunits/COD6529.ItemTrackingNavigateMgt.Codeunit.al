codeunit 6529 "Item Tracking Navigate Mgt."
{
    ...
    //   Support fom maintenance ledger entries
    ...
    var
        ...
        TempWorkOrder: Record "Work Order" temporary;
    ...
    begin
        ...
        FindMaintLedgerEntry(ItemFilters); // P800-MegaApp
    ...
    local procedure FindMaintLedgerEntry(var ItemFilters: Record Item)
    begin
        // P8000466A
        if not MaintLedgerEntry.ReadPermission then
            exit;

        with MaintLedgerEntry do begin
            Reset;
            if ItemFilters.GetFilter("Lot No. Filter") <> '' then
                if SetCurrentKey("Lot No.") then;
            if ItemFilters.GetFilter("Serial No. Filter") <> '' then
            ...
            if FindSet then
                repeat
                    RecRef.GetTable(MaintLedgerEntry);
                    // Upgrade18.0
                    Clear(ItemTrackingSetup);
                    ItemTrackingSetup.CopyTrackingFromMaintLedgerEntry(MaintLedgerEntry);
                    // Upgrade18.0
                    InsertBufferRec(RecRef, ItemTrackingSetup, "Item No.", '');
                    WorkOrder.Get("Work Order No.");
                    RecRef.GetTable(WorkOrder);
                    InsertBufferRec(RecRef, ItemTrackingSetup, "Item No.", '');
                    TempWorkOrder := WorkOrder;
                    if TempWorkOrder.Insert then;
                until Next = 0;
        end;
    end;
    ...
    begin
        case TableNo of
            ...
            DATABASE::"Maintenance Ledger":
                PAGE.Run(0, MaintLedgerEntry);
            DATABASE::"Work Order":
                PAGE.Run(0, TempWorkOrder);
                // P8000466A
        end;
