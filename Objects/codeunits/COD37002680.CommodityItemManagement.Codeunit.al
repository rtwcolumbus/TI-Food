    ...
    begin
        if (CostAdjmt <> 0) or (CostAdjmtACY <> 0) then
            with CostElementBuf do begin
                CostExists := GetElement("Cost Entry Type".FromInteger(CostElementType), "Cost Variance Type"::" "); // P800144605
    ...
    begin
        ...
        if (ShareOfTotalCost <> 0) then
            with CurrEntryCostBuf do
                if GetElement("Cost Entry Type"::"Direct Cost", "Cost Variance Type"::" ") then // P800144605
                    OverrideCommVariance := ("Actual Cost" <> 0) or ("Actual Cost (ACY)" <> 0);
        if OverrideCommVariance then begin
            with ProdOrderCostBuf do begin
                if not GetElement("Cost Entry Type"::Variance, "Cost Variance Type"::Material) then // P800144605
                    Insert;
                FindSet;
            end;
            with NewEntryCostBuf do begin
                repeat
                    ...
                    CurrEntryCostBuf.GetElement(Type, "Variance Type"); // P800144605
                ...
                GetElement("Cost Entry Type"::Variance, "Cost Variance Type"::Material); // P800144605
