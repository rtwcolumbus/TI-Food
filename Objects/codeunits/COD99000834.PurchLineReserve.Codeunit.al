...
{
    // PR3.60.03
    //   Call SetItemTrackingHandling in DeleteLine to delete tracking lines automatically
    // 
    // PR3.70.07
    // P8000121A, Myers Nissi, Jack Reynolds, 23 SEP 04
    //   RetrieveInvoiceSpecification - call AdjustInvoiceRounding in item tracking management codeunit
    // 
    // PR4.00.01
    ...
    //   Modify DeleteLine to preserve tracking specification lines
    // 
    // PRW15.00.01
    // P8000549A, VerticalSoft, Jack Reynolds, 15 APR 08
    //   SetDeleteItemTracking - set DeleteItemTracking to TRUE
    // 
    // PRW110.0.02
    ...
    // 
    // PRW111.00.01
    // P80059665, To-Increase, Dayakar Battini, 13 JUN 18
    //   Fix issue with purchase return order posting
    // 
    // PRW111.00.02
    // P80068196, To-Increase, Jack Reynolds, 10 DEC 18
    //   Add rounding to code change from P80047943
    ...
    begin
        ...
        with PurchLine do begin
            ...
            ReservMgt.DeleteReservEntries(true, 0);
            if not RetainTrackingLines then         // P8000410A
                DeleteInvoiceSpecFromLine(PurchLine);
            RetainTrackingLines := false;           // P8000410A
    ...
    begin
        ...
        repeat
            ...
            // P80047943
            if (PurchLine."Receipt No." <> '') or (PurchLine."Return Shipment No." <> '') then
                if TrackingSpecification."Qty. to Invoice (Base)" <> 0 then
                    TempInvoicingSpecification."Qty. to Invoice (Alt.)" := Round(TempInvoicingSpecification."Qty. to Invoice (Alt.)" / TrackingSpecification."Qty. to Invoice (Base)" * ReservEntry."Quantity (Base)", 0.00001); // P80068196
                                                                                                                                                                                                                                 // P80047943
    ...
    procedure RetainTrackingSpec()
    begin
        // P8000410A
        RetainTrackingLines := true;
    end;
    ...
    local procedure GetSourceValue(ReservEntry: Record "Reservation Entry"; var SourceRecRef: RecordRef; ReturnOption: Option "Net Qty. (Base)","Gross Qty. (Base)",,,,"Gross Qty. (Alt.)"): Decimal
    var
        PurchLine: Record "Purchase Line";
    begin
        ...
        case ReturnOption of
            ...
            ReturnOption::"Gross Qty. (Alt.)":     // P8000267B
                exit(PurchLine."Quantity (Alt.)"); // P8000267B
        end;
    end;
