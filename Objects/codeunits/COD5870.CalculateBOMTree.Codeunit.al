codeunit 5870 "Calculate BOM Tree"
{
    ...
    begin
        with BOMBuffer do begin
            ...
            ParentItem.InitFromSKU("Variant Code", "Location Code"); // P8001144
    ...
    begin
        ...
        with ProdBOMLine do begin
            ...
            //SETRANGE("Version Code",VersionMgt.GetBOMVersion(ParentItem."Production BOM No.",WORKDATE,TRUE));           // P8001213
            SetRange("Version Code", GetBOMVersionCode(ParentItem."Production BOM No.", ParentBOMBuffer."Needed by Date")); // P8001144
            ...
            if FindSet() then begin
                if ParentItem."Replenishment System" <> ParentItem."Replenishment System"::"Prod. Order" then
                    exit(true);
                CoProdBOMFactor := GetCoProdBOMFactor(ParentBOMBuffer, "Version Code"); // P8001144
                repeat
                    IsHandled := FALSE;
                            ...
                            case Type of
                                Type::Item:
                                    begin
                                        ...
                                        BOMBuffer.TransferFromProdComp(
                                          ...
                                          //     ParentBOMBuffer."Scrap Qty. per Top Item" *
                                          //     UOMMgt.GetQtyPerUnitOfMeasure(ParentItem, ParentBOMBuffer."Unit of Measure Code"), UOMMgt.QtyRndPrecision),
                                          ParentBOMBuffer."Qty. per Top Item" * ParentBOMBuffer."Qty. per Unit of Measure",
                                          ParentBOMBuffer."Qty. per Unit of Measure",
                                          CoProdBOMFactor,
                                          ParentBOMBuffer."Scrap Qty. per Top Item" * ParentBOMBuffer."Qty. per Unit of Measure",
                                          // P8001144
                                ...
                                Type::"Production BOM":
                                    begin
                                        ...
                                        BOMBuffer."Qty. per Unit of Measure" := BOMBuffer."Qty. per Unit of Measure" * Quantity; // P8001144
    ...
    begin
        ...
        with ProdOrderComp do begin
            ...
            // SETRANGE("Prod. Order Line No.",ProdOrderLine."Line No.");      // P8001144
            SetFilter("Prod. Order Line No.", '%1|0', ProdOrderLine."Line No."); // P8001144
            if FindSet() then begin
                repeat
                    if "Item No." <> '' then begin
                        // OldProdOrderLine.GET(Status,"Prod. Order No.","Prod. Order Line No."); // P8001144
                        OldProdOrderLine.Get(Status, "Prod. Order No.", ProdOrderLine."Line No.");  // P8001144
    ...
    begin
        ...
        AvailableForFieldNo := LotStatusMgmt.AvailableForToFieldNo(AvailableFor);                   // P8001144
        LotStatusExclusionFilter := LotStatusMgmt.SetLotStatusExclusionFilter(AvailableForFieldNo); // P8001144
        ...
        with BOMBuffer do
            repeat
                if not AvailByDateExists(BOMBuffer) then begin
                    ...
                    LotStatusMgmt.SetInboundExclusions(BOMItem, AvailableForFieldNo, ExcludePurch, ExcludeSalesRet, ExcludeOutput); // P8001144
                    AvailableToPromise.SetAvailableFor(LotStatusExclusionFilter, ExcludePurch, ExcludeSalesRet, ExcludeOutput);    // P8001144
                    TempItemAvailByDate."Available Qty" :=
                      ...
                      //    BOMItem, "Gross Requirement", "Scheduled Receipts", "Needed by Date", "Analysis Period Type"::Day, ZeroDF);
                          BOMItem, "Gross Requirement", "Scheduled Receipts", "Needed by Date", "Analysis Period Type"::Day, ZeroDF) / "Qty. per Unit of Measure";
                    // P8001144
        ...
        with BOMBuffer do begin
            while (Next <> 0) and (ParentBOMBuffer.Indentation < Indentation) do
                if ParentBOMBuffer.Indentation + 1 = Indentation then begin
                    ...
                    // IF AvailQty < ExpectedQty THEN BEGIN // P8001144
                    if (AvailQty <= ExpectedQty) then begin // P8001144
                        if "Is Leaf" then begin
    ...
    begin
        ...
        CalcStdCost.CalcRtngLineCost(
          RoutingLine, CostCalculationMgt.CalcQtyAdjdForBOMScrap(LotSize, ScrapPct), CapCost, SubcontractedCapCost, CapOverhead, // P8001132
          ParentItem, ''); // P8001132
    ...
    local procedure GetBOMVersionCode(ProdBOMNo: Code[20]; DemandDate: Date) VersionCode: Code[10]
    begin
        // P8001144
        if (DemandDate = 0D) then
            DemandDate := WorkDate;
        VersionCode := VersionMgt.GetBOMVersion(ProdBOMNo, DemandDate, true);
        if (VersionCode = '') then
            VersionCode := VersionMgt.GetBOMVersion(ProdBOMNo, DemandDate, false);
    end;

    local procedure GetCoProdBOMFactor(BOMBuffer: Record "BOM Buffer"; BOMVersionCode: Code[10]): Decimal
    var
    ...
    begin
        ...
        with BOMBuffer do
            if (Type = Type::Item) then
                if ParentBOMHeader.Get("Production BOM No.") then
                    exit(ParentBOMHeader.GetCoProdBOMFactor(BOMVersionCode, "No."));
        exit(1);
    end;
