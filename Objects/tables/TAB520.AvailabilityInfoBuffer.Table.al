table 520 "Availability Info. Buffer"
{
    ...
    fields
    {
        ...
        field(37002001; "Lot Status Code"; Code[10])
        {
            Caption = 'Lot Stauts Code';
        }
        field(37002002; Exclude; Boolean)
        {
            Caption = 'Exclude';
        }
        field(37002003; "Supplier Lot No."; Code[50])
        {
            Caption = 'Supplier Lot No.';
            FieldClass = FlowField;
            CalcFormula = lookup("Lot No. Information"."Supplier Lot No." where("Item No." = field("Item No."),
                                                                                "Variant Code" = field("Variant Code"),
                                                                                "Lot No." = field("Lot No.")));
        }
        field(37002004; "Creation Date"; Date)
        {
        ...
        field(37002005; "Expiration Date-Lot Info"; Date)
        {
    ...
    keys
    {
        key(Key1; "Lot Status Code", "Variant Code", "Lot No.", "Serial No.", "Package No.")
        {
            Clustered = true;
        }
    }
    ...
    procedure SetLotStatus(Positive: Boolean)
    var
        ...
        LotStatusMgt: Codeunit "Lot Status Management";
    begin
        if LotNoInformation.Get("Item No.", "Variant Code", "Lot No.") then
            if LotnoInformation.Posted then begin
                "Lot Status Code" := LotnoInformation."Lot Status Code";
                exit;
            end;

        if Positive then begin
            Item.Get("Item No.");
            "Lot Status Code" := LotStatusMgt.GetQCStatus(Item);
            if "Lot Status Code" = '' then begin
                InvSetup.Get();
                ReservationEntry.SetRange("Item No.", "Item No.");
                ReservationEntry.SetRange("Variant Code", "Variant Code");
                ReservationEntry.SetRange("Lot No.", "Lot No.");
                ...
                if ReservationEntry.FindFirst() then
                    case ReservationEntry."Source Type" of
                        Database::"Sales Line":
                            "Lot Status Code" := InvSetup."Sales Lot Status";
                        Database::"Purchase Line":
                            "Lot Status Code" := InvSetup."Purchase Lot Status";
                        Database::"Prod. Order Line",
                        Database::"Assembly Header":
                            "Lot Status Code" := InvSetup."Output Lot Status";
                        Database::"Requisition Line":
                            begin
                                ...
                                case RequisitionLine."Replenishment System" of
                                    RequisitionLine."Replenishment System"::Purchase:
                                        "Lot Status Code" := InvSetup."Purchase Lot Status";
                                    RequisitionLine."Replenishment System"::"Prod. Order",
    ...
    procedure UpdateExclude(LotStatusExclusionFilter: Text)
    var
        LotStatus: Record "Lot Status Code";
    begin
        if ("Lot Status Code" = '') or (LotStatusExclusionFilter = '') then
            Exclude := false
        else begin
            LotStatus.SetFilter(Code, LotStatusExclusionFilter);
            LotStatus.Code := "Lot Status Code"; 
            Exclude := LotStatus.Find();
        end;
    end;
