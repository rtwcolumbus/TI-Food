table 5771 "Warehouse Source Filter"
{
    ...
    fields
    {
        ...
        field(12; "Shipping Agent Code Filter"; Code[100])
        {
            ...
            trigger OnValidate()
            begin
                Validate("Create Delivery Trip"); //N138F0000.n
            end;
        }
        ...
        field(11028580; "Create Delivery Trip"; Boolean)
        {
            Caption = 'Create Delivery Trip';
            ...
            var
                ...
                CurrText000: Label 'You are not allowed to set multiple Shipping Agent Services';
            begin
                if "Create Delivery Trip" then
                    if not ShippingAgentServices.Get("Shipping Agent Code Filter", "Shipping Agent Service Filter") then
                        Error(CurrText000);
            end;
        }
        field(11028581; "Pick Class Code Sub Filter"; Code[100])
        {
            Caption = 'Pick Class Code Sub Filter';
    ...
    procedure SetFilters(var GetSourceBatch: Report "Get Source Documents"; LocationCode: Code[10]; DeliveryTrip: Code[20])
    var
        WhseRequest: Record "Warehouse Request";
    begin
        SetFilters(GetSourceBatch, LocationCode);
        GetSourceBatch.GetWarehouseRequest(WhseRequest);
        WhseRequest.SetFilter("Delivery Trip", '''''|%1', DeliveryTrip); //N138F0000.n
        GetSourceBatch.SetTableView(WhseRequest);
    end;
