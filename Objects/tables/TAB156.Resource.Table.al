...
{
    // PR1.00
    //   Added "Other" as option for Type
    //   Indirect Cost % - remove MaxValue
    ...
    // 
    // PR1.00.04
    //   Update Prod. BOM Activity Cost after changes to Unit Cost or Direct Unit Cost
    // 
    // PR2.00
    //   Set InitValue on Capacity Level % to 100
    // 
    // PR4.00
    // P8000197A, Myers Nissi, Jack Reynolds, 21 SEP 05
    //   Add field for Location Code
    // 
    // PR4.00.04
    // P8000333A, VerticalSoft, Jack Reynolds, 30 AUG 06
    //   Check for assets before deletion; update asset locations if location code changes
    // 
    // PRW16.00.04
    // P8000877, Columbus IT, Jack Reynolds, 02 MAR 11
    //   Add field for Machine Type
    // 
    // PRW16.00.06
    ...
    // 
    // PRW18.00.02
    // P8004218, to-Increase, Jack Reynolds, 02 Oct 15
    //   Add Location code to drop down field group
    // 
    // PRW111.00.01
    ...
    // 
    // PRW111.00.02
    // P80070316,To-Increase, Gangabhushan, 12 FEB 19
    // TI-12799 - When trying to create resources without a licence to Process Manufacturing returns an error
    // 
    // P80070978, To-Increase, Gangabhushan, 28 FEB 19
    //   TI-12898 - Resource replenishment area length causes error
    // 
    // PRW113.00
    ...
    fields
    {
        ...
        field(2; Type; Enum "Resource Type")
        {
            ...
            begin
                // P8000877
                if Type <> Type::Machine then begin // P8001234
                    "Machine Type" := 0;
                    DeleteLotSegment; // P8001234
                end;                // P8001234
                // P8000877
            end;
        }
        ...
        field(19; "Direct Unit Cost"; Decimal)
        {
            ...
            begin
                ...
                if ProcessFns.ProcessInstalled then      // PR3.60
                    P800BOMFns.ResourceUpdateABCCost(Rec); // PR3.60
            end;
        }
        ...
        field(37002460; "Equipment Capacity"; Decimal)
        {
            BlankZero = true;
            ...
            trigger OnValidate()
            begin
                ValidateEquipmentQtys("Equipment Capacity", "Capacity Level %", "Minimum Equipment Qty."); // P8001107
            end;
        }
        field(37002461; "Capacity UOM"; Code[10])
        {
            Caption = 'Capacity UOM';
            Description = 'PR1.00';
            TableRelation = "Unit of Measure";
        }
        field(37002462; "Capacity Level %"; Decimal)
        {
        ...
        field(37002463; "Location Code"; Code[10])
        {
            ...
            begin
                ...
                if "Location Code" <> xRec."Location Code" then begin
                    "Replenishment Area Code" := ''; // P8001279
                    Asset.SetCurrentKey("Resource No.");
                    Asset.SetRange("Resource No.", "No.");
                    if Asset.FindSet(true, false) then
                        repeat
                            Asset.Validate("Location Code", "Location Code");
                            Asset.Modify;
                        until Asset.Next = 0;
                end;
                // P8000333A
        ...
        field(37002464; "Machine Type"; Option)
        {
            ...
            begin
                // P8000877
                if "Machine Type" > 0 then
                    TestField(Type, Type::Machine);
            end;
        }
        field(37002465; "Minimum Equipment Qty."; Decimal)
        {
            BlankZero = true;
    ...
    fieldgroups
    {
        fieldgroup(DropDown; "No.", Name, Type, "Location Code", "Base Unit of Measure")
        {
        }
    ...
    var
        ...
        Text37002001: Label '%1 cannot exceed %2% of %3 (%4).';
    ...
    procedure ValidateEquipmentQtys(EquipmentCapacity: Decimal; CapacityLevelPct: Decimal; MinEquipmentQty: Decimal)
    var
        NetCapacity: Decimal;
    begin
        // P8001107
        NetCapacity := EquipmentCapacity * CapacityLevelPct / 100;
        if (MinEquipmentQty > NetCapacity) then
            Error(Text37002001,
              FieldCaption("Minimum Equipment Qty."), CapacityLevelPct,
              FieldCaption("Equipment Capacity"), NetCapacity);
    end;

    procedure DeleteLotSegment()
    var
        LotSegmentValue: Record "Lot No. Segment Value";
    begin
        // P8001234
        if ProcessFns.TrackingInstalled then begin
            if LotSegmentValue.Get(LotSegmentValue.Type::Equipment, "No.") then
