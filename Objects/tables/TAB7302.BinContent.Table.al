table 7302 "Bin Content"
{
    ...
    //   Add field - Open Quantity
    // 
    // PR4.00.04
    // P8000322A, VerticalSoft, Don Bresee, 08 AUG 06
    //   Add Ship Bin and Receive Bin fields
    //   Staged Picks
    // 
    // PR5.00
    // P8000503A, VerticalSoft, Don Bresee, 1 NOV 06
    //   Rounding Issues
    //   Rework open warehouse entry changes
    //   Add validation disable for Bin Status window
    // 
    // PRW15.00.03
    // P8000631A, VerticalSoft, Don Bresee, 12 JAN 09
    //   Add fields for container quantities
    // 
    // PRW16.00.02
    // P8000756, VerticalSoft, Jack Reynolds, 08 JAN 10
    //   P800 code removed since it is now standard
    //   P800 code removed - rounding problem corrected in base NAV (Don Bresee)
    // 
    // PRW16.00.06
    ...
    //    Fix incorrect Qty. in Container (Base)
    ...
    fields
    {
        ...
        field(37002020; "Remaining Quantity"; Decimal)
        {
            CalcFormula = Sum ("Warehouse Entry"."Remaining Quantity" WHERE("Location Code" = FIELD("Location Code"),
                                                                            "Bin Code" = FIELD("Bin Code"),
                                                                            "Item No." = FIELD("Item No."),
                                                                            "Variant Code" = FIELD("Variant Code"),
                                                                            "Unit of Measure Code" = FIELD("Unit of Measure Code"),
        ...
        field(37002021; "Remaining Qty. (Base)"; Decimal)
        {
            CalcFormula = Sum ("Warehouse Entry"."Remaining Qty. (Base)" WHERE("Location Code" = FIELD("Location Code"),
                                                                               "Bin Code" = FIELD("Bin Code"),
                                                                               "Item No." = FIELD("Item No."),
                                                                               "Variant Code" = FIELD("Variant Code"),
                                                                               "Unit of Measure Code" = FIELD("Unit of Measure Code"),
        ...
        field(37002100; "Ship Bin"; Boolean)
        {
        ...
        field(37002101; "Receive Bin"; Boolean)
        {
        ...
        field(37002102; "Quantity in Container"; Decimal)
        {
            BlankZero = true;
            CalcFormula = Sum ("Container Line".Quantity WHERE("Location Code" = FIELD("Location Code"),
                                                               "Bin Code" = FIELD("Bin Code"),
                                                               "Item No." = FIELD("Item No."),
                                                               "Variant Code" = FIELD("Variant Code"),
                                                               "Unit of Measure Code" = FIELD("Unit of Measure Code"),
                                                               "Lot No." = FIELD("Lot No. Filter"),
                                                               "Serial No." = FIELD("Serial No. Filter")));
            Caption = 'Quantity in Container';
            DecimalPlaces = 0 : 5;
            Description = 'P8000631A';
            Editable = false;
            FieldClass = FlowField;
        }
        field(37002103; "Qty. in Container (Base)"; Decimal)
        {
            BlankZero = true;
            CalcFormula = Sum ("Container Line"."Quantity (Base)" WHERE("Location Code" = FIELD("Location Code"),
                                                                        "Bin Code" = FIELD("Bin Code"),
                                                                        "Item No." = FIELD("Item No."),
                                                                        "Variant Code" = FIELD("Variant Code"),
                                                                        "Unit of Measure Code" = FIELD("Unit of Measure Code"),
                                                                        "Lot No." = FIELD("Lot No. Filter"),
                                                                        "Serial No." = FIELD("Serial No. Filter")));
            Caption = 'Qty. in Container (Base)';
            DecimalPlaces = 0 : 5;
            Description = 'P8000631A';
            Editable = false;
            FieldClass = FlowField;
        }
        field(37002104; "Container Qty. in Use"; Integer)
        {
            BlankZero = true;
            CalcFormula = Count ("Container Header" WHERE("Container Item No." = FIELD("Item No."),
                                                          "Location Code" = FIELD("Location Code"),
                                                          "Bin Code" = FIELD("Bin Code")));
            Caption = 'Container Qty. in Use';
            Description = 'P8000631A';
            Editable = false;
            FieldClass = FlowField;
        }
        field(37002560; "Allocated Container Qty."; Decimal)
        {
            BlankZero = true;
            CalcFormula = Sum ("Warehouse Activity Line"."Container Qty." WHERE("Location Code" = FIELD("Location Code"),
                                                                                "Bin Code" = FIELD("Bin Code"),
                                                                                "Item No." = FIELD("Item No."),
                                                                                "Variant Code" = FIELD("Variant Code"),
                                                                                "Unit of Measure Code" = FIELD("Unit of Measure Code"),
                                                                                "Action Type" = CONST(Take),
                                                                                "Activity Type" = CONST(Pick),
                                                                                "Lot No." = FIELD("Lot No. Filter")));
            Caption = 'Allocated Container Qty.';
            DecimalPlaces = 0 : 5;
            Editable = false;
            FieldClass = FlowField;
        }
        field(37002561; "Allocated Container Count"; Integer)
        {
            BlankZero = true;
            CalcFormula = Count ("Warehouse Activity Line" WHERE("Location Code" = FIELD("Location Code"),
                                                                 "Bin Code" = FIELD("Bin Code"),
                                                                 "Item No." = FIELD("Item No."),
                                                                 "Variant Code" = FIELD("Variant Code"),
                                                                 "Unit of Measure Code" = FIELD("Unit of Measure Code"),
                                                                 ...
                                                                 "Activity Type" = CONST(Pick),
                                                                 "Lot No." = FIELD("Lot No. Filter"),
                                                                 "Container Qty." = FILTER(> 0)));
            Caption = 'Allocated Container Count';
            Editable = false;
            FieldClass = FlowField;
        }
    }
    ...
    keys
    {
        ...
        key(Key8; "Location Code", "Item No.", "Variant Code", "Cross-Dock Bin", "Bin Ranking")
        {
        }
    }
    ...
    begin
        ...
        if (LotstatusExclusionFilter <> '') and (GetFilter("Lot No. Filter") = '') then            // P8001083
            LotStatusMgmt.QuantityAdjForBinContent(Rec, LotstatusExclusionFilter, "Quantity (Base)");  // P8001083
        exit(
          "Quantity (Base)" -
    ...
    begin
        ...
        // if QtyAvailToPickBase < QtyBase then begin                                                       // P8001039
        if ((QtyAvailToPickBase + RoundingAdjmtMgmt.GetNearZeroQtyForItem("Item No.")) < QtyBase) then begin // P8001039
            GetItem("Item No.");
            QtyAvailToPick :=
    ...
    procedure SetDisableValidation(NewDisableValidation: Boolean)
    begin
        DisableValidation := NewDisableValidation; // P8000322A
    end;

    procedure SetLotStatusExclusion(ExclusionFilter: Text[1024])
    begin
        // P8001083
        LotstatusExclusionFilter := ExclusionFilter
    end;

    procedure SetStagingFromBin(LocationCode: Code[10]; StagingFromPickLine: Record "Whse. Staged Pick Line")
    var
        Location: Record Location;
    begin
        // P80096141
        if StagingFromPickLine."No." <> '' then
            SetRange("Bin Code", StagingFromPickLine."Bin Code") 
        else begin
            Location.Get(LocationCode);
            if (Location."Staging Bin Code" <> '') and (Location."Sample Staging Bin Code" <> '') then
                SetFilter("Bin Code", '<>%1&<>%2', Location."Staging Bin Code", Location."Sample Staging Bin Code")
            else
                if Location."Staging Bin Code" <> '' then
                    SetFilter("Bin Code", '<>%1', Location."Staging Bin Code")
                else
                    if Location."Sample Staging Bin Code" <> '' then
                        SetFilter("Bin Code", '<>%1', Location."Sample Staging Bin Code");
        end;
    end;

    procedure GetBinContent(ItemNo: Code[20]; VariantCode: Code[10]; UOMCode: Code[10]; LocationCode: Code[10]; BinCode: Code[20]; UseCrossDock: Boolean; UseRanking: Boolean; UseTracking: Boolean; WhseItemTrackingSetup: Record "Item Tracking Setup"; StagingFromPickLine: Record "Whse. Staged Pick Line"): Boolean
    begin
        // P80096141
        SetStagingFromBin(LocationCode, StagingFromPickLine);
        exit(GetBinContent(ItemNo, VariantCode, UOMCode, LocationCode, BinCode, UseCrossDock, UseRanking, UseTracking, WhseItemTrackingSetup)) 
    end;
