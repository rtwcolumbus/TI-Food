report 10143 "Item List" // Version: FOODNA
{
    // PR3.70.08
    // P8000168A, Myers Nissi, Jack Reynolds, 14 JAN 05
    //   Use costing quantities when calculating inventory value
    // 
    // PR4.00.01
    // P8000267B, VerticalSoft, Jack Reynolds, 12 DEC 05
    //   Introduction of data item for stockkeeping unit require changes to code similar to that for items
    //     with respect to alternate quantities
    // 
    // PRNA7.10
    // P8001252, Columbus IT, Jack Reynolds, 09 JAN 14
    //   Restore P8002067, missing from NAV 2013
    
    DefaultLayout = RDLC;
    RDLCLayout = './layout/local/ItemList.rdlc';
    Caption = 'Item List';
        ...
        dataitem(Item; Item)
        {
            ...
            dataitem("Stockkeeping Unit"; "Stockkeeping Unit")
            {
                ...
                begin
                    ...
                    if ItemLedgerEntry.Find('-') then
                        repeat
                            if ItemLedgerEntry.GetCostingQty <> 0 then begin // P8000267B
                                ItemLedgerEntry.CalcFields("Cost Amount (Actual)", "Cost Amount (Expected)");
                                if ItemLedgerEntry.GetCostingInvQty <> ItemLedgerEntry.GetCostingQty then // P8000267B
                                    ItemLedgerEntry."Cost Amount (Actual)" := ItemLedgerEntry."Cost Amount (Expected)";
                                TotalValue := TotalValue +
                                  ItemLedgerEntry.GetCostingRemQty * ItemLedgerEntry."Cost Amount (Actual)" / ItemLedgerEntry.GetCostingQty; // P8000267B
                            end; // P8000267B
            ...
            begin
                ...
                if not UseSKU then begin
                    ...
                    if ItemLedgerEntry.Find('-') then
                        repeat
                            if ItemLedgerEntry.GetCostingQty <> 0 then begin // P8000168A
                                ItemLedgerEntry.CalcFields("Cost Amount (Actual)", "Cost Amount (Expected)");
                                if ItemLedgerEntry.GetCostingInvQty <> ItemLedgerEntry.GetCostingQty then // P8000168A
                                    ItemLedgerEntry."Cost Amount (Actual)" := ItemLedgerEntry."Cost Amount (Expected)";
                                TotalValue := TotalValue +
                                  ItemLedgerEntry.GetCostingRemQty * ItemLedgerEntry."Cost Amount (Actual)" / ItemLedgerEntry.GetCostingQty; // P8000168A
                            end; // P8000168A
                        until ItemLedgerEntry.Next() = 0;
                end;
                NewTotalValue := NewTotalValue + TotalValue;
