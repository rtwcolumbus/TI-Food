report 10135 "Item Sales Statistics" // Version: FOODNA
{
    // PR4.00
    // P8000239A, Myers Nissi, Jack Reynolds, 11 AUG 05
    //   Modify for alternate quantities
    // 
    // PRW16.00.03
    // P8000827, VerticalSoft, Rick Tweedle, 27 MAY 10
    //   Upgraded for RTC
    // 
    // PRNA7.10
    ...
    DefaultLayout = RDLC;
    RDLCLayout = './layout/local/ItemSalesStatistics.rdlc';
    ...
    dataset
    {
        dataitem(Item; Item)
        {
            ...
            column(ItemAltUOM; Item."Alternate Unit of Measure")
            {
            }
            column(SalesQtyAlt; SalesQtyAlt)
            {
                DecimalPlaces = 0 : 5;
            }
            column(QuantityReturnedAlt; QuantityReturnedAlt)
            {
                DecimalPlaces = 0 : 5;
            }
            dataitem("Item Variant"; "Item Variant")
            {
                ...
                begin
                    ...
                    ItemLedgerEntry.SetRange("Variant Code", Code); // P8000239A
                    //Item.CALCFIELDS("Sales (Qty.)","Sales (LCY)","COGS (LCY)"); // P8000239A
                    Item.CalcFields("Sales (LCY)", "COGS (LCY)");                  // P8000239A
                    GetSalesQty(Item."Sales (Qty.)", SalesQtyAlt);                 // P8000239A
                    ...
                    if Item."Sales (LCY)" <> 0 then
                        ...
                        ItemProfitPct := 0;
                    QuantityReturned := 0;
                    QuantityReturnedAlt := 0; // P8000239A
                    ItemLedgerEntry.SetRange("Item No.", Item."No.");
                    //ItemLedgerEntry.SETRANGE("Variant Code",Code);     // P8000239A
                    ItemLedgerEntry.SetFilter("Invoiced Quantity", '>0'); // P8000239A
                    if ItemLedgerEntry.Find('-') then
                        repeat
                            //IF ItemLedgerEntry."Invoiced Quantity" > 0 THEN BEGIN // P8000239A
                            ...
                            QuantityReturnedAlt := QuantityReturnedAlt + ItemLedgerEntry."Invoiced Quantity (Alt.)"; // P8000239A
                            SalesQtyAlt := SalesQtyAlt + ItemLedgerEntry."Invoiced Quantity (Alt.)";                 // P8000239A
                                                                                                                     //END;                                                  // P8000239A
                        until ItemLedgerEntry.Next() = 0;
                    if (Item."Sales (Qty.)" = 0) and (QuantityReturned = 0) and
                       (Item."Sales (LCY)" = 0) and (Item."COGS (LCY)" = 0) and // P8000239A
                       PrintOnlyIfSales                                         // P8000239A
                    then
                        CurrReport.Skip();
                end;
                ...
                begin
                    ...
                    CurrReport.CreateTotals(Item."Sales (Qty.)", Item."Sales (LCY)", Item."COGS (LCY)",
                      SalesQtyAlt, QuantityReturnedAlt, // P8000239A
            ...
            begin
                ...
                //CALCFIELDS("Sales (Qty.)","Sales (LCY)","COGS (LCY)"); // P8000239A
                CalcFields("Sales (LCY)", "COGS (LCY)");                  // P8000239A
                GetSalesQty("Sales (Qty.)", SalesQtyAlt);                 // P8000239A
    ...
    labels
    {
        ItemAltUOMCaption = 'Alternate UOM';
        SalesQtyAltCaption = 'Quantity Sold (Alt.)';
        QuantityReturnedAltCaption = 'Quantity Ret''d (Alt.)';
    }
    ...
    var
        ...
        SalesQtyAlt: Decimal;
        QuantityReturnedAlt: Decimal;
        SalesQtyAltCaptionLbl: Label 'Quantity Sold (Alt.)';
        QuantityReturnedAltCaptionLbl: Label 'Quantity Ret''d (Alt.)';
        Item__Alternate_Unit_of_Measure_CaptionLbl: Label 'Alternate UOM';
    ...
    procedure GetSalesQty(var Qty: Decimal; var QtyAlt: Decimal)
    var
        ItemLedger2: Record "Item Ledger Entry";
    begin
        // P8000239A
        ItemLedger2.Copy(ItemLedgerEntry);
        ItemLedger2.SetRange("Item No.", Item."No.");
        ItemLedger2.SetRange("Invoiced Quantity");
        ItemLedger2.CalcSums("Invoiced Quantity", "Invoiced Quantity (Alt.)");
        Qty := -ItemLedger2."Invoiced Quantity";
        QtyAlt := -ItemLedger2."Invoiced Quantity (Alt.)";
    end;
}
