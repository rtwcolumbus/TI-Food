report 5753 "Get Source Documents"
{
    ...
    // 
    // PRW111.00.03
    // P80082821, To-Increase, Gangabhushan, 18 SEP 19
    //   CS00076235 - Error on pick when we have container and reservation entries in Sales Orders
    //
    // PRW118.00.03
    // P800124023, To-Increase, Gangabhushan, 17 MAY 21
    //   While create warehouse receipt from PO Lot information not get updated.
            ...
            dataitem("Sales Header"; "Sales Header")
            {
                ...
                dataitem("Sales Line"; "Sales Line")
                {
                    ...
                    begin
                        ...
                        AssignContainersToDocument(DATABASE::"Sales Line", "Sales Header"."Document Type", "Sales Header"."No."); // P80046533
            ...
            dataitem("Purchase Header"; "Purchase Header")
            {
                ...
                dataitem("Purchase Line"; "Purchase Line")
                {
                    ...
                    begin
                        ...
                        AssignContainersToDocument(DATABASE::"Purchase Line", "Purchase Header"."Document Type", "Purchase Header"."No."); // P80046533
            ...
            dataitem("Transfer Header"; "Transfer Header")
            {
                ...
                dataitem("Transfer Line"; "Transfer Line")
                {
                    ...
                    begin
                        ...
                        case RequestType of
                            RequestType::Receive:
                                AssignContainersToDocument(DATABASE::"Transfer Line", 1, "Transfer Header"."No.");
                            RequestType::Ship:
                                AssignContainersToDocument(DATABASE::"Transfer Line", 0, "Transfer Header"."No.");
                        end;
                        // P80046533
    ...
    requestpage
    {
        ...
        trigger OnClosePage()
        begin
            DefaultDoNotFill := false; // P80039780
        end;
    }
    ...
    begin
        DefaultDoNotFill := true; // P80039780
    end;
    ...
    begin
        ...
        WhseShptHeader."Delivery Trip" := "Warehouse Request"."Delivery Trip"; //N138F0000.n
    ...
    procedure SetUserInteraction()
    begin
        UserInteraction := true;
        WhseActivityCreate.SetUserInteraction; //N138F0000.n
    end;

    procedure GetShipmentLines(var WhseShipmentLine: Record "Warehouse Shipment Line" temporary)
    begin
        WhseActivityCreate.GetShipmentLines(WhseShipmentLine); //N138F0000.n
    end;

    procedure SetAssignContainers()
    begin
        // P8001323
        // P80046533 - renamed from AssignContainers
        AssignContainers := true; // P80046533
    end;

    procedure GetWarehouseRequest(var WhseRequest: Record "Warehouse Request")
    begin
        WhseRequest.Copy("Warehouse Request");
    end;

    local procedure AssignContainersToDocument(DocumentType: Integer; DocumentSubtype: Integer; DocumentNo: Code[20])
    var
        ContainerHeader: Record "Container Header";
        WarehouseDocumentNo: Code[20];
    begin
        // P80046533
        if AssignContainers then begin
            case RequestType of
                RequestType::Receive:
                    WarehouseDocumentNo := WhseReceiptHeader."No.";
                RequestType::Ship:
                    WarehouseDocumentNo := WhseShptHeader."No.";
            end;

            if DocumentType <> DATABASE::"Transfer Line" then
            ...
            if ContainerHeader.FindSet(true) then
                repeat
                    ContainerHeader."Whse. Document Type" := RequestType + 1;
                    ContainerHeader."Whse. Document No." := WarehouseDocumentNo;
    ...
    local procedure FinalizeDocument()
    var
        WarehouseReceiptLine: Record "Warehouse Receipt Line";
        WarehouseShipmentLine: Record "Warehouse Shipment Line";
    begin
        // P80039780
        if (OneHeaderCreated and LineCreated) or ((not OneHeaderCreated) and WhseHeaderCreated) then begin
            // P80082821
            if DefaultDoNotFill then
            ...
            case RequestType of
                RequestType::Receive:
                    begin
                        ...
                        if DoNotFillQtytoHandle then begin
                            WarehouseReceiptLine.SetRange("No.", WhseReceiptHeader."No.");
                            WarehouseReceiptLine.SetFilter("Line No.", '>%1', LastLineNo);
                ...
                RequestType::Ship:
                    begin
                        WhseShptHeader.SortWhseDoc;
                        WhseShptHeader.UpdateDocumentStatus;
                        if DoNotFillQtytoHandle then begin
                            WarehouseShipmentLine.SetRange("No.", WhseShptHeader."No.");
                            WarehouseShipmentLine.SetFilter("Line No.", '>%1', LastLineNo);
