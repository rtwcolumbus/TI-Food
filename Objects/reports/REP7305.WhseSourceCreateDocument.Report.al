report 7305 "Whse.-Source - Create Document"
{
    ...
    //   Add option to skip COMMITs in OnPostReport
    // 
    // PRW15.00.03
    ...
    // 
    // PRW17.00
    // P8001142, Columbus IT, Don Bresee, 13 MAR 13
    ...
    dataset
    {
        ...
        dataitem("Prod. Order Component"; "Prod. Order Component")
        {
            ...
            begin
                ...
                if Location.Get("Location Code") then                                                                        // P8001142
                    "Bin Code" := Location.GetToProductionBin("Prod. Order No.", "Prod. Order Line No.", "Line No.", "Item No."); // P8001142, P8001319
                ...
                if not WhseWkshLine.FindFirst() then begin
                    TestField("Qty. per Unit of Measure");
                    // CalcFields("Pick Qty. (Base)"); // P8000503A
                    CalcPickQtys;                      // P8000503A
                    ...
                    */
                    ...
                    if QtyToPick > 0 then begin
                        ...
                        if Location."Pick Production by Line" then
                            LineNo := "Prod. Order Line No."
                        else
                            LineNo := 0;
                        // P80056710
                        CreatePick.SetProdOrderCompLine("Prod. Order Component", LineNo); // P80056710
                        CreatePick.SetTempWhseItemTrkgLine(
        ...
        dataitem("Whse. Staged Pick Line"; "Whse. Staged Pick Line")
        {
            DataItemTableView = SORTING("No.", "Line No.");
            ...
            begin
                // P8000322A
                WMSMgt.CheckInboundBlockedBin("Location Code", "Bin Code", "No.", "Variant Code", "Unit of Measure Code"); // P8001132
            ...
            begin
                // P8000322A
                if WhseDoc <> WhseDoc::"Staged Pick" then
                    CurrReport.Break;
                ...
                CopyFilters(WhseStagedPickLine);
                SetFilter("Qty. to Stage", '>0');

                WhseWkshLine.SetCurrentKey("Whse. Document Type", "Whse. Document No.", "Whse. Document Line No.");
                WhseWkshLine.SetRange(
                  "Whse. Document Type", WhseWkshLine."Whse. Document Type"::FOODStagedPick);
                WhseWkshLine.SetRange("Whse. Document No.", WhseStagedPickLine."No.");
                // P8000322A
            end;
        }
    }
    ...
    begin
        ...
        if not (WhseDoc in
                [WhseDoc::"Put-away Worksheet", WhseDoc::"Posted Receipt", WhseDoc::"Internal Put-away"]) // P8000630A
        then begin
            ...
            if not SkipCommit then // P8000549A
                Commit();
        end else
        ...
        case WhseDoc of
            ...
            WhseDoc::"Posted Receipt", WhseDoc::"Put-away Worksheet",      // P8000630A
          WhseDoc::"Internal Put-away":                                 // P8000630A
    ...
    var
        ...
        AssignedID: Code[50];
        WhseDoc: Option "Whse. Mov.-Worksheet","Posted Receipt","Internal Pick","Internal Put-away",Production,"Put-away Worksheet",Assembly,"Service Order",Job,"Staged Pick";
    ...
    begin
        case WhseDoc of
          ...
          WhseDoc::"Whse. Mov.-Worksheet":
            ...
            // P8001132
            WhseDoc::"Staged Pick":
                exit(WhseStagedPickLine."Location Code");
        // P8001132
        end;
    end;
    ...
    procedure SetWhseStagedPickLine(var WhseStagedPickLine2: Record "Whse. Staged Pick Line"; AssignedID2: Code[20])
    begin
        // P8000322A
        WhseStagedPickLine.Copy(WhseStagedPickLine2);
        WhseDoc := WhseDoc::"Staged Pick";
        SourceTableCaption := WhseStagedPickLine.TableCaption;
        AssignedID := AssignedID2;
        //GetLocation(WhseStagedPickLine."Location Code"); // P8001132
        // P8000322A
    end;

    procedure SetSkipCommit(Flag: Boolean)
    begin
        // P8000549A
        SkipCommit := Flag;
    end;
