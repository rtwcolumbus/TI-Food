...
{
    // PRW18.00
    // P8001361, Columbus IT, Jack Reynolds, 06 NOV 14
    //   Adjust for off invoice allowances and alternate quantity
    // 
    // PRW111.00
    // P80053245, To Increase, Jack Reynolds, 23 MAR 18
    //   Upgrade for NAV 2018
    RDLCLayout = './layout/StandardSalesOrderConf.rdlc';
    WordLayout = './layout/StandardSalesOrderConf.docx';
        ...
        dataitem(Header; "Sales Header")
        {
            ...
            dataitem(Line; "Sales Line")
            {
                ...
                begin
                    ...
                    GetUnitOfMeasure(Line); // P8001361
                    ...
                    // P80053245
                    if Type <> Type::" " then
                        FormattedQuantity := Format(Line.GetPricingQty);
                    // P80053245
            ...
            begin
                ...
                AdjustForAllowances(Line); // P8001361
    ...
    procedure AdjustForAllowances(var SalesLine: Record "Sales Line" temporary)
    var
    ...
    begin
        ...
        if SalesLine.FindSet(true) then
            repeat
                if TempAllowance.Get(SalesLine."Off-Invoice Allowance Code") then begin
                    TempAllowance.Allowance += SalesLine."Unit Price";
                    TempAllowance.Modify;
                end else begin
                    TempAllowance.Init;
                    TempAllowance."Allowance Code" := SalesLine."Off-Invoice Allowance Code";
                    TempAllowance.Allowance += SalesLine."Unit Price";
                    TempAllowance."G/L Account" := SalesLine."No.";
        ...
        OrderAllowance.SetRange("Document Type", Header."Document Type");
        OrderAllowance.SetRange("Document No.", Header."No.");
        OrderAllowance.SetRange("Grant Allowance", true);
        if OrderAllowance.FindSet then begin
            OffInvoiceFns.SumSalesLines(Header, Weight, Volume, Quantity, Amount, 'OUT');
            repeat
                OffInvoiceFns.CalcAllowance(Header, OrderAllowance."Allowance Code", Weight, Volume, Quantity, Amount, AllowanceLine);
                if AllowanceLine.Allowance <> 0 then begin
                    if TempAllowance.Get(OrderAllowance."Allowance Code") then begin
                        TempAllowance.Allowance -= AllowanceLine.Allowance;
                        TempAllowance.Modify;
                    end else begin
                        TempAllowance.Init;
                        TempAllowance."Allowance Code" := OrderAllowance."Allowance Code";
                        TempAllowance.Allowance := -AllowanceLine.Allowance;
                        TempAllowance."G/L Account" := AllowanceLine."G/L Account";
                        TempAllowance.Insert;
                    end;
                end;
        ...
        if TempAllowance.FindSet then
            repeat
                Allowance.Get(TempAllowance."Allowance Code");
                SalesLine.Init;
                SalesLine.SuspendStatusCheck(true);
                SalesLine."Document Type" := Header."Document Type";
                SalesLine."Document No." := Header."No.";
                SalesLine."Line No." := LineNo + 10000;
                LineNo := SalesLine."Line No.";
                SalesLine.Validate(Type, SalesLine.Type::"G/L Account");
                SalesLine.Validate("No.", TempAllowance."G/L Account");
                SalesLine.Description := Allowance.Description;
                SalesLine.Validate(Quantity, 1);
                SalesLine.Validate("Unit Price", TempAllowance.Allowance);
                SalesLine."Off-Invoice Allowance Code" := Allowance.Code;
                SalesLine.Insert;
            until TempAllowance.Next = 0;
    end;

    procedure GetUnitOfMeasure(var SalesLine: Record "Sales Line")
    var
        Item: Record Item;
        UnitOfMeasure: Record "Unit of Measure";
    ...
    begin
        ...
        if (SalesLine.Type = SalesLine.Type::Item) and (SalesLine."No." <> '') then begin
            ...
            if not Item.PriceInAlternateUnits then
                exit;

            if UnitOfMeasure.Get(Item."Alternate Unit of Measure") then;
            SalesLine."Unit of Measure" := UnitOfMeasure.Description;
            if Header."Language Code" <> '' then begin
                UnitOfMeasureTranslation.SetRange(Code, Item."Alternate Unit of Measure");
                UnitOfMeasureTranslation.SetRange("Language Code", Header."Language Code");
                if UnitOfMeasureTranslation.Find('-') then
                    SalesLine."Unit of Measure" := UnitOfMeasureTranslation.Description;
            end;
        end;
    end;
