...
{
    // PR2.00.05
    //   Use variant code when creating lines
    // 
    // PRW16.00.06
    ...
    dataset
    {
        dataitem("Production Order"; "Production Order")
        {
            ...
            begin
                ...
                // if CalcLines then begin                                                 // P8001082
                if CalcLines and (not IsPreProcessBlending("Production Order")) then begin // P8001082
    ...
    begin
        ...
        if not (CalcLines or CalcComponents) then
            exit;
        if IsPreProcessBlending("Production Order") then // P8001082
            exit;                                          // P8001082
    ...
    local procedure IsPreProcessBlending(var ProdOrder: Record "Production Order"): Boolean
    begin
        // P8001082
        if ProcessFns.ProcessInstalled() then
            exit(PreProcessMgmt.IsPreProcessBlending(ProdOrder));
    end;
