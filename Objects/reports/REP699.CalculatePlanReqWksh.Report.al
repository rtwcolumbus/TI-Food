report 699 "Calculate Plan - Req. Wksh."
{
    ...
    //   Set date defaults based on worksheet name; update dates for worksheet name
    // 
    // PR5.00.01
    // P8000599A, VerticalSoft, Don Bresee, 13 MAY 08
    //   OrderDate was removed in SP1
    // 
    // PRW15.00.02
    // P8000613A, VerticalSoft, Jack Reynolds, 07 AUG 08
    //   Fix overlapping control on request form
    // 
    // PRW16.00
    ...
    // 
    // P8008567, To-Increase, Dayakar Battini, 20 MAR 17
    //   Fix Item exclude from planning error when LM is active.
    ...
    dataset
    {
        dataitem(Item; Item)
        {
            ...
            RequestFilterFields = "No.", "Search Description", "Supply Chain Group Code", "Location Filter";
    ...
    requestpage
    {
        ...
        begin
            ...
            // P8000312A
            if (ReqTemplate.Type = ReqTemplate.Type::"Req.") and (not ReqTemplate.Recurring) then begin
                ReqWkshName.Get(CurrTemplateName, CurrWorksheetName);
                // OrderDate := WORKDATE;                         // P8000599A
                // ToDate := OrderDate + ReqWkshName."Days View"; // P8000599A
                FromDate := WorkDate;                             // P8000599A
                ToDate := FromDate + ReqWkshName."Days View";     // P8000599A
                ShowAllItems := ReqWkshName."Show All Items";
                ShowAllItemsEnable := true;
            end else begin
                ShowAllItems := false;
                ShowAllItemsEnable := false;
            end;
            // P8000312A
    ...
    begin
        ...
        if (ReqTemplate.Type = ReqTemplate.Type::"Req.") and (not ReqTemplate.Recurring) then begin
            ReqWkshName.Get(CurrTemplateName, CurrWorksheetName);
            // ReqWkshName."Last Date" := OrderDate; // P8000599A
            ReqWkshName."Last Date" := FromDate;     // P8000599A
            ReqWkshName."End Date" := ToDate;
            if '' <> Format(ReqWkshName.Frequency) then
                // ReqWkshName."Next Date" := CALCDATE(ReqWkshName.Frequency,OrderDate); // P8000599A
                ReqWkshName."Next Date" := CalcDate(ReqWkshName.Frequency, FromDate);     // P8000599A
            ReqWkshName.Modify;
        end;
        // P8000312A
    ...
    begin
        ...
        ReqTemplate.Get(TemplateName); // P8000312A
    end;
